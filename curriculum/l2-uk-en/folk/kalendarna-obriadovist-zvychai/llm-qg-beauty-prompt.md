...
teach while applying the existing `beauty` rubric.
...
1. **List 3 specific evidence quotes from the Generated Content related to
   `beauty`.** Quote them verbatim — character-for-character strings that
...
2. **For each quote, state how it maps to the residual-judgment rubric for
   `beauty`** (see scope section above; deterministic checks already ran).
...
The JSON response MUST include `evidence_quotes` with 3 verbatim quotes from step 1 and `rubric_mapping` explaining how each quote maps to `beauty` before the score. The `evidence` field MUST be one of those verbatim quotes, wrapped in escaped quotes. A summary or paraphrase in any evidence field is a reviewer-protocol failure.
...
that touches dimension `beauty`. The audit feeds the evidence list above:
...
Review the assigned dimension `beauty` and return the required JSON object now.
