# Module 70: Сталі вирази та колокації

**Template:** b1-vocab-module-template.md | **Compliance:** ✅ PASS
**Overall Score:** 9.5/10
**Status:** ✅ PASS
**Generated:** 2026-01-24 23:54:00
**Reviewer:** Gemini

## Scores Breakdown

| Dimension           | Score | Notes                                                                                               |
| ------------------- | ----- | --------------------------------------------------------------------------------------------------- |
| Coherence           | 10/10 | Logical groups: Business, Action, Influence, Resistance.                                            |
| Relevance           | 10/10 | Essential for sounding native.                                                                      |
| Educational         | 10/10 | Excellent correction of calques: *приймати участь* -> *брати участь*.                               |
| Language            | 10/10 | High-quality idioms.                                                                                |
| Pedagogy            | 9/10  | Pylyp Orlyk narrative (Constitution) is a great historical anchor for "Law/Agreement" collocations. |
| Immersion           | 10/10 | Authentic history.                                                                                  |
| Activities          | 10/10 | Strong error-correction and matching tasks.                                                         |
| Richness            | 9/10  | Good depth.                                                                                         |
| Humanity            | 9/10  | Engaging.                                                                                           |
| LLM Fingerprint     | 9/10  | Natural.                                                                                            |
| Linguistic Accuracy | 10/10 | No issues.                                                                                          |

## Linguistic Accuracy Issues

- None.

## Strengths

- **Anti-Surzhyk:** The module actively fights common errors like `приймати участь` (Rus: принимать участие) teaching the correct Ukrainian `брати участь`.
- **Patriotism:** Teaching `чинити опір` (to resist) in the context of Cossacks/Orlyk resonates with modern Ukrainian resistance.

## Issues

- None.

## Recommendation

✅ PASS — Excellent collocations module.

## Action Items

1. None.
