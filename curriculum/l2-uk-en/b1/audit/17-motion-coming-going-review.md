# Audit Report: M17 — 17-motion-coming-going.md
**Level:** B1 | **Module:** M17 | **Phase:** B1.2 | **Pedagogy:** TTT | **Target:** 1500
**Naturalness:** 9/10 (PASS)
**Overall Status:** ✅ PASS
**Generated:** 2026-01-25 19:22:15

## Configuration
**Type:** B1-grammar
**Word Target:** 1500 words
**Activities:** 8-12 required
**Items per Activity:** ≥12 items
**Unique Types:** ≥4 types required
**Priority Types:** cloze, error-correction, fill-in, unjumble
**Required Types:** cloze, fill-in, match-up, quiz
**Engagement:** ≥5 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥25 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Значення префіксів руху | 8 | 8 | ✅ |
| 2 | match-up | Пари дієслів | 12 | 8 | ✅ |
| 3 | group-sort | Напрямок руху | 18 | 12 | ✅ |
| 4 | fill-in | Оберіть правильний префікс | 12 | 8 | ✅ |
| 5 | true-false | Граматичні правила | 12 | 8 | ✅ |
| 6 | cloze | Ранкова поїздка | 15 | 12 | ✅ |
| 7 | unjumble | Складіть речення | 8 | 6 | ✅ |
| 8 | error-correction | Виправте помилки префіксів | 8 | 6 | ✅ |
| 9 | mark-the-words | Знайдіть дієслова прибуття (при-) | 7 | 6 | ✅ |
| 10 | select | Оберіть правильну ситуацію | 6 | 6 | ✅ |
| 11 | translate | Перекладіть фрази руху | 6 | 6 | ✅ |
| 12 | cloze | Історія в аеропорту | 13 | 12 | ✅ |

**Summary:**
- Total activities: 12 (target: 8-12) ✅
- Unique types: 11 (minimum: 4) ✅
- Priority types used: 4/4 (cloze, error-correction, fill-in, unjumble) ✅
- Required types used: 4/4 (cloze, fill-in, match-up, quiz) ✅
- Low density activities: 0

## Gates
- **Words:** ✅ 1742/1500 (raw: 2240)
- **Activities:** ✅ 12/8
- **Density:** ✅ All > 12
- **Unique_types:** ✅ 11/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 7/5
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 90/25
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 92.5% (target 85-100% (B1.2 Motion))
- **Richness:** ✅ 99% (grammar)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ✅ 9/10 (High)

## Richness Details
**Score:** 99% (minimum: 95%)
**Module Type:** grammar

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| examples | 37 | 24 | 100% | 20% | 20.0% |
| engagement | 6 | 5 | 100% | 15% | 15.0% |
| dialogues | 16 | 4 | 100% | 15% | 15.0% |
| variety | 0.99 | - | 99% | 10% | 9.9% |
| cultural | 3 | 3 | 100% | 10% | 10.0% |
| realworld | 8 | 3 | 100% | 10% | 10.0% |
| visual | 7 | 3 | 100% | 5% | 5.0% |
| paragraph_var | 0.98 | - | 98% | 5% | 4.9% |
| questions | 24 | 5 | 100% | 5% | 5.0% |
| proverbs | 1 | 1 | 100% | 5% | 5.0% |
| **TOTAL** | | | | | **99.8%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 157 | Included in Core |
| **Тест** | ⚪️ | 104 | Skipped |
| **Пояснення** | ⚪️ | 444 | Skipped |
| **Практика** | ⚪️ | 536 | Skipped |
| **Діалоги** | ✅ | 274 | Included in Core |
| **Підсумок** | ✅ | 117 | Included in Core |
| **Потрібно більше практики?** | ⚪️ | 110 | Skipped |