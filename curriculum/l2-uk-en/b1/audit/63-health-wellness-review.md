# Audit Report: M63 — 63-health-wellness.md
**Level:** B1 | **Module:** M63 | **Phase:** B1.6 | **Pedagogy:** PPP | **Target:** 1500
**Naturalness:** 9/10 (PASS)
**Overall Status:** ✅ PASS
**Generated:** 2026-01-27 19:01:34

## Configuration
**Type:** B1-vocab
**Word Target:** 1500 words
**Activities:** 8-12 required
**Items per Activity:** ≥12 items
**Unique Types:** ≥4 types required
**Priority Types:** mark-the-words, match-up, quiz, translate
**Required Types:** cloze, fill-in, match-up, quiz
**Engagement:** ≥5 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥35 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Розуміння лексики здоров'я та самопочуття | 8 | 8 | ✅ |
| 2 | match-up | Колокації — іменник + дієслово | 12 | 8 | ✅ |
| 3 | match-up | Синоніми та переклад | 12 | 8 | ✅ |
| 4 | group-sort | Групування за значенням | 18 | 12 | ✅ |
| 5 | cloze | Вибір слова з контексту | 12 | 12 | ✅ |
| 6 | select | Правильні колокації | 8 | 6 | ✅ |
| 7 | true-false | Правила використання лексики здоров'я | 12 | 8 | ✅ |
| 8 | cloze | Заповніть пропуски в тексті про здоров'я | 14 | 12 | ✅ |
| 9 | unjumble | Складіть речення про здоров'я | 8 | 6 | ✅ |
| 10 | error-correction | Виправте помилки в колокаціях | 8 | 6 | ✅ |
| 11 | translate | Переклад речень про здоров'я | 8 | 6 | ✅ |
| 12 | mark-the-words | Знайдіть слова про здоров'я та самопочуття | 14 | 6 | ✅ |
| 13 | fill-in | Доповніть речення лексикою здоров'я | 8 | 8 | ✅ |

**Summary:**
- Total activities: 13 (target: 8-12) ❌
- Unique types: 11 (minimum: 4) ✅
- Priority types used: 4/4 (mark-the-words, match-up, quiz, translate) ✅
- Required types used: 4/4 (cloze, fill-in, match-up, quiz) ✅
- Low density activities: 0

## Gates
- **Words:** ✅ 1734/1500 (raw: 2467)
- **Activities:** ✅ 13/8
- **Density:** ✅ All > 12
- **Unique_types:** ✅ 11/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 9/5
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 45/35
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 99.0% (target 85-100% (B1.5-6 Vocab))
- **Richness:** ✅ 99% (grammar)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ✅ 9/10 (High)

## Richness Details
**Score:** 99% (minimum: 95%)
**Module Type:** grammar

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| examples | 25 | 24 | 100% | 20% | 20.0% |
| engagement | 7 | 5 | 100% | 15% | 15.0% |
| dialogues | 17 | 4 | 100% | 15% | 15.0% |
| variety | 0.97 | - | 97% | 10% | 9.7% |
| cultural | 4 | 3 | 100% | 10% | 10.0% |
| realworld | 3 | 3 | 100% | 10% | 10.0% |
| visual | 6 | 3 | 100% | 5% | 5.0% |
| paragraph_var | 1.00 | - | 100% | 5% | 5.0% |
| questions | 34 | 5 | 100% | 5% | 5.0% |
| proverbs | 10 | 1 | 100% | 5% | 5.0% |
| **TOTAL** | | | | | **99.7%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 52 | Included in Core |
| **Вступ** | ✅ | 107 | Included in Core |
| **Вживання** | ⚪️ | 286 | Skipped |
| **Читання** | ✅ | 376 | Included in Core |
| **Діалоги** | ✅ | 335 | Included in Core |
| **Підсумок** | ✅ | 181 | Included in Core |
| **Лексика** | ⚪️ | 287 | Skipped |
| **Потрібно більше практики?** | ⚪️ | 110 | Skipped |