# Audit Report: M30 — 30-purpose-shchob-past-form.md
**Level:** B1 | **Module:** M30 | **Phase:** B1.3a | **Pedagogy:** TTT | **Target:** 1700
**Naturalness:** 9/10 (PASS)
**Overall Status:** ✅ PASS
**Generated:** 2026-02-01 17:52:24

## Configuration
**Type:** B1-grammar
**Word Target:** 1700 words
**Activities:** 8-12 required
**Items per Activity:** ≥12 items
**Unique Types:** ≥4 types required
**Priority Types:** cloze, error-correction, fill-in, unjumble
**Required Types:** error-correction, fill-in, translate, unjumble
**Engagement:** ≥5 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥25 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Конструкція щоб плюс минула форма | 12 | 8 | ✅ |
| 2 | match-up | Дієслово і приклад конструкції | 12 | 8 | ✅ |
| 3 | cloze | Оберіть правильну форму дієслова | 12 | 12 | ✅ |
| 4 | true-false | Правда чи неправда? | 12 | 8 | ✅ |
| 5 | group-sort | Категорії дієслів з конструкцією щоб | 14 | 12 | ✅ |
| 6 | unjumble | Розставте слова в правильному порядку | 8 | 6 | ✅ |
| 7 | error-correction | Знайдіть і виправте помилку | 8 | 6 | ✅ |
| 8 | cloze | Щоб плюс минула форма в контексті | 14 | 12 | ✅ |
| 9 | mark-the-words | Знайдіть минулі форми після щоб | 8 | 6 | ✅ |
| 10 | select | Оберіть усі правильні варіанти | 8 | 6 | ✅ |
| 11 | translate | Перекладіть речення | 8 | 6 | ✅ |
| 12 | quiz | Контекст і Культура | 8 | 8 | ✅ |
| 13 | fill-in | Доповніть речення минулою формою | 12 | 8 | ✅ |

**Summary:**
- Total activities: 13 (target: 8-12) ❌
- Unique types: 11 (minimum: 4) ✅
- Priority types used: 4/4 (cloze, error-correction, fill-in, unjumble) ✅
- Required types used: 4/4 (error-correction, fill-in, translate, unjumble) ✅
- Low density activities: 0

## Gates
- **Words:** ✅ 1725/1700 (raw: 2352)
- **Activities:** ✅ 13/8
- **Density:** ✅ All > 12
- **Unique_types:** ✅ 11/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 10/5
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 56/25
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 96.9% (target 85-100% (B1.3-4 Complex))
- **Richness:** ✅ 99% (grammar)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ✅ 9/10 (High)

## Richness Details
**Score:** 99% (minimum: 95%)
**Module Type:** grammar

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| examples | 42 | 24 | 100% | 20% | 20.0% |
| engagement | 10 | 5 | 100% | 15% | 15.0% |
| dialogues | 14 | 4 | 100% | 15% | 15.0% |
| variety | 0.98 | - | 98% | 10% | 9.8% |
| cultural | 3 | 3 | 100% | 10% | 10.0% |
| realworld | 4 | 3 | 100% | 10% | 10.0% |
| visual | 12 | 3 | 100% | 5% | 5.0% |
| paragraph_var | 1.00 | - | 100% | 5% | 5.0% |
| questions | 26 | 5 | 100% | 5% | 5.0% |
| proverbs | 10 | 1 | 100% | 5% | 5.0% |
| **TOTAL** | | | | | **99.8%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 61 | Included in Core |
| **Тест: Перевірте себе** | ⚪️ | 117 | Skipped |
| **Пояснення** | ⚪️ | 517 | Skipped |
| **Практика** | ⚪️ | 412 | Skipped |
| **Діалоги** | ✅ | 353 | Included in Core |
| **Підсумок** | ✅ | 155 | Included in Core |
| **Потрібно більше практики?** | ⚪️ | 110 | Skipped |