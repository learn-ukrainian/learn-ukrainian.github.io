# Audit Report: 57-health-wellness.md
**Phase:** B1 | **Level:** B1 | **Pedagogy:** "TTT" | **Target:** 1500
**Overall Status:** ✅ PASS

## Gates
- **Words:** ✅ 1533/1500
- **Activities:** ✅ 12/12
- **Density:** ✅ All > 14
- **Unique_types:** ✅ 12/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 8/5
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 30/30
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Immersion:** 🇺🇦 65.0% (target 60-65% (B1.5-6 Vocab))

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Здоров'я та благополуччя** | ⚪️ | 68 | Skipped |
| **Діагностика** | ✅ | 191 | Included in Core |
| **Аналіз** | ✅ | 1000 | Included in Core |
| **Поглиблення** | ⚪️ | 139 | Skipped |
| **Підсумок** | ✅ | 83 | Included in Core |
| **Need More Practice?** | ⚪️ | 52 | Skipped |
| **Activities** | ➖ | 216 | Excluded Type |
| **match-up: Здоров'я — Health Terms** | 🎮 | 14 | Activity (14 items, min 8) |
| **fill-in: Симптоми та лікування** | 🎮 | 8 | Activity (8 items, min 8) |
| **group-sort: Корисне vs Шкідливе** | 🎮 | 12 | Activity (12 items, min 8) |
| **true-false: Факти про здоров'я** | 🎮 | 8 | Activity (8 items, min 8) |
| **cloze: У кабінеті лікаря** | 🎮 | 6 | Activity (6 items, min 6) |
| **error-correction: Виправте помилки** | 🎮 | 6 | Activity (6 items, min 6) |
| **unjumble: Побудуйте речення** | 🎮 | 8 | Activity (8 items, min 6) |
| **dialogue-reorder: У лікаря** | 🎮 | 6 | Activity (6 items, min 5) |
| **translate: Переклад** | 🎮 | 8 | Activity (8 items, min 6) |
| **mark-the-words: Знайдіть слова про здоров'я** | 🎮 | 11 | Activity (11 items, min 6) |
| **select: Що потрібно для здоров'я?** | 🎮 | 10 | Activity (10 items, min 6) |
| **quiz: Перевірка знань** | 🎮 | 8 | Activity (8 items, min 8) |
| **Vocabulary** | ➖ | 0 | Excluded Type |