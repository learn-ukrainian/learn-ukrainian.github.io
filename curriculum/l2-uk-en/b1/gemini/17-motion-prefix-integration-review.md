# Audit Report: 17-motion-prefix-integration.md
**Phase:** B1 | **Level:** B1 | **Pedagogy:** "TTT" | **Target:** 1500
**Overall Status:** ✅ PASS

## Gates
- **Words:** ✅ 1621/1500
- **Activities:** ✅ 12/12
- **Density:** ✅ All > 14
- **Unique_types:** ✅ 12/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 6/5
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 20/20
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Immersion:** 🇺🇦 56.0% (target 40-68% (B1.2 Motion))

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Рух — Повна інтеграція префіксів** | ⚪️ | 32 | Skipped |
| **Діагностика** | ✅ | 0 | Included in Core |
| **The Complete Prefix Matrix** | ⚪️ | 200 | Skipped |
| **Аналіз** | ✅ | 0 | Included in Core |
| **The Complete Prefix System for Motion Verbs** | ⚪️ | 537 | Skipped |
| **Поглиблення** | ⚪️ | 0 | Skipped |
| **Combining Multiple Directions: Complex Movements** | ✅ | 252 | Included in Core |
| **Practice Dialogues: Prefixes in Context** | ✅ | 194 | Included in Core |
| **Need More Practice?** | ⚪️ | 78 | Skipped |
| **Практика** | ⚪️ | 0 | Skipped |
| **Exercise 1: Prefix Prediction** | ⚪️ | 65 | Skipped |
| **Exercise 2: Complete the Journey** | ⚪️ | 223 | Skipped |
| **Summary** | ✅ | 0 | Included in Core |
| **The Complete Motion Prefix System** | ⚪️ | 40 | Skipped |
| **Activities** | ➖ | 0 | Excluded Type |
| **fill-in: Префікси руху** | 🎮 | 14 | Activity (14 items, min 8) |
| **match-up: Префікс і напрямок** | 🎮 | 14 | Activity (14 items, min 8) |
| **quiz: Теорія префіксів** | 🎮 | 14 | Activity (14 items, min 8) |
| **error-correction: Виправте помилку** | 🎮 | 14 | Activity (14 items, min 6) |
| **group-sort: Дієслова за напрямком** | 🎮 | 18 | Activity (18 items, min 8) |
| **unjumble: Складіть речення** | 🎮 | 14 | Activity (14 items, min 6) |
| **true-false: Правила префіксів** | 🎮 | 14 | Activity (14 items, min 8) |
| **cloze: Опис подорожі** | 🎮 | 14 | Activity (14 items, min 6) |
| **select: Вибір префікса** | 🎮 | 14 | Activity (14 items, min 6) |
| **translate: Переклад напрямків** | 🎮 | 14 | Activity (14 items, min 6) |
| **dialogue-reorder: Як доїхати?** | 🎮 | 7 | Activity (7 items, min 5) |
| **mark-the-words: Знайдіть префіксні дієслова** | 🎮 | 14 | Activity (14 items, min 6) |
| **Словник** | ⚪️ | 0 | Skipped |