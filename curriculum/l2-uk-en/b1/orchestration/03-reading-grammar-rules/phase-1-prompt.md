...
```
curriculum/l2-uk-en/b1/research/03-reading-grammar-rules-research.md
```
...
**Note:** No old meta file exists — this is a clean rebuild. Build from plan + research only.
...
5. Each section's `points` list is specific and actionable — not vague ("cover grammar" → bad; "Each instruction verb gets its own H3 with definition, example sentence, typical context" → good)
...
- **Bridge module (immersion 80%):** Note the plan's `immersion: 80%` field. The intro section should explicitly list English scaffolding requirements — English only in tip/note callouts for tricky concepts.
- The plan has a thin "Comparison Patterns" section (137 words). Either merge it into a related section or expand it to ≥200 words by redistributing from nearby sections.
