===RESEARCH_START===

# Дослідження: Новини: як читати

## State Standard Reference
§3.4: "преса, радіо, телебачення, інтернет" та §3.6: "війна і мир; суспільно-політичні події"
Alignment: Цей модуль прямо реалізує вимоги Стандарту для В1, надаючи інструменти та словниковий запас для навігації в українськомовному інформаційному просторі (преса, інтернет) та розуміння суспільно-політичного контексту.

## Vocabulary Frequency
| Word | Frequency / Source | Key collocations |
|------|-------------------|------------------|
| новини | High (News/Media) | свіжі новини, стрічка новин, останні новини |
| стаття | High (Publishing) | публіцистична стаття, редакційна стаття |
| заголовок | Medium-High | гучний заголовок (clickbait), винести в заголовок |
| джерело | High (Journalism) | надійне джерело, перевірене джерело, посилатися на джерело |
| джинса | Slang/Media specific | розпізнати джинсу, політична джинса |

## Cultural Hooks
1. Інститут масової інформації (ІМІ) та феномен "джинси": "Джинса" — це унікальний сленговий термін в українському медіапросторі, що означає приховану рекламу або проплачений матеріал, поданий як об'єктивна новина. Громадянське суспільство активно з цим бореться.
2. Суспільне Мовлення (Suspilne): Створене після Революції Гідності, воно стало еталоном журналістських стандартів в Україні, значно відрізняючись від олігархічних телеканалів.
3. Національний телемарафон "Єдині новини": Запущений у лютому 2022 року для інформаційної консолідації на початку повномасштабного вторгнення.

## Common Learner Errors
1. Хибний друг "актуальний" → Означає "current / timely" (напр., актуальні новини), а НЕ "actual" (фактичний).
2. Хибний друг "оперативний" → Означає "prompt / rapid" (оперативна інформація, діяти оперативно), а НЕ медичний чи військовий термін (operative).
3. Використання русизмів → "получати новини" замість правильного "отримувати новини" або "дізнаватися новини".

## Cross-References
- Builds on: b1-86 (Ukrainian Holidays and Festivals)
- Prepares for: b1-88 (Interviews and Podcasts)

## Notes for Content Writing
- Decolonized framing: Уникати порівнянь українського медіа-ринку з російським. Підкреслювати розвиток незалежної журналістики (від заснування "Української правди" до "Суспільного") як прояв української демократії та суб'єктності.
- Immersion: Цей модуль має 100% занурення (B1.8). Жодного англійського тексту в поясненнях чи вступі. Англійська дозволена ТІЛЬКИ в перекладах у словникових таблицях. Усі заголовки та підзаголовки виключно українською.
- Заголовки (H2) повинні точно збігатися з українськими назвами в мета-файлі, без дублювання ключових слів між різними H2.
- Використовувати тільки українські кутові лапки «...» для цитат. Звернути увагу на правильне відображення транскрипції: о = [ɔ], е = [ɛ].

===RESEARCH_END===

===META_OUTLINE_START===
content_outline:
  - section: "Вступ"
    words: 600
    points:
      - "Пояснити роль медіа для мовного занурення згідно зі Стандартом §3.4 (інтернет, преса). Встановити 100% україномовний контекст уроку."
      - "Розповісти про еволюцію незалежних медіа в Україні: від заснування «Української правди» до «Суспільного мовлення», фокусуючись на українській демократичній традиції."
  - section: "Стратегії читання: Заголовки та ліди"
    words: 1100
    points:
      - "Проаналізувати граматику заголовків: використання номінативних речень та еліпсу (пропущених дієслів) згідно зі стилістичними нормами §4.5.1."
      - "Розібрати структуру ліду: як швидко орієнтуватися в тексті, шукати підмет і присудок серед активних дієприкметників."
      - "Подати візуальний словник сайту новин: терміни «шапка», «стрічка новин», «бічна панель» із контекстними автентичними прикладами."
  - section: "Медіаграмотність: Факти та маніпуляції"
    words: 1000
    points:
      - "Навчити розрізняти об'єктивні новини та авторські колонки (op-eds) через аналіз маркерів суб'єктивності."
      - "Розкрити поняття «джинса» (прихована політична або комерційна реклама) та пояснити, як громадянське суспільство вчиться її розпізнавати."
      - "Описати правило «трьох джерел» для фактчекінгу, практикуючи лексику: «надійне джерело», «посилатися на джерело»."
  - section: "Лексичні пастки та автентичний контекст"
    words: 900
    points:
      - "Опрацювати типові помилки та хибних друзів перекладача: розрізнення «актуальний» (current) від actual, та «оперативний» (prompt) від operative."
      - "Ввести лексику офіційного стилю щодо суспільно-політичних подій (Стандарт §3.6), уникаючи русизмів (тільки «отримувати», а не «получати»)."
      - "Проілюструвати термінологію цитування: введення прямої та непрямої мови з посиланням на офіційні або анонімні джерела."
  - section: "Продукція та підсумок"
    words: 400
    points:
      - "Організувати практику: міні-діалоги або дискусійні запитання для висловлення власної думки щодо прочитаних новин."
      - "Зробити підсумок «Читай, думай, перевіряй», згадати марафон «Єдині новини» як специфіку воєнного часу та заохотити до самостійного читання українських ЗМІ."
===META_OUTLINE_END===

===FRICTION_START===
**Phase**: Phase A: Meta + Research (Core)
**Step**: Full Phase A
**Friction Type**: NONE
**Raw Error**: None
**Self-Correction**: Stripped English translations from the plan's outline section headers to strictly enforce the "Section names must be in Ukrainian" rule, ensuring compliance with Phase B heading standards. Replaced literal "§4.5.1" with specific themes mapping from B1 "§3.4" and "§3.6" directly found in the State Standard.
**Proposed Tooling Fix**: N/A
===FRICTION_END===
