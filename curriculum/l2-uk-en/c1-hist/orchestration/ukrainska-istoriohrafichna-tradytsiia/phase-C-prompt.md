...
| Target | Value |
|--------|-------|
| Skill identity | Professor of Ukrainian Arts (historiography) |
| Module persona | Academic Historiographer, acting as Dean of Historiography |
...
```
/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/c1-hist/ukrainska-istoriohrafichna-tradytsiia.md
```

**Plan file** (vocabulary_hints — vocabulary list to follow):
```
/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/plans/c1-hist/ukrainska-istoriohrafichna-tradytsiia.yaml
```

**Meta file** (activity count targets, pedagogy):
```
/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/c1-hist/meta/ukrainska-istoriohrafichna-tradytsiia.yaml
```

**Activity schema** (CRITICAL — defines allowed fields per activity type):
```
schemas/activities-c1-hist.schema.json
```
...
- **Schema violations**: `additionalProperties: false` means ANY unlisted field = instant fail. Read `schemas/activities-c1-hist.schema.json` before writing.
...
3. **Seminar style** (c1-hist): Reading Input → Analytical Output. Focus on comprehension, analysis, and critical thinking — NOT drill exercises.
...
### CRITICAL: Activity Type Constraints for c1-hist

**ALLOWED types (use ONLY these):** reading, essay-response, critical-analysis, comparative-study, quiz, true-false
...
### Correct Reading Schema (REFERENCE — seminar tracks)

**Reading activities MUST have BOTH `text` AND `tasks`. Missing either = instant schema failure.**

```yaml
- type: reading
  id: reading-hrushevsky
  ...
  text: |
    ...
    королі чи владні гетьмани, а саме широкі народні маси, що
    творять культуру й будують державність...
  tasks:                              # ← REQUIRED — without this, audit FAILS
    - "Хто є головним рушієм історії за Грушевським?"
    - "Яку роль відводить автор народним масам?"
```

**WRONG — missing `text` and `tasks` (this is the #1 reading failure):**
```yaml
# ❌ FAILS AUDIT — no text, no tasks
- type: reading
...
**Required fields for reading**: `type`, `title`, `text` (the passage), `tasks` (comprehension questions). Optional: `id`, `source`, `instruction`.
