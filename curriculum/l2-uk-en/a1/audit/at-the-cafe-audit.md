# Audit Report: M41 — at-the-cafe.md
**Level:** A1 | **Module:** M41 | **Phase:** A1.4 | **Pedagogy:** PPP | **Target:** 1200
**Overall Status:** ✅ PASS
**Generated:** 2026-03-07 19:35:56

## Configuration
**Type:** A1
**Word Target:** 1200 words
**Activities:** 8-12 required
**Items per Activity:** ≥6 items
**Unique Types:** ≥4 types required
**Priority Types:** anagram, classify, fill-in, image-to-letter, match-up, quiz, unjumble, watch-and-repeat
**Required Types:** fill-in, quiz
**Engagement:** ≥3 callouts
**Immersion:** 0-100%
**Vocab Target:** ≥1 words
**Transliteration:** Allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | match-up | Cafe Vocabulary | 8 | 8 | ✅ |
| 2 | match-up | Cafe Phrases | 8 | 8 | ✅ |
| 3 | true-false | Cafe Etiquette and Vocabulary | 9 | 8 | ✅ |
| 4 | quiz | Ordering Rules in a Cafe | 8 | 8 | ✅ |
| 5 | fill-in | Complete the Cafe Order | 8 | 8 | ✅ |
| 6 | fill-in | Customer and Waiter Conversation | 8 | 8 | ✅ |
| 7 | unjumble | Put the Words in Order | 8 | 6 | ✅ |
| 8 | group-sort | Drinks vs Other Items | 11 | 6 | ✅ |

**Summary:**
- Total activities: 8 (target: 8-12) ✅
- Unique types: 6 (minimum: 4) ✅
- Priority types used: 4/8 (fill-in, match-up, quiz, unjumble) ✅
- Required types used: 2/2 (fill-in, quiz) ✅
- Low density activities: 0

## Gates
- **Words:** ✅ 1494/1200 (raw: 1684)
- **Activities:** ✅ 8/8
- **Density:** ✅ All > 6
- **Unique_types:** ✅ 6/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 4/3
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 20/1
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 45.0% (target 35-55% (M41))
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review
- **Research:** ✅ Content aligned with research

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 0 | Included in Core |
| **At the Café** | ✅ | 85 | Included in Core |
| **Вступ (Introduction)** | ✅ | 251 | Included in Core |
| **Презентація (Presentation)** | ✅ | 468 | Included in Core |
| **Практика (Practice)** | ✅ | 323 | Included in Core |
| **Продукція та Підсумок (Production and Summary)** | ➖ | 205 | Excluded Type |
| **Summary** | ✅ | 162 | Included in Core |