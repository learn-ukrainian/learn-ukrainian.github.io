# Audit Report: M53 — health.md
**Level:** A1 | **Module:** M53 | **Phase:** A1.8 | **Pedagogy:** PPP | **Target:** 1200
**Overall Status:** ✅ PASS
**Generated:** 2026-04-03 14:02:06

## Configuration
**Type:** A1-vocab
**Word Target:** 1200 words
**Activities:** 0-4 required
**Items per Activity:** ≥6 items
**Unique Types:** ≥0 types required
**Priority Types:** anagram, classify, fill-in, image-to-letter, match-up, quiz, unjumble, watch-and-repeat
**Engagement:** ≥0 callouts
**Immersion:** 0-100%
**Vocab Target:** ≥1 words
**Transliteration:** Allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | match-up |  | 10 | 6 | ✅ |
| 2 | fill-in |  | 6 | 6 | ✅ |
| 3 | fill-in |  | 6 | 6 | ✅ |
| 4 | quiz |  | 6 | 6 | ✅ |
| 5 | fill-in |  | 6 | 6 | ✅ |
| 6 | true-false |  | 8 | 6 | ✅ |
| 7 | match-up |  | 6 | 6 | ✅ |
| 8 | group-sort |  | 16 | 6 | ✅ |
| 9 | quiz |  | 7 | 6 | ✅ |
| 10 | translate |  | 6 | 6 | ✅ |
| 11 | error-correction |  | 6 | 6 | ✅ |

**Summary:**
- Total activities: 11 (target: 0-4) ❌
- Unique types: 7 (minimum: 0) ✅
- Priority types used: 3/8 (fill-in, match-up, quiz) ✅
- Low density activities: 0

## PEDAGOGICAL VIOLATIONS
- **[HEADING_LEVEL]** Main section 'Summary' uses H2 (##) but spec requires H1 (#)
  - FIX: Change '## Summary' to '# Summary' for top-level TOC compliance
- **[YAML_SCHEMA_VIOLATION]** Schema error in health.yaml: Schema validation error at key '0': 'голова' is not of type 'object'
  - FIX: Fix the activity YAML to match the schema in schemas/activity-v2.schema.json

## Recommendation
**📝 UPDATE** (severity 5/100)

- 2 violations (minor)

## Gates
- **Words:** ✅ 1304/1200 (raw: 1338)
- **Activities:** ✅ 11/0
- **Density:** ✅ All > 6
- **Unique_types:** ✅ 7/0 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 0/0
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 38/1
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 26.3% (target 20-40% (M53))
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review
- **Research:** ⚠️ Refresh recommended: Research has 3+ learner errors but content doesn't address common mistakes

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 357 | Included in Core |
| **Ті́ло (The Body)** | ✅ | 351 | Included in Core |
| **У мене болить... (It Hurts...)** | ✅ | 357 | Included in Core |
| **Summary** | ✅ | 239 | Included in Core |