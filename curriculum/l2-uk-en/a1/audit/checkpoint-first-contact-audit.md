# Audit Report: M14 — checkpoint-first-contact.md
**Level:** A1 | **Module:** M14 | **Phase:** A1.1 | **Pedagogy:** TTT | **Target:** 1200
**Overall Status:** ✅ PASS
**Generated:** 2026-03-10 20:38:14

## Configuration
**Type:** A1-checkpoint
**Word Target:** 1200 words
**Activities:** 8-12 required
**Items per Activity:** ≥10 items
**Unique Types:** ≥4 types required
**Priority Types:** fill-in, match-up, quiz
**Required Types:** quiz
**Engagement:** ≥2 callouts
**Immersion:** 0-100%
**Vocab Target:** ≥1 words
**Transliteration:** Allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Alphabet and Sounds | 8 | 6 | ✅ |
| 2 | group-sort | Noun Gender | 11 | 10 | ✅ |
| 3 | match-up | Adjective Agreement | 8 | 6 | ✅ |
| 4 | true-false | Grammar Rules | 8 | 6 | ✅ |
| 5 | fill-in | Complete the Sentence | 8 | 6 | ✅ |
| 6 | unjumble | Sentence Building | 6 | 6 | ✅ |
| 7 | quiz | Cafe Culture | 8 | 6 | ✅ |
| 8 | match-up | Vocabulary Review | 8 | 6 | ✅ |

**Summary:**
- Total activities: 8 (target: 8-12) ✅
- Unique types: 7 (minimum: 4) ✅
- Priority types used: 3/3 (fill-in, match-up, quiz) ✅
- Required types used: 1/1 (quiz) ✅
- Low density activities: 0

## PEDAGOGICAL VIOLATIONS
- **[VOCAB_NOT_IN_CONTENT]** Only 10/20 (50%) vocabulary words appear in content+activities. Missing: говорити, де, знати, ні, писати, питати, розуміти, хто (+2 more)
  - FIX: Integrate missing vocabulary words into the prose or activities. Each vocab word should appear at least once in context.

## Recommendation
**📝 UPDATE** (severity 5/100)

- 1 violations (minor)

## Gates
- **Words:** ⚠️ 1179/1200 (raw: 1523) (21 short)
- **Activities:** ✅ 8/8
- **Density:** ✅ All > 10
- **Unique_types:** ✅ 7/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 4/2
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 20/1
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 14.8% (checkpoint - no gate)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review
- **Research:** ✅ Content aligned with research

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 0 | Included in Core |
| **Checkpoint: First Contact** | ➖ | 34 | Excluded Type |
| **Огляд (Overview)** | ✅ | 173 | Included in Core |
| **Навичка 1: Читання та Рід (Skill 1: Reading and Gender)** | ⚪️ | 0 | Skipped (using YAML) |
| **Навичка 2: Прикметники та Питання (Skill 2: Adjectives, Plurals and Questions)** | ✅ | 276 | Included in Core |
| **Культурний контекст: Кафе (Cultural Context: Cafe)** | ✅ | 221 | Included in Core |
| **Підсумок — Summary** | ✅ | 150 | Included in Core |