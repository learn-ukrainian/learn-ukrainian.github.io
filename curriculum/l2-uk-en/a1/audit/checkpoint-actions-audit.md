# Audit Report: M21 — checkpoint-actions.md
**Level:** A1 | **Module:** M21 | **Phase:** A1.3 | **Pedagogy:** PPP | **Target:** 1200
**Overall Status:** ✅ PASS
**Generated:** 2026-04-11 00:57:16

## Configuration
**Type:** A1
**Word Target:** 1200 words
**Activities:** 0-4 required
**Items per Activity:** ≥6 items
**Unique Types:** ≥0 types required
**Priority Types:** anagram, classify, fill-in, image-to-letter, match-up, quiz, unjumble, watch-and-repeat
**Engagement:** ≥0 callouts
**Immersion:** 0-100%
**Vocab Target:** ≥1 words
**Transliteration:** Allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | fill-in |  | 6 | 6 | ✅ |
| 2 | group-sort |  | 12 | 6 | ✅ |
| 3 | quiz |  | 10 | 6 | ✅ |
| 4 | fill-in |  | 8 | 6 | ✅ |
| 5 | true-false |  | 6 | 6 | ✅ |
| 6 | match-up |  | 6 | 6 | ✅ |
| 7 | error-correction |  | 6 | 6 | ✅ |
| 8 | translate |  | 6 | 6 | ✅ |

**Summary:**
- Total activities: 8 (target: 0-4) ❌
- Unique types: 8 (minimum: 0) ✅
- Priority types used: 3/8 (fill-in, match-up, quiz) ✅
- Low density activities: 0

## PEDAGOGICAL VIOLATIONS
- **[INFO]** Dative case used at A1: 'мені' (taught formally at A2)
  - FIX: No action needed — incidental dative exposure is acceptable.
- **[CONTENT_REDUNDANCY]** Redundant information detected in lesson (100% overlap): "Their present tense endings follow this pattern: **-ю** (or **-у**), **-иш**, **-ить**, **-имо**, **...". Shares significant keywords with sentence at index 38.
  - FIX: Remove redundant paragraphs. Ensure each section adds new unique value.
- **[ROBOTIC_STRUCTURE]** Robotic structure: 3 sentences start with 'for example,...'.
  - FIX: Vary sentence structure.

## Recommendation
**📝 UPDATE** (severity 5/100)

- 3 violations (minor)

## Gates
- **Words:** ✅ 1809/1200 (raw: 1849)
- **Activities:** ✅ 8/0
- **Density:** ✅ All > 6
- **Unique_types:** ✅ 8/0 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 1/0
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 59/1
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** ⚠️ 14.4% (target 15-35%, within tolerance (M21))
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review
- **Research:** ✅ Content aligned with research

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 337 | Included in Core |
| **Чита́ння (Reading Practice)** | ⚪️ | 0 | Skipped (using YAML) |
| **Грама́тика (Grammar Summary)** | ✅ | 505 | Included in Core |
| **Діало́г (Connected Dialogue)** | ✅ | 360 | Included in Core |
| **Підсумок — Summary** | ✅ | 254 | Included in Core |