# Audit Report: M15 — the-living-verb-i.md
**Level:** A1 | **Module:** M15 | **Phase:** A1.2 | **Pedagogy:** PPP | **Target:** 1200
**Overall Status:** ✅ PASS
**Generated:** 2026-03-09 21:49:56

## Configuration
**Type:** A1-grammar
**Word Target:** 1200 words
**Activities:** 8-12 required
**Items per Activity:** ≥6 items
**Unique Types:** ≥4 types required
**Priority Types:** anagram, classify, fill-in, image-to-letter, match-up, quiz, unjumble, watch-and-repeat
**Required Types:** fill-in, match-up
**Engagement:** ≥3 callouts
**Immersion:** 0-100%
**Vocab Target:** ≥1 words
**Transliteration:** Allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | match-up | Match the Pronoun to the Verb Form | 8 | 8 | ✅ |
| 2 | quiz | Check Your Knowledge: Meanings | 8 | 8 | ✅ |
| 3 | fill-in | Complete the Sentence with the Correct Verb Form | 8 | 8 | ✅ |
| 4 | true-false | Grammar Rules: True or False? | 8 | 8 | ✅ |
| 5 | unjumble | Put the Words in Order | 8 | 6 | ✅ |
| 6 | group-sort | Sort the Verb Forms | 12 | 6 | ✅ |
| 7 | match-up | Match the Verb to the Object | 8 | 8 | ✅ |
| 8 | fill-in | Find the Missing Pronoun | 8 | 8 | ✅ |
| 9 | quiz | Quick Translation Quiz | 8 | 8 | ✅ |

**Summary:**
- Total activities: 9 (target: 8-12) ✅
- Unique types: 6 (minimum: 4) ✅
- Priority types used: 4/8 (fill-in, match-up, quiz, unjumble) ✅
- Required types used: 2/2 (fill-in, match-up) ✅
- Low density activities: 0

## Gates
- **Words:** ✅ 1585/1200 (raw: 1770)
- **Activities:** ✅ 9/8
- **Density:** ✅ All > 6
- **Unique_types:** ✅ 6/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 3/3
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 20/1
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 28.7% (target 25-45% (M15))
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review
- **Research:** ✅ Content aligned with research

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 0 | Included in Core |
| **The Living Verb I** | ✅ | 31 | Included in Core |
| **Вступ (Introduction)** | ✅ | 221 | Included in Core |
| **Презентація (Presentation)** | ✅ | 459 | Included in Core |
| **Практика (Practice)** | ✅ | 529 | Included in Core |
| **Підсумок та культурний аспект (Summary and Cultural Insight)** | ✅ | 345 | Included in Core |