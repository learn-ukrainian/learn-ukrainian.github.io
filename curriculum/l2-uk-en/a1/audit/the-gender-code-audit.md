# Audit Report: M07 — the-gender-code.md
**Level:** A1 | **Module:** M07 | **Phase:** A1.1 | **Pedagogy:** PPP | **Target:** 1200
**Overall Status:** ❌ FAIL
**Generated:** 2026-03-19 00:26:57

## Configuration
**Type:** A1-grammar
**Word Target:** 1200 words
**Activities:** 0-4 required
**Items per Activity:** ≥6 items
**Unique Types:** ≥0 types required
**Priority Types:** anagram, classify, fill-in, image-to-letter, match-up, quiz, unjumble, watch-and-repeat
**Required Types:** fill-in, match-up, quiz
**Engagement:** ≥1 callouts
**Immersion:** 0-100%
**Vocab Target:** ≥1 words
**Transliteration:** Allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | match-up | Match Nouns to Their Meanings | 8 | 6 | ✅ |
| 2 | quiz | Identify the Gender | 8 | 6 | ✅ |
| 3 | fill-in | Choose the Correct Possessive | 8 | 6 | ✅ |
| 4 | match-up | Match the Descriptive Phrase | 8 | 6 | ✅ |
| 5 | group-sort | Sort Nouns by Gender | 15 | 6 | ✅ |
| 6 | true-false | True or False? | 8 | 6 | ✅ |
| 7 | anagram | Unscramble the Noun | 8 | 6 | ✅ |
| 8 | unjumble | Put the Words in Order | 6 | 6 | ✅ |

**Summary:**
- Total activities: 8 (target: 0-4) ❌
- Unique types: 7 (minimum: 0) ✅
- Priority types used: 5/8 (anagram, fill-in, match-up, quiz, unjumble) ✅
- Required types used: 3/3 (fill-in, match-up, quiz) ✅
- Low density activities: 0

## PEDAGOGICAL VIOLATIONS
- **[INFO]** Dative case used at A1: 'чолові' (taught formally at A2)
  - FIX: No action needed — incidental dative exposure is acceptable.
- **[INFO]** Dative case used at A1: 'чолові' (taught formally at A2)
  - FIX: No action needed — incidental dative exposure is acceptable.
- **[LEVEL_RESTRICTION]** Activity 'unjumble' not appropriate for A1 M01-M10 (current: M07)
  - FIX: A1 M01-M10 students are still learning letters. Use anagram (letter scramble) instead of unjumble (sentence reorder).
- **[ROBOTIC_STRUCTURE]** Robotic structure: 3 sentences start with 'it goes...'.
  - FIX: Vary sentence structure.

## Recommendation
**📝 UPDATE** (severity 35/100)

- 4 violations (moderate)
- Structure issue: Missing '## Summary'

## Gates
- **Words:** ✅ 1726/1200 (raw: 1754)
- **Activities:** ✅ 8/0
- **Density:** ✅ All > 6
- **Unique_types:** ✅ 7/0 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 2/1
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 24/1
- **Structure:** ❌ Missing '## Summary'
- **Lint:** ✅ Clean Format
- **Pedagogy:** ❌ 1 violations
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 10.0% (target 10-20% (M07))
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review
- **Research:** ✅ Content aligned with research

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 274 | Included in Core |
| **Презентація правил (Presentation of Rules)** | ✅ | 424 | Included in Core |
| **Практичні вправи (Practice Exercises)** | ✅ | 379 | Included in Core |
| **Самостійна робота (Independent Work/Production)** | ➖ | 317 | Excluded Type |
| **Культурний код та підсумок (Cultural Code and Summary)** | ✅ | 332 | Included in Core |