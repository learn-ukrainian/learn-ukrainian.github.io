...
3. Verify with Grep: `Grep pattern="first 5-6 words" path="/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/a1/at-the-cafe.md"`
...
1. **Content** (the lesson you're reviewing): `/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/a1/at-the-cafe.md`
...
4. **Plan** (source of truth for scope): `/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/plans/a1/at-the-cafe.yaml`
5. **Research notes** (seminar tracks only — for fact verification): `/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/a1/research/at-the-cafe-research.md`
...
unverified_terms: []
review_focus:
  - "Check that Accusative endings are explained simply and naturally for A1."
  - "Verify all required target vocabulary is used in natural contexts."
  - "Ensure no IPA or Russianisms slipped in."
rag_tools_used: []
```
...
Word count:       2008 / 1200 (167.3%)
Activities:       4
Vocabulary items: 20
Engagement boxes: 3
Immersion:        19.1% (target: 20-35%)
Richness:         71% (threshold: 60%)
...
1. **[MORPHOLOGICAL_VIOLATION]** (severity: HIGH)
   Location: ~line 28
   Text: Imperative 'Прочита́йте' (VESUM: verb:perf:impr:p:2) — imperatives not taught until M47.
   Fix: Replace 'Прочита́йте' with English instruction. E.g., use 'Remember that...' instead of Ukrainian imperatives.
2. **[MORPHOLOGICAL_VIOLATION]** (severity: HIGH)
   Location: ~line 28
   Text: Imperative 'Уяві́ть' (VESUM: verb:perf:impr:p:2) — imperatives not taught until M47.
   Fix: Replace 'Уяві́ть' with English instruction. E.g., use 'Remember that...' instead of Ukrainian imperatives.
3. **[AGREEMENT_ERROR]** (severity: HIGH)
   Location: ~line 32
   Text: Agreement mismatch: 'вели́ку' (f) + 'Вам' (p)
   Fix: Change 'вели́ку' to match the gender/case of 'Вам', or vice versa.
4. **[AGREEMENT_ERROR]** (severity: HIGH)
   Location: ~line 93
   Text: Agreement mismatch: 'дев'я́тій' (f) + 'ра́нку' (f/m)
   Fix: Change 'дев'я́тій' to match the gender/case of 'ра́нку', or vice versa.
5. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 10
   Text: Wrong stress: 'каже́' → should be 'ка́же'
   Fix: Replace 'каже́' with 'ка́же'.
6. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 13
   Text: Wrong stress: 'мо́мент' → should be 'моме́нт'
   Fix: Replace 'мо́мент' with 'моме́нт'.
7. **[STRESS_UNKNOWN]** (severity: INFO)
   Location: ~line 26
   Text: Stressed word not in dictionary: Бари́ста (бариста)
   Fix: Verify stress manually — word not found in ukrainian-word-stress dictionary.
8. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 28
   Text: Wrong stress: 'вголо́с' → should be 'вго́лос'
   Fix: Replace 'вголо́с' with 'вго́лос'.
9. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 93
   Text: Wrong stress: 'Олена́' → should be 'оле́на'
   Fix: Replace 'Олена́' with 'оле́на'.
10. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 93
   Text: Wrong stress: 'вікна́' → should be 'ві́кна'
   Fix: Replace 'вікна́' with 'ві́кна'.
11. **[PLAN_SECTION_MISSING]** (severity: HIGH)
   Location: (plan vs content)
   Text: Missing 1 plan section(s): Продукція та Підсумок (Production and Summary)
   Fix: Add content for the missing plan sections or update section headings to match plan.
12. **[ACTIVITY_VESUM_FAIL]** (severity: HIGH)
   Location: at-the-cafe.yaml
   Text: Activity answers contain VESUM-failed words: менюм, менюу, менює
...
**Words checked:** 172 | **VESUM coverage:** 167/172 (97.1%)

**❌ Not found in VESUM or textbooks (4):**
- `Львові` (source: prose)
- `менюм` (source: activities)
- `менюу` (source: activities)
- `менює` (source: activities)
