...
> **Your #1 job: Write 315 words for the section "Вступ: Простір та Місце (Де?)".**
...
Write the section prose for **Вступ: Простір та Місце (Де?)** within the module **Prepositions III**.
...
### Rhetorical Openers Already Used (DO NOT REPEAT)

These openers/patterns have been used in previous sections: None yet

**Do NOT start paragraphs with these phrases again.** Vary your rhetorical toolkit:
- Instead of repeating «Уявіть собі», try: a question, a comparison, a direct statement, a historical anecdote
- Instead of repeating «Це не просто X, а Y», try: «X виконує функцію Y» or just state the fact directly
- Each section should feel like a different voice within the same author
...
```
===WORD_COUNTS===
Section "Вступ: Простір та Місце (Де?)": {count} words (budget: 315-315×2)
===WORD_COUNTS===
```
...
```
===FRICTION_START===
**Phase**: Phase 2: Section Content (Вступ: Простір та Місце (Де?))
