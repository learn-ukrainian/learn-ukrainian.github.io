...
unverified_terms: []
review_focus:
  - "Check that natural gender override explanations are clear to L2 learners."
  - "Verify strict adherence to A1 grammar constraints."
rag_tools_used:
  - "read_file: extracted textbook references from the research notes without needing external API calls."
```
...
Word count:       1477 / 1200 (123.1%)
Activities:       4
Vocabulary items: 20
Engagement boxes: 0
Immersion:        8.1% (target: 10-20%)
Richness:         61% (threshold: 60%)
Richness gaps:    engagement: 0/2, examples: 7/8, video_embeds: 0/2
Audit status:     FAIL
```
...
2. **[ACTIVITY_VESUM_FAIL]** (severity: HIGH)
   Location: the-gender-code.yaml
   Text: Activity answers contain VESUM-failed words: ий, ій
