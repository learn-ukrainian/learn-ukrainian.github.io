...
| Module persona | Patient Supportive Tutor, acting as Sports Commentator |
| Activities required | 6–10 |
| Items per activity | ≥12 |
| Required types | quiz, fill-in, match-up |
| Priority types | anagram, unjumble |
| Vocabulary items | 20 |
...
```
curriculum/l2-uk-en/a1/the-living-verb-i.md
```

**Plan file** (vocabulary_hints — vocabulary list to follow):
```
curriculum/l2-uk-en/plans/a1/the-living-verb-i.yaml
```

**Meta file** (activity count targets, pedagogy):
```
curriculum/l2-uk-en/a1/meta/the-living-verb-i.yaml
```
...
4. **Activity count**: 8 activities (4-9 for seminar tracks)
...
**ALLOWED types (use ONLY these):** quiz, fill-in, match-up, anagram, unjumble, mark-the-words, reorder

**FORBIDDEN types (audit will auto-FAIL if you use these):** cloze
...
**Count your activities before outputting.** You MUST generate 8 activities. Previous rebuilds consistently underproduced (8 activities when 12-16 were needed). If under target, ADD MORE activities before submitting. Types to add when short: quiz (8+ items), fill-in (8+ items), match-up (8+ pairs).
