...
3. Verify with Grep: `Grep pattern="first 5-6 words" path="/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/a1/weather-and-nature.md"`
...
1. **Content** (the lesson you're reviewing): `/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/a1/weather-and-nature.md`
2. **Activities**: `/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/a1/activities/weather-and-nature.yaml`
3. **Vocabulary**: `/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/a1/vocabulary/weather-and-nature.yaml`
4. **Plan** (source of truth for scope): `/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/plans/a1/weather-and-nature.yaml`
5. **Research notes** (seminar tracks only — for fact verification): `/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/a1/research/weather-and-nature-research.md`
...
unverified_terms: []
review_focus:
  - "Check that all target vocabulary words are naturally integrated."
  - "Verify the use of impersonal adverbs versus literal translations."
  - "Ensure no disallowed verb forms or grammar structures were used (strictly A1.4 limits)."
rag_tools_used:
  - "local_knowledge: Used verified curriculum vocabulary and known stress rules from Goroh definitions"
```
...
Vocabulary items: 20
Engagement boxes: 2
Immersion:        21.8% (target: 20-35%)
Richness:         92% (threshold: 60%)
Richness gaps:    none
Audit status:     FAIL
```
...
2. Погода та безособові форми (Weather & Impersonal Forms)
3. Пори року та природа (Seasons & Nature)
4. Складні речення та прогноз (Complex Sentences & Forecast)
...
1. **[LLM_FILLER]** (severity: MEDIUM)
   Location: ~line 13
   Text: Let's explore (2x)
   Fix: Rephrase -- this phrase appears repeatedly (LLM pattern)
...
3. **[MORPHOLOGICAL_VIOLATION]** (severity: HIGH)
   Location: ~line 124
   Text: Imperative 'Візьмі́ть' (VESUM: verb:perf:impr:p:2) — imperatives not taught until M47.
   Fix: Replace 'Візьмі́ть' with English instruction. E.g., use 'Remember that...' instead of Ukrainian imperatives.
4. **[MORPHOLOGICAL_VIOLATION]** (severity: HIGH)
   Location: ~line 130
   Text: Imperative 'Запам'ята́йте' (VESUM: verb:perf:impr:p:2) — imperatives not taught until M47.
   Fix: Replace 'Запам'ята́йте' with English instruction. E.g., use 'Remember that...' instead of Ukrainian imperatives.
5. **[AGREEMENT_ERROR]** (severity: HIGH)
   Location: ~line 93
   Text: Agreement mismatch: 'га́рна' (f) + 'ро́ку' (m)
   Fix: Change 'га́рна' to match the gender/case of 'ро́ку', or vice versa.
6. **[STRESS_UNKNOWN]** (severity: INFO)
   Location: ~line 11
   Text: Stressed word not in dictionary: па́рку (парку)
   Fix: Verify stress manually — word not found in ukrainian-word-stress dictionary.
7. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 11
   Text: Wrong stress: 'зби́рають' → should be 'збира́ють'
   Fix: Replace 'зби́рають' with 'збира́ють'.
8. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 49
   Text: Wrong stress: 'га́рячий' → should be 'гаря́чий'
   Fix: Replace 'га́рячий' with 'гаря́чий'.
9. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 93
   Text: Wrong stress: 'чоти́рьох' → should be 'чотирьо́х'
   Fix: Replace 'чоти́рьох' with 'чотирьо́х'.
10. **[STRESS_UNKNOWN]** (severity: INFO)
   Location: ~line 93
   Text: Stressed word not in dictionary: по́рів (порів)
   Fix: Verify stress manually — word not found in ukrainian-word-stress dictionary.
11. **[STRESS_UNKNOWN]** (severity: INFO)
   Location: ~line 93
   Text: Stressed word not in dictionary: прокида́ється (прокидається)
   Fix: Verify stress manually — word not found in ukrainian-word-stress dictionary.
12. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 93
   Text: Wrong stress: 'де́рева' → should be 'дере́ва'
   Fix: Replace 'де́рева' with 'дере́ва'.
13. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 93
   Text: Wrong stress: 'пта́хи' → should be 'птахи́'
   Fix: Replace 'пта́хи' with 'птахи́'.
14. **[STRESS_UNKNOWN]** (severity: INFO)
   Location: ~line 93
   Text: Stressed word not in dictionary: лі́стя (лістя)
   Fix: Verify stress manually — word not found in ukrainian-word-stress dictionary.
15. **[STRESS_UNKNOWN]** (severity: INFO)
   Location: ~line 93
   Text: Stressed word not in dictionary: пора́ (пора)
   Fix: Verify stress manually — word not found in ukrainian-word-stress dictionary.
16. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 124
   Text: Wrong stress: 'ви́хідні' → should be 'вихідні́'
   Fix: Replace 'ви́хідні' with 'вихідні́'.
17. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 124
   Text: Wrong stress: 'Темпера́тура' → should be 'температу́ра'
   Fix: Replace 'Темпера́тура' with 'температу́ра'.
18. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 124
   Text: Wrong stress: 'Мо́жливий' → should be 'можли́вий'
   Fix: Replace 'Мо́жливий' with 'можли́вий'.
19. **[STRESS_MISMATCH]** (severity: HIGH)
   Location: ~line 124
   Text: Wrong stress: 'пара́сольку' → should be 'парасо́льку'
   Fix: Replace 'пара́сольку' with 'парасо́льку'.
...
**Words checked:** 165 | **VESUM coverage:** 162/165 (98.2%)
...
- "Let's explore (2x)" at ~line 13
- "let's explore (2x)" at ~line 74
