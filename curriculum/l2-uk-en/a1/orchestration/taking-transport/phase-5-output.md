I will locate and read the module files to ensure I am reviewing the current state of the content on disk, as instructed.
I will use the `glob` tool to locate the module files since `run_shell_command` is not available.
I will read the content, activities, vocabulary, plan, and metadata files to perform the review.
===REVIEW_START===
