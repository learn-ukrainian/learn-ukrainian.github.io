...
deviations:
  - section: "Типові наголоси — Common Stress Patterns"
    reason: "The outline specified 'далеко' and 'Україна' as having last-syllable stress. I included them under the last-syllable category but added a pedagogical note that standard dictionaries list them with penultimate stress (дале́ко, Украї́на) to maintain factual linguistic accuracy while satisfying the outline constraint."
frictions: []
research_gaps: []
unverified_terms: []
review_focus:
  - "Check that the handling of the 'далеко/Україна' stress contradiction is pedagogically sound."
  - "Verify strict adherence to VESUM-verified vocabulary for all fill-in distractors."
rag_tools_used: []
```
...
Word count:       1878 / 1200 (156.5%)
Activities:       4
Vocabulary items: 21
Engagement boxes: 2
Immersion:        9.6% (target: 5-15%)
...
1. **[AGREEMENT_ERROR]** (severity: HIGH)
   Location: ~line 20
   Text: Agreement mismatch: 'доро́га' (f) + 'мі́сто' (n)
   Fix: Change 'доро́га' to match the gender/case of 'мі́сто', or vice versa.
...
**Words checked:** 80 | **VESUM coverage:** 71/80 (88.8%)

**❌ Not found in VESUM or textbooks (8):**
- `ва` (source: activities)
- `да` (source: activities)
- `ка` (source: activities)
- `книж` (source: activities)
- `ло` (source: activities)
- `ру` (source: activities)
- `сес` (source: activities)
- `шко` (source: activities)
