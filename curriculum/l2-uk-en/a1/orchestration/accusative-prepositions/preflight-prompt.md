...
|------|----------------|
| `/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/a1/research/accusative-prepositions-research.md` | Background knowledge, engagement hooks |
| `/Users/krisztiankoos/projects/learn-ukrainian/curriculum/l2-uk-en/plans/a1/accusative-prepositions.yaml` | Objectives, vocabulary_hints (source of truth) |
...
| Tool | When | Example |
|------|------|---------|
| `search_text` | Find textbook pedagogy | `search_text("Accusative prepositions (в/у, на, за, через, про) Direction expressions with Accusative", grade=3-5)` |
...
**Modules completed before this one:** 27
**Previous module:** The Accusative II: People

**Cumulative vocabulary (353 words):**
...
братик, сестричка, знайомий

**Grammar already taught (89 topics):**
...
**Coming next (module after this):** Locative case endings, Prepositions в/у and на, Location expressions
...
- магазин (shop) — іду в магазин, у магазині; Direction context
- пошта (post office) — іду на пошту, на пошті; на + Acc direction
- концерт (concert) — їду на концерт; Event destination
- допомога (help) — дякую за допомогу; За + Acc pattern
- година (hour) — за годину, через годину; Time expressions
- парк (park) — через парк, у парку; Spatial preposition context
- через (through/because of) — через парк, через дощ; Preposition
- про (about) — книга про історію, думати про; Preposition
- за (for/behind) — дякую за, за будинок; Preposition

**Recommended** (use in your content to reach the vocabulary target):
- площа (square) — на площу, на площі; Direction/location pair
- острів (island) — на острів, на острові; на + Acc direction
- стадіон (stadium) — на стадіон, на стадіоні; Event venue
- пляж (beach) — на пляж, на пляжі; на + Acc direction
...
- **Dobra Forma: Accusative Case after Prepositions в/у, на and про** (dobraforma)
  URL: https://opentext.ku.edu/dobraforma/chapter/8-2/
  Relevance: 0.7
  Topics: accusative, cases, prepositions, grammar
...
чень. 4. Створіть плакат «Порада другові» та складіть невеликий текс...
...
Write **Accusative Prepositions** for the a1 track.
...
- `## Куди? з Acc (Direction with Acc)` (~300 words)
  - Прийменник в/у + Знахідний для напрямку руху: Я іду в магазин, Я їду у Київ. Евфонічний вибір в/у залежно від фонетичного контексту (§4.2.3.2).
  - Прийменник на + Знахідний для поверхонь та подій: Я іду на пошту, Я їду на концерт, Я кладу на стіл. Контраст в/у (enclosed space) vs на (open/surface/event).
- `## За + Acc (Behind/For)` (~250 words)
  - За + Знахідний у значенні «за/for/in exchange»: Дякую за допомогу. Платити за квиток. Вибачте за запізнення.
  - За + Знахідний у часовому значенні «через (проміжок часу)»: за годину (in an hour), за тиждень (in a week). За + Знахідний для напрямку «за»: зайти за будинок (go behind the building).
- `## Через + Acc (Through/Because of)` (~200 words)
  - Просторове значення: через парк (through the park), через дорогу (across the road), через міст (across the bridge).
  - Часове та причинне значення: через годину (in an hour), через дощ (because of rain), через роботу (because of work). Контраст із «за + Acc» у часовому значенні.
- `## Про + Acc (About)` (~200 words)
  - Про + Знахідний у значенні «про/about»: розповідати про Україну, думати про маму, книга про історію, фільм про кохання.
  - Типові дієслова з «про»: говорити про, думати про, читати про, знати про, забувати про. Конструкція «Про що?» як питання.
- `## Практика (Practice)` (~250 words)
  - Вправи на вибір правильного прийменника залежно від контексту: напрямок (в/у/на), обмін/подяка (за), простір/причина (через), тема (про).
  - Діалоги про напрямки руху та щоденні ситуації з використанням усіх чотирьох прийменників. Контрастні вправи.
...
| Section | Minimum |
|---------|---------|
| Куди? з Acc (Direction with Acc) | 300+ |
| За + Acc (Behind/For) | 250+ |
| Через + Acc (Through/Because of) | 200+ |
| Про + Acc (About) | 200+ |
...
### Workflow
1. **Research first**: `search_text("Accusative prepositions (в/у, на, за, через, про) Direction expressions with Accusative", grade=3-5)` — find how textbooks teach this
