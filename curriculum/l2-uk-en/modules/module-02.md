---
module: 2
title: "The Cyrillic Code II"
subtitle: "Completing the Alphabet"
level: A1
phase: A1.1
duration: 45
transliteration: full
tags: [alphabet, basics, cyrillic, iotated, cursive]
objectives:
  - Learn the remaining 15 Cyrillic letters
  - Recognize Ukraine's unique letters (Ї, Є, Ґ)
  - Understand the soft sign (Ь) function
  - Be aware of cursive/italic letter traps
grammar:
  - Cyrillic alphabet groups 4-6 (15 letters)
  - Iotated vowels
  - Soft sign function
  - Print vs cursive awareness
---

# Lesson Content

## warm-up

### Completing Your Alphabet

In Module 01, you learned 18 letters. Now let's complete the Ukrainian alphabet with the remaining 15 letters. After this module, you'll be able to read ANY Ukrainian word!

Today's letters include some special ones that make Ukrainian unique - letters that don't exist in Russian or other Slavic languages.

> 🔍 **Myth Buster: "Ukrainian is a Russian Dialect"**
>
> Some people claim Ukrainian is just "a dialect of Russian." This is **completely false** - and politically charged misinformation. Ukrainian and Russian are separate languages that split from a common ancestor about 1,000 years ago. They have:
> - Different sounds and phonology
> - Different grammar rules
> - Different vocabulary (only ~60% lexical similarity)
> - **Different letters** - Ukrainian has Ї, Є, Ґ that Russian lacks!
>
> Would you call Spanish a "dialect of Italian"? Same logic applies here.

## presentation

### Group 4: Unique Consonants (7 letters)

These consonants have sounds that English speakers need to practice:

| Letter | Sound | English approximation |
|--------|-------|----------------------|
| Г г | H | "H" in "hello" (soft, breathy) |
| Ґ ґ | G | "G" in "go" (hard g - rare in Ukrainian!) |
| Ж ж | ZH | "S" in "pleasure" or "vision" |
| Ш ш | SH | "SH" in "ship" |
| Щ щ | SHCH | "SH+CH" combined (fresh cheese) |
| Ч ч | CH | "CH" in "church" |
| Ц ц | TS | "TS" in "cats" |

> 💡 **Did You Know?**
>
> The letter **Ґ** (hard G) has a fascinating history! It was **banned** during the Soviet era as part of Russification policies. After Ukrainian independence in 1991, it was officially restored. Today, words with Ґ like **ґанок** (porch) and **ґудзик** (button) are markers of authentic Ukrainian spelling. When you see Ґ, you're seeing a letter that survived cultural suppression!

> ⚡ **Pro Tip: Г vs Ґ**
>
> - **Г** = soft H sound (like "hello") - VERY common
> - **Ґ** = hard G sound (like "go") - rare, mostly in borrowed words
>
> Russian has only Г (pronounced as G), so Russians often mispronounce Ukrainian Г. This is a telltale accent marker!

### Group 5: Iotated Vowels (4 letters)

These vowels start with a "Y" sound (like "yes"):

| Letter | Sound | Example |
|--------|-------|---------|
| Є є | YE | "YE" in "yes" |
| Ї ї | YI | "YEE" (yi as in "yeast") |
| Ю ю | YU | "YU" in "universe" |
| Я я | YA | "YA" in "yard" |

> 📜 **History Bite: The Iotated Vowels**
>
> The term "iotated" comes from the Greek letter **iota** (ι). These vowels combine a "y" glide (called "iot") with a regular vowel:
> - Є = Й + Е (y + e)
> - Ї = Й + І (y + i)
> - Ю = Й + У (y + u)
> - Я = Й + А (y + a)
>
> After a consonant, they soften that consonant instead of adding "y". This is key to Ukrainian pronunciation!

> 🎯 **Fun Fact: Ї - Ukraine's Signature Letter**
>
> The letter **Ї** exists ONLY in Ukrainian - no other language in the world uses it! It's so iconic that there's a famous Ukrainian cultural magazine called "Ї" (Yi). When you see this letter, you know 100% you're reading Ukrainian, not Russian, Bulgarian, or any other Cyrillic language.

> 🎬 **Pop Culture Moment: "I am Groot"**
>
> In Ukrainian, Groot would say: **Я є Ґрут** (Ya ye Grut). Notice the letters you just learned:
> - **Я** (ya) - iotated vowel for "I"
> - **є** (ye) - iotated vowel meaning "am/is"
> - **Ґ** (g) - Ukraine's rare hard G!
>
> Three new letters in one iconic phrase. You're already reading real Ukrainian!

### Group 6: Special Letters (4 letters)

| Letter | Sound | Function |
|--------|-------|----------|
| И и | Y (short) | Like "i" in "bit" (not "ee"!) |
| Й й | Y (consonant) | Like "y" in "yes" |
| Ь ь | — (silent) | Softens the previous consonant |
| ' | — (pause) | Apostrophe - separates sounds |

> 🔗 **Language Link: И vs І vs Ї**
>
> Ukrainian has THREE "i" sounds - confusing at first, but logical:
> - **І** = "ee" as in "m**ee**t" (True Friend from Module 01)
> - **И** = short "i" as in "b**i**t" (no English equivalent - between "i" and "u")
> - **Ї** = "yee" as in "**yea**st" (y + ee combined)
>
> Listen carefully to native speakers - these are distinct sounds!

> 💡 **Did You Know? The Silent Letter**
>
> The **soft sign (Ь)** makes no sound by itself! Instead, it **changes** the consonant before it:
> - **кон** (horse) → **кінь** (different horse form, softer ending)
> - **мат** (checkmate) → **мать** (mother - archaic)
>
> Think of Ь as an instruction: "soften what came before me!"

## practice

### The Cursive Trap

**WARNING:** Ukrainian cursive (italic) letters can look VERY different from print!

| Print | Cursive | Trap Level |
|-------|---------|------------|
| т | m-like | ⚠️ HIGH - looks like Latin "m"! |
| п | n-like | ⚠️ HIGH - looks like Latin "n"! |
| д | g-like | ⚠️ HIGH - looks like Latin "g"! |
| и | u-like | ⚠️ MEDIUM - looks like Latin "u" |
| л | hook | ⚠️ MEDIUM - connected differently |
| г | ∂-like | ⚠️ MEDIUM - loops differently |

> 🎭 **Culture Corner: Handwriting in Ukraine**
>
> Ukrainians learn cursive writing in first grade and use it throughout life. Official documents, signatures, and handwritten notes are almost always in cursive. If you ever receive a handwritten letter from Ukraine, remember: that "m" is probably **т**, that "n" is probably **п**, and that "g" is probably **д**!

### Reading Practice

Now try reading words with ALL 33 letters:

**Basic words:**
- УКРАЇНА → U-K-R-A-YI-N-A → Ukraine
- КИЇВ → K-Y-YI-V → Kyiv
- МОВА → M-O-V-A → language
- ДЯКУЮ → D-YA-K-U-YU → thank you

**Phrases:**
- ДОБРЕ → D-O-B-R-E → good/okay
- БУДЬ ЛАСКА → BUD' LAS-KA → please
- ДО ПОБАЧЕННЯ → DO PO-BA-CHEN-NYA → goodbye

> 🌍 **Real World: Your First Ukrainian Phrases**
>
> You just learned some of the most useful Ukrainian phrases:
> - **Привіт!** (pryvit) - Hi!
> - **Дякую!** (dyakuyu) - Thank you!
> - **Будь ласка** (bud' laska) - Please / You're welcome
> - **Так** (tak) - Yes
> - **Ні** (ni) - No
>
> With just these words, you can already be polite in Ukraine!

## production

### Complete Alphabet Challenge

You now know all 33 letters! Here's the full Ukrainian alphabet:

**А Б В Г Ґ Д Е Є Ж З И І Ї Й К Л М Н О П Р С Т У Ф Х Ц Ч Ш Щ Ь Ю Я**

Read these words that use letters from both modules:

1. ШКОЛА (shkola) - school
2. ЛЮДИНА (lyudyna) - person
3. ЇЖА (yizha) - food
4. ЩАСТЯ (shchastya) - happiness
5. ЦЕНТР (tsentr) - center

### Alphabet Song Order

The Ukrainian alphabet has a specific order (like ABC in English). Practice saying:

А, Б, В, Г, Ґ, Д, Е, Є, Ж, З, И, І, Ї, Й, К, Л, М, Н, О, П, Р, С, Т, У, Ф, Х, Ц, Ч, Ш, Щ, Ь, Ю, Я

> 🎯 **Fun Fact: No Ъ in Ukrainian!**
>
> Russian has a "hard sign" (Ъ), but Ukrainian doesn't! Ukrainian uses an apostrophe (') instead to separate sounds. This is another easy way to tell Ukrainian from Russian text - if you see Ъ, it's not Ukrainian!

---

## Euphony Preview: І vs Й

### Why Ukrainian Sounds So Melodic

Ukrainian has built-in rules that make words flow together smoothly. One simple rule you can use right now:

**The word "and" has two forms: і and й**

**Rule:** Use **й** after vowels, **і** after consonants.

| Context | Correct | Why |
|---------|---------|-----|
| мама **й** тато | "й" after vowel "а" flows smoothly |
| хліб **і** сіль | "і" after consonant "б" is easier to say |
| кава **й** чай | "й" after vowel "а" |
| борщ **і** каша | "і" after consonant "щ" |

> 💡 **Memory Trick**
>
> Think of **й** as the "lazy" version of **і**. After a vowel, your mouth is already open, so it takes the shortcut "й". After a consonant, you need the full "і" to open up the sound again.

**More euphony rules** (у/в, з/із/зі) will be covered in later modules when you learn the grammar they require.

---

# Activities

## fill-in: Ukrainian Sounds & Letters

> Test your knowledge of Ukrainian letter sounds and unique features.

1. The letter **Ї** (unique to Ukrainian) makes the sound ___.
   > [!answer] [yi] like "yeast"
   > [!options] [i] like "machine" | [yi] like "yeast" | [y] like "yes" | [ee] like "see"

2. The letter **Г** in Ukrainian is pronounced as ___.
   > [!answer] soft [h] like "hello"
   > [!options] hard [g] like "go" | soft [h] like "hello" | [gh] like Arabic | silent

3. The letter **Ґ** (restored after independence) sounds like ___.
   > [!answer] hard [g] like "go"
   > [!options] soft [h] like "hello" | hard [g] like "go" | [k] like "cat" | [ng] like "sing"

4. The letter combination **Щ** is pronounced as ___.
   > [!answer] [shch] like "fresh cheese"
   > [!options] [sh] like "ship" | [ch] like "church" | [shch] like "fresh cheese" | [sk] like "sky"

5. The iotated vowel **Є** sounds like ___.
   > [!answer] [ye] like "yes"
   > [!options] [e] like "bed" | [ye] like "yes" | [ay] like "day" | [ee] like "see"

6. The soft sign **Ь** functions to ___.
   > [!answer] soften the preceding consonant
   > [!options] add a vowel sound | soften the preceding consonant | indicate stress | separate syllables

7. Which letter does NOT exist in Russian but exists in Ukrainian?
   > [!answer] Ї
   > [!options] А | О | Ї | К

8. In the word "Україна", the letter Ї is pronounced ___.
   > [!answer] as [yi] - two sounds
   > [!options] as [i] - one sound | as [yi] - two sounds | silently | as [y] only

9. The letter **Ж** sounds like ___.
   > [!answer] [zh] like "vision"
   > [!options] [z] like "zoo" | [zh] like "vision" | [j] like "jam" | [sh] like "ship"

10. The letter **Ц** sounds like ___.
    > [!answer] [ts] like "cats"
    > [!options] [s] like "sun" | [k] like "cat" | [ts] like "cats" | [ch] like "church"

11. The letter **Ч** sounds like ___.
    > [!answer] [ch] like "church"
    > [!options] [sh] like "ship" | [ch] like "church" | [k] like "cat" | [ts] like "cats"

12. The iotated vowel **Ю** sounds like ___.
    > [!answer] [yu] like "universe"
    > [!options] [u] like "moon" | [yu] like "universe" | [oo] like "book" | [y] like "yes"

## unjumble: Build the Sentence

> Arrange the words in the correct order. Sentences have 5-7 words.

1. мова / Це / українська / є / і
   > [!answer] Це є українська мова.
   > (This is the Ukrainian language.) [4 words]

2. моя / сім'я / Це / мама / й / тато
   > [!answer] Це моя сім'я: мама й тато.
   > (This is my family: mom and dad.) [6 words]

3. є / Тут / борщ / і / кава / чай
   > [!answer] Тут є борщ, кава і чай.
   > (Here is borscht, coffee, and tea.) [6 words]

4. є / місто / Київ / Україна / і
   > [!answer] Київ є місто і Україна.
   > (Kyiv is a city and Ukraine.) [5 words]

5. мама / Це / моя / вона / і / жінка
   > [!answer] Це моя мама і вона жінка.
   > (This is my mom and she is a woman.) [6 words]

6. тато / мій / Це / і / він / чоловік
   > [!answer] Це мій тато і він чоловік.
   > (This is my dad and he is a man.) [6 words]

7. чай / є / і / кава / Тут / там
   > [!answer] Тут є чай і там кава.
   > (Here is tea and there is coffee.) [6 words]

8. є / Там / школа / і / центр
   > [!answer] Там є школа і центр.
   > (There is a school and center.) [5 words]

9. добре / Дякую / все / у / мене
   > [!answer] Дякую, у мене все добре.
   > (Thanks, everything is fine with me.) [5 words]

10. чай / й / Тут / є / кава
    > [!answer] Тут є кава й чай.
    > (Here is coffee and tea.) [5 words]

11. моя / Це / школа / тут / і / вона
    > [!answer] Це моя школа і вона тут.
    > (This is my school and it is here.) [6 words]

12. тато / мама / Там / і / є
    > [!answer] Там є мама і тато.
    > (There are mom and dad.) [5 words]

## match-up: Match the Pairs

> Match Ukrainian words to their English meanings.

| Left | Right |
|------|-------|
| чай | tea |
| кава | coffee |
| борщ | borscht |
| школа | school |
| місто | city |
| сім'я | family |
| жінка | woman |
| чоловік | man |
| дякую | thank you |
| привіт | hi, hello |
| мова | language |
| ім'я | name |
| буква | letter (alphabet) |
| слово | word |

## group-sort: Letter Sound Categories

> Sort these Ukrainian letters by their sound type.

### Unique Consonants (New sounds)
- Г (soft H like "hello")
- Ґ (hard G like "go")
- Ж (ZH like "vision")
- Ш (SH like "ship")
- Щ (SHCH like "fresh cheese")
- Ч (CH like "church")
- Ц (TS like "cats")

### Iotated Vowels (Y + vowel)
- Є (YE like "yes")
- Ї (YI like "yeast")
- Ю (YU like "universe")
- Я (YA like "yard")

### Special Letters
- И (short I, between "bit" and "but")
- Й (consonant Y like "yes")
- Ь (soft sign - silent, softens consonant)

## quiz: Ukrainian Alphabet & Phonetics

> Choose the correct answer.

1. The letter **Ї** (unique to Ukrainian) is pronounced as:
   - [ ] Single [i] sound like "meet"
   - [x] Double sound [yi] like "yeast"
   - [ ] Short [i] like "bit"
   - [ ] Silent
   > Ї = iotated vowel, always two sounds: Y + I

2. What distinguishes **Г** from **Ґ**?
   - [ ] Г is hard G, Ґ is soft H
   - [x] Г is soft H, Ґ is hard G
   - [ ] They are the same sound
   - [ ] Г is voiced, Ґ is unvoiced
   > Г = soft H (hello), Ґ = hard G (go) - Ґ was banned during Soviet era

3. The soft sign **Ь** functions to:
   - [ ] Add a vowel sound
   - [ ] Mark stress position
   - [x] Soften the preceding consonant
   - [ ] Separate syllables
   > Ь makes no sound - it modifies the consonant before it

4. Which statement about iotated vowels is correct?
   - [ ] They are pronounced as single sounds
   - [x] They combine a Y-glide with a vowel (Є=ye, Ю=yu, Я=ya)
   - [ ] They only exist in Russian
   - [ ] They are silent in some positions
   > Iotated = "iota" (Greek) = Y-sound + vowel

5. The letter **И** sounds like:
   - [ ] Long "ee" as in "meet"
   - [x] Short "i" as in "bit" (between i and u)
   - [ ] "Y" as in "yes"
   - [ ] "A" as in "cat"
   > И is unique - between English "i" and "u", not found in English

6. How is **Щ** pronounced?
   - [ ] [sh] like "ship"
   - [ ] [ch] like "church"
   - [x] [shch] like "fresh cheese"
   - [ ] [sk] like "sky"
   > Щ = Ш + Ч combined = "shch"

7. What is the apostrophe (') used for in Ukrainian?
   - [ ] To indicate stress
   - [ ] To mark possessives
   - [x] To separate sounds (similar to Russian Ъ)
   - [ ] To soften consonants
   > Ukrainian uses ' instead of Russian's hard sign Ъ

8. In cursive, Ukrainian **т** looks like:
   - [x] Latin "m"
   - [ ] Latin "n"
   - [ ] Latin "g"
   - [ ] Latin "u"
   > Cursive trap: т→m, п→n, д→g - major reading challenge!

9. Why is **Ґ** called a "survivor letter"?
   - [ ] It's the oldest Cyrillic letter
   - [x] It was banned during Soviet Russification and restored after independence
   - [ ] It survived from Greek alphabet
   - [ ] It's rarely used
   > Ґ was banned 1933-1991 as part of suppressing Ukrainian identity

10. The letter **Й** is classified as:
    - [x] A consonant (Y-sound in "yes")
    - [ ] A vowel
    - [ ] A soft sign
    - [ ] An iotated letter
    > Й = consonantal Y, used at word endings (край) and in diphthongs

11. How many letters are in the Ukrainian alphabet?
    - [ ] 26 (same as English)
    - [ ] 30
    - [x] 33
    - [ ] 36
    > Ukrainian has 33 letters - you learned all of them in Modules 1-2!

12. The letter **Ш** sounds like:
    - [x] [sh] like "ship"
    - [ ] [s] like "sun"
    - [ ] [ch] like "church"
    - [ ] [zh] like "vision"
    > Ш = [sh], always a consistent "sh" sound

## true-false: Alphabet Rules

> Determine if each statement about Ukrainian letters is true or false.

- [x] The letter Ї exists only in Ukrainian, not in Russian or other Slavic languages.
   > Correct! Ї is Ukraine's unique signature letter.

- [ ] The letters Г and Ґ are pronounced the same way.
   > Incorrect. Г = soft H (hello), Ґ = hard G (go).

- [x] Iotated vowels (Є, Ї, Ю, Я) all start with a Y-glide sound.
   > Correct! They combine Y + vowel: ye, yi, yu, ya.

- [ ] The soft sign (Ь) is pronounced as a short vowel.
   > Incorrect. Ь is silent - it softens the preceding consonant.

- [x] Ukrainian uses an apostrophe (') instead of the Russian hard sign (Ъ).
   > Correct! This is one way to identify Ukrainian vs Russian text.

- [ ] The letter И sounds exactly like English "ee" in "meet."
   > Incorrect. И is a short sound between "i" and "u" - unique to Ukrainian.

- [x] In cursive writing, Ukrainian т looks like Latin m.
   > Correct! This is a major "trap" for learners reading handwriting.

- [x] The letter Ґ was restored to Ukrainian after independence in 1991.
   > Correct! It was banned during Soviet Russification (1933-1991).

- [ ] The letters Ш and Щ are pronounced identically.
   > Incorrect. Ш = [sh], Щ = [shch] (two sounds combined).

- [x] Ukrainian has 33 letters in its alphabet.
   > Correct! More than English (26) but same as Russian (33).

- [x] The letter Ц makes a "ts" sound like in "cats."
   > Correct! Ц = [ts], a single letter for a common sound combination.

- [ ] The letter Й is a vowel in Ukrainian.
   > Incorrect. Й is a consonant (the "y" sound in "yes"), not a vowel.

## fill-in: Euphony - І vs Й

> Choose the correct form: **і** or **й**. Remember: **й** after vowels, **і** after consonants!

1. мама ___ тато (mom and dad)
   > [!answer] й
   > [!options] і | й

2. хліб ___ сіль (bread and salt)
   > [!answer] і
   > [!options] і | й

3. кава ___ чай (coffee and tea)
   > [!answer] й
   > [!options] і | й

4. борщ ___ вареники (borscht and varenyky)
   > [!answer] і
   > [!options] і | й

5. сестра ___ брат (sister and brother)
   > [!answer] й
   > [!options] і | й

6. Київ ___ Львів (Kyiv and Lviv)
   > [!answer] і
   > [!options] і | й

7. жінка ___ чоловік (woman and man)
   > [!answer] й
   > [!options] і | й

8. суп ___ салат (soup and salad)
   > [!answer] і
   > [!options] і | й

9. школа ___ університет (school and university)
   > [!answer] й
   > [!options] і | й

10. місто ___ село (city and village)
    > [!answer] й
    > [!options] і | й

11. день ___ ніч (day and night)
    > [!answer] і
    > [!options] і | й

12. сонце ___ місяць (sun and moon)
    > [!answer] й
    > [!options] і | й

## match-up: Ukrainian Phrases

> Match Ukrainian phrases and words to their English meanings.

| Left | Right |
|------|-------|
| дякую | thank you |
| будь ласка | please |
| привіт | hi, hello |
| до побачення | goodbye |
| вибачте | excuse me |
| прошу | you're welcome |
| ласкаво просимо | welcome |
| так | yes |
| ні | no |
| добре | good, okay |
| що | what |
| як | how |

---

# Vocabulary

| Word | IPA | English | POS | Gender | Note |
|------|-----|---------|-----|--------|------|
| буква | /ˈbukvɑ/ | letter (of alphabet) | noun | f | Б, У - uses new letters |
| слово | /ˈslɔvɔ/ | word | noun | n | Basic vocabulary |
| алфавіт | /ɑlfɑˈvʲit/ | alphabet | noun | m | Also: абетка (traditional) |
| мова | /ˈmɔvɑ/ | language | noun | f | Українська мова = Ukrainian language |
| українська | /ukrɑˈjinʲsʲkɑ/ | Ukrainian (adj) | adj | f | Uses Ї! |
| англійська | /ɑnˈɦlʲijsʲkɑ/ | English (adj) | adj | f | Г = H sound |
| так | /tɑk/ | yes | particle | - | Essential word |
| ні | /nʲi/ | no | particle | - | Essential word |
| добре | /ˈdɔbrɛ/ | good, okay | adv | - | Common response |
| дякую | /ˈdʲakuju/ | thank you | verb | - | Я, Ю = iotated |
| будь ласка | /budʲ ˈlɑskɑ/ | please | phrase | - | Ь = soft sign |
| привіт | /prɪˈvʲit/ | hi, hello | interj | - | Informal greeting |
| до побачення | /dɔ pɔˈbɑtʃɛnʲːɑ/ | goodbye | phrase | - | Ч = CH |
| вибачте | /vɪˈbɑtʃtɛ/ | excuse me, sorry | verb | - | Ч = CH |
| прошу | /ˈprɔʃu/ | please/you're welcome | verb | - | Ш = SH |
| ласкаво просимо | /ˈlɑskɑvɔ ˈprɔsɪmɔ/ | welcome | phrase | - | Formal welcome |
| що | /ʃtʃɔ/ | what | pron | - | Щ = SHCH |
| як | /jɑk/ | how | adv | - | Я = YA |
| де | /dɛ/ | where | adv | - | Simple question word |
| хто | /xtɔ/ | who | pron | - | Х = KH |
| чай | /tʃɑj/ | tea | noun | m | Ч = CH, Й = Y |
| кава | /ˈkɑvɑ/ | coffee | noun | f | Basic word |
| борщ | /bɔrʃtʃ/ | borscht | noun | m | Щ = SHCH |
| школа | /ˈʃkɔlɑ/ | school | noun | f | Ш = SH |
| центр | /t͡sɛntr/ | center | noun | m | Ц = TS |
| місто | /ˈmʲistɔ/ | city | noun | n | Basic word |
| сім'я | /sʲiˈmjɑ/ | family | noun | f | ' = apostrophe, Я = YA |
| ім'я | /iˈmjɑ/ | name | noun | n | ' = apostrophe, Я = YA |
| жінка | /ˈʒinkɑ/ | woman | noun | f | Ж = ZH |
| чоловік | /tʃɔlɔˈvʲik/ | man/husband | noun | m | Ч = CH |
| і/й | /i/, /j/ | and | conj | - | Euphonic pair: й after vowel, і after consonant |

---

# Letter Groups Summary

## Module 01 Letters (18)
А Е І О К М Т (True Friends)
В Н Р С У Х (False Friends)
Б Д З Л П Ф (New Letters)

## Module 02 Letters (15)
Г Ґ Ж Ш Щ Ч Ц (Unique Consonants)
Є Ї Ю Я (Iotated Vowels)
И Й Ь (Special Letters)

## Complete Ukrainian Alphabet (33)
А Б В Г Ґ Д Е Є Ж З И І Ї Й К Л М Н О П Р С Т У Ф Х Ц Ч Ш Щ Ь Ю Я

---

# Summary

> 💡 **What You Learned Today**
>
> - All 33 letters of the Ukrainian alphabet - you can now read anything!
> - **Unique consonants:** Г (h), Ґ (g), Ж (zh), Ш (sh), Щ (shch), Ч (ch), Ц (ts)
> - **Iotated vowels:** Є (ye), Ї (yi), Ю (yu), Я (ya) - they start with "y"
> - **Special letters:** И (short i), Й (y consonant), Ь (softens consonants)
> - The letter **Ї** exists ONLY in Ukrainian!
> - Cursive letters can look very different - watch out!
> - **Euphony preview:** Why Ukrainian sounds melodic - **і/й** (and): й after vowels, і after consonants (мама **й** тато)
> - Your first Ukrainian phrases: Дякую! Будь ласка! Привіт!

> 🎯 **Coming Next**
>
> In Module 03, you'll dive deeper into **Noun Grammar**:
> - Grouping nouns into **4 Declensions** (Families)
> - Why "ending rules" aren't enough for fluent speech
> - Connecting Gender to these Declension Groups
> - Preparing for Case endings!
