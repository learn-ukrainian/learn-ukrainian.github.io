---
module: a2-09
title: "All Cases Practice"
subtitle: "Integrating All 7 Ukrainian Cases"
version: "1.0"
phase: "A2.1"
pedagogy: "PPP"
duration: "60 min"
transliteration: none
tags:
  - grammar
  - cases
  - practice
  - review
grammar:
  - 7-case integration
  - case recognition
  - case selection
objectives:
  - "Learner can identify all 7 Ukrainian cases"
  - "Learner can choose the correct case for each context"
  - "Learner can transform nouns through different cases"
  - "Learner can use cases in connected speech"
vocabulary_count: 25
---

## Warm-up

Congratulations — you've now learned all seven Ukrainian cases! This is a major milestone in your Ukrainian journey. In this module, we'll bring everything together in one comprehensive practice session.

**Why this matters:** The seven-case system is the backbone of Ukrainian grammar. Being able to choose the right case quickly and accurately will make your Ukrainian sound natural and fluent. This review module will cement your understanding and give you the confidence to use cases correctly in real conversations.

Unlike English, which relies heavily on word order and prepositions to show relationships between words, Ukrainian uses morphological endings — changes to the word itself — to indicate the grammatical function of each noun, pronoun, and adjective in a sentence. This means that while an English speaker must rely on position to distinguish between "The dog bites the man" and "The man bites the dog," a Ukrainian speaker can theoretically rearrange words more freely because the case endings clearly mark which noun is the subject and which is the object.

**The Seven Ukrainian Cases — Quick Reference:**

| # | Case | Ukrainian | Question | Primary Function |
|---|------|-----------|----------|------------------|
| 1 | Nominative | Називний | хто? що? | Subject of sentence |
| 2 | Genitive | Родовий | кого? чого? | Possession, absence, quantity |
| 3 | Dative | Давальний | кому? чому? | Indirect object, recipient |
| 4 | Accusative | Знахідний | кого? що? | Direct object, direction |
| 5 | Instrumental | Орудний | ким? чим? | Means, accompaniment |
| 6 | Locative | Місцевий | на кому? на чому? | Location (always with preposition) |
| 7 | Vocative | Кличний | — | Direct address |

**How to use this module:** Work through each section systematically. First, review the theory for each case. Then, practice identifying cases in example sentences. Finally, complete the activities to test your understanding. By the end, you should be able to quickly identify which case is needed in any given context.

> 💡 **Learning Strategy**
>
> The key to mastering cases is to always ask yourself the right question. For example, when you see a noun after a verb, ask: "Who or what is receiving the action?" (accusative) or "To whom is something being given?" (dative). The question you ask determines the case you need.


## Presentation

## Understanding the Case System

Before we dive into reviewing each case, let's understand why cases matter and how they work together. In Ukrainian, cases serve several crucial functions that English handles differently.

**Function 1: Marking grammatical relationships.** Cases tell you who does what to whom. The nominative case marks the doer (subject), while the accusative marks the receiver (direct object). This allows for flexible word order without losing meaning.

**Function 2: Working with prepositions.** Many Ukrainian prepositions require specific cases. Some prepositions can take different cases depending on whether you're expressing motion (going somewhere) or location (being somewhere). This is one of the trickiest aspects for English speakers.

**Function 3: Expressing subtle meanings.** Cases can convey nuances that English expresses through separate words. For example, the genitive can express absence ("There is no..."), possession ("my friend's"), or quantity ("a cup of...").

> [!tip] The Golden Rule
> Always identify the FUNCTION of the word first, then choose the case. Ask: What role does this word play in the sentence? Is it the subject? The direct object? The recipient? The location? The answer determines the case.

## Огляд відмінків

Кожен відмінок має своє головне призначення. Ось короткий огляд:

### Називний відмінок — Nominative Case

**Питання:** Хто? Що?
**Функція:** Підмет речення.

Приклади:
- **Книга** лежить на столі. (The book is on the table.)
- **Мама** готує обід. (Mom is cooking dinner.)
- **Студент** читає. (The student is reading.)

### Родовий відмінок — Genitive Case

**Питання:** Кого? Чого?
**Функція:** Possession, absence, quantity, after certain prepositions.

Приклади:
- Немає **книги**. (There is no book.)
- Чашка **кави**. (A cup of coffee.)
- Я йду з **роботи**. (I'm coming from work.)

### Давальний відмінок — Dative Case

**Питання:** Кому? Чому?
**Функція:** Indirect object — who receives something.

Приклади:
- Я даю книгу **другові**. (I give a book to a friend.)
- Вона пише **мамі**. (She writes to mom.)
- Мені холодно. (I'm cold. — lit. "To me it's cold.")

> 💡 **Remember This!**
>
> Dative case is also used to express physical and emotional states: Мені добре (I feel good). Тобі цікаво? (Are you interested?) Йому сумно (He feels sad).

### Знахідний відмінок — Accusative Case

**Питання:** Кого? Що?
**Функція:** Direct object, direction of motion.

Приклади:
- Я читаю **книгу**. (I'm reading a book.)
- Він бачить **маму**. (He sees mom.)
- Вона йде у **школу**. (She's going to school.)

### Орудний відмінок — Instrumental Case

**Питання:** Ким? Чим?
**Функція:** Means, accompaniment, profession.

Приклади:
- Писати **ручкою**. (To write with a pen.)
- Йти з **другом**. (To go with a friend.)
- Він був **студентом**. (He was a student.)

### Місцевий відмінок — Locative Case

**Питання:** На кому? На чому?
**Функція:** Location — always with prepositions.

Приклади:
- Книга на **столі**. (The book is on the table.)
- Він у **школі**. (He's at school.)
- Ми говоримо про **друга**. (We're talking about a friend.)

> 📜 **Interesting Fact**
>
> Locative case in Ukrainian is ALWAYS used with prepositions: в/у, на, по, при. Without a preposition, it doesn't exist! This is different from some other Slavic languages.

### Кличний відмінок — Vocative Case

**Функція:** Addressing people directly.

Приклади:
- **Мамо**! (Mom!)
- **Друже**! (Friend!)
- **Вікторе**! (Viktor!)
- **Оксано**! (Oksana!)

---

## Cultural Insight

## Як відмінки формують речення

Порівняйте ці речення. Кожне слово змінює форму залежно від функції в реченні:

**Базове слово:** мама

| Відмінок | Приклад | Значення |
|----------|---------|----------|
| Називний | **Мама** готує. | Mom cooks. |
| Родовий | Немає **мами**. | There's no mom. |
| Давальний | Даю **мамі**. | I give to mom. |
| Знахідний | Бачу **маму**. | I see mom. |
| Орудний | Йду з **мамою**. | I go with mom. |
| Місцевий | Думаю про **маму**. | I think about mom. |
| Кличний | **Мамо**! | Mom! |

> 🔍 **Important Note**
>
> Accusative and genitive have the same question for animate nouns (кого?), but different endings. Genitive: мами, Accusative: маму. For inanimate nouns: Genitive книги, Accusative книгу.

## Як обрати правильний відмінок

Щоб обрати правильний відмінок, запитайте себе:

1. **Хто це робить?** → Називний
2. **Немає чого? Скільки?** → Родовий
3. **Кому даю/кажу?** → Давальний
4. **Що бачу/беру?** → Знахідний
5. **Чим користуюся? З ким?** → Орудний
6. **Де знаходиться?** → Місцевий
7. **До кого звертаюся?** → Кличний

> [!tip] The Golden Rule
> First identify the function of the word in the sentence, then choose the case. Not the other way around! Ask yourself: "What role does this word play?"

---

## Practice

## Практичні вправи

### Вправа 1: Визначте відмінок

Прочитайте речення і визначте відмінок виділених слів:

1. Я бачу **маму**. — Знахідний
2. Немає **часу**. — Родовий
3. **Книга** цікава. — Називний
4. Він дає подарунок **сестрі**. — Давальний
5. Ми їдемо **автобусом**. — Орудний
6. Вона в **університеті**. — Місцевий
7. **Петре**, іди сюди! — Кличний

### Вправа 2: Провідмінюйте слово

Провідмінюйте слово "друг" через усі відмінки:

| Відмінок | Форма |
|----------|-------|
| Називний | друг |
| Родовий | друга |
| Давальний | другові | другу |
| Знахідний | друга |
| Орудний | другом |
| Місцевий | друзі | другові |
| Кличний | друже |

### Вправа 3: Оберіть правильну форму

Мій **___** приїхав. (брат | брата | братові)
→ **брат** (називний — підмет)

Я дзвоню **___**. (мама | мами | мамі)
→ **мамі** (давальний — кому дзвоню?)

---

# Summary

# Підсумок

У цьому модулі ми повторили всі сім відмінків української мови:

**Називний** — хто? що? — підмет речення
**Родовий** — кого? чого? — відсутність, кількість, після прийменників
**Давальний** — кому? чому? — адресат дії
**Знахідний** — кого? що? — прямий об'єкт, напрямок руху
**Орудний** — ким? чим? — засіб, супровід, професія
**Місцевий** — на кому? на чому? — місце (завжди з прийменником)
**Кличний** — звертання до людини

Тепер ви готові використовувати всі відмінки! 🎯

**Українська приказка:** «Учітеся, брати мої, думайте, читайте!» — Тарас Шевченко.

Продовжуйте практикувати відмінки. Вони — ключ до вільного володіння українською мовою!

## Need More Practice?

> [!resources] External Resources
> - 🎥 [All 7 Cases](https://www.youtube.com/@LetsLearnUkrainian) — Let's Learn Ukrainian
> - 📖 [Cases Introduction](https://www.ukrainianlessons.com/intro-cases/)
> - 📝 [Cases Chart](https://www.ukrainianlessons.com/ukrainian-cases-chart/)
> - 🎙️ [Ukrainian Lessons Podcast](https://www.ukrainianlessons.com/podcast/)

---

# Activities

## match-up: Case Questions

- Називний :: хто? що?
- Родовий :: кого? чого?
- Давальний :: кому? чому?
- Знахідний :: кого? що?
- Орудний :: ким? чим?
- Місцевий :: на кому? на чому?
- Кличний :: звертання
- Nominative :: subject
- Genitive :: possession, absence
- Dative :: recipient
- Accusative :: direct object
- Instrumental :: means, with

## fill-in: Choose the Correct Case Form

> Complete each sentence with the correct form.

1. Я бачу ___.
   > [!answer] маму
   > [!options] мама | маму | мамі | мамою

2. Немає ___.
   > [!answer] часу
   > [!options] час | часу | часом | часі

3. Він дає книгу ___.
   > [!answer] другові
   > [!options] друг | друга | другові | другом

4. ___ готує обід.
   > [!answer] Мама
   > [!options] Мама | Маму | Мамі | Мамою

5. Ми їдемо ___.
   > [!answer] автобусом
   > [!options] автобус | автобуса | автобусу | автобусом

6. Вона в ___.
   > [!answer] університеті
   > [!options] університет | університету | університетом | університеті

7. Чашка ___.
   > [!answer] кави
   > [!options] кава | кави | каву | кавою

8. Я пишу ___.
   > [!answer] ручкою
   > [!options] ручка | ручки | ручці | ручкою

9. ___, іди сюди!
   > [!answer] Друже
   > [!options] Друг | Друга | Друже | Другом

10. Книга на ___.
    > [!answer] столі
    > [!options] стіл | стола | столом | столі

11. Це подарунок для ___.
    > [!answer] сестри
    > [!options] сестра | сестри | сестрі | сестрою

12. Маленький ___ спить.
    > [!answer] кіт
    > [!options] кіт | кота | котові | котом

## quiz: Which Case?

1. In the sentence "Я читаю книгу," which grammatical case is the word "книгу"?

- [x] Accusative
- [ ] Nominative
- [ ] Genitive
- [ ] Dative

---

2. In the Ukrainian expression "Немає часу," which grammatical case is the word "часу"?

- [x] Genitive
- [ ] Dative
- [ ] Locative
- [ ] Instrumental

---

3. In the sentence "Вона дає подарунок мамі," which grammatical case is the word "мамі"?

- [x] Dative
- [ ] Locative
- [ ] Genitive
- [ ] Accusative

---

4. In the sentence "Студент читає," which grammatical case is the word "студент"?

- [x] Nominative
- [ ] Accusative
- [ ] Vocative
- [ ] Genitive

---

5. In the sentence "Ми їдемо автобусом," which grammatical case is the word "автобусом"?

- [x] Instrumental
- [ ] Dative
- [ ] Locative
- [ ] Accusative

---

6. In the sentence "Книга на столі," which grammatical case is the word "столі"?

- [x] Locative
- [ ] Instrumental
- [ ] Genitive
- [ ] Dative

---

7. In the greeting "Друже, привіт!" which grammatical case is the word "друже"?

- [x] Vocative
- [ ] Nominative
- [ ] Dative
- [ ] Accusative

---

8. In the sentence "Я йду з другом," which grammatical case is the word "другом"?

- [x] Instrumental
- [ ] Genitive
- [ ] Dative
- [ ] Locative

---

9. When saying "Я бачу маму" in Ukrainian, which grammatical case should you use for "маму"?

- [x] Accusative
- [ ] Nominative
- [ ] Dative
- [ ] Genitive

---

10. When expressing absence with "Немає хліба," which grammatical case is required for "хліба"?

- [x] Genitive
- [ ] Accusative
- [ ] Nominative
- [ ] Dative

---

11. When expressing location "в університеті" in Ukrainian, which grammatical case is required?

- [x] Locative
- [ ] Genitive
- [ ] Accusative
- [ ] Instrumental

---

12. When saying "Я даю другові" in Ukrainian, which grammatical case is used for "другові"?

- [x] Dative
- [ ] Accusative
- [ ] Instrumental
- [ ] Genitive

## true-false: Case Rules
> Визначте, чи правильне твердження про відмінки. | Decide if the statement about cases is true or false.

> Decide if each statement is true or false.

- [x] Nominative case is used for the subject of a sentence.
  > Correct! The subject is always in nominative.

- [x] Locative case always requires a preposition.
  > Yes! You can't use locative without в/у, на, по, or при.

- [ ] Accusative and genitive always have the same endings.
  > They have the same QUESTION for animate nouns, but different endings.

- [x] Vocative is used when addressing someone directly.
  > Correct! Друже! Мамо! Оксано!

- [x] Dative case shows who receives something.
  > Yes! Даю книгу другові = I give a book to a friend.

- [ ] Instrumental case is only for tools.
  > It's also used for accompaniment and professions.

- [x] Genitive can express absence with "немає."
  > Correct! Немає часу = There's no time.

- [ ] Every noun has a unique vocative form.
  > Not all nouns — masculine -о/-е, feminine -о, but many are just nominative.

- [x] Ukrainian has 7 grammatical cases.
  > Yes! Nominative, Genitive, Dative, Accusative, Instrumental, Locative, Vocative.

- [x] Prepositions determine which case to use.
  > Many prepositions require specific cases: до + Gen, з + Instr, etc.

- [ ] Nominative and accusative are always the same.
  > Only for inanimate masculine and neuter nouns.

- [x] "Про" requires accusative case.
  > Yes! Я думаю про тебе.

## group-sort: Case Functions
> З'єднайте функцію з правильним відмінком. | Match the function to the correct case.

> Sort these functions into the correct case category.

### Nominative/Genitive
- subject of sentence
- хто? що?
- possession
- absence
- quantity

### Dative/Accusative
- recipient
- кому?
- direct object
- direction

### Instrumental/Locative
- with someone
- tool/means
- profession
- location
- always with preposition

## unjumble: Case Sentences

> Put the words in the correct order.

1. я / бачу / маму / і / тата / у / магазині / сьогодні
   > [!answer] Я бачу маму і тата у магазині сьогодні

2. книга / на / столі / лежить / вже / давно / тут / у
   > [!answer] Книга лежить на столі вже давно тут у

3. він / дає / подарунок / сестрі / на / день / народження / сьогодні
   > [!answer] Він дає подарунок сестрі на день народження сьогодні

4. немає / часу / сьогодні / у / мене / на / це / зовсім
   > [!answer] Немає часу сьогодні у мене на це зовсім

5. ми / їдемо / автобусом / до / центру / міста / щодня / вранці
   > [!answer] Ми їдемо автобусом до центру міста щодня вранці

6. вона / пише / ручкою / у / зошиті / на / уроці / завжди
   > [!answer] Вона пише ручкою у зошиті на уроці завжди

7. друже / привіт / як / ти / сьогодні / себе / почуваєш / добре
   > [!answer] Друже привіт як ти сьогодні себе почуваєш добре

8. я / йду / з / другом / у / парк / щодня / ввечері
   > [!answer] Я йду з другом у парк щодня ввечері

9. студент / читає / книгу / про / Україну / в / бібліотеці / сьогодні
   > [!answer] Студент читає книгу про Україну в бібліотеці сьогодні

10. їхня / чашка / кави / стоїть / на / столі / в / кафе
    > [!answer] Їхня чашка кави стоїть на столі в кафе

11. мама / готує / обід / для / всієї / родини / щодня / ввечері
    > [!answer] Мама готує обід для всієї родини щодня ввечері

12. вона / в / університеті / вчиться / вже / три / роки / зараз
    > [!answer] Вона в університеті вчиться вже три роки зараз

## error-correction: Fix the Case

> Each sentence has ONE case error. Find and correct it.

1. Я бачу мама.
   > [!error] мама
   > [!answer] маму
   > [!explanation] Direct object requires accusative case.
   > [!options] мама | маму | мамі | мамою

2. Немає час.
   > [!error] час
   > [!answer] часу
   > [!explanation] "Немає" requires genitive case.
   > [!options] час | часу | часом | часі

3. Він дає книгу друг.
   > [!error] друг
   > [!answer] другові
   > [!explanation] Recipient requires dative case.
   > [!options] друг | друга | другові | другом

4. Книга на стіл.
   > [!error] стіл
   > [!answer] столі
   > [!explanation] Location requires locative case with "на."
   > [!options] стіл | столу | столом | столі

5. Ми їдемо автобус.
   > [!error] автобус
   > [!answer] автобусом
   > [!explanation] Means of transport requires instrumental.
   > [!options] автобус | автобуса | автобусу | автобусом

6. Чашка кава.
   > [!error] кава
   > [!answer] кави
   > [!explanation] "Cup of X" uses genitive case.
   > [!options] кава | кави | каву | кавою

7. Друг, привіт!
   > [!error] Друг
   > [!answer] Друже
   > [!explanation] Direct address requires vocative case.
   > [!options] Друг | Друга | Друже | Другом

8. Я пишу ручка.
   > [!error] ручка
   > [!answer] ручкою
   > [!explanation] Tool/means requires instrumental case.
   > [!options] ручка | ручки | ручці | ручкою

9. Вона в університет.
   > [!error] університет
   > [!answer] університеті
   > [!explanation] Location requires locative case with "в."
   > [!options] університет | університету | університетом | університеті

10. Подарунок для сестра.
    > [!error] сестра
    > [!answer] сестри
    > [!explanation] "Для" requires genitive case.
    > [!options] сестра | сестри | сестрі | сестрою

11. Я йду з друг.
    > [!error] друг
    > [!answer] другом
    > [!explanation] "З" for accompaniment requires instrumental.
    > [!options] друг | друга | другові | другом

12. Мама, привіт!
    > [!error] Мама
    > [!answer] Мамо
    > [!explanation] Direct address requires vocative case.
    > [!options] Мама | Мами | Мамо | Мамою

## cloze: Complete the Story

> Fill in the blanks with the correct case forms.

Це моя [___:1]. Вона працює в [___:2]. Я даю їй [___:3]. Вона їде [___:4] на роботу. Після [___:5] вона готує [___:6].

1. сестра | сестри | сестрі
   > [!answer] сестра

2. банк | банку | банком | банці
   > [!answer] банці

3. подарунок | подарунка | подарунку
   > [!answer] подарунок

4. автобус | автобуса | автобусом
   > [!answer] автобусом

5. робота | роботи | роботі
   > [!answer] роботи

6. вечеря | вечері | вечерю
   > [!answer] вечерю

---

[___:1] вчиться в [___:2]. Він читає [___:3]. Потім він пише [___:4]. Після [___:5] він йде [___:6].

1. Студент | Студента | Студентові
   > [!answer] Студент

2. університет | університету | університеті
   > [!answer] університеті

3. книга | книги | книгу
   > [!answer] книгу

4. ручка | ручки | ручкою
   > [!answer] ручкою

5. урок | уроку | уроком
   > [!answer] уроку

6. додому | до дому | домом
   > [!answer] додому

## dialogue-reorder: At the Bookstore

- Добрий день! Чим можу допомогти?
- Я шукаю книгу для друга.
- Які книги він любить?
- Він любить історичні романи.
- Ось хороша книга про Україну.
- Дякую! Скільки це коштує?

> [!answer]
> 1. Добрий день! Чим можу допомогти?
> 2. Я шукаю книгу для друга.
> 3. Які книги він любить?
> 4. Він любить історичні романи.
> 5. Ось хороша книга про Україну.
> 6. Дякую! Скільки це коштує?

---

- У мене проблема з рахунком.
- Який саме рахунок?
- Ощадний рахунок.
- Покажіть, будь ласка, картку.
- Ось, будь ласка.
- Зачекайте хвилинку, я перевірю.

> [!answer]
> 1. У мене проблема з рахунком.
> 2. Який саме рахунок?
> 3. Ощадний рахунок.
> 4. Покажіть, будь ласка, картку.
> 5. Ось, будь ласка.
> 6. Зачекайте хвилинку, я перевірю.

## mark-the-words: Find Case Usage

Click on words that show case usage.

[Мама](correct) готує [обід](correct). Вона дає [тарілку](correct) [синові](correct). [Хлопчик](correct) їсть [ложкою](correct).

---

Я читаю [книгу](correct) про [Україну](correct). [Книга](correct) дуже цікава. Після [читання](correct) я пишу [листа](correct) [подрузі](correct).

---

[Друже](correct)! Давай підемо в [кіно](correct)! Я маю [квитки](correct). Після [фільму](correct) можемо поїсти [піци](correct).

## translate: English to Ukrainian

I see a book.

- [x] Я бачу книгу.
- [ ] Я бачу книга.
- [ ] Я бачу книги.

---

There is no time.

- [x] Немає часу.
- [ ] Немає час.
- [ ] Немає часом.

---

I give a gift to mom.

- [x] Я даю подарунок мамі.
- [ ] Я даю подарунок мама.
- [ ] Я даю подарунок мамою.

---

The book is on the table.

- [x] Книга на столі.
- [ ] Книга на стіл.
- [ ] Книга на столом.

---

We go by bus.

- [x] Ми їдемо автобусом.
- [ ] Ми їдемо автобус.
- [ ] Ми їдемо автобуса.

---

Friend, hello!

- [x] Друже, привіт!
- [ ] Друг, привіт!
- [ ] Друга, привіт!

---

A cup of coffee.

- [x] Чашка кави.
- [ ] Чашка кава.
- [ ] Чашка кавою.

---

I'm writing with a pen.

- [x] Я пишу ручкою.
- [ ] Я пишу ручка.
- [ ] Я пишу ручки.

---

She's at the university.

- [x] Вона в університеті.
- [ ] Вона в університет.
- [ ] Вона в університету.

---

A gift for sister.

- [x] Подарунок для сестри.
- [ ] Подарунок для сестра.
- [ ] Подарунок для сестрі.

---

I'm going with a friend.

- [x] Я йду з другом.
- [ ] Я йду з друг.
- [ ] Я йду з друга.

---

Mom cooks dinner.

- [x] Мама готує обід.
- [ ] Маму готує обід.
- [ ] Мамі готує обід.

---

# Vocabulary

| Word | IPA | English | POS | Gender | Note |
|------|-----|---------|-----|--------|------|
| кого | /kɔˈɦɔ/ | whom | pron | — | Gen/Acc of хто |
| чого | /t͡ʃɔˈɦɔ/ | of what | pron | — | Gen of що |
| кому | /kɔˈmu/ | to whom | pron | — | Dat of хто |
| ким | /kɪm/ | by whom | pron | — | Instr of хто |
| чим | /t͡ʃɪm/ | with what | pron | — | Instr of що |
| підмет | /ˈpidmɛt/ | subject | noun | m | grammar term |
| додаток | /ˈdɔdɑtɔk/ | object | noun | m | grammar term |
| присудок | /prɪˈsudɔk/ | predicate | noun | m | grammar term |
| звертатися | /zʋɛrˈtɑtɪsʲɑ/ | to address | verb | — | uses vocative |

---
