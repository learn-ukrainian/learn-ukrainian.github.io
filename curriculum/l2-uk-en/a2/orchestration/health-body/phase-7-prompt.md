...
1. **Plan** (source of truth): `curriculum/l2-uk-en/plans/a2/health-body.yaml`
2. **Meta** (build config): `curriculum/l2-uk-en/a2/meta/health-body.yaml`
3. **Content** (the lesson): `curriculum/l2-uk-en/a2/health-body.md`
4. **Activities**: `curriculum/l2-uk-en/a2/activities/health-body.yaml`
5. **Vocabulary**: `curriculum/l2-uk-en/a2/vocabulary/health-body.yaml`
6. **Review** (Green Team review from Phase 6): `curriculum/l2-uk-en/a2/review/health-body-review.md`
7. **Phase 6b fix log** (if exists): `curriculum/l2-uk-en/a2/orchestration/health-body/phase-6b-fixes.md`
...
```bash
scripts/audit_module.sh curriculum/l2-uk-en/a2/health-body.md
```
...
Check against `curriculum/l2-uk-en/plans/a2/health-body.yaml`:
...
Read `curriculum/l2-uk-en/a2/health-body.md` in FULL. Check for:

### 3a: Russianisms (auto-fail)
```bash
grep -inP 'кушати|получати|приймати участь|слідуючий|любий\b|дякуючи' curriculum/l2-uk-en/a2/health-body.md
grep -inP 'кушати|получати|приймати участь|слідуючий|любий\b|дякуючи' curriculum/l2-uk-en/a2/activities/health-body.yaml
```

### 3b: Russian characters (auto-fail)
```bash
grep -nP '[ыэёъ]' curriculum/l2-uk-en/a2/health-body.md
grep -nP '[ыэёъ]' curriculum/l2-uk-en/a2/activities/health-body.yaml
```

### 3c: IPA errors
```bash
grep -n '/w/' curriculum/l2-uk-en/a2/health-body.md
grep -n '/ʊ/' curriculum/l2-uk-en/a2/health-body.md
grep -n '/w/' curriculum/l2-uk-en/a2/vocabulary/health-body.yaml
grep -n '/ʊ/' curriculum/l2-uk-en/a2/vocabulary/health-body.yaml
```
Fix: /w/ -> /ʋ/, /ʊ/ -> /u/
...
```bash
grep -nP '\[/[^]]+/\]|(?<!\[)/[а-яіїєґА-ЯІЇЄҐ][^/]*/(?!\])' curriculum/l2-uk-en/a2/health-body.md
```
If found and track is B1+: move to vocabulary, remove from content.

### 3e: English leakage
```bash
grep -inP '\b(the|is|are|was|were|has|have|this|that|with|from|for|and|but|not|can|will)\b' curriculum/l2-uk-en/a2/health-body.md
```
...
```bash
.venv/bin/python scripts/calc_immersion.py curriculum/l2-uk-en/a2/health-body.md
```
...
Read `curriculum/l2-uk-en/a2/activities/health-body.yaml` in FULL.
...
Read `curriculum/l2-uk-en/a2/review/health-body-review.md`. For EACH issue in "Critical Issues Found" or "Fix Plan":
...
```bash
.venv/bin/python scripts/generate_mdx.py l2-uk-en a2 55
```
