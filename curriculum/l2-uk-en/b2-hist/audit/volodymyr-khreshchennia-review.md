# Audit Report: M11 — volodymyr-khreshchennia.md
**Level:** B2 | **Module:** M11 | **Phase:** B2 | **Pedagogy:** CBI | **Target:** 4000
**Naturalness:** 10/10 (PASS)
**Overall Status:** ✅ PASS
**Generated:** 2026-01-25 09:46:22

## Configuration
**Type:** B2-history
**Word Target:** 4000 words
**Activities:** 3-9 required
**Items per Activity:** ≥1 items
**Unique Types:** ≥2 types required
**Priority Types:** comparative-study, critical-analysis, essay-response, reading
**Required Types:** comparative-study, essay-response, fill-in, match-up, quiz, reading
**Engagement:** ≥5 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥20 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | reading | Вибір віри: Легенда і реальність | 3 | 1 | ✅ |
| 2 | reading | Хрещення киян | 3 | 1 | ✅ |
| 3 | critical-analysis | Значення цивілізаційного вибору Володимира | 1 | 1 | ✅ |
| 4 | comparative-study | Порівняння історичних епох | 1 | 1 | ✅ |
| 5 | essay-response | Християнство як державотворчий акт | 1 | 1 | ✅ |
| 6 | match-up | Релігійна та історична термінологія | 12 | 8 | ✅ |
| 7 | quiz | Факти про хрещення Русі | 8 | 8 | ✅ |
| 8 | fill-in | Релігійна та політична лексика | 15 | 8 | ✅ |
| 9 | quiz | Поширення християнства | 8 | 8 | ✅ |
| 10 | match-up | Хронологія подій | 12 | 8 | ✅ |

**Summary:**
- Total activities: 10 (target: 3-9) ❌
- Unique types: 7 (minimum: 2) ✅
- Priority types used: 4/4 (comparative-study, critical-analysis, essay-response, reading) ✅
- Required types used: 6/6 (comparative-study, essay-response, fill-in, match-up, quiz, reading) ✅
- Low density activities: 0

## Gates
- **Words:** ⚠️ 3965/4000 (raw: 4256) (35 short)
- **Activities:** ✅ 10/3
- **Density:** ✅ All > 1
- **Unique_types:** ✅ 7/2 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 12/5
- **Audio:** ℹ️ No audio
- **Vocab:** ⚠️ 17 < 20 (soft target)
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ⚠️ Too many activities: 10 (target 3-9)
- **Immersion:** 🇺🇦 98.3% (target 90-100% (history))
- **Richness:** ✅ 98% (history)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ✅ 10/10 (High)

## Richness Details
**Score:** 98% (minimum: 95%)
**Module Type:** history

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| primary_sources | 14 | 3 | 100% | 24% | 23.8% |
| engagement | 12 | 6 | 100% | 14% | 14.3% |
| timeline_markers | 30 | 10 | 100% | 14% | 14.3% |
| decolonization | 15 | 2 | 100% | 14% | 14.3% |
| cultural | 9 | 4 | 100% | 10% | 9.5% |
| visual | 12 | 4 | 100% | 10% | 9.5% |
| variety | 0.98 | - | 98% | 5% | 4.7% |
| paragraph_var | 0.77 | - | 77% | 5% | 3.7% |
| questions | 13 | 3 | 100% | 5% | 4.8% |
| **TOTAL** | | | | | **98.8%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 0 | Included in Core |
| **Володимир і хрещення — 988 рік** | ⚪️ | 57 | Skipped |
| **Розминка** | ⚪️ | 162 | Skipped |
| **Читання: Князь Володимир: від язичника до святого** | ✅ | 525 | Included in Core |
| **Корсунський похід і шлюб із Анною** | ⚪️ | 459 | Skipped |
| **Первинні джерела** | ✅ | 251 | Included in Core |
| **Деколонізаційний погляд** | ✅ | 355 | Included in Core |
| **Значення хрещення для культури** | ✅ | 1970 | Included in Core |
| **Підсумок** | ✅ | 76 | Included in Core |
| **Потрібно більше практики?** | ⚪️ | 110 | Skipped |