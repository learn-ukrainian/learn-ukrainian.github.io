# Audit Report: M48 — kozatske-viisko.md
**Level:** B2 | **Module:** M48 | **Phase:** HIST.5 | **Pedagogy:** CBI | **Target:** 4000
**Naturalness:** 9/10 (PASS)
**Overall Status:** ✅ PASS
**Generated:** 2026-01-26 23:52:28

## Configuration
**Type:** B2-history
**Word Target:** 4000 words
**Activities:** 3-9 required
**Items per Activity:** ≥1 items
**Unique Types:** ≥2 types required
**Priority Types:** comparative-study, critical-analysis, essay-response, reading
**Required Types:** comparative-study, essay-response, quiz, reading, true-false
**Engagement:** ≥5 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥20 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | reading | Опис козацького вишколу та побуту | 3 | 1 | ✅ |
| 2 | critical-analysis | Психологія козацького воїна | 1 | 1 | ✅ |
| 3 | quiz | Структура та озброєння козацького війська | 8 | 8 | ✅ |
| 4 | comparative-study | Козацьке військо vs Регулярні армії Європи | 1 | 1 | ✅ |
| 5 | true-false | Військові факти: Шабля, гармата, дисципліна | 10 | 8 | ✅ |
| 6 | essay-response | Спадкоємність мілітарних традицій | 1 | 1 | ✅ |

**Summary:**
- Total activities: 6 (target: 3-9) ✅
- Unique types: 6 (minimum: 2) ✅
- Priority types used: 4/4 (comparative-study, critical-analysis, essay-response, reading) ✅
- Required types used: 5/5 (comparative-study, essay-response, quiz, reading, true-false) ✅
- Low density activities: 0

## Gates
- **Words:** ⚠️ 3904/4000 (raw: 4129) (96 short)
- **Activities:** ✅ 6/3
- **Density:** ✅ All > 1
- **Unique_types:** ✅ 6/2 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 5/5
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 25/20
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ✅ Content-heavy OK (6 activities)
- **Immersion:** 🇺🇦 98.8% (target 90-100% (history))
- **Richness:** ✅ 97% (history)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ✅ 9/10 (High)

## Richness Details
**Score:** 97% (minimum: 95%)
**Module Type:** history

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| primary_sources | 8 | 3 | 100% | 24% | 23.8% |
| engagement | 5 | 6 | 83% | 14% | 11.9% |
| timeline_markers | 12 | 10 | 100% | 14% | 14.3% |
| decolonization | 15 | 2 | 100% | 14% | 14.3% |
| cultural | 4 | 4 | 100% | 10% | 9.5% |
| visual | 5 | 4 | 100% | 10% | 9.5% |
| variety | 0.99 | - | 99% | 5% | 4.7% |
| paragraph_var | 1.00 | - | 100% | 5% | 4.8% |
| questions | 10 | 3 | 100% | 5% | 4.8% |
| **TOTAL** | | | | | **97.5%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 0 | Included in Core |
| **Козацьке військо та озброєння** | ⚪️ | 103 | Skipped |
| **Вступ: Феномен козацького війська** | ✅ | 383 | Included in Core |
| **Структура та Ієрархія: Від коша до полку** | ⚪️ | 634 | Skipped |
| **Озброєння: Шабля, самопал та гармата** | ⚪️ | 662 | Skipped |
| **Тактика бою: Козацький табір та розвідка** | ⚪️ | 523 | Skipped |
| **Символіка та Клейноди: Візуальні знаки влади** | ⚪️ | 392 | Skipped |
| **Читання: Опис козацького вишколу** | ✅ | 242 | Included in Core |
| **Первинні джерела: Голоси епохи** | ✅ | 295 | Included in Core |
| **Деколонізаційний погляд** | ✅ | 434 | Included in Core |
| **Підсумок** | ✅ | 115 | Included in Core |
| **Потрібно більше практики?** | ⚪️ | 121 | Skipped |