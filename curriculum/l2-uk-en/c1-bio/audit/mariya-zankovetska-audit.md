# Audit Report: M50 — mariya-zankovetska.md
**Level:** C1-BIO | **Module:** M50 | **Phase:** C1 | **Pedagogy:** Not Specified | **Target:** 4000
**Naturalness:** 10/10 (PASS)
**Overall Status:** ✅ PASS
**Generated:** 2026-02-11 00:43:38

## Configuration
**Type:** C1-biography
**Word Target:** 4000 words
**Activities:** 3-9 required
**Items per Activity:** ≥1 items
**Unique Types:** ≥3 types required
**Priority Types:** authorial-intent, comparative-study, critical-analysis, essay-response, quiz, reading
**Required Types:** essay-response, reading
**Engagement:** ≥5 callouts
**Immersion:** 95-100%
**Vocab Target:** ≥24 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | reading | Мемуари: Зустріч з Глитаєм | 4 | 1 | ✅ |
| 2 | reading | Публіцистика: Симон Петлюра про актрису | 4 | 1 | ✅ |
| 3 | critical-analysis | Аналіз: Вибір актриси | 1 | 1 | ✅ |
| 4 | essay-response | Есе: Феномен Заньковецької | 1 | 1 | ✅ |
| 5 | comparative-study | Порівняння: Театр корифеїв та сучасний театр | 1 | 1 | ✅ |
| 6 | quiz | Вікторина: Марія Заньковецька | 5 | 5 | ✅ |
| 7 | true-false | Правда чи Міф: Життя Марії Заньковецької | 10 | 5 | ✅ |

**Summary:**
- Total activities: 7 (target: 3-9) ✅
- Unique types: 6 (minimum: 3) ✅
- Priority types used: 5/6 (comparative-study, critical-analysis, essay-response, quiz, reading) ✅
- Required types used: 2/2 (essay-response, reading) ✅
- Low density activities: 0

## Gates
- **Words:** ✅ 4548/4000 (raw: 4585)
- **Activities:** ✅ 7/3
- **Density:** ✅ All > 1
- **Unique_types:** ✅ 6/3 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 13/5
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 27/24
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ✅ Content-heavy OK (7 activities)
- **Immersion:** 🇺🇦 99.9% (target 95-100% (biography))
- **Richness:** ✅ 99% (biography)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ✅ 10/10 (High)

## Richness Details
**Score:** 99% (minimum: 95%)
**Module Type:** biography

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| primary_sources | 4 | 4 | 100% | 19% | 19.0% |
| engagement | 11 | 6 | 100% | 14% | 14.3% |
| quotes | 3 | 3 | 100% | 14% | 14.3% |
| cultural | 9 | 4 | 100% | 10% | 9.5% |
| visual | 14 | 4 | 100% | 10% | 9.5% |
| timeline_markers | 30 | 8 | 100% | 10% | 9.5% |
| legacy | 15 | 2 | 100% | 10% | 9.5% |
| variety | 0.99 | - | 99% | 5% | 4.7% |
| paragraph_var | 1.00 | - | 100% | 5% | 4.8% |
| questions | 4 | 3 | 100% | 5% | 4.8% |
| **TOTAL** | | | | | **100.0%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 0 | Included in Core |
| **Вступ — Королева українського театру** | ✅ | 622 | Included in Core |
| **Дитинство та юність (1854-1875)** | ⚪️ | 709 | Skipped |
| **Початок кар'єри (1875-1885)** | ⚪️ | 739 | Skipped |
| **Зоряні роки (1885-1900)** | ⚪️ | 820 | Skipped |
| **Пізній період та УНР (1900-1920)** | ⚪️ | 647 | Skipped |
| **Спадщина та пам'ять** | ⚪️ | 824 | Skipped |
| **Підсумок** | ✅ | 187 | Included in Core |