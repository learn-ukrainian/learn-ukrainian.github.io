# Audit Report: M146 — mariya-matios.md
**Level:** C1-BIO | **Module:** M146 | **Phase:** C1 | **Pedagogy:** Not Specified | **Target:** 4000
**Naturalness:** 9/10 (PASS)
**Overall Status:** ❌ FAIL
**Generated:** 2026-02-09 23:25:16

## Configuration
**Type:** C1-biography
**Word Target:** 4000 words
**Activities:** 3-9 required
**Items per Activity:** ≥1 items
**Unique Types:** ≥3 types required
**Priority Types:** authorial-intent, comparative-study, critical-analysis, essay-response, quiz, reading
**Required Types:** essay-response, reading
**Engagement:** ≥5 callouts
**Immersion:** 95-100%
**Vocab Target:** ≥24 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | essay-response | Творча робота: Солодка та Гірка Даруся | 1 | 1 | ✅ |
| 2 | comparative-study | Марія Матіос та Ольга Кобилянська: Діалог через століття | 1 | 1 | ✅ |

**Summary:**
- Total activities: 2 (target: 3-9) ❌
- Unique types: 2 (minimum: 3) ❌
- Priority types used: 2/6 (comparative-study, essay-response) ✅
- Required types used: 1/2 (essay-response) ❌
- Low density activities: 0

## PEDAGOGICAL VIOLATIONS
- **[MISSING_RESEARCH]** No research file found for seminar module. Expected: research/mariya-matios-research.md
  - FIX: Run /full-rebuild c1-bio or /research to create research notes before content generation.
- **[YAML_SCHEMA_VIOLATION]** Schema error in mariya-matios.yaml: Insufficient activities: 2 found, minimum 3 required for C1-BIO
  - FIX: Fix the activity YAML to match the schema in schemas/activities-base.schema.json

## Recommendation
**📝 UPDATE** (severity 40/100)

- Revision recommended (severity 40/100)
- 2 violations (minor)
- Structure issue: Missing '## Vocabulary' header OR vocabulary sidecar
- Activity count below minimum

## Gates
- **Words:** ❌ 2109/4000 (raw: 2389)
- **Activities:** ❌ 2/3
- **Density:** ✅ All > 1
- **Unique_types:** ❌ 2/3 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 9/5
- **Audio:** ℹ️ No audio
- **Vocab:** ⚠️ 0 < 24 (soft target)
- **Structure:** ❌ Missing '## Vocabulary' header OR vocabulary sidecar
- **Lint:** ✅ Clean Format
- **Pedagogy:** ❌ 2 violations
- **Content_heavy:** ⚠️ Too few activities: 2 (target 3-9)
- **Immersion:** 🇺🇦 100.0% (target 95-100% (biography))
- **Richness:** ✅ 97% (biography)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ✅ 9/10 (High)

## Richness Details
**Score:** 97% (minimum: 95%)
**Module Type:** biography

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| primary_sources | 4 | 4 | 100% | 19% | 19.0% |
| engagement | 9 | 6 | 100% | 14% | 14.3% |
| quotes | 8 | 3 | 100% | 14% | 14.3% |
| cultural | 3 | 4 | 75% | 10% | 7.1% |
| visual | 12 | 4 | 100% | 10% | 9.5% |
| timeline_markers | 17 | 8 | 100% | 10% | 9.5% |
| legacy | 15 | 2 | 100% | 10% | 9.5% |
| variety | 0.98 | - | 98% | 5% | 4.7% |
| paragraph_var | 1.00 | - | 100% | 5% | 4.8% |
| questions | 19 | 3 | 100% | 5% | 4.8% |
| **TOTAL** | | | | | **97.5%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 72 | Included in Core |
| **Вступ** | ✅ | 191 | Included in Core |
| **Життєпис** | ⚪️ | 580 | Skipped |
| **Внесок** | ⚪️ | 56 | Skipped |
| **Сучасний етап** | ⚪️ | 120 | Skipped |
| **Історичний контекст** | ✅ | 225 | Included in Core |
| **Порівняльний аналіз** | ✅ | 159 | Included in Core |
| **Есе** | ⚪️ | 361 | Skipped |
| **Підсумок** | ✅ | 46 | Included in Core |
| **Практикум рефлексії** | ⚪️ | 188 | Skipped |
| **Потрібно більше практики?** | ⚪️ | 111 | Skipped |