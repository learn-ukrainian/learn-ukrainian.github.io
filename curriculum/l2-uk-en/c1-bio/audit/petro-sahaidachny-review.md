# Audit Report: M20 — petro-sahaidachny.md
**Level:** C1-BIO | **Module:** M20 | **Phase:** C1 | **Pedagogy:** CBI | **Target:** 4000
**Naturalness:** 10/10 (PASS)
**Overall Status:** ❌ FAIL
**Generated:** 2026-02-04 12:08:41

## Configuration
**Type:** C1-biography
**Word Target:** 4000 words
**Activities:** 3-9 required
**Items per Activity:** ≥1 items
**Unique Types:** ≥3 types required
**Priority Types:** authorial-intent, comparative-study, critical-analysis, essay-response, quiz, reading
**Required Types:** essay-response, reading
**Engagement:** ≥5 callouts
**Immersion:** 95-100%
**Vocab Target:** ≥24 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | reading | Вірші на жалісний погреб Сагайдачного | 1 | 1 | ✅ |
| 2 | critical-analysis | Аналіз стратегії 'Меча і Хреста' | 1 | 1 | ✅ |
| 3 | authorial-intent | Аналіз Хотинської перемоги як геополітичного акту | 1 | 1 | ✅ |
| 4 | comparative-study | Порівняння лідерів: Сагайдачний vs Наливайко | 1 | 1 | ✅ |
| 5 | essay-response | Есе: Сагайдачний — творець українського Києва | 1 | 1 | ✅ |

**Summary:**
- Total activities: 5 (target: 3-9) ✅
- Unique types: 5 (minimum: 3) ✅
- Priority types used: 5/6 (authorial-intent, comparative-study, critical-analysis, essay-response, reading) ✅
- Required types used: 2/2 (essay-response, reading) ✅
- Low density activities: 0

## PEDAGOGICAL VIOLATIONS
- **[ROBOTIC_STRUCTURE]** Robotic structure: 3 sentences start with 'основна частина...'.
  - FIX: Vary sentence structure.

## Recommendation
**📝 UPDATE** (severity 25/100)

- 1 violations (minor)
- Structure issue: Missing '## Vocabulary' header OR vocabulary sidecar

## Gates
- **Words:** ❌ 1676/4000 (raw: 1931)
- **Activities:** ✅ 5/3
- **Density:** ✅ All > 1
- **Unique_types:** ✅ 5/3 types
- **Priority:** ✅ Priority types used
- **Engagement:** ❌ 3/5
- **Audio:** ℹ️ No audio
- **Vocab:** ⚠️ 0 < 24 (soft target)
- **Structure:** ❌ Missing '## Vocabulary' header OR vocabulary sidecar
- **Lint:** ✅ Clean Format
- **Pedagogy:** ❌ 1 violations
- **Content_heavy:** ✅ Content-heavy OK (5 activities)
- **Immersion:** 🇺🇦 99.7% (target 95-100% (biography))
- **Richness:** ❌ 61% < 95% min (biography)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ✅ 10/10 (High)

## Richness Details
**Score:** 61% (minimum: 95%)
**Module Type:** biography

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| primary_sources | 1 | 4 | 25% | 19% | 4.8% |
| engagement | 2 | 6 | 33% | 14% | 4.7% |
| quotes | 0 | 3 | 0% | 14% | 0.0% |
| cultural | 6 | 4 | 100% | 10% | 9.5% |
| visual | 4 | 4 | 100% | 10% | 9.5% |
| timeline_markers | 30 | 8 | 100% | 10% | 9.5% |
| legacy | 12 | 2 | 100% | 10% | 9.5% |
| variety | 1.00 | - | 100% | 5% | 4.8% |
| paragraph_var | 1.00 | - | 100% | 5% | 4.8% |
| questions | 10 | 3 | 100% | 5% | 4.8% |
| **TOTAL** | | | | | **61.9%** |

### Dryness Flags & Fixes
- ❌ **NO_QUOTES**
  - FIX:
    Add 2+ direct quotes from the subject. Use this format:
    
    > «[Exact quote from the person]»
    > — *[Person name], [context/year]*

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 91 | Included in Core |
| **Вступ** | ✅ | 172 | Included in Core |
| **Життєпис** | ⚪️ | 555 | Skipped |
| **Історичний контекст** | ✅ | 100 | Included in Core |
| **Внесок та Спадщина** | ⚪️ | 134 | Skipped |
| **Порівняльний аналіз** | ✅ | 72 | Included in Core |
| **Есе** | ⚪️ | 408 | Skipped |
| **Підсумок** | ✅ | 67 | Included in Core |
| **Потрібно більше практики?** | ⚪️ | 77 | Skipped |