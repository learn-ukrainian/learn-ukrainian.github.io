# Audit Report: M11 — 11-summary-paraphrase.md
**Level:** C1 | **Module:** M11 | **Phase:** C1.1 | **Pedagogy:** Not Specified | **Target:** 4000
**Naturalness:** None/10 (PENDING)
**Overall Status:** ❌ FAIL
**Generated:** 2026-02-06 20:12:52

## Configuration
**Type:** C1-grammar
**Word Target:** 4000 words
**Activities:** 12-16 required
**Items per Activity:** ≥12 items
**Unique Types:** ≥4 types required
**Priority Types:** error-correction, fill-in, unjumble
**Required Types:** cloze, essay-response, fill-in, match-up, quiz
**Engagement:** ≥7 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥25 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Розуміння концепції переказу | 9 | 5 | ✅ |
| 2 | fill-in | Академічна лексика: Резюмування та Парафраз | 8 | 6 | ✅ |
| 3 | match-up | Термінологія академічного письма | 8 | 6 | ✅ |
| 4 | error-correction | Граматика та стиль у переказі | 6 | 5 | ✅ |
| 5 | group-sort | Класифікація академічних прийомів | 12 | 12 | ✅ |
| 6 | select | Аналіз академічної етики | 6 | 5 | ✅ |
| 7 | cloze | Мистецтво стислості | 14 | 12 | ✅ |
| 8 | unjumble | Маркери парафразу | 6 | 5 | ✅ |
| 9 | mark-the-words | Маркери логічного зв'язку | 8 | 5 | ✅ |
| 10 | true-false | Правда чи Міф: Академічне резюмування | 8 | 5 | ✅ |
| 11 | quiz | Техніки скорочення тексту | 8 | 5 | ✅ |
| 12 | fill-in | Синоніми в академічному контексті | 8 | 6 | ✅ |
| 13 | match-up | Контекстуальні заміни | 8 | 6 | ✅ |
| 14 | quiz | Ознаки якісного резюме | 8 | 5 | ✅ |
| 15 | essay-response | Практичне завдання: Резюмування тексту | 1 | 1 | ✅ |
| 16 | critical-analysis | Аналіз 'Езопової мови' | 1 | 1 | ✅ |

**Summary:**
- Total activities: 16 (target: 12-16) ✅
- Unique types: 12 (minimum: 4) ✅
- Priority types used: 3/3 (error-correction, fill-in, unjumble) ✅
- Required types used: 5/5 (cloze, essay-response, fill-in, match-up, quiz) ✅
- Low density activities: 0

## PEDAGOGICAL VIOLATIONS
- **[YAML_SCHEMA_VIOLATION]** Schema error in 11-summary-paraphrase.yaml: Schema validation error at key 'min_words': 100 is less than the minimum of 200
  - FIX: Fix the activity YAML to match the schema in schemas/activities-base.schema.json

## Recommendation
**📝 UPDATE** (severity 25/100)

- 1 violations (minor)
- Structure issue: Missing '## Vocabulary' header OR vocabulary sidecar

## Gates
- **Words:** ❌ 2026/4000 (raw: 2110)
- **Activities:** ✅ 16/12
- **Density:** ✅ All > 12
- **Unique_types:** ✅ 12/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 7/7
- **Audio:** ℹ️ No audio
- **Vocab:** ⚠️ 0 < 25 (soft target)
- **Structure:** ❌ Missing '## Vocabulary' header OR vocabulary sidecar
- **Lint:** ✅ Clean Format
- **Pedagogy:** ❌ 1 violations
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 99.4% (target 90-100% (grammar))
- **Richness:** ❌ 92% < 95% min (grammar)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review

## Richness Details
**Score:** 92% (minimum: 95%)
**Module Type:** grammar

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| examples | 40 | 24 | 100% | 20% | 20.0% |
| engagement | 7 | 5 | 100% | 15% | 15.0% |
| dialogues | 3 | 4 | 75% | 15% | 11.2% |
| variety | 0.99 | - | 99% | 10% | 9.9% |
| cultural | 2 | 3 | 67% | 10% | 6.7% |
| realworld | 16 | 3 | 100% | 10% | 10.0% |
| visual | 11 | 3 | 100% | 5% | 5.0% |
| paragraph_var | 1.00 | - | 100% | 5% | 5.0% |
| questions | 14 | 5 | 100% | 5% | 5.0% |
| proverbs | 10 | 1 | 100% | 5% | 5.0% |
| **TOTAL** | | | | | **92.8%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 60 | Included in Core |
| **Вступ: Академічний текст та основи переказу** | ✅ | 209 | Included in Core |
| **Резюме: Як «вичавити сік» і залишити вітаміни** | ⚪️ | 167 | Skipped |
| **Діалог: Обговорення резюме з професором** | ✅ | 491 | Included in Core |
| **Аналіз** | ✅ | 363 | Included in Core |
| **Етика цитування: Де межа між натхненням і крадіжкою** | ⚪️ | 481 | Skipped |
| **Підсумок** | ✅ | 179 | Included in Core |
| **Потрібно більше практики?** | ⚪️ | 76 | Skipped |