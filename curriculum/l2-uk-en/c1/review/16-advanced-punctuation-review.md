# Audit Report: M16 — 16-advanced-punctuation.md
**Level:** C1 | **Module:** M16 | **Phase:** C1.1 | **Pedagogy:** Not Specified | **Target:** 4000
**Naturalness:** None/10 (PENDING)
**Overall Status:** ❌ FAIL
**Generated:** 2026-02-06 20:12:54

## Configuration
**Type:** C1-grammar
**Word Target:** 4000 words
**Activities:** 12-16 required
**Items per Activity:** ≥12 items
**Unique Types:** ≥4 types required
**Priority Types:** error-correction, fill-in, unjumble
**Required Types:** cloze, error-correction, fill-in, group-sort, match-up, quiz
**Engagement:** ≥7 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥25 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Теорія знаків | 8 | 5 | ✅ |
| 2 | match-up | Тире чи Дефіс? | 8 | 6 | ✅ |
| 3 | true-false | Кома: Бути чи не бути? | 8 | 5 | ✅ |
| 4 | group-sort | Класифікація знаків | 18 | 12 | ✅ |
| 5 | error-correction | Виправлення помилок | 6 | 5 | ✅ |
| 6 | unjumble | Складання переліку | 6 | 5 | ✅ |
| 7 | match-up | Синтаксичний пазл | 8 | 6 | ✅ |
| 8 | fill-in | Кома у складному реченні | 8 | 6 | ✅ |
| 9 | select | Пунктуаційна синонімія | 6 | 5 | ✅ |
| 10 | match-up | Переклад термінів | 8 | 6 | ✅ |
| 11 | fill-in | Апостроф чи Ні? | 8 | 6 | ✅ |
| 12 | group-sort | Пунктуаційна культура | 18 | 12 | ✅ |
| 13 | unjumble | Складання речення: Лапки | 6 | 5 | ✅ |
| 14 | quiz | Вибір знака: Складний список | 8 | 5 | ✅ |
| 15 | match-up | Синтаксична синонімія | 8 | 6 | ✅ |
| 16 | essay-response | Письмове завдання: Редагування пунктуації | 1 | 1 | ✅ |

**Summary:**
- Total activities: 16 (target: 12-16) ✅
- Unique types: 9 (minimum: 4) ✅
- Priority types used: 3/3 (error-correction, fill-in, unjumble) ✅
- Required types used: 5/6 (error-correction, fill-in, group-sort, match-up, quiz) ❌
- Low density activities: 0

## PEDAGOGICAL VIOLATIONS
- **[YAML_SCHEMA_VIOLATION]** Schema error in 16-advanced-punctuation.yaml: Schema validation error at key 'min_words': 100 is less than the minimum of 200
  - FIX: Fix the activity YAML to match the schema in schemas/activities-base.schema.json

## Recommendation
**📝 UPDATE** (severity 25/100)

- 1 violations (minor)
- Structure issue: Missing '## Vocabulary' header OR vocabulary sidecar

## Gates
- **Words:** ❌ 2053/4000 (raw: 2183)
- **Activities:** ✅ 16/12
- **Density:** ✅ All > 12
- **Unique_types:** ✅ 9/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 7/7
- **Audio:** ℹ️ No audio
- **Vocab:** ⚠️ 0 < 25 (soft target)
- **Structure:** ❌ Missing '## Vocabulary' header OR vocabulary sidecar
- **Lint:** ✅ Clean Format
- **Pedagogy:** ❌ 1 violations
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 99.9% (target 90-100% (grammar))
- **Richness:** ✅ 96% (grammar)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review

## Richness Details
**Score:** 96% (minimum: 95%)
**Module Type:** grammar

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| examples | 36 | 24 | 100% | 20% | 20.0% |
| engagement | 12 | 5 | 100% | 15% | 15.0% |
| dialogues | 3 | 4 | 75% | 15% | 11.2% |
| variety | 0.99 | - | 99% | 10% | 9.9% |
| cultural | 8 | 3 | 100% | 10% | 10.0% |
| realworld | 6 | 3 | 100% | 10% | 10.0% |
| visual | 7 | 3 | 100% | 5% | 5.0% |
| paragraph_var | 1.00 | - | 100% | 5% | 5.0% |
| questions | 10 | 5 | 100% | 5% | 5.0% |
| proverbs | 10 | 1 | 100% | 5% | 5.0% |
| **TOTAL** | | | | | **96.2%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 125 | Included in Core |
| **Вступ** | ✅ | 194 | Included in Core |
| **Аналіз: Кома у складних конструкціях** | ✅ | 211 | Included in Core |
| **Розділ 2: Тире — знак інтелектуального акценту** | ⚪️ | 165 | Skipped |
| **Розділ 3: Двокрапка та Крапка з комою в переліках** | ⚪️ | 173 | Skipped |
| **Розділ 4: Лапки та цитування** | ⚪️ | 120 | Skipped |
| **Стилістичний вимір: Пунктуація як інтонація** | ⚪️ | 186 | Skipped |
| **Розділ 5: Пунктуація та однозначність** | ⚪️ | 136 | Skipped |
| **Розділ 6: Ситуативний діалог про редагування** | ✅ | 166 | Included in Core |
| **Розділ 7: Пунктуаційна культура в цифрову добу** | ✅ | 156 | Included in Core |
| **Розділ 8: Етика розділових знаків та цитування** | ⚪️ | 107 | Skipped |
| **Розділ 9: Пунктуація у математичних та статистичних виразах** | ⚪️ | 104 | Skipped |
| **Розділ 10: Історія українських розділових знаків** | ⚪️ | 129 | Skipped |
| **Підсумок** | ✅ | 67 | Included in Core |
| **Потрібно більше практики?** | ⚪️ | 14 | Skipped |