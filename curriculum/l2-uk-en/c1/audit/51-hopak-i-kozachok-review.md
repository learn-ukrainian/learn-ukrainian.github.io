# Audit Report: M51 — 51-hopak-i-kozachok.md
**Level:** C1 | **Module:** M51 | **Phase:** C1.5 | **Pedagogy:** CBI | **Target:** 3000
**Overall Status:** ❌ FAIL
**Generated:** 2026-01-30 21:14:28

## Configuration
**Type:** C1-history
**Word Target:** 3000 words
**Activities:** 3-9 required
**Items per Activity:** ≥1 items
**Unique Types:** ≥3 types required
**Priority Types:** comparative-study, critical-analysis, essay-response, reading
**Required Types:** critical-analysis, essay-response, reading
**Engagement:** ≥6 callouts
**Immersion:** 95-100%
**Vocab Target:** ≥25 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Теоретичні основи гопака та козачка | 5 | 5 | ✅ |
| 2 | match-up | Термінологія руху та елементи костюма | 10 | 6 | ✅ |
| 3 | fill-in | Еволюція та деталі хореографії | 6 | 6 | ✅ |
| 4 | critical-analysis | Аналіз явища 'шароварщини' | 1 | 1 | ✅ |
| 5 | comparative-study | Порівняння гопака та козачка | 1 | 1 | ✅ |
| 6 | essay-response | Танець як зброя дипломатії | 1 | 1 | ✅ |
| 7 | authorial-intent | Намір Миколи Гоголя в описі танцю | 1 | 1 | ✅ |
| 8 | true-false | Правда та вигадки про українські танці | 6 | 5 | ✅ |
| 9 | select | Елементи козацької хореографії | 5 | 5 | ✅ |
| 10 | error-correction | Виправлення хореографічних помилок | 6 | 5 | ✅ |
| 11 | group-sort | Класифікація танцювальних стилів та рухів | 12 | 1 | ✅ |
| 12 | unjumble | Цитати про магію українського танцю | 5 | 5 | ✅ |

**Summary:**
- Total activities: 12 (target: 3-9) ❌
- Unique types: 12 (minimum: 3) ✅
- Priority types used: 3/4 (comparative-study, critical-analysis, essay-response) ✅
- Required types used: 2/3 (critical-analysis, essay-response) ❌
- Low density activities: 0

## Recommendation
**📝 UPDATE** (severity 20/100)

- Structure issue: Missing '## Vocabulary' header OR vocabulary sidecar

## Gates
- **Words:** ❌ 1993/3000 (raw: 2193)
- **Activities:** ✅ 12/3
- **Density:** ✅ All > 1
- **Unique_types:** ✅ 12/3 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 7/6
- **Audio:** ℹ️ No audio
- **Vocab:** ⚠️ 0 < 25 (soft target)
- **Structure:** ❌ Missing '## Vocabulary' header OR vocabulary sidecar
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ⚠️ Too many activities: 12 (target 3-9)
- **Immersion:** 🇺🇦 99.1% (target 95-100% (history))
- **Richness:** ✅ 100% (history)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ❌ 0/10 (PENDING) - Naturalness check required

## Richness Details
**Score:** 100% (minimum: 95%)
**Module Type:** history

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| primary_sources | 7 | 3 | 100% | 24% | 23.8% |
| engagement | 7 | 6 | 100% | 14% | 14.3% |
| timeline_markers | 19 | 10 | 100% | 14% | 14.3% |
| decolonization | 11 | 2 | 100% | 14% | 14.3% |
| cultural | 7 | 4 | 100% | 10% | 9.5% |
| visual | 9 | 4 | 100% | 10% | 9.5% |
| variety | 1.00 | - | 100% | 5% | 4.8% |
| paragraph_var | 1.00 | - | 100% | 5% | 4.8% |
| questions | 8 | 3 | 100% | 5% | 4.8% |
| **TOTAL** | | | | | **100.0%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 82 | Included in Core |
| **Вступ: Танець як вираження ідентичності** | ✅ | 88 | Included in Core |
| **Гопак: Від бойового гарту до світової слави** | ⚪️ | 241 | Skipped |
| **Козачок: Швидкість та витонченість** | ⚪️ | 135 | Skipped |
| **Деколонізаційний погляд: Подолання «шароварщини»** | ✅ | 108 | Included in Core |
| **Хореографічна спадщина Павла Вірського** | ⚪️ | 127 | Skipped |
| **Аналіз руху: Від «повзунця» до «розніжки»** | ✅ | 157 | Included in Core |
| **Хронологія танцювальної волі** | ✅ | 112 | Included in Core |
| **Читання: Опис танцю у світовій літературі** | ✅ | 184 | Included in Core |
| **Первинні джерела: Спогади очевидців** | ✅ | 108 | Included in Core |
| **Регіональні варіації: Не гопаком єдиним** | ⚪️ | 118 | Skipped |
| **Порівняльна стилістика: Гопак vs Козачок** | ⚪️ | 0 | Skipped |
| **Сценічний костюм: Глибока мова кольорів та функцій** | ⚪️ | 206 | Skipped |
| **Вплив на світову поп-культуру та сучасне мистецтво** | ✅ | 111 | Included in Core |
| **Need More Practice?** | ⚪️ | 88 | Skipped |
| **Підсумок** | ✅ | 128 | Included in Core |