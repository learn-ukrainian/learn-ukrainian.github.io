# Audit Report: M52 — 52-rehionalni-tantsi.md
**Level:** C1 | **Module:** M52 | **Phase:** C1 | **Pedagogy:** CBI | **Target:** 4000
**Overall Status:** ❌ FAIL
**Generated:** 2026-01-26 22:29:26

## Configuration
**Type:** C1-history
**Word Target:** 4000 words
**Activities:** 3-9 required
**Items per Activity:** ≥1 items
**Unique Types:** ≥3 types required
**Priority Types:** comparative-study, critical-analysis, essay-response, reading
**Required Types:** critical-analysis, essay-response, reading
**Engagement:** ≥6 callouts
**Immersion:** 95-100%
**Vocab Target:** ≥25 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Теоретичні основи регіональної хореографії | 5 | 5 | ✅ |
| 2 | match-up | Регіональні назви та специфічні терміни | 10 | 6 | ✅ |
| 3 | fill-in | Особливості виконання та символізм | 6 | 6 | ✅ |
| 4 | critical-analysis | Аналіз ритуалу ініціації в Аркані | 1 | 1 | ✅ |
| 5 | comparative-study | Порівняння ландшафту та хореографії | 1 | 1 | ✅ |
| 6 | essay-response | Регіональне різноманіття як багатство нації | 1 | 1 | ✅ |
| 7 | authorial-intent | Мета етнографічного опису Гната Хоткевича | 1 | 1 | ✅ |
| 8 | true-false | Правда та міфи про регіональну хореографію | 6 | 5 | ✅ |
| 9 | select | Комплексний аналіз ознак регіональних танців | 5 | 5 | ✅ |
| 10 | error-correction | Виправлення етнографічних помилок | 6 | 5 | ✅ |
| 11 | group-sort | Класифікація танцювальних регіонів | 12 | 1 | ✅ |
| 12 | unjumble | Філософія регіонального руху | 5 | 5 | ✅ |

**Summary:**
- Total activities: 12 (target: 3-9) ❌
- Unique types: 12 (minimum: 3) ✅
- Priority types used: 3/4 (comparative-study, critical-analysis, essay-response) ✅
- Required types used: 2/3 (critical-analysis, essay-response) ❌
- Low density activities: 0

## Recommendation
**📝 UPDATE** (severity 20/100)

- Structure issue: Missing '## Vocabulary' header OR vocabulary sidecar

## Gates
- **Words:** ❌ 1903/4000 (raw: 2130)
- **Activities:** ✅ 12/3
- **Density:** ✅ All > 1
- **Unique_types:** ✅ 12/3 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 7/6
- **Audio:** ℹ️ No audio
- **Vocab:** ⚠️ 0 < 25 (soft target)
- **Structure:** ❌ Missing '## Vocabulary' header OR vocabulary sidecar
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ⚠️ Too many activities: 12 (target 3-9)
- **Immersion:** 🇺🇦 99.7% (target 95-100% (history))
- **Richness:** ✅ 99% (content)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ❌ 0/10 (PENDING) - Naturalness check required

## Richness Details
**Score:** 99% (minimum: 95%)
**Module Type:** content

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| examples | 16 | 15 | 100% | 25% | 25.0% |
| engagement | 7 | 5 | 100% | 19% | 18.7% |
| variety | 1.00 | - | 100% | 12% | 12.5% |
| cultural | 6 | 4 | 100% | 12% | 12.5% |
| realworld | 3 | 3 | 100% | 12% | 12.5% |
| visual | 9 | 4 | 100% | 6% | 6.2% |
| paragraph_var | 1.00 | - | 100% | 6% | 6.2% |
| questions | 5 | 4 | 100% | 6% | 6.2% |
| **TOTAL** | | | | | **100.0%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 86 | Included in Core |
| **Вступ: Географія українського руху** | ✅ | 80 | Included in Core |
| **Аркан: Священний танець чоловічої ініціації** | ⚪️ | 266 | Skipped |
| **Коломийка: Нескінченний вихор народної душі** | ⚪️ | 185 | Skipped |
| **Танці Центральної України: Метелиця та Голуб** | ⚪️ | 91 | Skipped |
| **Деколонізаційний погляд: Проти примітивізації культури** | ✅ | 91 | Included in Core |
| **Танці Півдня та Полісся: Між степом і лісом** | ⚪️ | 72 | Skipped |
| **Закарпатський Чардаш та Буковинська Полька: Гібридність краси** | ⚪️ | 78 | Skipped |
| **Спадщина: Від сільської площі до Карнеґі-холу** | ⚪️ | 77 | Skipped |
| **Аналіз руху: Бартка як продовження душі** | ✅ | 71 | Included in Core |
| **Хронологія наукової документації танцю** | ✅ | 103 | Included in Core |
| **Читання: Екзистенційний опис коломийки** | ✅ | 103 | Included in Core |
| **Первинні джерела: Методологія Василя Верховинця** | ✅ | 84 | Included in Core |
| **Порівняльна стилістика: Гірські vs Степові танці** | ⚪️ | 0 | Skipped |
| **Специфіка Буковини: Аристократизм та статус** | ⚪️ | 72 | Skipped |
| **Сучасний стан регіональних танців** | ⚪️ | 95 | Skipped |
| **Музична магія: Троїсті музики** | ⚪️ | 129 | Skipped |
| **Need More Practice?** | ⚪️ | 87 | Skipped |
| **Підсумок** | ✅ | 133 | Included in Core |