# Audit Report: M54 — 54-vyshyvanka.md
**Level:** C1 | **Module:** M54 | **Phase:** C1.5 | **Pedagogy:** CBI | **Target:** 4000
**Overall Status:** ❌ FAIL
**Generated:** 2026-01-25 19:24:37

## Configuration
**Type:** C1-history
**Word Target:** 4000 words
**Activities:** 3-9 required
**Items per Activity:** ≥1 items
**Unique Types:** ≥3 types required
**Priority Types:** comparative-study, critical-analysis, essay-response, reading
**Required Types:** critical-analysis, essay-response, reading
**Engagement:** ≥6 callouts
**Immersion:** 95-100%
**Vocab Target:** ≥25 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Теоретичні основи вишивального мистецтва | 5 | 5 | ✅ |
| 2 | match-up | Техніки та символи української вишивки | 10 | 6 | ✅ |
| 3 | fill-in | Історія та сучасність вишиванки | 6 | 6 | ✅ |
| 4 | critical-analysis | Аналіз феномену Борщівської вишивки | 1 | 1 | ✅ |
| 5 | comparative-study | Порівняння регіональних шкіл: Полтава vs Гуцульщина | 1 | 1 | ✅ |
| 6 | essay-response | Вишиванка як культурний бронежилет | 1 | 1 | ✅ |
| 7 | authorial-intent | Аналіз 'Пісні про рушник' Андрія Малишка | 1 | 1 | ✅ |
| 8 | true-false | Правда та міфи про українську вишивку | 6 | 5 | ✅ |
| 9 | select | Комплексний аналіз вишивальних орнаментів | 5 | 5 | ✅ |
| 10 | error-correction | Виправлення помилок у семантиці кольорів | 6 | 5 | ✅ |
| 11 | group-sort | Класифікація орнаментів за типом | 12 | 1 | ✅ |
| 12 | unjumble | Філософські твердження про вишиванку | 5 | 5 | ✅ |

**Summary:**
- Total activities: 12 (target: 3-9) ❌
- Unique types: 12 (minimum: 3) ✅
- Priority types used: 3/4 (comparative-study, critical-analysis, essay-response) ✅
- Required types used: 2/3 (critical-analysis, essay-response) ❌
- Low density activities: 0

## TEMPLATE COMPLIANCE
- ❌ **[MISSING_REQUIRED_SECTION]** Missing required section 'Аналіз' per template 'c1-module-template.md'
  - FIX: Add '## Аналіз' section as specified in docs/l2-uk-en/templates/c1-module-template.md.md

## Recommendation
**📝 UPDATE** (severity 25/100)

- 1 violations (minor)
- Structure issue: Missing '## Vocabulary' header OR vocabulary sidecar

## Gates
- **Words:** ❌ 1956/4000 (raw: 2073)
- **Activities:** ✅ 12/3
- **Density:** ✅ All > 1
- **Unique_types:** ✅ 12/3 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 7/6
- **Audio:** ℹ️ No audio
- **Vocab:** ⚠️ 0 < 25 (soft target)
- **Structure:** ❌ Missing '## Vocabulary' header OR vocabulary sidecar
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ⚠️ Too many activities: 12 (target 3-9)
- **Immersion:** 🇺🇦 99.5% (target 95-100% (history))
- **Richness:** ✅ 99% (history)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ❌ 0/10 (PENDING) - Naturalness check required

## Richness Details
**Score:** 99% (minimum: 95%)
**Module Type:** history

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| primary_sources | 6 | 3 | 100% | 24% | 23.8% |
| engagement | 7 | 6 | 100% | 14% | 14.3% |
| timeline_markers | 12 | 10 | 100% | 14% | 14.3% |
| decolonization | 6 | 2 | 100% | 14% | 14.3% |
| cultural | 9 | 4 | 100% | 10% | 9.5% |
| visual | 8 | 4 | 100% | 10% | 9.5% |
| variety | 0.97 | - | 97% | 5% | 4.6% |
| paragraph_var | 1.00 | - | 100% | 5% | 4.8% |
| questions | 7 | 3 | 100% | 5% | 4.8% |
| **TOTAL** | | | | | **99.9%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 96 | Included in Core |
| **Вступ: Генетичний код нації у стібках** | ✅ | 138 | Included in Core |
| **Семантика орнаменту: Геометрія духу та природи** | ⚪️ | 278 | Skipped |
| **Техніки вишивки: Крізь терни до візуальної досконалості** | ⚪️ | 233 | Skipped |
| **Символіка чоловічої vs жіночої сорочки** | ⚪️ | 102 | Skipped |
| **Регіональні стилі: Етнографічна карта України на полотні** | ⚪️ | 173 | Skipped |
| **Деколонізаційний погляд: Вишиванка як політичний маніфест** | ✅ | 126 | Included in Core |
| **Хронологія нитки, голки та волі народу** | ✅ | 128 | Included in Core |
| **Читання: Поезія вишитого рушника як метафора долі** | ✅ | 131 | Included in Core |
| **Первинні джерела: Інтелектуальна праця голки** | ✅ | 65 | Included in Core |
| **Специфіка білого вишивання: Світло на світлі** | ⚪️ | 77 | Skipped |
| **Вишиванка як сучасна інтелектуальна зброя та дипломатія** | ⚪️ | 85 | Skipped |
| **Обрядова культура догляду** | ✅ | 150 | Included in Core |
| **Need More Practice?** | ⚪️ | 55 | Skipped |
| **Підсумок** | ✅ | 119 | Included in Core |