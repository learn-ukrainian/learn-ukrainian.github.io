# Audit Report: M12 — 12-research-article.md
**Level:** C1 | **Module:** M12 | **Phase:** C1.1 | **Pedagogy:** Not Specified | **Target:** 4000
**Naturalness:** None/10 (PENDING)
**Overall Status:** ❌ FAIL
**Generated:** 2026-02-06 20:12:52

## Configuration
**Type:** C1-grammar
**Word Target:** 4000 words
**Activities:** 12-16 required
**Items per Activity:** ≥12 items
**Unique Types:** ≥4 types required
**Priority Types:** error-correction, fill-in, unjumble
**Required Types:** cloze, essay-response, fill-in, group-sort, match-up, quiz
**Engagement:** ≥7 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥25 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | match-up | Архітектура статті (IMRaD) | 8 | 6 | ✅ |
| 2 | match-up | Анатомія статті | 8 | 6 | ✅ |
| 3 | quiz | Теорія жанру | 8 | 5 | ✅ |
| 4 | fill-in | Академічна лексика | 8 | 6 | ✅ |
| 5 | match-up | Вдалий чи невдалий заголовок? | 8 | 6 | ✅ |
| 6 | fill-in | Структура Вступу | 8 | 6 | ✅ |
| 7 | fill-in | Формулювання мети | 8 | 6 | ✅ |
| 8 | quiz | Вибір кращого стилю | 8 | 5 | ✅ |
| 9 | match-up | Українські vs Англійські терміни | 8 | 6 | ✅ |
| 10 | quiz | Етика публікації | 8 | 5 | ✅ |
| 11 | fill-in | Ключові слова | 8 | 6 | ✅ |
| 12 | match-up | Дієслова для розділів | 8 | 6 | ✅ |
| 13 | fill-in | Розділ 'Методи' | 8 | 6 | ✅ |
| 14 | fill-in | Складання речень (Методологія) | 8 | 6 | ✅ |
| 15 | quiz | Вибір журналу | 8 | 5 | ✅ |
| 16 | match-up | Пошук синонімів (Register Repair) | 8 | 6 | ✅ |
| 17 | essay-response | Критичний аналіз анотації | 1 | 1 | ✅ |
| 18 | quiz | Хеджування (Hedging) | 8 | 5 | ✅ |

**Summary:**
- Total activities: 18 (target: 12-16) ❌
- Unique types: 4 (minimum: 4) ✅
- Priority types used: 1/3 (fill-in) ✅
- Required types used: 4/6 (essay-response, fill-in, match-up, quiz) ❌
- Low density activities: 0

## PEDAGOGICAL VIOLATIONS
- **[ROBOTIC_STRUCTURE]** Robotic structure: 3 sentences start with '[ ]...'.
  - FIX: Vary sentence structure.

## Recommendation
**📝 UPDATE** (severity 25/100)

- 1 violations (minor)
- Structure issue: Missing '## Vocabulary' header OR vocabulary sidecar

## Gates
- **Words:** ❌ 2056/4000 (raw: 2158)
- **Activities:** ✅ 18/12
- **Density:** ✅ All > 12
- **Unique_types:** ✅ 4/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 8/7
- **Audio:** ℹ️ No audio
- **Vocab:** ⚠️ 0 < 25 (soft target)
- **Structure:** ❌ Missing '## Vocabulary' header OR vocabulary sidecar
- **Lint:** ✅ Clean Format
- **Pedagogy:** ❌ 1 violations
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 98.5% (target 90-100% (grammar))
- **Richness:** ❌ 82% < 95% min (grammar) - REWRITE needed
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review

## Richness Details
**Score:** 82% (minimum: 95%)
**Module Type:** grammar

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| examples | 95 | 24 | 100% | 20% | 20.0% |
| engagement | 7 | 5 | 100% | 15% | 15.0% |
| dialogues | 1 | 4 | 25% | 15% | 3.8% |
| variety | 1.00 | - | 100% | 10% | 10.0% |
| cultural | 1 | 3 | 33% | 10% | 3.3% |
| realworld | 9 | 3 | 100% | 10% | 10.0% |
| visual | 8 | 3 | 100% | 5% | 5.0% |
| paragraph_var | 1.00 | - | 100% | 5% | 5.0% |
| questions | 17 | 5 | 100% | 5% | 5.0% |
| proverbs | 10 | 1 | 100% | 5% | 5.0% |
| **TOTAL** | | | | | **82.0%** |

### Dryness Flags & Fixes
- ❌ **LOW_DIALOGUE**
  - FIX:
    Add more mini-dialogues (need 4+ total). Use this exact format:
    
    **Діалог: [Location in Ukraine]**
    
    > — [Speaker 1 line with **bolded** grammar examples]
    > — [Speaker 2 response with **bolded** grammar examples]
    > — [Speaker 1 continuation]
    > — [Speaker 2 conclusion]
- ❌ **LOW_CULTURAL_ANCHOR**
  - FIX:
    Add more cultural references (need 3+ total). Include:
    - Named Ukrainian places (Поділ, Бесарабський ринок, Острозька академія)
    - Ukrainian traditions or customs
    - Contemporary Ukrainian life examples

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 89 | Included in Core |
| **Структура IMRaD: Архітектура наукового тексту** | ✅ | 516 | Included in Core |
| **Культура «Фахових видань»** | ✅ | 316 | Included in Core |
| **Стилістика та мовні кліше** | ⚪️ | 131 | Skipped |
| **Академічна доброчесність та Штучний Інтелект** | ⚪️ | 144 | Skipped |
| **Аналіз тексту: Фрагмент статті з соціолінгвістики** | ✅ | 285 | Included in Core |
| **Етика наукової публікації** | ⚪️ | 316 | Skipped |
| **Чек-лист автора** | ⚪️ | 83 | Skipped |
| **Практичні поради з оформлення** | ⚪️ | 76 | Skipped |
| **Підсумок** | ✅ | 60 | Included in Core |
| **Потрібно більше практики?** | ⚪️ | 40 | Skipped |