# Audit Report: 12-research-article.md
**Phase:** C1.1 | **Level:** C1 | **Pedagogy:** Academic | **Target:** 1750
**Overall Status:** ✅ PASS

## PEDAGOGICAL VIOLATIONS
- **[VOCAB_PLAN_MISSING]** Missing vocabulary from plan (1 words): завдання...
  - FIX: Add missing words from curriculum plan to module vocabulary section.

## Recommendation
**📝 UPDATE** (severity 10/100)

- 1 pedagogical violations (minor)

## Gates
- **Words:** ✅ 2067/1750
- **Activities:** ✅ 17/16
- **Density:** ✅ All > 18
- **Unique_types:** ✅ 4/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 7/7
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 47/25
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 98.1% (target 98-100%)
- **Richness:** ✅ 99% (academic)
- **Grammar:** ⏳ Pending validation

## Richness Details
**Score:** 99% (minimum: 95%)
**Module Type:** academic

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| citations | 6 | 5 | 100% | 24% | 23.8% |
| data_refs | 6 | 3 | 100% | 19% | 19.0% |
| visual | 9 | 5 | 100% | 14% | 14.3% |
| questions | 16 | 4 | 100% | 14% | 14.3% |
| engagement | 7 | 5 | 100% | 10% | 9.5% |
| variety | 1.00 | - | 100% | 10% | 9.5% |
| cultural | 1 | - | 100% | 5% | 4.8% |
| paragraph_var | 1.00 | - | 100% | 5% | 4.8% |
| **TOTAL** | | | | | **100.0%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 89 | Included in Core |
| **IMRaD: Архітектура наукового тексту** | ✅ | 516 | Included in Core |
| **Культура "Фахових видань"** | ✅ | 316 | Included in Core |
| **Стилістика та мовні кліше** | ⚪️ | 131 | Skipped |
| **Академічна доброчесність та Штучний Інтелект** | ⚪️ | 144 | Skipped |
| **Текст для аналізу: Фрагмент статті з соціолінгвістики** | ✅ | 285 | Included in Core |
| **Етика наукової публікації** | ⚪️ | 316 | Skipped |
| **Чек-лист автора** | ⚪️ | 83 | Skipped |
| **Практичні поради з оформлення** | ⚪️ | 76 | Skipped |
| **Підсумок** | ✅ | 111 | Included in Core |