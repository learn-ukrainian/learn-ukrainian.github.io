# Audit Report: M102 — 102-kotsiubynskyi.md
**Level:** C1 | **Module:** M102 | **Phase:** C1 | **Pedagogy:** Not Specified | **Target:** 3000
**Overall Status:** ❌ FAIL
**Generated:** 2026-02-06 20:12:49

## Configuration
**Type:** C1
**Word Target:** 3000 words
**Activities:** 12-16 required
**Items per Activity:** ≥12 items
**Unique Types:** ≥4 types required
**Priority Types:** error-correction, fill-in, unjumble
**Engagement:** ≥7 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥25 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Розуміння аналізу імпресіонізму Коцюбинського | 12 | 5 | ✅ |
| 2 | match-up | Символи та образи у творчості Коцюбинського | 10 | 6 | ✅ |
| 3 | fill-in | Термінологія імпресіонізму | 10 | 6 | ✅ |
| 4 | group-sort | Intermezzo vs Тіні забутих предків | 16 | 12 | ✅ |
| 5 | unjumble | Побудова тез аналізу творчості Коцюбинського | 8 | 5 | ✅ |
| 6 | unjumble | Тези про імпресіонізм (частина 2) | 8 | 5 | ✅ |
| 7 | cloze | Аналіз стилю Коцюбинського | 13 | 12 | ✅ |
| 8 | error-correction | Виправлення помилок у тексті про імпресіонізм | 8 | 5 | ✅ |
| 9 | select | Аналіз стилю та образів | 8 | 5 | ✅ |
| 10 | translate | Переклад тез про Коцюбинського | 8 | 5 | ✅ |
| 11 | essay-response | Критичне есе про роль природи | 1 | 1 | ✅ |
| 12 | critical-analysis | Аналіз уривка з «Intermezzo» | 1 | 1 | ✅ |
| 13 | reading | Коцюбинський і кіно | 3 | 3 | ✅ |

**Summary:**
- Total activities: 13 (target: 12-16) ✅
- Unique types: 12 (minimum: 4) ✅
- Priority types used: 3/3 (error-correction, fill-in, unjumble) ✅
- Low density activities: 0

## PEDAGOGICAL VIOLATIONS
- **[CONTENT_REDUNDANCY]** Redundant information detected in lesson (100% overlap): "---

### Первинний аналіз

**Питання для першого читання:**

1.". Shares significant keywords with sentence at index 40.
  - FIX: Remove redundant paragraphs. Ensure each section adds new unique value.
- **[YAML_SCHEMA_VIOLATION]** Schema error in 102-kotsiubynskyi.yaml: Schema validation error at key 'words': ['Мова', 'Коцюбинського', 'відзначається', 'вишуканістю', 'ритмічністю', 'та', 'багатством', 'художніх', 'засобів', 'особливо', 'епітетів'] is too short
  - FIX: Fix the activity YAML to match the schema in schemas/activities-base.schema.json

## TEMPLATE COMPLIANCE
- ❌ **[DUPLICATE_SYNONYMOUS_HEADERS]** Multiple headers contain 'Аналіз': Порівняльний аналіз: Коцюбинський vs. Стефаник, Первинний аналіз, Аналіз «Intermezzo»: Конфлікт і Образна система, Літературний аналіз, Аналіз «Тіней забутих предків»: Тематика і Стиль
  - FIX: RENAME one header to NOT contain 'Аналіз'. Example: 'Агіографічна спадщина' → 'Житійна творчість' (removes the duplicate word).

## Recommendation
**📝 UPDATE** (severity 25/100)

- 3 violations (minor)
- Structure issue: Missing '## Vocabulary' header OR vocabulary sidecar

## Gates
- **Words:** ❌ 1917/3000 (raw: 2132)
- **Activities:** ✅ 13/12
- **Density:** ✅ All > 12
- **Unique_types:** ✅ 12/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ❌ 6/7
- **Audio:** ℹ️ No audio
- **Vocab:** ⚠️ 0 < 25 (soft target)
- **Structure:** ❌ Missing '## Vocabulary' header OR vocabulary sidecar
- **Lint:** ✅ Clean Format
- **Pedagogy:** ❌ 2 violations
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 99.0% (target 90-100%)
- **Richness:** ❌ 84% < 95% min (content)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review

## Richness Details
**Score:** 84% (minimum: 95%)
**Module Type:** content

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| examples | 44 | 15 | 100% | 25% | 25.0% |
| engagement | 6 | 5 | 100% | 19% | 18.7% |
| variety | 0.98 | - | 98% | 12% | 12.2% |
| cultural | 3 | 4 | 75% | 12% | 9.4% |
| realworld | 0 | 3 | 0% | 12% | 0.0% |
| visual | 7 | 4 | 100% | 6% | 6.2% |
| paragraph_var | 1.00 | - | 100% | 6% | 6.2% |
| questions | 28 | 4 | 100% | 6% | 6.2% |
| **TOTAL** | | | | | **84.1%** |

### Dryness Flags & Fixes
- ❌ **ABSTRACT_ONLY**
  - FIX:
    Add 3+ real-world boxes. Use this exact format:
    
    > 🌍 **У реальному житті**
    >
    > [Specific scenario: "На співбесіді...", "У магазині...", "На вокзалі..."]
    > [Example sentence showing grammar in that context]

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 71 | Included in Core |
| **Біографічний контекст: Європеєць в українській літературі** | ✅ | 385 | Included in Core |
| **Текст** | ✅ | 301 | Included in Core |
| **Літературний аналіз** | ✅ | 469 | Included in Core |
| **Порівняльний аналіз: Коцюбинський vs. Стефаник** | ✅ | 80 | Included in Core |
| **Вплив та Спадщина: Магія кіно** | ⚪️ | 54 | Skipped |
| **Критичне есе** | ⚪️ | 393 | Skipped |
| **Підсумок** | ✅ | 48 | Included in Core |
| **Потрібно більше практики?** | ⚪️ | 116 | Skipped |