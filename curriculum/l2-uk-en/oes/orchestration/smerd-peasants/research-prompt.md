...
version: '2.0'
title: Смерди (селяни)
subtitle: The Smerd (Peasants)
focus: Feudal relationships
phase: 2
word_target: 5000
content_outline:
- section: Вступ
  points:
  - Introduction to The Smerd (Peasants)
  words: 1250
...
Research **Смерди (селяни)** for the **oes** track. Produce structured research notes that will drive content writing in Phase B.
...
- `query_wikipedia(query="Смерди (селяни)", mode="extract")` — factual backbone
- `search_literary(query="Смерди (селяни)")` — primary source excerpts
