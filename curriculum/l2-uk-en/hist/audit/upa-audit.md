# Audit Report: M107 — upa.md
**Level:** HIST | **Module:** M107 | **Phase:** HIST.10 | **Pedagogy:** CBI | **Target:** 5000
**Overall Status:** ✅ PASS
**Generated:** 2026-02-26 00:27:30

## Configuration
**Type:** history
**Word Target:** 5000 words
**Activities:** 3-9 required
**Items per Activity:** ≥1 items
**Unique Types:** ≥2 types required
**Priority Types:** comparative-study, critical-analysis, essay-response, reading
**Required Types:** essay-response, reading
**Engagement:** ≥5 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥20 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | reading | Первинне джерело: Присяга воїна УПА | 1 | 1 | ✅ |
| 2 | essay-response | Аналітичне есе: Війна на два фронти | 1 | 1 | ✅ |
| 3 | true-false | Перевірка фактів: Історія УПА | 10 | 8 | ✅ |
| 4 | critical-analysis | Критичний аналіз: Пропаганда та ідеологія | 1 | 1 | ✅ |
| 5 | comparative-study | Порівняльний аналіз: Радянські міфи та реальність | 1 | 1 | ✅ |

**Summary:**
- Total activities: 5 (target: 3-9) ✅
- Unique types: 5 (minimum: 2) ✅
- Priority types used: 4/4 (comparative-study, critical-analysis, essay-response, reading) ✅
- Required types used: 2/2 (essay-response, reading) ✅
- Low density activities: 0

## Gates
- **Words:** ✅ 5809/5000 (raw: 6289)
- **Activities:** ✅ 5/3
- **Density:** ✅ All > 1
- **Unique_types:** ✅ 5/2 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 6/5
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 25/20
- **Structure:** ✅ Valid Structure
- **Ipa:** ✅ Clean IPA
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ✅ Content-heavy OK (5 activities)
- **Immersion:** 🇺🇦 98.9% (target 90-100% (history))
- **Richness:** ✅ 97% (history)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review
- **Research:** ⚠️ Refresh recommended: Research has 5+ sources but content cites 0

## Richness Details
**Score:** 97% (minimum: 95%)
**Module Type:** history

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| primary_sources | 15 | 3 | 100% | 24% | 23.8% |
| engagement | 5 | 6 | 83% | 14% | 11.9% |
| timeline_markers | 30 | 10 | 100% | 14% | 14.3% |
| decolonization | 15 | 2 | 100% | 14% | 14.3% |
| cultural | 9 | 4 | 100% | 10% | 9.5% |
| visual | 6 | 4 | 100% | 10% | 9.5% |
| variety | 0.97 | - | 97% | 5% | 4.6% |
| paragraph_var | 1.00 | - | 100% | 5% | 4.8% |
| questions | 10 | 3 | 100% | 5% | 4.8% |
| **TOTAL** | | | | | **97.4%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 0 | Included in Core |
| **Вступ: Війна на два фронти** | ✅ | 858 | Included in Core |
| **Створення та організація: Об'єднання сил** | ✅ | 700 | Included in Core |
| **Створення та організація: Структура і тил** | ✅ | 883 | Included in Core |
| **Бойовий шлях: Антинімецький фронт** | ✅ | 633 | Included in Core |
| **Бойовий шлях: Волинська трагедія** | ✅ | 573 | Included in Core |
| **Бойовий шлях: Протистояння з СРСР та Польщею** | ✅ | 648 | Included in Core |
| **Первинні джерела** | ✅ | 486 | Included in Core |
| **Деколонізаційний погляд** | ✅ | 500 | Included in Core |
| **Підсумок: Спадщина** | ✅ | 335 | Included in Core |
| **Підсумок** | ✅ | 193 | Included in Core |