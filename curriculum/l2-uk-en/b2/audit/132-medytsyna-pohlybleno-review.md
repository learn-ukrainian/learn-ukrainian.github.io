# Audit Report: 132-medytsyna-pohlybleno.md
**Phase:** B2.4 | **Level:** B2 | **Pedagogy:** CBI | **Target:** 1750
**Overall Status:** ❌ FAIL

## Recommendation
**📝 UPDATE** (severity 10/100)

- Activity density below minimum

## Gates
- **Words:** ✅ 1956/1750
- **Activities:** ✅ 14/14
- **Density:** ❌ 1 < 16
- **Unique_types:** ✅ 11/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 8/6
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 25/25
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 99.4% (target 98-100% (skills))
- **Richness:** ❌ 79% < 95% min (content)
- **Grammar:** ⏳ Pending validation

## Richness Details
**Score:** 79% (minimum: 95%)
**Module Type:** content

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| examples | 51 | 15 | 100% | 25% | 25.0% |
| engagement | 6 | 5 | 100% | 19% | 18.7% |
| variety | 0.99 | - | 99% | 12% | 12.4% |
| cultural | 2 | 4 | 50% | 12% | 6.2% |
| realworld | 0 | 3 | 0% | 12% | 0.0% |
| visual | 3 | 4 | 75% | 6% | 4.7% |
| paragraph_var | 1.00 | - | 100% | 6% | 6.2% |
| questions | 21 | 4 | 100% | 6% | 6.2% |
| **TOTAL** | | | | | **79.6%** |

### Dryness Flags & Fixes
- ❌ **ABSTRACT_ONLY**
  - FIX:
    Add 3+ real-world boxes. Use this exact format:
    
    > 🌍 **У реальному житті**
    >
    > [Specific scenario: "На співбесіді...", "У магазині...", "На вокзалі..."]
    > [Example sentence showing grammar in that context]

## Low Density Activities
| Activity | Type | Items | Required | Fix |
|----------|------|-------|----------|-----|
| На прийомі у лікаря | cloze | 13 | 16 | Add 3 more items |


## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 68 | Included in Core |
| **Вступ** | ⚪️ | 393 | Skipped |
| **Діалог лікар-пацієнт** | ✅ | 372 | Included in Core |
| **Спеціалізації та спеціалісти** | ⚪️ | 110 | Skipped |
| **Медичні обстеження та аналізи** | ✅ | 255 | Included in Core |
| **Ліки та рецепти** | ⚪️ | 237 | Skipped |
| **Профілактика та щеплення** | ⚪️ | 149 | Skipped |
| **Екстрені ситуації** | ⚪️ | 250 | Skipped |
| **Підсумок** | ✅ | 122 | Included in Core |