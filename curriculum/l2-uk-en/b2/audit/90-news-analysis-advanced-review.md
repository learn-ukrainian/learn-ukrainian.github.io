# Audit Report: M90 — 90-news-analysis-advanced.md
**Level:** B2 | **Module:** M90 | **Phase:** B2 | **Pedagogy:** CBI | **Target:** 4000
**Naturalness:** 8/10 (PASS)
**Overall Status:** ❌ FAIL
**Generated:** 2026-01-26 22:28:46

## Configuration
**Type:** B2-history
**Word Target:** 4000 words
**Activities:** 3-9 required
**Items per Activity:** ≥1 items
**Unique Types:** ≥2 types required
**Priority Types:** comparative-study, critical-analysis, essay-response, reading
**Required Types:** essay-response, reading
**Engagement:** ≥5 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥20 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Розпізнавання упередженості в новинах | 10 | 8 | ✅ |
| 2 | match-up | Терміни медіааналізу | 16 | 8 | ✅ |
| 3 | fill-in | Аналіз медіатекстів | 15 | 8 | ✅ |
| 4 | true-false | Твердження про медіаграмотність | 15 | 8 | ✅ |
| 5 | unjumble | Відновіть правильний порядок речень | 12 | 6 | ✅ |
| 6 | error-correction | Виправте помилки в аналізі медіатекстів | 10 | 6 | ✅ |
| 7 | cloze | Текст з пропусками про медіаманіпуляції | 15 | 1 | ✅ |
| 8 | group-sort | Класифікація медіатехнік | 24 | 1 | ✅ |
| 9 | mark-the-words | Знайдіть маркери упередженості | 6 | 6 | ✅ |
| 10 | select | Множинний вибір ознак маніпуляції | 8 | 6 | ✅ |
| 11 | translate | Переклад термінів медіааналізу | 8 | 6 | ✅ |
| 12 | quiz | Глибокий аналіз медіатекстів | 10 | 8 | ✅ |
| 13 | essay-response | Критичний аналіз новинного матеріалу | 1 | 1 | ✅ |
| 14 | comparative-study | Порівняльний аналіз висвітлення події | 1 | 1 | ✅ |

**Summary:**
- Total activities: 14 (target: 3-9) ❌
- Unique types: 13 (minimum: 2) ✅
- Priority types used: 2/4 (comparative-study, essay-response) ✅
- Required types used: 1/2 (essay-response) ❌
- Low density activities: 0

## Gates
- **Words:** ✅ 4057/4000 (raw: 4179)
- **Activities:** ✅ 14/3
- **Density:** ✅ All > 1
- **Unique_types:** ✅ 13/2 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 5/5
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 34/20
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ⚠️ Too many activities: 14 (target 3-9)
- **Immersion:** 🇺🇦 96.8% (target 90-100% (history))
- **Richness:** ✅ 95% (content)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ✅ 8/10 (High)

## Richness Details
**Score:** 95% (minimum: 95%)
**Module Type:** content

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| examples | 20 | 15 | 100% | 25% | 25.0% |
| engagement | 5 | 5 | 100% | 19% | 18.7% |
| variety | 0.99 | - | 99% | 12% | 12.4% |
| cultural | 6 | 4 | 100% | 12% | 12.5% |
| realworld | 5 | 3 | 100% | 12% | 12.5% |
| visual | 1 | 4 | 25% | 6% | 1.6% |
| paragraph_var | 1.00 | - | 100% | 6% | 6.2% |
| questions | 26 | 4 | 100% | 6% | 6.2% |
| **TOTAL** | | | | | **95.2%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 109 | Included in Core |
| **Аналіз новин: Поглиблено** | ✅ | 66 | Included in Core |
| **Вступ** | ✅ | 353 | Included in Core |
| **Імерсивна розповідь: Одна подія — п'ять наративів** | ⚪️ | 1886 | Skipped |
| **Аналіз: Медіаекосистема та інформаційні бульбашки** | ✅ | 986 | Included in Core |
| **Граматика: Хеджування та модальність** | ⚪️ | 283 | Skipped |
| **Підсумок** | ✅ | 238 | Included in Core |
| **Потрібно більше практики?** | ⚪️ | 136 | Skipped |