# Audit Report: 08-active-participles-past.md
**Phase:** B2.1a | **Level:** B2 | **Pedagogy:** TTT | **Target:** 1750
**Overall Status:** ✅ PASS

## PEDAGOGICAL VIOLATIONS
- **[VOCAB_PLAN_MISSING]** Missing vocabulary from plan (8 words): теперішній, попередній, вживаний, майбутній, останній...
  - FIX: Add missing words from curriculum plan to module vocabulary section.

## Recommendation
**📝 UPDATE** (severity 10/100)

- 1 pedagogical violations (minor)

## Gates
- **Words:** ⚠️ 1738/1750 (12 short)
- **Activities:** ✅ 13/13
- **Density:** ✅ All > 16
- **Unique_types:** ✅ 11/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 12/6
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 145/25
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ℹ️ N/A (standard module)
- **Immersion:** 🇺🇦 100.0% (target 98-100% (grammar))
- **Richness:** ✅ 98% (grammar)
- **Grammar:** ⏳ Pending validation

## Richness Details
**Score:** 98% (minimum: 95%)
**Module Type:** grammar

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| examples | 28 | 24 | 100% | 20% | 20.0% |
| engagement | 8 | 5 | 100% | 15% | 15.0% |
| dialogues | 12 | 4 | 100% | 15% | 15.0% |
| variety | 0.94 | - | 94% | 10% | 9.4% |
| cultural | 3 | 3 | 100% | 10% | 10.0% |
| realworld | 3 | 3 | 100% | 10% | 10.0% |
| visual | 9 | 3 | 100% | 5% | 5.0% |
| paragraph_var | 0.81 | - | 81% | 5% | 4.1% |
| questions | 25 | 5 | 100% | 5% | 5.0% |
| proverbs | 1 | 1 | 100% | 5% | 5.0% |
| **TOTAL** | | | | | **98.5%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 87 | Included in Core |
| **Тест: Прочитайте текст** | ✅ | 236 | Included in Core |
| **Пояснення** | ⚪️ | 397 | Skipped |
| **Синтетичний майбутній час** | ⚪️ | 272 | Skipped |
| **Культурний контекст** | ✅ | 172 | Included in Core |
| **Практика** | ⚪️ | 165 | Skipped |
| **Діалоги** | ✅ | 292 | Included in Core |
| **Підсумок** | ✅ | 0 | Included in Core |
| **Що ви вивчили** | ⚪️ | 65 | Skipped |
| **Самоперевірка** | ⚪️ | 52 | Skipped |