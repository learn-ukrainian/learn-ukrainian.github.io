# Audit Report: 86-kozatska-kultura.md
**Phase:** B2.3b | **Level:** B2 | **Pedagogy:** CBI | **Target:** 2000
**Naturalness:** 9/10 (PASS)
**Overall Status:** ✅ PASS

## Gates
- **Words:** ✅ 2292/2000
- **Activities:** ✅ 14/10
- **Density:** ✅ All > 14
- **Unique_types:** ✅ 13/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 7/5
- **Audio:** ℹ️ No audio
- **Vocab:** ✅ 102/20
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ✅ Content-heavy OK (14 activities)
- **Immersion:** 🇺🇦 99.7% (target 90-100% (history))
- **Richness:** ✅ 99% (history)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ✅ 9/10 (High)

## Richness Details
**Score:** 99% (minimum: 95%)
**Module Type:** history

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| primary_sources | 9 | 3 | 100% | 24% | 23.8% |
| engagement | 7 | 6 | 100% | 14% | 14.3% |
| timeline_markers | 15 | 10 | 100% | 14% | 14.3% |
| decolonization | 12 | 2 | 100% | 14% | 14.3% |
| cultural | 7 | 4 | 100% | 10% | 9.5% |
| visual | 6 | 4 | 100% | 10% | 9.5% |
| variety | 0.97 | - | 97% | 5% | 4.6% |
| paragraph_var | 1.00 | - | 100% | 5% | 4.8% |
| questions | 8 | 3 | 100% | 5% | 4.8% |
| **TOTAL** | | | | | **99.9%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 91 | Included in Core |
| **Контекст** | ✅ | 310 | Included in Core |
| **Кобзарі та лірники: голос народу** | ⚪️ | 327 | Skipped |
| **Читання** | ✅ | 150 | Included in Core |
| **Первинні джерела** | ⚪️ | 187 | Skipped |
| **Козацьке бароко та архітектура** | ⚪️ | 214 | Skipped |
| **Освіта та книговидання** | ⚪️ | 156 | Skipped |
| **Деколонізаційний погляд** | ⚪️ | 212 | Skipped |
| **Есе** | ⚪️ | 409 | Skipped |
| **Підсумок** | ✅ | 103 | Included in Core |
| **Потрібно більше практики?** | ⚪️ | 133 | Skipped |