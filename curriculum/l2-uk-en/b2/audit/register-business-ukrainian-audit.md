# Audit Report: M17 — register-business-ukrainian.md
**Level:** B2 | **Module:** M17 | **Phase:** B2.1b | **Pedagogy:** Not Specified | **Target:** 4000
**Overall Status:** ✅ PASS
**Generated:** 2026-02-24 17:17:13

## Configuration
**Type:** B2-grammar
**Word Target:** 4000 words
**Activities:** 10-14 required
**Items per Activity:** ≥14 items
**Unique Types:** ≥4 types required
**Priority Types:** cloze, error-correction, fill-in, unjumble
**Required Types:** error-correction, essay-response, fill-in, match-up, reading
**Engagement:** ≥6 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥25 words
**Transliteration:** Not allowed

## Gates
- **Words:** ✅ 4272/4000 (raw: 4628)
- **Activities:** ⏳ Deferred (content-only audit)
- **Density:** ⏳ Deferred (content-only audit)
- **Unique_types:** ⏳ Deferred (content-only audit)
- **Priority:** ⏳ Deferred (content-only audit)
- **Engagement:** ✅ 13/6
- **Audio:** ℹ️ No audio
- **Vocab:** ⏳ Deferred (content-only audit)
- **Structure:** ✅ Valid Structure
- **Ipa:** ✅ Clean IPA
- **Lint:** ✅ Clean Format
- **Pedagogy:** ✅ Level-appropriate
- **Content_heavy:** ⏳ Deferred (content-only audit)
- **Immersion:** 🇺🇦 99.6% (target 90-100% (grammar))
- **Richness:** ✅ 99% (style)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ℹ️ PENDING — awaiting review
- **Research:** ✅ Content aligned with research

## Richness Details
**Score:** 99% (minimum: 95%)
**Module Type:** style

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| exemplar_texts | 15 | 2 | 100% | 25% | 25.0% |
| model_answers | 100 | 3 | 100% | 20% | 20.0% |
| engagement | 10 | 5 | 100% | 15% | 15.0% |
| register_analysis | 15 | 5 | 100% | 15% | 15.0% |
| visual | 13 | 4 | 100% | 10% | 10.0% |
| variety | 0.99 | - | 99% | 5% | 5.0% |
| cultural | 2 | - | 100% | 5% | 5.0% |
| paragraph_var | 0.83 | - | 83% | 5% | 4.2% |
| **TOTAL** | | | | | **99.1%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 0 | Included in Core |
| **Ділова українська мова** | ✅ | 50 | Included in Core |
| **Вступ: Поняття ділового стилю** | ✅ | 489 | Included in Core |
| **Ділове та електронне листування** | ✅ | 933 | Included in Core |
| **Переговори та укладання угод** | ✅ | 892 | Included in Core |
| **Корпоративна комунікація та етика** | ✅ | 708 | Included in Core |
| **Типові помилки та мовна норма** | ✅ | 654 | Included in Core |
| **Підсумок** | ✅ | 546 | Included in Core |