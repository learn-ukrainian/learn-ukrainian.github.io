# Audit Report: M79 — 79-capstone-research.md
**Level:** B2 | **Module:** M79 | **Phase:** B2.4 | **Pedagogy:** integration | **Target:** 2000
**Naturalness:** 9/10 (PASS)
**Overall Status:** ❌ FAIL
**Generated:** 2026-02-05 23:09:20

## Configuration
**Type:** B2-checkpoint
**Word Target:** 2000 words
**Activities:** 15-19 required
**Items per Activity:** ≥14 items
**Unique Types:** ≥4 types required
**Priority Types:** cloze, error-correction, fill-in, quiz
**Required Types:** essay-response, reading, true-false
**Engagement:** ≥4 callouts
**Immersion:** 90-100%
**Vocab Target:** ≥10 words
**Transliteration:** Not allowed

## Activity Breakdown
| # | Type | Title | Items | Min | Status |
|---|------|-------|-------|-----|--------|
| 1 | quiz | Фази підготовчої роботи | 10 | 8 | ✅ |
| 2 | true-false | Етика та методологія проєкту | 10 | 8 | ✅ |
| 3 | fill-in | Словник проєктного менеджменту | 10 | 8 | ✅ |
| 4 | unjumble | Логіка планування | 8 | 6 | ✅ |
| 5 | group-sort | Класифікація джерел інформації | 15 | 14 | ✅ |
| 6 | mark-the-words | Аналіз плану-проспекту | 9 | 6 | ✅ |
| 7 | cloze | Консультація щодо теми проєкту | 19 | 14 | ✅ |
| 8 | error-correction | Редагування дослідницьких тез | 8 | 6 | ✅ |
| 9 | translate | Проєктна термінологія | 8 | 6 | ✅ |
| 10 | select | Якість дослідницьких питань | 8 | 6 | ✅ |
| 11 | cloze | Архівна справа в цифрову епоху | 16 | 14 | ✅ |
| 12 | translate | Від чернетки до академічного вислову | 8 | 6 | ✅ |
| 13 | quiz | Об'єкт та предмет дослідження | 10 | 8 | ✅ |
| 14 | match-up | Дослідницькі колокації | 12 | 8 | ✅ |
| 15 | translate | Ключові фрази дослідження | 8 | 6 | ✅ |
| 16 | reading | Текст для аналізу: Підсумковий проєкт: Дослідження | 3 | 3 | ✅ |
| 17 | essay-response | Письмова відповідь: Підсумковий проєкт: Дослідження | 1 | 1 | ✅ |

**Summary:**
- Total activities: 17 (target: 15-19) ✅
- Unique types: 13 (minimum: 4) ✅
- Priority types used: 4/4 (cloze, error-correction, fill-in, quiz) ✅
- Required types used: 3/3 (essay-response, reading, true-false) ✅
- Low density activities: 0

## PEDAGOGICAL VIOLATIONS
- **[COMPLEXITY_WORD_COUNT]** quiz 'Фази підготовчої роботи' Q9 prompt length 3 (target: 5-25)
  - FIX: Adjust prompt length to 5-25 words.
- **[COMPLEXITY_WORD_COUNT]** quiz 'Фази підготовчої роботи' Q10 prompt length 3 (target: 5-25)
  - FIX: Adjust prompt length to 5-25 words.
- **[COMPLEXITY_WORD_COUNT]** quiz 'Об'єкт та предмет дослідження' Q9 prompt length 3 (target: 5-25)
  - FIX: Adjust prompt length to 5-25 words.
- **[COMPLEXITY_WORD_COUNT]** quiz 'Об'єкт та предмет дослідження' Q10 prompt length 3 (target: 5-25)
  - FIX: Adjust prompt length to 5-25 words.

## Recommendation
**📝 UPDATE** (severity 15/100)

- 4 violations (moderate)

## Gates
- **Words:** ❌ 1829/2000 (raw: 1989)
- **Activities:** ✅ 17/15
- **Density:** ✅ All > 14
- **Unique_types:** ✅ 13/4 types
- **Priority:** ✅ Priority types used
- **Engagement:** ✅ 7/4
- **Audio:** ℹ️ No audio
- **Vocab:** ⚠️ 6 < 10 (soft target)
- **Structure:** ✅ Valid Structure
- **Lint:** ✅ Clean Format
- **Pedagogy:** ❌ 4 violations
- **Content_heavy:** ✅ Content-heavy OK (17 activities)
- **Immersion:** 🇺🇦 99.0% (checkpoint - no gate)
- **Richness:** ✅ 99% (checkpoint)
- **Grammar:** ℹ️ N/A (covered by naturalness)
- **Naturalness:** ✅ 9/10 (High)

## Richness Details
**Score:** 99% (minimum: 85%)
**Module Type:** checkpoint

### Score Breakdown
| Metric | Count | Target | Score | Weight | Contribution |
|--------|-------|--------|-------|--------|--------------|
| activity_types | 13 | 8 | 100% | 25% | 25.0% |
| review_sections | 25 | 3 | 100% | 20% | 20.0% |
| variety | 0.96 | - | 96% | 15% | 14.4% |
| engagement | 7 | 3 | 100% | 10% | 10.0% |
| cultural | 3 | - | 100% | 10% | 10.0% |
| visual | 13 | 3 | 100% | 10% | 10.0% |
| paragraph_var | 1.00 | - | 100% | 10% | 10.0% |
| **TOTAL** | | | | | **99.4%** |

## Section Audit
| Section | Status | Count | Notes |
|---|---|---|---|
| **Intro/Narrative** | ✅ | 0 | Included in Core |
| **Огляд — Capstone проєкт** | ⚪️ | 167 | Skipped |
| **Навичка 1: Ключові знання модуля** | ⚪️ | 38 | Skipped |
| **Вибір теми дослідження** | ⚪️ | 312 | Skipped |
| **Пошук та оцінка джерел** | ⚪️ | 365 | Skipped |
| **Складання плану-проспекту** | ⚪️ | 135 | Skipped |
| **Написання чернетки** | ⚪️ | 537 | Skipped |
| **Підсумок та завдання** | ✅ | 275 | Included in Core |