===RESEARCH_START===

# Дослідження: Релігія в Україні

## State Standard Reference
§4.3.4.1: "Складнопідрядні речення зі з’ясувальною підрядною частиною (сполучники що, щоб, сполучні слова куди, звідки, скільки, чому, навіщо, який): Батько радив мені, щоб я надрукував цю статтю. Треба вміти слухати, про що співає птах."
§4.3.4.2: "Складнопідрядні речення з означальною частиною зі сполучними словами який, котрий, що в називному та непрямих відмінках: Як радію за ту я людину, що загублену радість знайшла."
Alignment: This directly maps to the module's grammar focus on formulating complex sentences (explanatory and attributive) to discuss abstract concepts like church doctrines, historical decisions, and interfaith dialogue.

## Vocabulary Frequency
| Word | Frequency / Source | Key collocations |
|------|-------------------|------------------|
| віра | High (General & abstract) | віра в Бога, символ віри, прийняти на віру, віра в ЗСУ/перемогу |
| релігія | High (Academic/Formal) | традиційна релігія, релігійне життя, релігійна громада, свобода релігії |
| церква | High (General) | помісна церква, глава церкви, церковне свято, православна церква |
| свято | High (General) | велике свято, з нагоди свята, відзначати свято, з прийдешнім святом |
| конфесія | Medium (Academic/News) | міжконфесійний діалог, приналежність до конфесії, поліконфесійність |
| святий | High (Attribute/Noun) | Святе Письмо, свята вода, Святий вечір, зарахувати до лику святих |
| капелан | High (Current context) | військовий капелан, служба військового капеланства |

## Cultural Hooks
1. Концепція «Київ — Другий Єрусалим»: У добу митрополита Петра Могили та гетьмана Івана Мазепи Київ розбудовувався як сакральний центр, де Софія Київська символічно відтворювала храм Соломона, а Золоті ворота підкреслювали богообраність міста.
2. Всеукраїнська рада церков і релігійних організацій (ВРЦіРО): Унікальний феномен українського громадянського суспільства, що об'єднує понад 90% вірян різних деномінацій та слугує платформою толерантності та опору під час війни.

## Common Learner Errors
1. Калькування з російської "З наступаючим святом!" (С наступающим!) → Correct form: «З прийдешнім святом!» або «Зі святом!» — The calque uses an active present participle unnatural to Ukrainian syntax.
2. Confusing the noun "Свято" with the adjective/noun "Святий" → Correct: «Сьогодні велике свято», «Настав Святий вечір» — Learners often map English "holiday" and "holy" improperly.
3. Prepositional government of "вірити" → Correct form: «вірити в» + Зн.в. (віра в Бога, віра в перемогу) vs «вірити» + Дав.в. (довіряти комусь: вірити капеланові, вірити другові).

## Cross-References
- Builds on: B1 sentence structures and abstract vocabulary foundations.
- Prepares for: HIST modules dealing with church history (e.g., Beresteiska Union, Kyiv-Mohyla Academy) and C1 level discussions on identity and society.

## Notes for Content Writing
- Decolonized framing is MANDATORY. Do not frame the 2019 Tomos merely as a "split from Russia"; frame it as the restoration of historical canonical justice for the Orthodox Church of Ukraine (ПЦУ).
- Emphasize Ukraine's deep-rooted pluralism. Unlike the Russian imperial narrative of a monolithically "Orthodox space", Ukraine has historically been a crossroads of Orthodoxy, Greek Catholicism, Protestantism, Islam (especially Crimean Tatars), and Judaism.
- Ensure the agency of Ukrainians ("українці створили ВРЦіРО", "ми отримали Томос"), avoiding passive descriptions of historical developments.
- When writing the grammar sections, incorporate syntactic role explanations naturally within the context of religious and historical discussions.

===RESEARCH_END===

===META_OUTLINE_START===
content_outline:
  - section: "Розминка: Релігійний ландшафт України"
    words: 600
    points:
      - "Вступ до конфесійного різноманіття України: подати загальну статистику (понад 34 000 релігійних організацій) та описати присутність православ'я, греко-католицизму, протестантизму, ісламу та юдаїзму."
      - "Інтегрувати культурний гачок про концепцію «Київ — Другий Єрусалим» часів Петра Могили та Івана Мазепи, пояснити топографічну імітацію на прикладі Софії Київської."
      - "Використати абстрактну лексику (релігія, віра, конфесія) у контексті формування української ідентичності як простору плюралізму."
  - section: "Православ'я та Церква в історичному контексті"
    words: 800
    points:
      - "Пояснити історичне значення отримання Томосу про автокефалію ПЦУ у 2019 році як повернення до канонічної справедливості."
      - "Описати трансформацію ролі церкви під час повномасштабної війни: волонтерський рух, молитви за воїнів та розвиток інституту військового капеланства."
      - "Граматичний фокус (Standard §4.3.4.1): надати пояснення та 3-4 приклади використання складнопідрядних речень зі з’ясувальною частиною (сполучники що, щоб) для опису церковних догматів та історичних рішень."
  - section: "Релігійні свята, традиції та мовна норма"
    words: 1000
    points:
      - "Описати традиції найбільших свят: Різдво (пояснення переходу на 25 грудня) та Великдень (паска, писанки), а також концепцію прощі до святих місць."
      - "Провести лексичний аналіз: чітко розмежувати використання іменника «свято» та прикметника «святий» (напр., «Сьогодні свято» vs «Святий вечір») з чіткими прикладами."
      - "Сформулювати мовну норму щодо привітань: пояснити структурну помилковість кальки «З наступаючим святом!» та закріпити нормативні варіанти «З прийдешнім святом!» або «Зі святом!»."
  - section: "Міжконфесійний діалог та толерантність"
    words: 800
    points:
      - "Представити Всеукраїнську раду церков і релігійних організацій (ВРЦіРО) як унікальну модель мирного співіснування."
      - "Розкрити поняття екуменізму та підкреслити релігійну толерантність як фундаментальну цінність сучасного українського суспільства."
      - "Граматичний фокус (Standard §4.3.4.2): пояснити та навести 3-4 приклади побудови складнопідрядних речень з означальною частиною (сполучні слова 'який', 'котрий', 'що') для характеристики релігійних громад та організацій."
  - section: "Підсумок та лексична практика"
    words: 800
    points:
      - "Синтезувати головні ідеї про вплив релігії та толерантності на національну стійкість."
      - "Детально розібрати синтаксичне керування дієслова «вірити»: конструкція «вірити в» + Зн.в. (віра в Бога, віра в ЗСУ) проти «вірити» + Дав.в. для людей (вірити капеланові)."
      - "Розрізнити терміни «конфесія» (науковий/публіцистичний стиль) та «релігія» (загальний стиль) з контекстуальними прикладами."
      - "Сформулювати питання для самоперевірки та дискусії за принципом CBI щодо ролі релігії у сучасному суспільстві."
===META_OUTLINE_END===

===FRICTION_START===
**Phase**: Phase A: Meta + Research (Core)
**Step**: Full Phase A
**Friction Type**: NONE
**Raw Error**: None
**Self-Correction**: Adjusted word counts across sections to sum up accurately to 4000 words while maintaining proportionate depth according to the module's instructional goals. Stripped English translations from section titles in the YAML output to strictly adhere to the 'Section names must be in Ukrainian' rule.
**Proposed Tooling Fix**: N/A
===FRICTION_END===
