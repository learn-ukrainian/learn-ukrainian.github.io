...
1. **Plan** (source of truth): `curriculum/l2-uk-en/plans/b2/passive-voice-system.yaml`
2. **Meta** (build config): `curriculum/l2-uk-en/b2/meta/passive-voice-system.yaml`
3. **Content** (the lesson): `curriculum/l2-uk-en/b2/passive-voice-system.md`
4. **Activities**: `curriculum/l2-uk-en/b2/activities/passive-voice-system.yaml`
5. **Vocabulary**: `curriculum/l2-uk-en/b2/vocabulary/passive-voice-system.yaml`
6. **Review** (Green Team review from Phase 6): `curriculum/l2-uk-en/b2/review/passive-voice-system-review.md`
7. **Phase 6b fix log** (if exists): `curriculum/l2-uk-en/b2/orchestration/passive-voice-system/phase-6b-fixes.md`
...
```bash
scripts/audit_module.sh curriculum/l2-uk-en/b2/passive-voice-system.md
```
...
Check against `curriculum/l2-uk-en/plans/b2/passive-voice-system.yaml`:
...
Read `curriculum/l2-uk-en/b2/passive-voice-system.md` in FULL. Check for:

### 3a: Russianisms (auto-fail)
```bash
grep -inP 'кушати|получати|приймати участь|слідуючий|любий\b|дякуючи' curriculum/l2-uk-en/b2/passive-voice-system.md
grep -inP 'кушати|получати|приймати участь|слідуючий|любий\b|дякуючи' curriculum/l2-uk-en/b2/activities/passive-voice-system.yaml
```

### 3b: Russian characters (auto-fail)
```bash
grep -nP '[ыэёъ]' curriculum/l2-uk-en/b2/passive-voice-system.md
grep -nP '[ыэёъ]' curriculum/l2-uk-en/b2/activities/passive-voice-system.yaml
```

### 3c: IPA errors
```bash
grep -n '/w/' curriculum/l2-uk-en/b2/passive-voice-system.md
grep -n '/ʊ/' curriculum/l2-uk-en/b2/passive-voice-system.md
grep -n '/w/' curriculum/l2-uk-en/b2/vocabulary/passive-voice-system.yaml
grep -n '/ʊ/' curriculum/l2-uk-en/b2/vocabulary/passive-voice-system.yaml
```
Fix: /w/ -> /ʋ/, /ʊ/ -> /u/
...
```bash
grep -nP '\[/[^]]+/\]|(?<!\[)/[а-яіїєґА-ЯІЇЄҐ][^/]*/(?!\])' curriculum/l2-uk-en/b2/passive-voice-system.md
```
If found and track is B1+: move to vocabulary, remove from content.

### 3e: English leakage
```bash
grep -inP '\b(the|is|are|was|were|has|have|this|that|with|from|for|and|but|not|can|will)\b' curriculum/l2-uk-en/b2/passive-voice-system.md
```
...
```bash
.venv/bin/python scripts/calc_immersion.py curriculum/l2-uk-en/b2/passive-voice-system.md
```
...
Read `curriculum/l2-uk-en/b2/activities/passive-voice-system.yaml` in FULL.
...
### 4e: Forbidden types
- A1: NO `cloze` type
- Check schema at `schemas/activities-b2.schema.json` for track-specific rules

---

## Step 5: Phase 6b Fix Verification

Read `curriculum/l2-uk-en/b2/review/passive-voice-system-review.md`. For EACH issue in "Critical Issues Found" or "Fix Plan":
...
```bash
.venv/bin/python scripts/generate_mdx.py l2-uk-en b2 1
```
...
```
===FINAL_REVIEW_START===
# Final Review: passive-voice-system
