...
approval (Fable, Astra; Kimi consult non-UA design/coding; roster may change); already-ordered work is item 10 (do not slice one outcome)** · **pre-dispatch outcome adequacy**: before
...
| `/effort` | Set model effort dynamically mid-session | Levels `low` / `medium` / `high` / `xhigh` / `max`, tuned for **Fable 5.1** (re-verify per seated model; never port Opus/`xhigh` habits — level names do not mean the same depth across models). Thinking is always on — depth is effort-only. **Default `high`** (API default; start here). **`medium`/`low`** for routine/quick interactive turns (`medium` ≈ Fable 5 cheaper; `low` often beats prior-gen `xhigh`/`max`). **`xhigh`** for hard debugging, large multi-file refactors, long autonomous runs — **and** for curriculum/linguistic review skills that pin `effort: xhigh` (row below; do not step those down). **`max` almost never** — after measured headroom at `xhigh`. **Long deliverables:** stay at `high` unless a measured quality gain says otherwise; at `xhigh`/`max` leave output-budget room for the final answer. Quirks: `high+` on simple tasks **over-gathers** (lower effort; do not prompt around it); `low` **searches less** (bump when retrieval is needed). Effort does not shorten visible output (see Concise by default). Canonical table: `docs/best-practices/fleet-shared-doctrine.md` § Fable 5.1 `/effort` decision topology. |
