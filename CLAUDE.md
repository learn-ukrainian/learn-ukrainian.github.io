...
| `/effort` | Set model effort dynamically mid-session | Levels `low` / `medium` / `high` / `xhigh` / `max` by task tier, tuned for **Fable 5.1** (re-verify per seated model via the `claude-api` skill; never port a previous model's defaults). Thinking is always on for Fable 5.1 — depth is controlled by effort only. `high` is the driving default: orchestration, epic driving, first-pass code review, dispatch briefs. `medium` for routine driving, merge-train babysitting and config fixes — Fable 5.1 at `medium` roughly matches Fable 5 at lower cost, and `low` still beats prior-generation models on routine work, so step down wherever the result holds. `xhigh` for the one hard turn inside a drive: review of record, content or plan review, module building, linguistic analysis, adversarial critique. `max` for deep architecture and contested verdicts only. At `xhigh`/`max` the model may draft a long deliverable in thinking and again in the reply — run long deliverables at `high` unless a measured quality gain says otherwise. Effort is per-turn and does not control response length (see Concise by default). |
