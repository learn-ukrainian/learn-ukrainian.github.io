# holidays — review-and-lock per #1412 rubric template

**Task ID:** `scale-holidays-review-and-lock`
**Worktree:** `.worktrees/scale-holidays`
**Branch:** `agent/scale-holidays`
**Hard timeout:** 5400s

## Why

PR #1415 shipped at-the-cafe as the worked example for the review-and-lock procedure. `docs/best-practices/wiki-plan-review-and-lock.md` is the canonical rubric. This dispatch applies it to **holidays**.
...
## Worktree instructions (mandatory)
    ...
    git worktree add -b agent/scale-holidays .worktrees/scale-holidays
    cd .worktrees/scale-holidays
...
4. **DO NOT touch other slugs' wikis or plans.** This dispatch = holidays ONLY.
...
- **Your slug's current state**:
  - `wiki/pedagogy/a1/holidays.md`
  - `wiki/pedagogy/a1/holidays.sources.yaml`
  - `curriculum/l2-uk-en/plans/a1/holidays.yaml`
  - `wiki/.reviews/pedagogy/a1/holidays-review*.md` (if prior reviews exist — read all)
...
Apply prior reviewer fixes (if any `<fixes>` blocks exist in prior review files), then close any documented gaps. Re-review and emit `wiki/.reviews/pedagogy/a1/holidays-review-LOCKED.md` with final 5-dimension scores (factual, language, decolonization, completeness, actionable, all ≥9).
...
6. Push, open PR with title `feat(quality): review-and-lock holidays wiki + plan (scale batch 2)`
7. STOP. Do not merge.
...
```
## Summary
Locks holidays (L2-UK-EN A1) wiki + plan to 9+/10 + LOCKED state per the #1412 rubric template (`docs/best-practices/wiki-plan-review-and-lock.md`).
...
## Test plan
- [ ] `yq '.lifecycle' curriculum/l2-uk-en/plans/a1/holidays.yaml` returns `locked`
