# sounds-letters-and-hello — review-and-lock per #1412 rubric template

**Task ID:** `scale-sounds-letters-and-hello-review-and-lock`
**Worktree:** `.worktrees/scale-sounds-letters-and-hello`
**Branch:** `agent/scale-sounds-letters-and-hello`
**Hard timeout:** 5400s

## Why

PR #1415 shipped at-the-cafe as the worked example for the review-and-lock procedure. `docs/best-practices/wiki-plan-review-and-lock.md` is the canonical rubric. This dispatch applies it to **sounds-letters-and-hello**.
...
## Worktree instructions (mandatory)
    ...
    git worktree add -b agent/scale-sounds-letters-and-hello .worktrees/scale-sounds-letters-and-hello
    cd .worktrees/scale-sounds-letters-and-hello
...
4. **DO NOT touch other slugs' wikis or plans.** This dispatch = sounds-letters-and-hello ONLY.
...
- **Your slug's current state**:
  ...
  - `wiki/.reviews/pedagogy/a1/sounds-letters-and-hello-review*.md` (if prior reviews exist — read all)
...
Apply prior reviewer fixes (if any `<fixes>` blocks exist in prior review files), then close any documented gaps. Re-review and emit `wiki/.reviews/pedagogy/a1/sounds-letters-and-hello-review-LOCKED.md` with final 5-dimension scores (factual, language, decolonization, completeness, actionable, all ≥9).
...
- Add lifecycle markers matching at-the-cafe schema:
  ...
  reviewed_by: '<agent-id>-sounds-letters-and-hello'
...
.venv/bin/python scripts/ai_agent_bridge/__main__.py ask-codex \
  "Adversarial review of review-and-lock for sounds-letters-and-hello. Read the diff in this branch. Apply the same 5 risk vectors from #1412 adversarial review (wiki gaps reintroduced, lifecycle field naming, plan-review checklist completeness, rubric-doc drift, hallucinated vocab). Report BLOCKER / MEDIUM / NIT bullets with file:line + concrete fix." \
  --task-id sounds-letters-and-hello-adv-review
```
...
6. Push, open PR with title `feat(quality): review-and-lock sounds-letters-and-hello wiki + plan (scale batch 1)`
7. STOP. Do not merge.
...
```
## Summary
Locks sounds-letters-and-hello (L2-UK-EN A1) wiki + plan to 9+/10 + LOCKED state per the #1412 rubric template (`docs/best-practices/wiki-plan-review-and-lock.md`).
...
Part of overnight scale batch 1 (alongside food-and-drink, colors, sounds-letters-and-hello).

## Test plan
- [ ] `yq '.lifecycle' curriculum/l2-uk-en/plans/a1/sounds-letters-and-hello.yaml` returns `locked`
