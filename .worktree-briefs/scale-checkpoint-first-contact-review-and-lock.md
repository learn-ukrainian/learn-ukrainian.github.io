# checkpoint-first-contact — review-and-lock per #1412 rubric template

**Task ID:** `scale-checkpoint-first-contact-review-and-lock`
**Worktree:** `.worktrees/scale-checkpoint-first-contact`
**Branch:** `agent/scale-checkpoint-first-contact`
**Hard timeout:** 5400s

## Why

PR #1415 shipped at-the-cafe as the worked example for the review-and-lock procedure. `docs/best-practices/wiki-plan-review-and-lock.md` is the canonical rubric. This dispatch applies it to **checkpoint-first-contact**.
...
## Worktree instructions (mandatory)
    ...
    git worktree add -b agent/scale-checkpoint-first-contact .worktrees/scale-checkpoint-first-contact
    cd .worktrees/scale-checkpoint-first-contact
...
4. **DO NOT touch other slugs' wikis or plans.** This dispatch = checkpoint-first-contact ONLY.
...
- **Your slug's current state**:
  ...
  - `wiki/pedagogy/a1/checkpoint-first-contact.sources.yaml`
  - `curriculum/l2-uk-en/plans/a1/checkpoint-first-contact.yaml`
  - `wiki/.reviews/pedagogy/a1/checkpoint-first-contact-review*.md` (if prior reviews exist — read all)
...
Apply prior reviewer fixes (if any `<fixes>` blocks exist in prior review files), then close any documented gaps. Re-review and emit `wiki/.reviews/pedagogy/a1/checkpoint-first-contact-review-LOCKED.md` with final 5-dimension scores (factual, language, decolonization, completeness, actionable, all ≥9).
...
- Add lifecycle markers matching at-the-cafe schema:
  ...
  reviewed_by: '<agent-id>-checkpoint-first-contact'
...
.venv/bin/python scripts/ai_agent_bridge/__main__.py ask-codex \
  "Adversarial review of review-and-lock for checkpoint-first-contact. Read the diff in this branch. Apply the same 5 risk vectors from #1412 adversarial review (wiki gaps reintroduced, lifecycle field naming, plan-review checklist completeness, rubric-doc drift, hallucinated vocab). Report BLOCKER / MEDIUM / NIT bullets with file:line + concrete fix." \
  --task-id checkpoint-first-contact-adv-review
```
...
6. Push, open PR with title `feat(quality): review-and-lock checkpoint-first-contact wiki + plan (scale batch 1)`
7. STOP. Do not merge.
...
```
## Summary
Locks checkpoint-first-contact (L2-UK-EN A1) wiki + plan to 9+/10 + LOCKED state per the #1412 rubric template (`docs/best-practices/wiki-plan-review-and-lock.md`).
...
Part of overnight scale batch 1 (alongside food-and-drink, checkpoint-first-contact, sounds-letters-and-hello).

## Test plan
- [ ] `yq '.lifecycle' curriculum/l2-uk-en/plans/a1/checkpoint-first-contact.yaml` returns `locked`
