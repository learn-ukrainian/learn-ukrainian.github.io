# stress-and-melody — review-and-lock per #1412 rubric template

**Task ID:** `scale-stress-and-melody-review-and-lock`
**Worktree:** `.worktrees/scale-stress-and-melody`
**Branch:** `agent/scale-stress-and-melody`
**Hard timeout:** 5400s

## Why

PR #1415 shipped at-the-cafe as the worked example for the review-and-lock procedure. `docs/best-practices/wiki-plan-review-and-lock.md` is the canonical rubric. This dispatch applies it to **stress-and-melody**.
...
## Worktree instructions (mandatory)
    ...
    git worktree add -b agent/scale-stress-and-melody .worktrees/scale-stress-and-melody
    cd .worktrees/scale-stress-and-melody
...
4. **DO NOT touch other slugs' wikis or plans.** This dispatch = stress-and-melody ONLY.
...
- **Your slug's current state**:
  ...
  - `wiki/pedagogy/a1/stress-and-melody.sources.yaml`
  - `curriculum/l2-uk-en/plans/a1/stress-and-melody.yaml`
  - `wiki/.reviews/pedagogy/a1/stress-and-melody-review*.md` (if prior reviews exist — read all)
...
Apply prior reviewer fixes (if any `<fixes>` blocks exist in prior review files), then close any documented gaps. Re-review and emit `wiki/.reviews/pedagogy/a1/stress-and-melody-review-LOCKED.md` with final 5-dimension scores (factual, language, decolonization, completeness, actionable, all ≥9).
...
- Add lifecycle markers matching at-the-cafe schema:
  ...
  reviewed_by: '<agent-id>-stress-and-melody'
...
6. Push, open PR with title `feat(quality): review-and-lock stress-and-melody wiki + plan (scale batch 1)`
7. STOP. Do not merge.
...
```
## Summary
Locks stress-and-melody (L2-UK-EN A1) wiki + plan to 9+/10 + LOCKED state per the #1412 rubric template (`docs/best-practices/wiki-plan-review-and-lock.md`).
...
Part of overnight scale batch 1 (alongside food-and-drink, stress-and-melody, sounds-letters-and-hello).

## Test plan
- [ ] `yq '.lifecycle' curriculum/l2-uk-en/plans/a1/stress-and-melody.yaml` returns `locked`
