# food-and-drink — review-and-lock per #1412 rubric template

**Task ID:** `scale-food-and-drink-review-and-lock`
**Worktree:** `.worktrees/scale-food-and-drink`
**Branch:** `agent/scale-food-and-drink`
**Hard timeout:** 5400s

## Why

PR #1415 shipped at-the-cafe as the worked example for the review-and-lock procedure. `docs/best-practices/wiki-plan-review-and-lock.md` is the canonical rubric. This dispatch applies it to **food-and-drink**.
...
## Worktree instructions (mandatory)
    ...
    git worktree add -b agent/scale-food-and-drink .worktrees/scale-food-and-drink
    cd .worktrees/scale-food-and-drink
...
4. **DO NOT touch other slugs' wikis or plans.** This dispatch = food-and-drink ONLY.
...
- **Your slug's current state**:
  ...
  - `wiki/pedagogy/a1/food-and-drink.sources.yaml`
  - `curriculum/l2-uk-en/plans/a1/food-and-drink.yaml`
  - `wiki/.reviews/pedagogy/a1/food-and-drink-review*.md` (if prior reviews exist — read all)
...
Apply prior reviewer fixes (if any `<fixes>` blocks exist in prior review files), then close any documented gaps. Re-review and emit `wiki/.reviews/pedagogy/a1/food-and-drink-review-LOCKED.md` with final 5-dimension scores (factual, language, decolonization, completeness, actionable, all ≥9).
...
- Add lifecycle markers matching at-the-cafe schema:
  ...
  reviewed_by: '<agent-id>-food-and-drink'
...
6. Push, open PR with title `feat(quality): review-and-lock food-and-drink wiki + plan (scale batch 1)`
7. STOP. Do not merge.
...
```
## Summary
Locks food-and-drink (L2-UK-EN A1) wiki + plan to 9+/10 + LOCKED state per the #1412 rubric template (`docs/best-practices/wiki-plan-review-and-lock.md`).
...
## Test plan
- [ ] `yq '.lifecycle' curriculum/l2-uk-en/plans/a1/food-and-drink.yaml` returns `locked`
