# hey-friend — review-and-lock per #1412 rubric template

**Task ID:** `scale-hey-friend-review-and-lock`
**Worktree:** `.worktrees/scale-hey-friend`
**Branch:** `agent/scale-hey-friend`
**Hard timeout:** 5400s

## Why

PR #1415 shipped at-the-cafe as the worked example for the review-and-lock procedure. `docs/best-practices/wiki-plan-review-and-lock.md` is the canonical rubric. This dispatch applies it to **hey-friend**.
...
## Worktree instructions (mandatory)
    ...
    git worktree add -b agent/scale-hey-friend .worktrees/scale-hey-friend
    cd .worktrees/scale-hey-friend
...
4. **DO NOT touch other slugs' wikis or plans.** This dispatch = hey-friend ONLY.
...
- **Your slug's current state**:
  ...
  - `wiki/pedagogy/a1/hey-friend.sources.yaml`
  - `curriculum/l2-uk-en/plans/a1/hey-friend.yaml`
  - `wiki/.reviews/pedagogy/a1/hey-friend-review*.md` (if prior reviews exist — read all)
...
Apply prior reviewer fixes (if any `<fixes>` blocks exist in prior review files), then close any documented gaps. Re-review and emit `wiki/.reviews/pedagogy/a1/hey-friend-review-LOCKED.md` with final 5-dimension scores (factual, language, decolonization, completeness, actionable, all ≥9).
...
- Add lifecycle markers matching at-the-cafe schema:
  ...
  reviewed_by: '<agent-id>-hey-friend'
...
6. Push, open PR with title `feat(quality): review-and-lock hey-friend wiki + plan (scale batch 2)`
7. STOP. Do not merge.
...
```
## Summary
Locks hey-friend (L2-UK-EN A1) wiki + plan to 9+/10 + LOCKED state per the #1412 rubric template (`docs/best-practices/wiki-plan-review-and-lock.md`).
...
Part of overnight scale batch 2 (alongside food-and-drink, colors, sounds-letters-and-hello).

## Test plan
- [ ] `yq '.lifecycle' curriculum/l2-uk-en/plans/a1/hey-friend.yaml` returns `locked`
