# my-day — review-and-lock per #1412 rubric template

**Task ID:** `scale-my-day-review-and-lock`
**Worktree:** `.worktrees/scale-my-day`
**Branch:** `agent/scale-my-day`
**Hard timeout:** 5400s

## Why

PR #1415 shipped at-the-cafe as the worked example for the review-and-lock procedure. `docs/best-practices/wiki-plan-review-and-lock.md` is the canonical rubric. This dispatch applies it to **my-day**.
...
## Worktree instructions (mandatory)
    ...
    git worktree add -b agent/scale-my-day .worktrees/scale-my-day
    cd .worktrees/scale-my-day
...
4. **DO NOT touch other slugs' wikis or plans.** This dispatch = my-day ONLY.
...
- **Your slug's current state**:
  - `wiki/pedagogy/a1/my-day.md`
  - `wiki/pedagogy/a1/my-day.sources.yaml`
  - `curriculum/l2-uk-en/plans/a1/my-day.yaml`
  - `wiki/.reviews/pedagogy/a1/my-day-review*.md` (if prior reviews exist — read all)
...
Apply prior reviewer fixes (if any `<fixes>` blocks exist in prior review files), then close any documented gaps. Re-review and emit `wiki/.reviews/pedagogy/a1/my-day-review-LOCKED.md` with final 5-dimension scores (factual, language, decolonization, completeness, actionable, all ≥9).
...
6. Push, open PR with title `feat(quality): review-and-lock my-day wiki + plan (scale batch 2)`
7. STOP. Do not merge.
...
```
## Summary
Locks my-day (L2-UK-EN A1) wiki + plan to 9+/10 + LOCKED state per the #1412 rubric template (`docs/best-practices/wiki-plan-review-and-lock.md`).
...
## Test plan
- [ ] `yq '.lifecycle' curriculum/l2-uk-en/plans/a1/my-day.yaml` returns `locked`
