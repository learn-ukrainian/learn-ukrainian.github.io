# shopping — review-and-lock per #1412 rubric template

**Task ID:** `scale-shopping-review-and-lock`
**Worktree:** `.worktrees/scale-shopping`
**Branch:** `agent/scale-shopping`
**Hard timeout:** 5400s

## Why

PR #1415 shipped at-the-cafe as the worked example for the review-and-lock procedure. `docs/best-practices/wiki-plan-review-and-lock.md` is the canonical rubric. This dispatch applies it to **shopping**.
...
## Worktree instructions (mandatory)
    ...
    git worktree add -b agent/scale-shopping .worktrees/scale-shopping
    cd .worktrees/scale-shopping
...
4. **DO NOT touch other slugs' wikis or plans.** This dispatch = shopping ONLY.
...
- **Your slug's current state**:
  - `wiki/pedagogy/a1/shopping.md`
  - `wiki/pedagogy/a1/shopping.sources.yaml`
  - `curriculum/l2-uk-en/plans/a1/shopping.yaml`
  - `wiki/.reviews/pedagogy/a1/shopping-review*.md` (if prior reviews exist — read all)
...
Apply prior reviewer fixes (if any `<fixes>` blocks exist in prior review files), then close any documented gaps. Re-review and emit `wiki/.reviews/pedagogy/a1/shopping-review-LOCKED.md` with final 5-dimension scores (factual, language, decolonization, completeness, actionable, all ≥9).
...
6. Push, open PR with title `feat(quality): review-and-lock shopping wiki + plan (scale batch 2)`
7. STOP. Do not merge.
...
```
## Summary
Locks shopping (L2-UK-EN A1) wiki + plan to 9+/10 + LOCKED state per the #1412 rubric template (`docs/best-practices/wiki-plan-review-and-lock.md`).
...
## Test plan
- [ ] `yq '.lifecycle' curriculum/l2-uk-en/plans/a1/shopping.yaml` returns `locked`
