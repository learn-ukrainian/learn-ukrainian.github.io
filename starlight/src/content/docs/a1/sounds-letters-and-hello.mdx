---
title: "Sounds Letters And Hello"
sidebar:
  order: 1
  label: "01. Sounds Letters And Hello"
pipeline: v6
build_status: draft
---

import Anagram from '@site/src/components/Anagram';
import FillIn from '@site/src/components/FillIn';
import FlashcardDeck from '@site/src/components/FlashcardDeck';
import GroupSort from '@site/src/components/GroupSort';
import LetterGrid from '@site/src/components/LetterGrid';
import MatchUp from '@site/src/components/MatchUp';
import Observe from '@site/src/components/Observe';
import Quiz from '@site/src/components/Quiz';
import YouTubeVideo from '@site/src/components/YouTubeVideo';
import { Tabs, TabItem } from '@astrojs/starlight/components';

<Tabs syncKey="module-tab">
<TabItem label="Урок">

## Зву́ки і лі́тери — Sounds and Letters

Every Ukrainian first-grader learns a golden rule from their very first textbook: **Ми чу́ємо і вимовля́ємо звуки, а ба́чимо і пи́шемо літери** — "We hear and pronounce sounds, but we see and write letters." The Ukrainian **абетка** (alphabet) has 33 **літери** (letters), but the language has 38 **звуків** (sounds). Why the mismatch? Because some letters pull double duty — they can represent more than one sound. The letters **я**, **ю**, **є**, and **ї** are the main culprits. Their exact rules are covered in the next module.

Ukrainian sounds split into two families. The first family is **голосні звуки** (vowel sounds). These are made with voice alone — your mouth stays open, air flows freely, nothing blocks it. A poem from a first-grade textbook captures this perfectly: **Голосні почу́єш в пісні** — "You'll hear vowels in a song." There are 6 vowel sounds in Ukrainian: [а], [о], [у], [е], [и], [і]. But there are 10 vowel letters: **а**, **о**, **у**, **е**, **и**, **і**, **я**, **ю**, **є**, **ї**. The extra four — **я**, **ю**, **є**, **ї** — are called **йотовані** (iotated) letters. They can represent two sounds at once. For now, just recognize them when you see them. Try pronouncing the six pure vowel sounds in sequence: [а] — [о] — [у] — [е] — [и] — [і]. Each one rings out clearly, like singing a note.

The second family is **приголосні звуки** (consonant sounds). These are made when your lips, teeth, or tongue create an obstruction — air hits a barrier and produces noise, either with voice or without it. Another first-grade poem describes them: **Приголосні деренча́ть, і тихе́нько шелестять** — "Consonants clatter and softly rustle." Ukrainian has 32 consonant sounds, produced by just 22 consonant letters (plus **ь**, the soft sign, which has no sound of its own — it only softens the consonant before it). Try this contrast: say [а] — your mouth is open, pure voice. Now say [м] — your lips press together, blocking the air. That vibration you feel through closed lips? That is the difference between **голосний** (vowel) and **приголосний** (consonant).

<GroupSort client:only="react" groups={{"Голосні (vowels)": ["А", "О", "У", "Е", "И", "І"], "Приголосні (consonants)": ["К", "М", "Т", "В", "Н", "Р", "С", "Х"]}} instruction={"Sort these Ukrainian letters into two groups: vowels (голосні) and consonants (приголосні)."} />

## Пе́рші слова́ — First Words

Some Cyrillic letters are friendly — they look like Latin letters and sound similar too. Meet your first five: **А** [а], **О** [о], **К** [к], **М** [м], and **Т** [т]. Look at the word **мама** (mother). Sound it out: м-а-м-а. Now try **тато** (father): т-а-т-о. Here are more words built from these five letters alone: **кома** (comma), **атом** (atom), **мак** (poppy), **око** (eye). Two small pronunciation notes: Ukrainian **Т** is dental — your tongue touches the back of your upper teeth, not the gum ridge the way it does in English. And both **К** and **Т** are unaspirated — no puff of air follows them. Hold your palm in front of your mouth and say the English word "top" — you feel a burst of air. Now say **тато** without that burst. Close enough for now — awareness matters more than perfection at this stage.

Some Cyrillic letters are **false friends**. They look exactly like Latin letters but make completely different sounds:

| Letter | Ukrainian sound | English trap | Example word |
|--------|----------------|--------------|-------------|
| **В** | [в] (like "v") | not "b" | **вода** (water) |
| **Н** | [н] (like "n") | not "h" | **ніс** (nose) |
| **Р** | [р] (rolled "r") | not "p" | **рука** (hand) |
| **С** | [с] (like "s") | not "c/k" | **сон** (dream) |
| **У** | [у] (like "oo") | not "y" | **вухо** (ear) |
| **Х** | [х] (like "ch" in Scottish "loch") | not "x" | **хата** (house) |

When you see **РЕСТОРАН** on a sign in Kyiv, the **Р** is a rolled "r" and the **С** is an "s."

<MatchUp client:only="react" pairs={[{"left": "В", "right": "[в] — like \"v\""}, {"left": "Н", "right": "[н] — like \"n\""}, {"left": "Р", "right": "[р] — rolled \"r\""}, {"left": "С", "right": "[с] — like \"s\""}, {"left": "У", "right": "[у] — like \"oo\""}, {"left": "Х", "right": "[х] — like \"ch\" in \"loch\""}]} instruction={"Match each false friend letter to its REAL Ukrainian sound — not the English sound it looks like!"} />

Beyond friendly letters and false friends, Ukrainian has letters with shapes entirely new to English speakers. You do not need to memorize all of them right now — just know they exist and start recognizing them through words. **Б** looks a bit like the number 6 with a hat. **Г** resembles an upside-down L. **Д** looks like a small triangle on legs. **Ж** spreads out like a snowflake. **З** is shaped like the number 3. **Л** looks like a tent. **П** is a doorway. **Ш** is a comb with three teeth. And **Щ** is **Ш** with a small tail — it always makes two sounds [шч]. Read these words to see the new letters in action: **банк** (bank), **дім** (home), **зима** (winter), **книга** (book), **школа** (school).

Four special letters deserve a quick introduction. **Ї** always sounds like [йі] — it never softens the consonant before it. This letter is unique to Ukrainian. You will not find it in Russian, Belarusian, or any other Slavic language — it belongs to Ukrainian alone. You will see it in words like **їжак** (hedgehog) and **Україна** (Ukraine). The letters **Я**, **Ю**, and **Є** serve double duty: at the start of a word or after a vowel, each one represents two sounds ([йа], [йу], [йе]); after a consonant, they soften it instead. The full rules come in the next module — for now, simply recognize these four special letters when you encounter them.

## Приві́т! — Hello!

These are informal greetings — use them with friends, family, and people your age or younger. The core phrases:

- **Привіт!** — Hi!
- **Як спра́ви?** — How are you?
- **Добре.** — Fine.
- **Чудо́во!** — Great!
- **Нормально.** — Okay.
- **А у тебе́?** — And you?

Listen for the rhythm when you say these phrases aloud. Ukrainian has a natural melodic rise and fall that differs from English — imitate the music, not just the words.

Here is what a greeting sounds like between two friends meeting on the street:

<div class="dialogue">

**Оленка:** Привіт! *(Hi!)*

**Тарас:** О, привіт! Як справи? *(Oh, hi! How are you?)*

**Оленка:** Добре, дякую. А у тебе? *(Fine, thanks. And you?)*

**Тарас:** Чудово! *(Great!)*

**Оленка:** Ра́да тебе ба́чити! *(Glad to see you! — said by a woman)*

</div>

Now imagine Тарас says the same thing back. He would say: **Радий тебе бачити!** Notice the difference? Women say **рада**, men say **радий**. This is your very first encounter with grammatical gender in Ukrainian — the language marks whether the speaker is male or female. This will become a major topic starting in Module 8. For now, just pick the form that matches you: **рада** if you are female, **радий** if you are male.

Read **Привіт** letter by letter to see how far you have already come. **П** — a new shape (the doorway). **р** — a false friend (rolled, not "p"). **и** — a new vowel (between English "i" and "ih"). **в** — a false friend (not "b"). **і** — a vowel (like English "ee"). **т** — familiar, but dental. Blend into syllables: При-віт. This single word uses letters from all three groups — friendly, false friends, and new shapes.

<FillIn client:only="react" items={[{"sentence": "— ____! Як справи?", "answer": "Привіт", "options": ["Привіт", "Добре", "Чудово"]}, {"sentence": "— ____. А у тебе?", "answer": "Добре", "options": ["Привіт", "Добре", "Нормально"]}, {"sentence": "— ____!", "answer": "Чудово", "options": ["Чудово", "Привіт", "Добре"]}, {"sentence": "— Як ____?", "answer": "справи", "options": ["справи", "добре", "тебе"]}]} instruction={"Complete this greeting dialogue by filling in the missing words."} />

## Чита́ємо — Reading Practice

Sound out each word letter by letter, then blend into syllables:

- **Аптека** (а-пте-ка) — pharmacy
- **Банк** (банк) — bank
- **Кафе** (ка-фе) — café
- **Метро** (ме-тро) — metro
- **Пошта** (по-шта) — post office
- **Школа** (шко-ла) — school
- **Зупинка** (зу-пин-ка) — bus stop

Notice how many of these words are recognizable from English or other European languages. Ukrainian has plenty of international vocabulary. The spelling follows Ukrainian rules, but the meaning is transparent.

Now try Ukrainian city names — proper nouns that carry real cultural weight. **Київ** (Ки-їв) — note the **ї**! **Львів** (Львів) — note the **ь** softening **Л**. **Одеса** (О-де-са). **Харків** (Хар-ків). **Дніпро** (Дні-про). **Полтава** (Пол-та-ва). These are not just reading exercises — they are real places. **Київ** is the capital. **Львів** is the cultural heart of western Ukraine. Pronouncing these names correctly is a sign of respect.

Now build your first sentences using **Це** (this is). The pattern is simple: **Це** + noun.

- **Це мама.** — This is mom.
- **Це банк.** — This is a bank.
- **Це Київ.** — This is Kyiv.
- **Це школа.** — This is a school.
- **Це аптека.** — This is a pharmacy.

Two essential question forms are: **Що це?** (What is this?) — for things and places, and **Хто це?** (Who is this?) — for people. Try a quick exchange:

> **Що це?** — **Це банк.** *(What is this? — This is a bank.)*
> **Хто це?** — **Це мама.** *(Who is this? — This is mom.)*

Notice the **що/хто** distinction: **що** for objects and places, **хто** for people. Ukrainian distinguishes animate from inanimate from the very start.

<Quiz client:only="react" questions={[{"question": "What do we hear and pronounce?", "options": [{"text": "звуки (sounds)", "correct": true}, {"text": "літери (letters)", "correct": false}, {"text": "слова (words)", "correct": false}]}, {"question": "What do we see and write?", "options": [{"text": "звуки (sounds)", "correct": false}, {"text": "літери (letters)", "correct": true}, {"text": "речення (sentences)", "correct": false}]}, {"question": "How many letters are in the Ukrainian абетка (alphabet)?", "options": [{"text": "26", "correct": false}, {"text": "33", "correct": true}, {"text": "38", "correct": false}]}, {"question": "How many sounds does Ukrainian have?", "options": [{"text": "33", "correct": false}, {"text": "36", "correct": false}, {"text": "38", "correct": true}]}, {"question": "Which family of sounds is described as 'you hear them in a song'?", "options": [{"text": "приголосні (consonants)", "correct": false}, {"text": "голосні (vowels)", "correct": true}, {"text": "йотовані (iotated)", "correct": false}]}, {"question": "How many голосні звуки (vowel sounds) does Ukrainian have?", "options": [{"text": "6", "correct": true}, {"text": "10", "correct": false}, {"text": "33", "correct": false}]}]} instruction={"Answer the questions about Ukrainian sounds and letters."} />

### Відео — Video

<YouTubeVideo client:only="react" url="https://www.youtube.com/watch?v=ksXIXj7CXwc" label="Overview — Ukrainian Lessons" />

[Повний плейлист / Full playlist](https://www.youtube.com/playlist?list=PLpkSIXDyaJi3mlJlKXWKhdiJZj67fPXQV)

## Підсумок — Summary

Here is what you learned in this first module. Ukrainian has 33 **літери** (letters) and 38 **звуків** (sounds) — we hear and pronounce sounds, we see and write letters. Sounds split into two families: **голосні** (6 vowel sounds, 10 vowel letters) and **приголосні** (32 consonant sounds). Cyrillic letters fall into three groups for English speakers: friendly ones (**А**, **О**, **К**, **М**, **Т**), false friends (**В**, **Н**, **Р**, **С**, **У**, **Х** — the biggest trap), and new shapes (**Б**, **Г**, **Д**, **Ж**, **Ш**, **Щ**, and more). You learned your first greeting: **Привіт! Як справи?** — **Добре!** You read real Ukrainian words and city names. You formed your first sentences with **Це**. 

Self-check: How many letters in the **абетка**? How many sounds? Why the difference? What are **голосні**? What are **приголосні**? What does **Привіт** mean? How do you answer **Як справи?** 

Next module: Reading Ukrainian — where we dive deeper into how letters combine into syllables.

</TabItem>
<TabItem label="Словник">

### Обов'язкові та рекомендовані слова

| Слово | Переклад | Частина мови | Рід |
|-------|----------|-------------|-----|
| **звук** | sound | ім. | ч. |
| **лі́тера** | letter (of the alphabet) | ім. | ж. |
| **абе́тка** | alphabet | ім. | ж. |
| **голосни́й** | vowel | прикм. |  |
| **при́голосний** | consonant | прикм. |  |
| **йото́вані** | iotated (letters representing two sounds) | прикм. |  |
| **ма́ма** | mother | ім. | ж. |
| **та́то** | father | ім. | ч. |
| **ко́ма** | comma | ім. | ж. |
| **а́том** | atom | ім. | ч. |
| **мак** | poppy | ім. | ч. |
| **о́ко** | eye | ім. | с. |
| **вода́** | water | ім. | ж. |
| **ніс** | nose | дієсл. |  |
| **рука́** | hand | ім. | ж. |
| **сон** | dream | ім. | ч. |
| **ву́хо** | ear | ім. | с. |
| **ха́та** | house | ім. | ж. |
| **банк** | bank | ім. | ч. |
| **дім** | home | ім. | ч. |
| **зима́** | winter | ім. | ж. |
| **кни́га** | book | ім. | ж. |
| **шко́ла** | school | ім. | ж. |
| **їжа́к** | hedgehog | ім. | ч. |
| **Украї́на** | Ukraine | ім. | ж. |
| **приві́т** | hi (informal) | ім. | ч. |
| **до́бре** | fine, good | присл. |  |
| **чудо́во** | great, wonderful | присл. |  |
| **норма́льно** | okay | присл. |  |
| **дякую** | thanks | дієсл. |  |
| **ра́да** | glad (feminine) | ім. | ч. |
| **ра́дий** | glad (masculine) | прикм. |  |
| **апте́ка** | pharmacy | ім. | ж. |
| **кафе́** | café | ім. | с. |
| **метро́** | metro | ім. | с. |
| **по́шта** | post office | ім. | ж. |
| **зупи́нка** | bus stop | ім. | ж. |
| **Ки́їв** | Kyiv (capital of Ukraine) | ім. |  |
| **Львів** | Lviv |  |  |
| **Оде́са** | Odesa |  |  |
| **Харків** | Kharkiv |  |  |
| **Дніпро́** | Dnipro |  |  |
| **Полта́ва** | Poltava |  |  |
| **це** | this is | ім. | с. |
| **що** | what | спол. |  |
| **хто** | who | ім. | ч. |

### Вирази

| Вираз | Переклад |
|-------|----------|
| **Як справи?** | How are you? |
| **А у тебе?** | And you? |
| **Рада тебе бачити!** | Glad to see you! (said by a woman) |
| **Радий тебе бачити!** | Glad to see you! (said by a man) |
| **Що це?** | What is this? |
| **Хто це?** | Who is this? |

### Картки — Flashcards

<FlashcardDeck client:only="react" cards={[{ front: "звук", back: "sound", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "лі́тера", back: "letter (of the alphabet)", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "абе́тка", back: "alphabet", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "голосни́й", back: "vowel", subtitle: "прикм." }, { front: "при́голосний", back: "consonant", subtitle: "прикм." }, { front: "йото́вані", back: "iotated (letters representing two sounds)", subtitle: "прикм." }, { front: "ма́ма", back: "mother", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "та́то", back: "father", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "ко́ма", back: "comma", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "а́том", back: "atom", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "мак", back: "poppy", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "о́ко", back: "eye", subtitle: "ім.", tag: "с.", tagColor: "#E65100" }, { front: "вода́", back: "water", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "ніс", back: "nose", subtitle: "дієсл." }, { front: "рука́", back: "hand", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "сон", back: "dream", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "ву́хо", back: "ear", subtitle: "ім.", tag: "с.", tagColor: "#E65100" }, { front: "ха́та", back: "house", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "банк", back: "bank", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "дім", back: "home", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "зима́", back: "winter", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "кни́га", back: "book", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "шко́ла", back: "school", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "їжа́к", back: "hedgehog", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "Украї́на", back: "Ukraine", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "приві́т", back: "hi (informal)", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "до́бре", back: "fine, good", subtitle: "присл." }, { front: "чудо́во", back: "great, wonderful", subtitle: "присл." }, { front: "норма́льно", back: "okay", subtitle: "присл." }, { front: "дякую", back: "thanks", subtitle: "дієсл." }, { front: "ра́да", back: "glad (feminine)", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "ра́дий", back: "glad (masculine)", subtitle: "прикм." }, { front: "апте́ка", back: "pharmacy", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "кафе́", back: "café", subtitle: "ім.", tag: "с.", tagColor: "#E65100" }, { front: "метро́", back: "metro", subtitle: "ім.", tag: "с.", tagColor: "#E65100" }, { front: "по́шта", back: "post office", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "зупи́нка", back: "bus stop", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "Ки́їв", back: "Kyiv (capital of Ukraine)", subtitle: "ім." }, { front: "Львів", back: "Lviv" }, { front: "Оде́са", back: "Odesa" }, { front: "Харків", back: "Kharkiv" }, { front: "Дніпро́", back: "Dnipro" }, { front: "Полта́ва", back: "Poltava" }, { front: "це", back: "this is", subtitle: "ім.", tag: "с.", tagColor: "#E65100" }, { front: "що", back: "what", subtitle: "спол." }, { front: "хто", back: "who", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }]} />

</TabItem>
<TabItem label="Зошит">

<Quiz client:only="react" questions={[{"question": "The letter В in Ukrainian sounds like...", "options": [{"text": "\"b\" as in boy", "correct": false}, {"text": "\"v\" as in voice", "correct": true}, {"text": "\"w\" as in water", "correct": false}]}, {"question": "The letter Р in Ukrainian sounds like...", "options": [{"text": "\"p\" as in park", "correct": false}, {"text": "a rolled \"r\"", "correct": true}, {"text": "\"r\" as in red", "correct": false}]}, {"question": "The letter Н in Ukrainian sounds like...", "options": [{"text": "\"h\" as in hat", "correct": false}, {"text": "\"n\" as in no", "correct": true}, {"text": "\"m\" as in map", "correct": false}]}, {"question": "The letter С in Ukrainian sounds like...", "options": [{"text": "\"k\" as in cat", "correct": false}, {"text": "\"s\" as in sun", "correct": true}, {"text": "\"c\" as in city", "correct": false}]}, {"question": "Which letter is unique to Ukrainian and not found in Russian or Belarusian?", "options": [{"text": "Я", "correct": false}, {"text": "Ї", "correct": true}, {"text": "Є", "correct": false}]}]} instruction={"Choose the correct answer about Ukrainian letters and sounds."} />

<MatchUp client:only="react" pairs={[{"left": "мама", "right": "mother"}, {"left": "тато", "right": "father"}, {"left": "вода", "right": "water"}, {"left": "рука", "right": "hand"}, {"left": "книга", "right": "book"}, {"left": "школа", "right": "school"}]} instruction={"Match each Ukrainian word to its English meaning."} />

<MatchUp client:only="react" pairs={[{"left": "Аптека", "right": "pharmacy"}, {"left": "Банк", "right": "bank"}, {"left": "Метро", "right": "metro"}, {"left": "Пошта", "right": "post office"}, {"left": "Зупинка", "right": "bus stop"}]} instruction={"Match each sign you might see in a Ukrainian city to its meaning."} />

<Quiz client:only="react" questions={[{"question": "Someone says \"Привіт! Як справи?\" — how do you reply?", "options": [{"text": "Привіт!", "correct": false}, {"text": "Добре!", "correct": true}, {"text": "Як справи?", "correct": false}]}, {"question": "You want to ask 'How are you?' in Ukrainian. What do you say?", "options": [{"text": "Добре!", "correct": false}, {"text": "А у тебе?", "correct": false}, {"text": "Як справи?", "correct": true}]}, {"question": "Your friend says things are fine and asks about you. What did they say?", "options": [{"text": "Чудово! Привіт!", "correct": false}, {"text": "Добре. А у тебе?", "correct": true}, {"text": "Нормально. Як справи?", "correct": false}]}, {"question": "A woman says she's glad to see you. She says:", "options": [{"text": "Радий тебе бачити!", "correct": false}, {"text": "Рада тебе бачити!", "correct": true}, {"text": "Добре тебе бачити!", "correct": false}]}]} instruction={"Choose the correct response for each greeting situation."} />

<GroupSort client:only="react" groups={{"Friendly letters (look and sound similar)": ["А", "О", "К", "М", "Т"], "False friends (look similar, sound different)": ["В", "Н", "Р", "С", "У", "Х"]}} instruction={"Sort these letters: which ones look like Latin letters but make DIFFERENT sounds (false friends), and which ones look AND sound similar (friendly letters)?"} />

<Quiz client:only="react" questions={[{"question": "You point at a school. What do you say?", "options": [{"text": "Це школа.", "correct": true}, {"text": "Це банк.", "correct": false}, {"text": "Це аптека.", "correct": false}]}, {"question": "You want to say 'This is mom.' What do you say?", "options": [{"text": "Хто це?", "correct": false}, {"text": "Це тато.", "correct": false}, {"text": "Це мама.", "correct": true}]}, {"question": "You ask about a thing (not a person). Which question word do you use?", "options": [{"text": "Хто це?", "correct": false}, {"text": "Що це?", "correct": true}, {"text": "Як справи?", "correct": false}]}, {"question": "You ask about a person. Which question word do you use?", "options": [{"text": "Що це?", "correct": false}, {"text": "Хто це?", "correct": true}, {"text": "Де це?", "correct": false}]}]} instruction={"Use Це (this is) to identify things. Choose the correct sentence."} />

<Anagram client:only="react" items={[{"scrambled": "і в т р и п", "answer": "привіт", "hint": "a greeting — hi!"}, {"scrambled": "а м а м", "answer": "мама", "hint": "mother"}, {"scrambled": "о а д в", "answer": "вода", "hint": "water"}, {"scrambled": "г н и к а", "answer": "книга", "hint": "book"}]} instruction={"Unscramble the letters to form a Ukrainian word you learned in this module."} />

<LetterGrid client:only="react" letters={[{"upper": "А", "lower": "а", "emoji": "🍍", "key_word": "ананас", "sound_type": "vowel"}, {"upper": "О", "lower": "о", "emoji": "👁️", "key_word": "око", "sound_type": "vowel"}, {"upper": "У", "lower": "у", "emoji": "🇺🇦", "key_word": "Україна", "sound_type": "vowel"}, {"upper": "И", "lower": "и", "emoji": "🐟", "key_word": "риба", "sound_type": "vowel"}, {"upper": "І", "lower": "і", "emoji": "🦃", "key_word": "індик", "sound_type": "vowel"}, {"upper": "Е", "lower": "е", "emoji": "📺", "key_word": "екран", "sound_type": "vowel"}, {"upper": "Я", "lower": "я", "emoji": "🍎", "key_word": "яблуко", "sound_type": "vowel"}, {"upper": "Ю", "lower": "ю", "emoji": "🧑", "key_word": "юнак", "sound_type": "vowel"}, {"upper": "Є", "lower": "є", "emoji": "🦝", "key_word": "єнот", "sound_type": "vowel"}, {"upper": "Ї", "lower": "ї", "emoji": "🦔", "key_word": "їжак", "sound_type": "vowel"}]} title={"Голосні — Vowels"} />

<LetterGrid client:only="react" letters={[{"upper": "М", "lower": "м", "emoji": "🥕", "key_word": "морква", "sound_type": "consonant"}, {"upper": "К", "lower": "к", "emoji": "🐱", "key_word": "кіт", "sound_type": "consonant"}, {"upper": "Т", "lower": "т", "emoji": "🐯", "key_word": "тигр", "sound_type": "consonant"}]} title={"Friendly letters"} />

<LetterGrid client:only="react" letters={[{"upper": "Н", "lower": "н", "emoji": "✂️", "key_word": "ножиці", "sound_type": "consonant"}, {"upper": "С", "lower": "с", "emoji": "👜", "key_word": "сумка", "sound_type": "consonant"}, {"upper": "Р", "lower": "р", "emoji": "🚀", "key_word": "ракета", "sound_type": "consonant"}, {"upper": "В", "lower": "в", "emoji": "🐺", "key_word": "вовк", "sound_type": "consonant"}, {"upper": "Х", "lower": "х", "emoji": "🍞", "key_word": "хліб", "sound_type": "consonant"}]} title={"False friends!"} />

<LetterGrid client:only="react" letters={[{"upper": "Л", "lower": "л", "emoji": "✈️", "key_word": "літак", "sound_type": "consonant"}, {"upper": "Б", "lower": "б", "emoji": "🍌", "key_word": "банан", "sound_type": "consonant"}, {"upper": "Д", "lower": "д", "emoji": "🏠", "key_word": "дім", "sound_type": "consonant"}, {"upper": "П", "lower": "п", "emoji": "🕷️", "key_word": "павук", "sound_type": "consonant"}, {"upper": "Г", "lower": "г", "emoji": "⛰️", "key_word": "гора", "sound_type": "consonant"}, {"upper": "Ґ", "lower": "ґ", "emoji": "🚪", "key_word": "ґанок", "sound_type": "consonant"}, {"upper": "З", "lower": "з", "emoji": "🦓", "key_word": "зебра", "sound_type": "consonant"}, {"upper": "Ж", "lower": "ж", "emoji": "🪲", "key_word": "жук", "sound_type": "consonant"}, {"upper": "Ш", "lower": "ш", "emoji": "🧢", "key_word": "шапка", "sound_type": "consonant"}, {"upper": "Й", "lower": "й", "emoji": "🥛", "key_word": "йогурт", "sound_type": "consonant"}, {"upper": "Ч", "lower": "ч", "emoji": "🐢", "key_word": "черепаха", "sound_type": "consonant"}, {"upper": "Щ", "lower": "щ", "emoji": "🪥", "key_word": "щітка", "sound_type": "consonant"}, {"upper": "Ц", "lower": "ц", "emoji": "🧅", "key_word": "цибуля", "sound_type": "consonant"}, {"upper": "Ф", "lower": "ф", "emoji": "🦩", "key_word": "фламінго", "sound_type": "consonant"}]} title={"New shapes"} />

<LetterGrid client:only="react" letters={[{"upper": "Я", "lower": "я", "emoji": "🍎", "key_word": "яблуко", "sound_type": "vowel"}, {"upper": "Ю", "lower": "ю", "emoji": "🧑", "key_word": "юнак", "sound_type": "vowel"}, {"upper": "Є", "lower": "є", "emoji": "🦝", "key_word": "єнот", "sound_type": "vowel"}, {"upper": "Ь", "lower": "ь", "emoji": "🧂", "key_word": "сіль", "sound_type": "special"}, {"upper": "Ї", "lower": "ї", "emoji": "🦔", "key_word": "їжак", "sound_type": "vowel"}]} title={"Special letters"} />

<Observe client:only="react" examples={["А — ананас — «У нас ананас.» — Video: https://www.youtube.com/watch?v=hvB3VpcR3ZE", "М — морква — «Мама сама.» — Video: https://www.youtube.com/watch?v=Ez95H4ibuJo", "Л — літак — «Мала лама.» — Video: https://www.youtube.com/watch?v=v6-3Xg52Buk", "У — Україна — «У нас мул.» — Video: https://www.youtube.com/watch?v=VB1O6PmtYRU", "Н — ножиці — «У нас лан.» — Video: https://www.youtube.com/watch?v=vNUfiKHPYaU", "С — сумка — «Сумна мама.» — Video: https://www.youtube.com/watch?v=7UsFBgSL91E"]} prompt={"Watch each video by Anna Ohoiko. Say the letter out loud, then the key word, then the short phrase. Can you hear how each letter sounds on its own and inside a word?"} />

<Observe client:only="react" examples={["К — кіт — Video: https://www.youtube.com/watch?v=J7sGEI4-xJo", "И — риба — Video: https://www.youtube.com/watch?v=W-1rCu0indE", "Р — ракета — Video: https://www.youtube.com/watch?v=fMGsQ5KPQgg", "Б — банан — Video: https://www.youtube.com/watch?v=V1hxBE_JbGg", "В — вовк — Video: https://www.youtube.com/watch?v=aFcvYfvQ2X4", "Д — дім — Video: https://www.youtube.com/watch?v=g4Bh-lqzd48", "І — індик — Video: https://www.youtube.com/watch?v=Z9TH0H4ShGo"]} prompt={"Watch each video and repeat after Anna. Pay special attention to В (sounds like \"v\", not \"b\") and Р (rolled, not like English \"p\"). Can you feel the difference?"} />

<Observe client:only="react" examples={["П — павук — Video: https://www.youtube.com/watch?v=JksSjjxyW5Y", "Т — тигр — Video: https://www.youtube.com/watch?v=m-jcLR_gK0k", "Г — гора — Video: https://www.youtube.com/watch?v=gVnclpSI0DU", "Ґ — ґанок — Video: https://www.youtube.com/watch?v=gNjHqjTW9WQ", "Е — екран — Video: https://www.youtube.com/watch?v=KFlsroBW0dk", "З — зебра — Video: https://www.youtube.com/watch?v=BhASNxitC1A", "Ж — жук — Video: https://www.youtube.com/watch?v=dIrGVcqPwqM", "Ш — шапка — Video: https://www.youtube.com/watch?v=1D-6MIw3OXY", "Х — хліб — Video: https://www.youtube.com/watch?v=vpr58zJSJKc"]} prompt={"These letters have new shapes you have not seen in English. Watch each video and try to memorize the shape alongside the sound. Which letters are hardest to remember?"} />

<Observe client:only="react" examples={["Й — йогурт — Video: https://www.youtube.com/watch?v=aq0cjB90s3w", "Ч — черепаха — Video: https://www.youtube.com/watch?v=UsJkbdsY2RA", "Щ — щітка — Video: https://www.youtube.com/watch?v=QmBLieIuf6Q", "Я — яблуко — Video: https://www.youtube.com/watch?v=yhSAf41LX8I", "Ю — юнак — Video: https://www.youtube.com/watch?v=9JdIBYCTWGw", "Є — єнот — Video: https://www.youtube.com/watch?v=O0bwRyyBQSc", "Ь — сіль — Video: https://www.youtube.com/watch?v=cJlal8XKBxo", "Ї — їжак — Video: https://www.youtube.com/watch?v=UcjdjQXhAY8", "Ц — цибуля — Video: https://www.youtube.com/watch?v=u44eCjR2Oz8", "Ф — фламінго — Video: https://www.youtube.com/watch?v=haHRsFFZRQI"]} prompt={"These are the remaining letters of the Ukrainian абетка. Watch each video and repeat. Notice that Ь (soft sign) has no sound of its own — it only softens the consonant before it. And Ї always sounds like [йі]."} />


</TabItem>
<TabItem label="Ресурси">

**Джерела — References**

- Большакова Grade 1 буквар, p.24
  _Голосні/приголосні taught through poems. 'Голосні почуєш в пісні.'_
- Захарійчук Grade 1 буквар (NUS 2025), p.13
  _Sound notation: [•] for vowels, [–] for consonants._
- Заболотний Grade 5, p.73
  _38 звуків: 6 голосних + 32 приголосні._
- [ULP Season 1, Episode 1 — Informal Greetings](https://www.ukrainianlessons.com/episode1/)
  _Привіт, Як справи?, response patterns._

</TabItem>
</Tabs>