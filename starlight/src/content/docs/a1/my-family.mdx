---
title: "My Family"
sidebar:
  order: 6
  label: "06. My Family"
pipeline: v6
build_status: draft
---

import Anagram from '@site/src/components/Anagram';
import FillIn from '@site/src/components/FillIn';
import FlashcardDeck from '@site/src/components/FlashcardDeck';
import GroupSort from '@site/src/components/GroupSort';
import MatchUp from '@site/src/components/MatchUp';
import Observe from '@site/src/components/Observe';
import Quiz from '@site/src/components/Quiz';
import YouTubeVideo from '@site/src/components/YouTubeVideo';
import { Tabs, TabItem } from '@astrojs/starlight/components';

<Tabs syncKey="module-tab">
<TabItem label="Урок">

## Діалоги (Dialogues)

Оля and Максим sit in a cafe. Maksym pulls out his phone — he has new photos from a family gathering. Showing phone photos to friends is universal, and in Ukraine it often turns into a full family introduction.

<div class="dialogue">

**Оля:** У тебе є брати чи сестри? *(Do you have brothers or sisters?)*

**Максим:** Так, у мене є два брати і одна сестра. *(Yes, I have two brothers and one sister.)*

**Оля:** Ого! У мене тільки один брат. *(Wow! I only have one brother.)*

**Максим:** Як його звати? *(What's his name?)*

**Оля:** Коля. *(Kolya.)*

</div>

A few words to notice: **чи** (or) appears in yes/no questions when offering a choice. **Тільки** (only) is a useful word — it softens a statement. **Як його звати?** literally means "How is he called?" — you already know **Як тебе звати?** from Module 5, and **його** (his/him) simply replaces **тебе** (you).

Maksym swipes to a group photo. Ukrainians often keep extended family photos on their phones — family ties run deep.

<div class="dialogue">

**Максим:** Це моя сім'я на фотографії. *(This is my family in a photo.)*

**Оля:** Класно! Хто це? *(Cool! Who is this?)*

**Максим:** Це моя мама Марина. *(This is my mom Maryna.)*

**Максим:** Це мій тато Євген. *(This is my dad Yevhen.)*

**Максим:** Це моя сестра Катя. *(This is my sister Katia.)*

**Максим:** І мої брати — Іван і Денис. *(And my brothers — Ivan and Denys.)*

**Оля:** А це твоя бабуся? *(And is this your grandmother?)*

**Максим:** Так, її звати Тетяна. *(Yes, her name is Tetiana.)*

</div>

**Хто це?** (Who is this?) is your go-to question when pointing at someone in a photo. **Її звати** (her name is) works exactly like **його звати** — just swap the pronoun.

Notice the pattern **Це** + possessive + noun: **Це мій тато**, **Це моя мама**, **Це мої брати**. Ukrainian uses **Це** (this is) the same way English uses "This is" for introductions. Already you can see that **мій** and **моя** are different — this is because they match the gender of the person you are introducing.

Now Оля introduces her own family — a connected monologue combining everything from Modules 1–5 with new family vocabulary:

<div class="dialogue">

**Оля:** Привіт! Мене звати Оля. *(Hi! My name is Olya.)*

**Оля:** Моя мама — вчителька. *(My mom is a teacher.)*

**Оля:** Мій тато — інженер. *(My dad is an engineer.)*

**Оля:** У мене є один брат. *(I have one brother.)*

**Оля:** Його звати Коля. *(His name is Kolya.)*

**Оля:** Моя бабуся Ганна живе в Києві. *(My grandmother Hanna lives in Kyiv.)*

**Оля:** У мене дружна сім'я. *(I have a close-knit family.)*

</div>

Three key patterns carry this entire module. First: **У мене є** + family member for saying what family you have. Second: **Це мій/моя** + person for introducing someone. Third: **Як його/її звати?** → **Його/Її звати...** for asking and giving names. Master these three and you can talk about any family.

## Сім'я (Family Vocabulary)

Ukrainian has two words for family: **сім'я** and **родина**. Both are common, both are correct, and you will hear them interchangeably. A Grade 1 textbook poem by Марія Братко captures both:

> Поділюся з вами я: В мене дружна є сім'я.

Notice the apostrophe in **сім'я** — this is the same apostrophe you learned in Module 4. It separates the **м** from the **я**, keeping them as distinct sounds. **Дружна** (close-knit, friendly) is a word Ukrainians often use to describe their families.

Here are the core family members you need. Each noun has a grammatical gender — this matters for possessive pronouns later:

| Ukrainian | English | Gender |
|-----------|---------|--------|
| **мама** / **мати** | mother | f |
| **тато** / **батько** | father | m |
| **брат** | brother | m |
| **сестра** | sister | f |
| **син** | son | m |
| **дочка** / **донька** | daughter | f |

**Мама** is everyday speech; **мати** is formal or literary. **Тато** is everyday; **батько** is formal. The plural **батьки** (parents) is one of the first words where the plural meaning differs from the singular — it does not mean "fathers."

Extended family members: **бабуся** / **баба** (grandmother), **дідусь** / **дід** (grandfather), **тітка** (aunt), **дядько** (uncle), **дружина** (wife), and **чоловік** (husband). A cultural note: Ukrainian has no single word for "grandparents" — you always say **бабуся і дідусь**. Diminutive forms are very common in families: **татусь** (daddy), **матуся** (mommy), **бабця** (granny). The word **дідусь** is already a diminutive of **дід**. A Grade 2 textbook has a fun unscrambling exercise with family words: "маам, отат, дусьід, басябу" — can you figure them out? мама, тато, дідусь, бабуся.

<MatchUp client:only="react" pairs={[{"left": "батьки", "right": "parents"}, {"left": "дядько", "right": "uncle"}, {"left": "тітка", "right": "aunt"}, {"left": "дідусь", "right": "grandfather"}, {"left": "бабуся", "right": "grandmother"}, {"left": "брат", "right": "brother"}, {"left": "сестра", "right": "sister"}, {"left": "мама і тато", "right": "mother and father"}]} instruction={"Match the Ukrainian family words to their English meanings."} />

## У мене є (I have)

While Ukrainian does have a verb for "to have" (*мати*), everyday speech prefers a different construction. It is literally "At me there-is": **У мене є брат** — "I have a brother." This is completely different from English "I have," and it is one of the first structures that shows you how Ukrainian thinks differently.

For A1, you need only three forms:

| Ukrainian | English |
|-----------|---------|
| **У мене є** | I have |
| **У тебе є** | You have (informal) |
| **У вас є** | You have (formal) |

Examples with family: **У мене є сестра** (I have a sister). **У тебе є брат?** (Do you have a brother?). **У вас є діти?** (Do you have children?). Other pronoun forms like **у нього є** (he has) and **у неї є** (she has) are also useful memorized phrases — the full genitive pronoun system comes in A2. For now, just recognize them when you hear them.

Questions use rising intonation only — no word-order change needed: **У тебе є сестра?** ↗ Compare English "Do you have...?" which requires a helper verb. Ukrainian is simpler here. Short answers: **Так, у мене є сестра.** Or simply: **Ні.** For now, if you want to say you don't have someone, just say **Ні** or offer a correction: **Ні, у мене тільки один брат.** We will learn how to say "I don't have" later.

<Quiz client:only="react" questions={[{"question": "У тебе є брат?", "options": [{"text": "Так, у мене є брат.", "correct": true}, {"text": "Так, у тебе є брат.", "correct": false}, {"text": "Так, мій брат.", "correct": false}]}, {"question": "У тебе є сестра?", "options": [{"text": "Ні, моя сестра.", "correct": false}, {"text": "Так, у мене є сестра.", "correct": true}, {"text": "Так, у тебе є сестра.", "correct": false}]}, {"question": "У тебе є бабуся?", "options": [{"text": "Так, у мене є бабуся.", "correct": true}, {"text": "Так, моя бабуся.", "correct": false}, {"text": "Так, у тебе є бабуся.", "correct": false}]}, {"question": "У тебе є два брати?", "options": [{"text": "Так, у тебе є два брати.", "correct": false}, {"text": "Ні.", "correct": true}, {"text": "Так, мої два брати.", "correct": false}]}, {"question": "У тебе є дідусь?", "options": [{"text": "Так, у тебе є дідусь.", "correct": false}, {"text": "Так, мій дідусь.", "correct": false}, {"text": "Так, у мене є дідусь.", "correct": true}]}, {"question": "У тебе є син?", "options": [{"text": "Ні.", "correct": true}, {"text": "Так, у тебе є син.", "correct": false}, {"text": "Ні, мій син.", "correct": false}]}]} instruction={"Answer the question using the pattern «У мене є...»."} />

Numbers make a brief appearance here, because family talk naturally involves counting. Two numbers show gender agreement: **один брат** (one brother, masculine) vs. **одна сестра** (one sister, feminine). **Два брати** (two brothers, masculine) vs. **дві сестри** (two sisters, feminine). **Один/одна** and **два/дві** change by gender — this is your first encounter with number-gender agreement. Keep it to just these two numbers for now. Examples: **У мене є один брат і дві сестри.** **У мене є одна бабуся і два дідусі.**

:::tip Pattern Summary
Four sentences that show the full **У мене є** pattern in action:

1. **У мене є два брати.** *(I have two brothers.)*
2. **У тебе є сестра?** — **Так, у мене є одна сестра.** *(Do you have a sister? — Yes, I have one sister.)*
3. **У вас є діти?** — **Так, у мене є син і дочка.** *(Do you have children? — Yes, I have a son and a daughter.)*
4. **У мене є бабуся. Її звати Ганна.** *(I have a grandmother. Her name is Hanna.)*

Notice how **У мене є** connects naturally to family introductions from the dialogues.
:::

## Мій, моя, моє (Possessive Pronouns)

Possessives in Ukrainian match the gender of the **thing possessed**, not the owner. This is different from English "my," which never changes form.

| | Masculine | Feminine | Neuter | Plural |
|---|-----------|----------|--------|--------|
| my | **мій** | **моя** | **моє** | **мої** |

**Мій брат** (my brother — masculine noun). **Моя сестра** (my sister — feminine noun). **Моє місто** (my city — neuter noun). **Мої батьки** (my parents — plural noun). The key: look at the noun's gender, not who is speaking. A man says **моя сестра** (not *мій сестра) because **сестра** is feminine. A woman says **мій брат** because **брат** is masculine. The speaker's gender is irrelevant.

**Твій/твоя/твоє/твої** (your, informal) follows the same pattern: **твій тато** (your dad, masculine), **твоя мама** (your mom, feminine), **твоє ім'я** (your name, neuter), **твої друзі** (your friends, plural). Third-person possessives are even easier: **його** (his) and **її** (her) never change form regardless of the noun. **Його мама, його тато, його місто** — always **його**. **Її брат, її сестра, її місто** — always **її**. This makes **його** and **її** the simplest possessives in Ukrainian.

The **Це** + possessive + noun pattern is the workhorse for family introductions: **Це моя мама. Це мій тато. Це моя сестра. Це мої брати.** We will learn other possessives like **наш** (our) and **ваш** (your, formal) later — for now, **мій/твій/його/її** covers everything you need for simple introductions. A quick exchange shows the contrast between **мій** and **твій**:

<div class="dialogue">

**Максим:** Це мій тато. *(This is my dad.)*

**Оля:** А це твій брат? *(And is this your brother?)*

</div>

<FillIn client:only="react" items={[{"sentence": "Це ____ мама.", "answer": "моя", "options": ["мій", "моя", "моє", "мої"]}, {"sentence": "Де ____ тато?", "answer": "мій", "options": ["мій", "моя", "моє", "мої"]}, {"sentence": "Ось ____ батьки.", "answer": "мої", "options": ["мій", "моя", "моє", "мої"]}, {"sentence": "Це ____ брат.", "answer": "мій", "options": ["мій", "моя", "моє", "мої"]}, {"sentence": "Це ____ сестра.", "answer": "моя", "options": ["мій", "моя", "моє", "мої"]}, {"sentence": "Це ____ місто.", "answer": "моє", "options": ["мій", "моя", "моє", "мої"]}, {"sentence": "Де ____ бабуся?", "answer": "моя", "options": ["мій", "моя", "моє", "мої"]}, {"sentence": "Це ____ дідусь.", "answer": "мій", "options": ["мій", "моя", "моє", "мої"]}]} instruction={"Choose the correct possessive pronoun (мій, моя, моє, or мої)."} />

<FillIn client:only="react" items={[{"sentence": "— Привіт! Це ____ брат?", "answer": "твій", "options": ["твій", "твоя", "мій", "мої"]}, {"sentence": "— Так, це мій брат. Ось мій ____.", "answer": "тато", "options": ["тато", "мама", "сестра", "батьки"]}, {"sentence": "— А це ____ мама?", "answer": "твоя", "options": ["твій", "твоя", "мій", "моя"]}, {"sentence": "— Так, її звати ____.", "answer": "Марина", "options": ["Марина", "Максим", "мама", "тато"]}]} instruction={"Complete the dialogue with the correct word."} />

## Підсумок — Summary

In this module you learned to talk about your family in Ukrainian. Three key patterns carry your family conversations: (1) **У мене є** + noun for saying what family you have, (2) **Це** + **мій/моя/моє** for introducing family members, and (3) **Як його/її звати?** for asking and giving names. You can now name **мама**, **тато**, **брат**, **сестра**, **бабуся**, **дідусь**, **дядько**, **тітка**, **син**, **дочка**, and **батьки**. You know that possessives change by gender — **мій брат** but **моя сестра** — and that **його** and **її** never change form.

Test yourself:

1. Name 5 family members in Ukrainian.
2. Say "I have a sister." — **У мене є сестра.**
3. What is the difference between **мій** and **моя**? (Gender of the noun!)
4. Introduce your family in 4–5 sentences using **Це мій/моя...** and **У мене є...**
5. Ask someone "Do you have a brother?" — **У тебе є брат?**

</TabItem>
<TabItem label="Словник">

### Обов'язкові та рекомендовані слова

| Слово | Переклад | Частина мови | Рід |
|-------|----------|-------------|-----|
| **сім'я́** | family | ім. | ж. |
| **роди́на** | family | ім. | ж. |
| **ма́ма** | mother | ім. | ж. |
| **ма́ти** | mother (formal) | ім. |  |
| **та́то** | father | ім. | ч. |
| **ба́тько** | father (formal) | ім. | ч. |
| **батьки́** | parents | ім. |  |
| **брат** | brother | ім. | ч. |
| **сестра́** | sister | ім. | ж. |
| **дочка́** | daughter | ім. | ж. |
| **до́нька** | daughter | ім. | ж. |
| **бабу́ся** | grandmother | ім. | ж. |
| **ба́ба** | grandmother (informal) | ім. | ж. |
| **діду́сь** | grandfather | ім. | ч. |
| **дід** | grandfather | ім. | ч. |
| **ті́тка** | aunt | ім. | ж. |
| **дя́дько** | uncle | ім. | ч. |
| **дружи́на** | wife | ім. | ж. |
| **чолові́к** | man / husband | ім. | ч. |
| **ді́ти** | children | ім. |  |
| **моя́** | my (feminine) | прикм. |  |
| **моє́** | my (neuter) | прикм. |  |
| **мої́** | my (plural) | прикм. |  |
| **твій** | your (masculine, informal) | прикм. |  |
| **твоя́** | your (feminine, informal) | прикм. |  |
| **твоє́** | your (neuter, informal) | прикм. |  |
| **твої́** | your (plural, informal) | прикм. |  |
| **йо́го** | his | ім. | ч. |
| **чи** | or (in questions) | спол. |  |
| **ті́льки** | only | присл. |  |
| **оди́н** | one (masculine) | числ. |  |
| **одна́** | one (feminine) | числ. |  |
| **два** | two (masculine) | числ. |  |
| **дві** | two (feminine) | числ. |  |
| **дру́жна** | close-knit, friendly | прикм. |  |
| **тату́сь** | daddy (diminutive) | ім. | ч. |
| **мату́ся** | mommy (diminutive) | ім. | ж. |
| **ба́бця** | granny (diminutive) | ім. | ж. |
| **фотогра́фія** | photo, photograph | ім. | ж. |
| **вчи́телька** | teacher (female) | ім. | ж. |
| **інжене́р** | engineer | ім. | ч. |
| **кла́сно** | cool, great | присл. |  |
| **ого́** | wow | виг. |  |

### Вирази

| Вираз | Переклад |
|-------|----------|
| **у мене є** | I have |
| **у тебе є** | you have (informal) |
| **у вас є** | you have (formal) |
| **Як його звати?** | What's his name? |
| **Його звати...** | His name is... |
| **Її звати...** | Her name is... |

### Картки — Flashcards

<FlashcardDeck client:only="react" cards={[{ front: "сім'я́", back: "family", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "роди́на", back: "family", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "ма́ма", back: "mother", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "ма́ти", back: "mother (formal)", subtitle: "ім." }, { front: "та́то", back: "father", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "ба́тько", back: "father (formal)", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "батьки́", back: "parents", subtitle: "ім." }, { front: "брат", back: "brother", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "сестра́", back: "sister", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "дочка́", back: "daughter", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "до́нька", back: "daughter", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "бабу́ся", back: "grandmother", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "ба́ба", back: "grandmother (informal)", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "діду́сь", back: "grandfather", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "дід", back: "grandfather", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "ті́тка", back: "aunt", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "дя́дько", back: "uncle", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "дружи́на", back: "wife", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "чолові́к", back: "man / husband", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "ді́ти", back: "children", subtitle: "ім." }, { front: "моя́", back: "my (feminine)", subtitle: "прикм." }, { front: "моє́", back: "my (neuter)", subtitle: "прикм." }, { front: "мої́", back: "my (plural)", subtitle: "прикм." }, { front: "твій", back: "your (masculine, informal)", subtitle: "прикм." }, { front: "твоя́", back: "your (feminine, informal)", subtitle: "прикм." }, { front: "твоє́", back: "your (neuter, informal)", subtitle: "прикм." }, { front: "твої́", back: "your (plural, informal)", subtitle: "прикм." }, { front: "йо́го", back: "his", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "чи", back: "or (in questions)", subtitle: "спол." }, { front: "ті́льки", back: "only", subtitle: "присл." }, { front: "оди́н", back: "one (masculine)", subtitle: "числ." }, { front: "одна́", back: "one (feminine)", subtitle: "числ." }, { front: "два", back: "two (masculine)", subtitle: "числ." }, { front: "дві", back: "two (feminine)", subtitle: "числ." }, { front: "дру́жна", back: "close-knit, friendly", subtitle: "прикм." }, { front: "тату́сь", back: "daddy (diminutive)", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "мату́ся", back: "mommy (diminutive)", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "ба́бця", back: "granny (diminutive)", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "фотогра́фія", back: "photo, photograph", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "вчи́телька", back: "teacher (female)", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "інжене́р", back: "engineer", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "кла́сно", back: "cool, great", subtitle: "присл." }, { front: "ого́", back: "wow", subtitle: "виг." }]} />

</TabItem>
<TabItem label="Зошит">

<MatchUp client:only="react" pairs={[{"left": "син", "right": "son"}, {"left": "дочка", "right": "daughter"}, {"left": "дружина", "right": "wife"}, {"left": "чоловік", "right": "husband"}, {"left": "сім'я", "right": "family"}]} instruction={"Match each Ukrainian word to its English meaning."} />

<Quiz client:only="react" questions={[{"question": "How do you say 'I have a sister' in Ukrainian?", "options": [{"text": "У мене є сестра.", "correct": true}, {"text": "Моя сестра є.", "correct": false}, {"text": "Мене є сестра.", "correct": false}]}, {"question": "Why is it «моя сестра» and not «мій сестра»?", "options": [{"text": "Because the speaker is female", "correct": false}, {"text": "Because «сестра» is a feminine noun", "correct": true}, {"text": "Because «мій» is only for brothers", "correct": false}]}, {"question": "What does «його» mean as a possessive?", "options": [{"text": "her", "correct": false}, {"text": "my", "correct": false}, {"text": "his", "correct": true}]}, {"question": "Which is correct for 'two sisters'?", "options": [{"text": "два сестри", "correct": false}, {"text": "дві сестри", "correct": true}, {"text": "дві сестра", "correct": false}]}]} instruction={"Choose the correct answer."} />

<GroupSort client:only="react" groups={{"Masculine (мій)": ["тато", "брат", "син", "дідусь", "дядько"], "Feminine (моя)": ["мама", "сестра", "бабуся", "дочка", "тітка"]}} instruction={"Sort these words by grammatical gender."} />

<FillIn client:only="react" items={[{"sentence": "Це ____ дядько?", "answer": "твій", "options": ["твій", "твоя", "твоє", "твої"]}, {"sentence": "Де ____ тітка?", "answer": "твоя", "options": ["твій", "твоя", "твоє", "твої"]}, {"sentence": "Це ____ батьки?", "answer": "твої", "options": ["твій", "твоя", "твоє", "твої"]}, {"sentence": "Як ____ ім'я?", "answer": "твоє", "options": ["твій", "твоя", "твоє", "твої"]}]} instruction={"Choose the correct possessive: твій, твоя, твоє, or твої."} />

<Quiz client:only="react" questions={[{"question": "— У тебе є брати чи сестри? — ...", "options": [{"text": "Так, у мене є один брат і одна сестра.", "correct": true}, {"text": "Так, мій брат і сестра.", "correct": false}, {"text": "У тебе є брат.", "correct": false}]}, {"question": "— Хто це на фотографії? — ...", "options": [{"text": "Це у мене є мама.", "correct": false}, {"text": "Це моя мама Марина.", "correct": true}, {"text": "Моя мама є Марина.", "correct": false}]}, {"question": "— Як його звати? — ...", "options": [{"text": "Його мій Андрій.", "correct": false}, {"text": "Він Андрій.", "correct": false}, {"text": "Його звати Андрій.", "correct": true}]}]} instruction={"Read the mini-dialogue and choose the best reply."} />

<Observe client:only="react" examples={["Його мама, його тато, його сестра — always «його».", "Її брат, її мама, її місто — always «її».", "Мій тато, моя мама, моє місто, мої батьки — «мій» changes!"]} prompt={"What is different about «його» and «її» compared to «мій»? (Hint: do «його» and «її» change form?)"} />

<Anagram client:only="react" items={[{"scrambled": "с е р т с а", "answer": "сестра", "hint": "sister"}, {"scrambled": "б а у б с я", "answer": "бабуся", "hint": "grandmother"}, {"scrambled": "д і д у с ь", "answer": "дідусь", "hint": "grandfather"}, {"scrambled": "б а т ь к и", "answer": "батьки", "hint": "parents"}]} instruction={"Unscramble the letters to find a family word (hint provided)."} />


</TabItem>
<TabItem label="Ресурси">

**Джерела — References**

- [ULP Season 1, Episode 6 — Family + I Have](https://www.ukrainianlessons.com/episode6/)
  _У мене є with family. Один/одна gender._
- [ULP Season 1, Episode 7 — Possessive Pronouns](https://www.ukrainianlessons.com/episode7/)
  _мій/моя/моє paradigm. Це моя мама._
- [ULP Season 1, Episode 10 — Review](https://www.ukrainianlessons.com/episode10/)
  _Connected self-introduction: Я і моя сім'я._

**Статті — Articles**

- [Family Vocabulary](https://www.ukrainianlessons.com/vocabulary-family/) (Сім'я)
- [ULP 1-06 Talking about your family in Ukrainian + I have, you have](https://www.ukrainianlessons.com/lesson/6/) (ukrainianlessons.com)
- [Talking about your family in Ukrainian + I have, you have](https://www.ukrainianlessons.com/episode6/) (ukrainianlessons.com)
- [More about your family in Ukrainian + Possessive Pronouns](https://www.ukrainianlessons.com/episode7/) (ukrainianlessons.com)
- [Talk Ukrainian: Family – Родина](https://talkukrainian.com/family/) (talkukrainian)

**Anna Ohoiko — Ukrainian Lessons**

- [Ukrainian Family Vocabulary](https://www.ukrainianlessons.com/family-members/)

**Подкасти — Podcasts**

- [ULP Episode 4 — Family Members](https://www.ukrainianlessons.com/episode4/)

</TabItem>
</Tabs>