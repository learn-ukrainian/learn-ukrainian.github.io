---
title: "Перші кроки в А2"
sidebar:
  order: 7
  label: "07. Перші кроки в А2"
pipeline: v6
build_status: draft
---

import ErrorCorrection from '@site/src/components/ErrorCorrection';
import FillIn from '@site/src/components/FillIn';
import FlashcardDeck from '@site/src/components/FlashcardDeck';
import GroupSort from '@site/src/components/GroupSort';
import MatchUp from '@site/src/components/MatchUp';
import Quiz from '@site/src/components/Quiz';
import Translate from '@site/src/components/Translate';
import TrueFalse from '@site/src/components/TrueFalse';
import Unjumble from '@site/src/components/Unjumble';
import YouTubeVideo from '@site/src/components/YouTubeVideo';
import { Tabs, TabItem } from '@astrojs/starlight/components';

<Tabs syncKey="module-tab">
<TabItem label="Урок">

## Сценарій 1: Плануємо вечірку (Scenario 1: Planning a Party)

Welcome to your first steps in the A2 level! You are now moving from simply memorizing basic phrases to engaging in realistic, dynamic communication. Imagine you are organizing a large housewarming party, which we call **новосілля** in Ukrainian. Planning such an important event requires a significant shift in how you talk about actions. You will need to learn how to discuss ongoing future plans, figure out exactly what is missing from your apartment, and confidently confirm concrete results when your tasks are finally finished. Let us look at how two friends handle this complex process.

<div class="dialogue">

<div class="dialogue-line"><span class="speaker">Максим:</span> Привіт, Олено! Давай планувати наше новосілля. *(Hi Olena! Let's plan our housewarming party.)*</div>

<div class="dialogue-line"><span class="speaker">Олена:</span> Привіт! Звісно. Ми будемо обговорювати меню сьогодні ввечері? *(Hi! Sure. Are we going to discuss the menu tonight?)*</div>

<div class="dialogue-line"><span class="speaker">Максим:</span> Так, нам обов'язково треба скласти довгий список. Що ми будемо готувати для друзів? *(Yes, we definitely need to make a long list. What are we going to cook for the friends?)*</div>

<div class="dialogue-line"><span class="speaker">Олена:</span> Я буду готувати великий святковий салат і запікати м'ясо. *(I will be cooking a big festive salad and roasting meat.)*</div>

<div class="dialogue-line"><span class="speaker">Максим:</span> Дуже добре, а я буду купувати всі необхідні напої та солодкі десерти. *(Very good, and I will be buying all the necessary drinks and sweet desserts.)*</div>

<div class="dialogue-line"><span class="speaker">Олена:</span> Чудово! А хто буде запрошувати наших гостей? *(Great! And who is going to invite our guests?)*</div>

<div class="dialogue-line"><span class="speaker">Максим:</span> Я думаю, ми будемо робити це разом у п'ятницю після роботи. *(I think we will be doing this together on Friday after work.)*</div>

</div>

When Ukrainians discuss the continuous process of planning or an ongoing future action, they use the imperfective future tense. This compound tense is formed by combining the correct conjugated form of the verb **бути** (to be) with the infinitive of an imperfective verb. 

Ми часто використовуємо недоконаний вид дієслова, коли дія є довгою, регулярною або рутинною. Якщо ми говоримо про успішний результат або швидку одноразову подію, ми завжди обираємо доконаний вид.

> *We often use the imperfective aspect of a verb when an action is long, regular, or routine. If we are talking about a successful result or a quick one-time event, we always choose the perfective aspect.*

The aspect pair **готувати** (to be cooking) and **приготувати** (to finish cooking) perfectly highlights this difference in focus. The same logic applies when choosing between the verbs for planning as an ongoing process versus successfully completing a plan.

As you carefully check your party list, you will inevitably find that some crucial things are missing. To express absence or lack in Ukrainian, we rely on the word **немає** (there is no). This word is a powerful trigger for the Genitive case.

Коли ми кажемо «немає», наступне слово завжди стоїть у родовому відмінку. Це суворе граматичне правило чудово працює для живих людей, фізичних предметів та абстрактних понять.

> *When we say "немає", the next word is always in the Genitive case. This strict grammatical rule works perfectly for living people, physical objects, and abstract concepts.*

You can use this structure to complain about an empty new apartment or missing items:
**У нас немає музики.** — *We have no music.*
**Немає торта.** — *There is no cake.*
**Немає великого столу.** — *There is no big table.*

The Genitive case is also absolutely essential when you are talking about quantities for your event. Words like **багато** (many) and **мало** (few) strictly require the noun that follows them to take the Genitive form. For countable party items, this is usually the Genitive plural.

Для гарної вечірки господарям треба купити багато свіжих фруктів і багато холодних напоїв. Якщо у вас дуже мало паперових серветок, гості будуть незадоволені.

> *For a good party, the hosts need to buy a lot of fresh fruits and a lot of cold drinks. If you have very few paper napkins, the guests will be unsatisfied.*

Remembering these specific plural endings will help you accurately describe the exact scale of your exciting preparations.

Let us rejoin our friends a few days later to see their actual progress. Now, they are completely focusing on their completed results, which requires switching to the perfective past tense.

<div class="dialogue">

<div class="dialogue-line"><span class="speaker">Максим:</span> Олено, як наші справи? Ти вже приготувала салат? *(Olena, how are our things? Have you already prepared the salad?)*</div>

<div class="dialogue-line"><span class="speaker">Олена:</span> Так, я все успішно зробила. А ти купив напої? *(Yes, I successfully did everything. And did you buy the drinks?)*</div>

<div class="dialogue-line"><span class="speaker">Максим:</span> Я вже купив торт і газовану воду, але зовсім забув про яблучний сік. *(I already bought the cake and sparkling water, but I completely forgot about the apple juice.)*</div>

<div class="dialogue-line"><span class="speaker">Олена:</span> Нічого страшного, ми замовили велику піцу, а кур'єр може привезти сік. *(No worries, we ordered a big pizza, and the courier can bring juice.)*</div>

<div class="dialogue-line"><span class="speaker">Максим:</span> Чудово! Я також успішно запланував музику для нашого вечора. *(Great! I also successfully planned the music for our evening.)*</div>

</div>

:::tip
**Did you know?**
Ukrainians absolutely love using diminutive suffixes in everyday speech, especially when talking about delicious food and warm hospitality. Calling a standard cake a **тортик** instead of «торт» or a fresh salad a **салатик** instead of «салат» makes the invitation sound much warmer and more welcoming. You might even hear informal slang like **винишко** or the more traditional Ukrainian diminutive **винце** for wine among very close friends. These small changes in word endings actively show deep affection and help create a beautifully cozy, relaxed atmosphere for your happy guests.
:::

<Quiz client:only="react" questions={[{"question": "Ми __________ нове меню для вечірки цілий день.", "options": [{"text": "обговорювали", "correct": true}, {"text": "обговорили", "correct": false}, {"text": "обговоримо", "correct": false}, {"text": "обговорить", "correct": false}]}, {"question": "Ти вже __________ смачний торт для гостей?", "options": [{"text": "приготувала", "correct": true}, {"text": "готувала", "correct": false}, {"text": "приготую", "correct": false}, {"text": "готуєш", "correct": false}]}, {"question": "Хто __________ наших гостей у п'ятницю?", "options": [{"text": "буде запрошувати", "correct": true}, {"text": "запрошував", "correct": false}, {"text": "запрошує", "correct": false}, {"text": "запросить", "correct": false}]}, {"question": "Я успішно __________ всі солодкі десерти.", "options": [{"text": "купила", "correct": true}, {"text": "купувала", "correct": false}, {"text": "купує", "correct": false}, {"text": "буде купувати", "correct": false}]}, {"question": "Що ми __________ робити завтра ввечері?", "options": [{"text": "будемо", "correct": true}, {"text": "буде", "correct": false}, {"text": "будуть", "correct": false}, {"text": "буду", "correct": false}]}, {"question": "Я обов'язково __________ велику піцу завтра.", "options": [{"text": "замовлю", "correct": true}, {"text": "замовляю", "correct": false}, {"text": "замовляв", "correct": false}, {"text": "замовлятиму", "correct": false}]}, {"question": "Він завжди __________ цю тему зі своїми друзями.", "options": [{"text": "обговорював", "correct": true}, {"text": "обговорив", "correct": false}, {"text": "обговорить", "correct": false}, {"text": "буде обговорити", "correct": false}]}, {"question": "Максим швидко __________ м'ясо і пішов відпочивати.", "options": [{"text": "приготував", "correct": true}, {"text": "готував", "correct": false}, {"text": "приготує", "correct": false}, {"text": "готує", "correct": false}]}]} instruction={"Оберіть правильний вид дієслова для кожного речення (зверніть увагу на процес чи результат)"} />

## Сценарій 2: На ринку (Scenario 2: At the Market)

To truly experience everyday Ukrainian life, you must visit a bustling local farmers market. A lively «ринок» like the famous Bessarabskyi or the authentic Lukianivskyi in Kyiv offers much more than just fresh seasonal food. It is a vibrant social hub where people negotiate, share community news, and build lasting relationships with their favorite vendors. For a language learner, the market is the ultimate practice ground for the Genitive case. You will constantly hear people asking for specific quantities, discussing prices, and navigating what is currently available or completely sold out. Let us listen to a typical conversation at a colorful produce stall.

<div class="dialogue">

<div class="dialogue-line"><span class="speaker">Покупець:</span> Добрий день! Скільки коштує кілограм меду? *(Good day! How much does a kilogram of honey cost?)*</div>

<div class="dialogue-line"><span class="speaker">Продавець:</span> Добрий день! Цей свіжий мед коштує триста гривень. *(Good day! This fresh honey costs three hundred hryvnias.)*</div>

<div class="dialogue-line"><span class="speaker">Покупець:</span> Добре. А у вас є домашній сир? *(Good. And do you have cottage cheese?)*</div>

<div class="dialogue-line"><span class="speaker">Продавець:</span> Так, звичайно. Скільки вам потрібно? *(Yes, of course. How much do you need?)*</div>

<div class="dialogue-line"><span class="speaker">Покупець:</span> Дайте мені, будь ласка, один кілограм сиру і пів кілограма масла. *(Give me, please, one kilogram of cheese and half a kilogram of butter.)*</div>

<div class="dialogue-line"><span class="speaker">Продавець:</span> Ось, тримайте. Щось ще? *(Here, take it. Anything else?)*</div>

<div class="dialogue-line"><span class="speaker">Покупець:</span> Ще дайте мені дві дині і п'ять лимонів. *(Also give me two melons and five lemons.)*</div>

</div>

When buying items by the piece or by weight, you must pay close attention to the numbers you use. Ukrainian grammar has a very specific rule for combining numbers with nouns, which we often call the "1, 2-4, 5+" rule.

Після числа «один» ми завжди використовуємо називний відмінок однини. Після чисел «два», «три» та «чотири» іменник чоловічого роду має форму називного відмінка множини. А після числа «п'ять» і більше ми завжди використовуємо родовий відмінок множини.

> *After the number "one", we always use the nominative case singular. After the numbers "two", "three", and "four", a masculine noun takes the form of the nominative case plural. And after the number "five" and more, we always use the genitive case plural.*

For example, if you want to buy lemons, you would say **один лимон** (one lemon), **три лимони** (three lemons), but **шість лимонів** (six lemons). This pattern applies to the word **кілограм** as well. You ask for **один кілограм**, but **два кілограми** or **п'ять кілограмів**.

The market is also the perfect place to practice the Genitive of negation and absence. When a vendor runs out of a product, they will use the word **немає**, which strictly requires the Genitive case. 

Вибачте, сьогодні у мене немає домашнього сиру, але є дуже смачне молоко. Я бачу жовті яблука, але я зовсім не бачу червоних помідорів на вашому столі.

> *Excuse me, today I do not have cottage cheese, but there is very tasty milk. I see yellow apples, but I absolutely do not see red tomatoes on your table.*

Notice how the shopper contrasts their statements. When they state what they see affirmatively, they use the simple Accusative case: **Я бачу жовті яблука.** But the moment they introduce a negative verb like **не бачу** (do not see) or express absence with **немає**, the object must immediately switch to the Genitive case.

You must also differentiate between uncountable goods and individual items when dealing with feminine nouns. When you buy vegetables in bulk, you usually treat them as a single mass.

Дайте мені, будь ласка, один кілограм картоплі і один кілограм моркви. Але я також хочу купити один кілограм солодких яблук і кілограм груш.

> *Give me, please, one kilogram of potatoes and one kilogram of carrots. But I also want to buy one kilogram of sweet apples and a kilogram of pears.*

Words like **картопля** (potatoes) and **морква** (carrots) remain in the Genitive singular because they are treated as an uncountable mass. However, items you can easily count individually, like **яблука** (apples) or **груші** (pears), take the Genitive plural ending after the word **кілограм**.

:::tip
**Did you know?**
At a Ukrainian market, you will hear a wonderful variety of diminutive suffixes. Vendors use these affectionate word forms to build rapport, show the high quality of their goods, and create a welcoming atmosphere.
:::

Продавці дуже часто пропонують покупцям свіжу картопельку, солодку морквочку або смачні ягідки. Якщо ви також використовуєте ці теплі слова, продавець може дати вам хорошу знижку.

> *Vendors very often offer buyers fresh little potatoes, sweet little carrots, or tasty little berries. If you also use these warm words, the vendor might give you a good discount.*

Using forms like **картопелька** instead of **картопля**, or **морквочка** instead of **морква**, demonstrates that you appreciate the local culture. It changes the interaction from a simple commercial transaction into a friendly conversation between neighbors.

Before you grab your reusable bag and head to the stalls, let us review the essential market verbs. You will frequently need the imperfective verb **коштувати** (to cost) to ask about prices. When you are looking at the produce, you need time to **вибирати** (to choose, process), but eventually, you must **вибрати** (to choose, result) the best items. The vendor will then **важити** (to weigh) your purchases on the scale. Finally, after you hand over your cash, always remember to wait for your **решта** (change).

<FillIn client:only="react" items={[{"sentence": "Вибачте, у нас сьогодні немає свіжого __________.", "answer": "сиру", "options": ["сир", "сиру", "сиром", "сирі"]}, {"sentence": "Дайте мені, будь ласка, п'ять __________.", "answer": "лимонів", "options": ["лимон", "лимони", "лимонів", "лимоном"]}, {"sentence": "Скільки коштує один кілограм __________?", "answer": "яблук", "options": ["яблуко", "яблука", "яблук", "яблуками"]}, {"sentence": "Я бачу жовті дині, але зовсім не бачу червоних __________.", "answer": "помідорів", "options": ["помідор", "помідори", "помідорів", "помідорам"]}, {"sentence": "Для вечірки нам треба купити багато холодних __________.", "answer": "напоїв", "options": ["напій", "напої", "напоїв", "напоями"]}, {"sentence": "У нас залишилося дуже мало __________ серветок.", "answer": "паперових", "options": ["паперові", "паперових", "паперовими", "паперової"]}, {"sentence": "Я хочу купити два кілограми __________.", "answer": "моркви", "options": ["морква", "моркву", "моркви", "морквою"]}, {"sentence": "На столі немає великого __________.", "answer": "столу", "options": ["стіл", "столу", "столом", "столі"]}]} instruction={"Вставте правильну форму слова, використовуючи родовий відмінок"} />

## Сценарій 3: Як пройшов твій день? (Scenario 3: How Was Your Day?)

Welcome to the biggest challenge and the most expressive tool in Ukrainian grammar: the aspect of the verb in the past tense. When you tell a story about your day, it is no longer enough to simply say "I was there" or "I worked." You must choose between describing the process of your action or the concrete result of your effort. Mastering this choice allows you to paint a vivid picture of your past experiences and answer the essential difference between "що робив?" (what were you doing?) and "що зробив?" (what did you get done?).

Опанування видів дієслова — це ваш головний ключ до вільного спілкування. Українці рідко використовують складні часові конструкції у розмовах. Але ми завжди звертаємо увагу на характер дії. Нам важливо знати процес і результат. Дія тривала довго, чи вона вже повністю завершена? Це робить нашу мову дуже точною та динамічною.

> *Mastering verb aspects is your main key to fluent communication. Ukrainians rarely use complex tense structures in conversations. But we always pay attention to the character of the action. It is important for us to know the process and the result. Did the action last a long time, or is it already completely finished? This makes our language very precise and dynamic.*

Let us listen to two colleagues, Олена and Максим, talking at the end of the workday. Notice how Олена describes her day as a continuous, exhausting flow of activities, while Максим lists his achievements as clear milestones.

<div class="dialogue">

<div class="dialogue-line"><span class="speaker">Олена:</span> Привіт, Максиме. Як пройшов твій день? *(Hi, Maksym. How was your day?)*</div>

<div class="dialogue-line"><span class="speaker">Максим:</span> Привіт! Дуже добре. Я написав три листи і провів важливу зустріч. А ти? *(Hi! Very good. I wrote three letters and held an important meeting. And you?)*</div>

<div class="dialogue-line"><span class="speaker">Олена:</span> Ох, а я цілий день писала звіти і читала довгі документи. Я так втомилася! *(Oh, and I was writing reports and reading long documents all day. I am so tired!)*</div>

<div class="dialogue-line"><span class="speaker">Максим:</span> Але ти закінчила ту презентацію? *(But did you finish that presentation?)*</div>

<div class="dialogue-line"><span class="speaker">Олена:</span> Ні, я ще не закінчила. Я буду працювати завтра. *(No, I haven't finished yet. I will work tomorrow.)*</div>

<div class="dialogue-line"><span class="speaker">Максим:</span> Розумію. Тоді ходімо пити каву, ти заслужила відпочинок. *(I understand. Then let's go drink coffee, you earned a rest.)*</div>

</div>

When you describe an action that took a long time, was repeated, or served as the background to your day, you must use the imperfective aspect. This form focuses entirely on the process itself, ignoring whether the task was ever finished. You will often see it paired with words that signal duration, such as **довго** (long), **цілий день** (all day), or **часто** (often).

Я вчора дуже довго читав цікаву книгу. Ми весь вечір уважно дивилися цей фільм, але так і не побачили фінал. Моя старша сестра часто купувала свіжі яблука на цьому ринку. Коли вона жила в Києві, вона щодня гуляла в парку.

> *Yesterday I read an interesting book for a very long time. We carefully watched this movie all evening, but we never saw the finale. My older sister often bought fresh apples at this market. When she lived in Kyiv, she walked in the park every day.*

:::info
**Grammar box**
Use the imperfective past tense to answer the question «що робив?» (what were you doing?). It acts like a continuous video recording, showing the action as it unfolds without a definitive end point.
:::

In contrast, the perfective aspect is used to state a clear result or a completed action. It answers the question "що зробив?" (what did you get done?). When you narrate a sequence of completed events, you are creating a chain of results. You build these perfective verbs by adding specific prefixes to the imperfective stem, such as **по-**, **при-**, or **з-**. For example, the process of doing is **робити**, while the result is **зробити**.

Спочатку я поснідав і швидко одягнувся. Потім я відразу пішов на роботу. На роботі я успішно зробив усі завдання і написав звіт. Увечері я повернувся додому і приготував вечерю. Я нарешті добре відпочив після важкого дня.

> *First I had breakfast and quickly got dressed. Then I immediately went to work. At work, I successfully did all the tasks and wrote a report. In the evening I returned home and cooked dinner. I finally rested well after a hard day.*

One of the most elegant ways to use verb aspects is the interruption pattern. You can create a rich narrative by combining both aspects in a single sentence: use the imperfective to set the ongoing background scene, and the perfective to introduce a sudden, completed event that interrupts it. Think of the imperfective verb as a continuous movie playing, while the perfective verb is a sudden snapshot.

Я спокійно готував смачну вечерю, коли ти несподівано подзвонив. Ми йшли додому через парк, коли почався сильний дощ. Вона читала цікаву книгу, коли в кімнату швидко зайшов її брат.

> *I was calmly cooking a tasty dinner when you unexpectedly called. We were walking home through the park when a heavy rain started. She was reading an interesting book when her brother quickly entered the room.*

As you talk about your past experiences, especially related to education or work, you must be careful with the verbs for taking exams. A very common error among learners is translating "to take an exam" literally as "здавати іспит". This is a calque from Russian. In proper Ukrainian, the verb **здавати** means to hand something over, like an apartment key. To express the process of taking an academic test, you must use the verb **складати**.

Мій молодший брат зараз навчається в університеті і складає складний іспит з історії. Я впевнений, що він складе його дуже добре.

> *My younger brother is currently studying at the university and is taking a difficult history exam. I am sure that he will pass it very well.*

:::note
**Quick tip**
Remember: **складати іспит** (imperfective) is the stressful process of taking the test, while **скласти іспит** (perfective) is the joyful result of successfully passing it.
:::

Let us expand your daily routine vocabulary with some essential aspect pairs. Many professional verbs form their perfective partner simply by adding the prefix **про-** or **об-**, such as **аналізувати** (to analyze) becoming **проаналізувати**. However, some verbs undergo an internal stem change. For instance, the verb **замовляти** (to order, process) loses the letter "л" to become **замовити** (to order, result).

Сьогодні ми довго обговорювали новий проєкт. Потім ми ретельно проаналізували всі важливі фінансові дані. Після цієї зустрічі ми вирішили замовити смачну піцу в офіс. Я часто замовляю тут їжу, тому що вони готують дуже швидко.

> *Today we discussed the new project for a long time. Then we carefully analyzed all the important financial data. After this meeting, we decided to order a tasty pizza to the office. I often order food here because they cook very quickly.*

Choosing the right aspect is what makes your Ukrainian sound natural and decolonized. Instead of translating a sequence of English verbs ending in "-ed", you must pause and think about the nature of the action. Was it a background process, or was it a completed step that moved the story forward? When you master this rhythm, you stop translating and start truly thinking in Ukrainian.

Я дуже довго вивчав українську граматику. Я читав складні правила і робив різні письмові вправи. Але саме сьогодні я нарешті добре зрозумів цю важливу тему. Тепер я можу вільно розповідати цікаві історії про своє життя.

> *I studied Ukrainian grammar for a very long time. I read difficult rules and did various written exercises. But exactly today I finally understood this important topic well. Now I can freely tell interesting stories about my life.*

<MatchUp client:only="react" pairs={[{"left": "Цілий день я читала довгі документи,", "right": "тому що це був довгий і нудний процес."}, {"left": "Я швидко написав три важливі листи,", "right": "і отримав успішний результат на роботі."}, {"left": "Вона спокійно готувала смачну вечерю,", "right": "коли несподівано подзвонив її брат."}, {"left": "Ми довго обговорювали наше новосілля,", "right": "але так і не закінчили складати меню."}, {"left": "Після довгої зустрічі ми вирішили", "right": "замовити велику піцу прямо в офіс."}, {"left": "Мій брат зараз складає іспит з історії,", "right": "і це дуже стресовий процес для нього."}, {"left": "Я впевнений, що він успішно складе іспит,", "right": "і ми будемо святкувати ввечері."}, {"left": "Ми весь вечір уважно дивилися цей фільм,", "right": "але так і не побачили фінал."}]} instruction={"З'єднайте частини речення, щоб завершити історію"} />
<MatchUp client:only="react" pairs={[{"left": "Що планували організувати Максим і Олена?", "right": "Велике новосілля для своїх друзів."}, {"left": "Що буде готувати Олена для всіх гостей?", "right": "Великий святковий салат і м'ясо."}, {"left": "Що потрібно сказати, якщо торта немає?", "right": "На жаль, у нас немає торта."}, {"left": "Скільки коштує свіжий мед на цьому ринку?", "right": "Триста гривень за один кілограм."}, {"left": "Який відмінок ми використовуємо після числа п'ять?", "right": "Ми завжди використовуємо родовий відмінок множини."}, {"left": "Що робила Олена цілий день на роботі?", "right": "Вона писала звіти і читала довгі документи."}, {"left": "Що забув купити Максим для їхньої вечірки?", "right": "Він зовсім забув про яблучний сік."}, {"left": "Як українці часто називають стандартний торт?", "right": "Вони використовують слово «тортик»."}]} instruction={"З'єднайте запитання про текст із правильною відповіддю"} />

## Підсумок — Summary

This module introduced the foundation of the A2 level. Before moving forward, take a moment to test your new skills.

Спробуйте відповісти на кілька запитань. Чи можете ви назвати три речі, які ви запланували, використовуючи слово «буду»? Як ви попросите п'ять кілограмів яблук на ринку? Яка різниця між реченнями «Я писав» і «Я написав»? Пам'ятайте головне: вид дієслова показує різницю між процесом і результатом. Водночас родовий відмінок допомагає говорити про кількість та слово «немає».

> *Try to answer a few questions. Can you name three things you planned using the word "буду"? How do you ask for five kilograms of apples at the market? What is the difference between the sentences "Я писав" and "Я написав"? Remember the main point: the aspect of a verb shows the difference between a process and a result. At the same time, the Genitive case helps to talk about quantity and the word "немає".*

:::note
**Quick tip**
Mastering verb aspects takes practice. Do not try to memorize every pair immediately; focus on the verbs you use most often in your daily life.
:::

</TabItem>
<TabItem label="Словник">

### Обов'язкові та рекомендовані слова

| Слово | Переклад | Частина мови | Рід |
|-------|----------|-------------|-----|
| **сцена́рій** | scenario | ім. | ч. |
| **діало́г** | dialogue | ім. | ч. |
| **обгово́рювати / обговори́ти** | to discuss |  |  |
| **замовля́ти / замо́вити** | to order |  |  |
| **планува́ти / запланува́ти** | to plan |  |  |
| **ри́нок** | market | ім. |  |
| **ко́штува́ти** | to cost | дієсл. |  |
| **кілогра́м** | kilogram | ім. | ч. |
| **вечі́рка** | party | ім. | ж. |
| **день** | day | ім. | ч. |
| **новосі́лля** | housewarming party | ім. | с. |
| **напо́ї** | drinks | ім. | ч. |
| **заку́ски** | snacks | ім. | ж. |
| **прибира́ти** | to clean | дієсл. |  |
| **ми́ти** | to wash | дієсл. |  |
| **запро́шувати** | to invite | дієсл. |  |
| **нема́є** | there is no / are no |  |  |
| **бага́то** | many / much | присл. |  |
| **ма́ло** | few / little | присл. |  |
| **свя́то** | holiday | присл. |  |
| **продаве́ць** | vendor | ім. | ч. |
| **покупе́ць** | shopper | ім. | ч. |
| **мед** | honey | ім. | ч. |
| **карто́пля** | potatoes | ім. | ж. |
| **мо́рква** | carrots | ім. | ж. |
| **помідо́ри** | tomatoes | ім. |  |
| **вибира́ти / ви́брати** | to choose |  |  |
| **ва́жити** | to weigh | дієсл. |  |
| **ре́шта** | change | ім. | ж. |
| **ціна́** | price | ім. | ж. |
| **звіт** | report | ім. | ч. |
| **лист** | letter | ім. | ч. |
| **зу́стріч** | meeting | ім. | ж. |
| **відпочива́ти** | to rest | дієсл. |  |
| **аналізува́ти / проаналізува́ти** | to analyze |  |  |
| **ра́птом** | suddenly | присл. |  |
| **гото́во** | ready | присл. |  |
| **зни́жка** | discount | ім. | ж. |
| **втоми́тися** | to get tired | дієсл. |  |
| **поверну́тися** | to return | дієсл. |  |
| **ра́дий** | glad | прикм. |  |
| **я́годи** | berries | ім. | ж. |
| **полуни́ця** | strawberries | ім. | ж. |

### Вирази

| Вираз | Переклад |
|-------|----------|
| **вільний час** | free time |
| **пів кілограма** | half a kilogram |
| **щось іще?** | anything else? |
| **це все** | that's all |
| **складати іспит** | to take an exam |
| **скласти іспит** | to pass an exam |

### Картки — Flashcards

<FlashcardDeck client:only="react" cards={[{ front: "сцена́рій", back: "scenario", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "діало́г", back: "dialogue", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "обгово́рювати / обговори́ти", back: "to discuss" }, { front: "замовля́ти / замо́вити", back: "to order" }, { front: "планува́ти / запланува́ти", back: "to plan" }, { front: "ри́нок", back: "market", subtitle: "ім." }, { front: "ко́штува́ти", back: "to cost", subtitle: "дієсл." }, { front: "кілогра́м", back: "kilogram", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "вечі́рка", back: "party", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "день", back: "day", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "новосі́лля", back: "housewarming party", subtitle: "ім.", tag: "с.", tagColor: "#E65100" }, { front: "напо́ї", back: "drinks", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "заку́ски", back: "snacks", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "прибира́ти", back: "to clean", subtitle: "дієсл." }, { front: "ми́ти", back: "to wash", subtitle: "дієсл." }, { front: "запро́шувати", back: "to invite", subtitle: "дієсл." }, { front: "нема́є", back: "there is no / are no" }, { front: "бага́то", back: "many / much", subtitle: "присл." }, { front: "ма́ло", back: "few / little", subtitle: "присл." }, { front: "свя́то", back: "holiday", subtitle: "присл." }, { front: "продаве́ць", back: "vendor", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "покупе́ць", back: "shopper", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "мед", back: "honey", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "карто́пля", back: "potatoes", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "мо́рква", back: "carrots", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "помідо́ри", back: "tomatoes", subtitle: "ім." }, { front: "вибира́ти / ви́брати", back: "to choose" }, { front: "ва́жити", back: "to weigh", subtitle: "дієсл." }, { front: "ре́шта", back: "change", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "ціна́", back: "price", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "звіт", back: "report", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "лист", back: "letter", subtitle: "ім.", tag: "ч.", tagColor: "#0057B8" }, { front: "зу́стріч", back: "meeting", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "відпочива́ти", back: "to rest", subtitle: "дієсл." }, { front: "аналізува́ти / проаналізува́ти", back: "to analyze" }, { front: "ра́птом", back: "suddenly", subtitle: "присл." }, { front: "гото́во", back: "ready", subtitle: "присл." }, { front: "зни́жка", back: "discount", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "втоми́тися", back: "to get tired", subtitle: "дієсл." }, { front: "поверну́тися", back: "to return", subtitle: "дієсл." }, { front: "ра́дий", back: "glad", subtitle: "прикм." }, { front: "я́годи", back: "berries", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }, { front: "полуни́ця", back: "strawberries", subtitle: "ім.", tag: "ж.", tagColor: "#C2185B" }]} />

</TabItem>
<TabItem label="Зошит">

<GroupSort client:only="react" groups={{"Недоконаний вид (Процес)": ["готувати", "планувати", "купувати", "обговорювати", "замовляти", "складати", "читати"], "Доконаний вид (Результат)": ["приготувати", "запланувати", "купити", "обговорити", "замовити", "скласти", "прочитати"]}} instruction={"Розподіліть дієслова за видами (недоконаний процес чи доконаний результат)"} />

<MatchUp client:only="react" pairs={[{"left": "готувати", "right": "приготувати"}, {"left": "планувати", "right": "запланувати"}, {"left": "купувати", "right": "купити"}, {"left": "замовляти", "right": "замовити"}, {"left": "обговорювати", "right": "обговорити"}, {"left": "складати", "right": "скласти"}, {"left": "вибирати", "right": "вибрати"}, {"left": "робити", "right": "зробити"}]} instruction={"З'єднайте недоконаний вид (процес) з доконаним (результат)"} />

<TrueFalse client:only="react" items={[{"statement": "Після слова «один» ми використовуємо родовий відмінок множини.", "isTrue": false, "explanation": "Після «один» використовується називний відмінок однини (один кілограм)."}, {"statement": "Слова «картопля» та «морква» залишаються в однині, коли ми купуємо їх кілограмами.", "isTrue": true, "explanation": "Це незлічувані іменники (один кілограм картоплі)."}, {"statement": "Якщо у вас немає яблук, ви кажете «немає яблука» (називний відмінок).", "isTrue": false, "explanation": "Після «немає» завжди використовується родовий відмінок (немає яблук)."}, {"statement": "Форма «три лимони» правильна, бо після 2, 3, 4 використовується називний відмінок множини.", "isTrue": true, "explanation": "Так, для чоловічого роду це правило працює (2-4 + називний множини)."}, {"statement": "Дієслово «коштувати» означає процес планування.", "isTrue": false, "explanation": "Дієслово «коштувати» означає «to cost» (вартість)."}, {"statement": "Слова «салатик» та «морквочка» — це зменшувально-пестливі форми.", "isTrue": true, "explanation": "Українці часто використовують такі форми для створення привітної атмосфери."}]} instruction={"Правда чи ні? Визначте правильні правила покупок на ринку"} />

<ErrorCorrection client:only="react" items={[{"sentence": "У мене немає великий стіл для гостей.", "errorWord": "великий стіл", "correctForm": "великого столу", "options": ["великого столу", "великим столом", "великому столу"], "explanation": "Після слова «немає» іменник і прикметник мають бути в родовому відмінку."}, {"sentence": "Дайте мені, будь ласка, п'ять лимони.", "errorWord": "лимони", "correctForm": "лимонів", "options": ["лимон", "лимонів", "лимонами"], "explanation": "Після числа 5 використовується родовий відмінок множини."}, {"sentence": "Вчора він дуже довго прочитав цю книгу.", "errorWord": "прочитав", "correctForm": "читав", "options": ["читав", "читає", "буде читати"], "explanation": "Для довгого процесу без результату потрібен недоконаний вид (читав)."}, {"sentence": "Ми будемо запланувати вечірку завтра.", "errorWord": "запланувати", "correctForm": "планувати", "options": ["планувати", "плануємо", "запланували"], "explanation": "Складний майбутній час (будемо + інфінітив) утворюється ТІЛЬКИ з дієсловами недоконаного виду."}, {"sentence": "Мій брат зараз успішно здає іспит в університеті.", "errorWord": "здає", "correctForm": "складає", "options": ["складає", "робить", "бере"], "explanation": "«Здавати іспит» — це калька з російської. Правильно українською: «складати іспит»."}, {"sentence": "Скільки коштує два кілограмів помідорів?", "errorWord": "кілограмів", "correctForm": "кілограми", "options": ["кілограм", "кілограми", "кілограму"], "explanation": "Після числа 2 використовується називний відмінок множини (два кілограми)."}]} instruction={"Знайдіть і виправте граматичну помилку в кожному реченні"} />

<Translate client:only="react" questions={[{"source": "We have **no big table**.", "options": [{"text": "У нас немає великого столу.", "correct": true}, {"text": "У нас немає великий стіл.", "correct": false}]}, {"source": "Give me **five lemons**, please.", "options": [{"text": "Дайте мені п'ять лимонів, будь ласка.", "correct": true}, {"text": "Дайте мені п'ять лимони, будь ласка.", "correct": false}]}, {"source": "How much does **one kilogram of honey** cost?", "options": [{"text": "Скільки коштує один кілограм меду?", "correct": true}, {"text": "Скільки коштує один кілограм мед?", "correct": false}]}, {"source": "I need to buy **two melons**.", "options": [{"text": "Мені треба купити дві дині.", "correct": true}, {"text": "Мені треба купити двох динь.", "correct": false}]}, {"source": "Are you going to **plan a party**?", "options": [{"text": "Ви будете планувати вечірку?", "correct": true}, {"text": "Ви будете запланувати вечірку?", "correct": false}]}, {"source": "He is **taking a difficult exam**.", "options": [{"text": "Він складає складний іспит.", "correct": true}, {"text": "Він здає складний іспит.", "correct": false}]}]} instruction={"Оберіть правильний переклад для виділених виразів"} />

<Unjumble client:only="react" items={[{"words": "вечірку. / планувати / Давай / нашу", "answer": "Давай планувати нашу вечірку."}, {"words": "коштує / кілограм / Скільки / яблук?", "answer": "Скільки коштує кілограм яблук?"}, {"words": "немає / напоїв. / холодних / У / нас", "answer": "У нас немає холодних напоїв."}, {"words": "успішно / написав / звіт. / Він / вже", "answer": "Він вже успішно написав звіт."}, {"words": "потрібно / Мені / лимонів. / п'ять / купити", "answer": "Мені потрібно купити п'ять лимонів."}, {"words": "піцу / офіс. / замовили / Ми / в / велику", "answer": "Ми замовили велику піцу в офіс."}]} instruction={"Складіть правильне речення зі слів (зверніть увагу на відмінки та вид)"} />

</TabItem>
<TabItem label="Ресурси">

_Ресурси будуть додані пізніше._

</TabItem>
</Tabs>