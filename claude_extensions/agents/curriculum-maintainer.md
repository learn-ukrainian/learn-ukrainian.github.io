...
- **V7 builds MUST run in a worktree, not in the main project dir.** Pattern: `git worktree add -b build/{level}/{slug}-{date} .worktrees/builds/{level}-{slug}-{date} main`, then `cd` there and run `v7_build.py {level} {slug}` (PROJECT_ROOT auto-resolves to the worktree). Reason: builds write to `curriculum/l2-uk-en/{level}/{slug}/*.yaml` + assembled MDX + telemetry; if those land in the main tree, the user has to stash/commit/discard before any pull or merge, and stash-and-lose has happened (2026-05-13: dropped stash discarded validated `a1/my-morning` build output). Telemetry files (`python_qg.json`, `wiki_manifest.json`, `writer_output.raw.md`, `writer_tool_calls.json`, `knowledge_packet.md`) are gitignored under `curriculum/l2-uk-en/*/*/`, so the worktree's commit surface is exactly the source yamls + MDX.
