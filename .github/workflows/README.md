...
|----------|---------|---------|-------|
| `ci.yml` | Required path: pytest shards, contracts (schema/MDX/atlas/BIO + secret scan), exact-head CF attest, frontend, coverage floor, **CI Gate**; advisory E2E after gate | PR (opened/synchronize/reopened/labeled) / push to main / merge_group | `CI Gate` is the only **required** status check. Gate stays red without independent exact-head CF. Always-on parallel fan-out is capped (#4811). PR-body edits do **not** rerun CI (#8505); concurrency keys by PR number so a push cancels the stale run. Gate is `if: always()` and fails when a required job was cancelled or skipped unexpectedly (a skipped required check counts as success); on `pull_request` it re-reads labels and fails if `full-ci` is set but the tier was not full. **Labels (#8505):** `full-ci` is an ordinary full-tier run (forks included) whose `full-ci-pending` job reports a failing `CI Gate` at once, so an earlier lighter-tier green Gate stops counting until the full-tier Gate supersedes it (branch protection reads the newest same-named check run). Use *Re-run all jobs* on such a run; *Re-run failed jobs* would rerun only the placeholder. Any other label is a no-op run in its own concurrency group: every job is skipped and the gate job is renamed, so it reports no `CI Gate` and cannot cancel, mask, or be mistaken for a real run. |
