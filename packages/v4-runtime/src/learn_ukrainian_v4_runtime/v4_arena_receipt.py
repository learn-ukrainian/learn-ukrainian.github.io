...
def _candidate_bindings(
    ...
    require(
        len({item["route_id"] for item in bindings.values()}) == len(bindings),
        "candidate map has duplicate route binding",
    )
    require(
        set(item["route_id"] for item in bindings.values()) == set(routes), "candidate map / route denominator drift"
    )
...
def _proposal_cases(
    ...
    for item in cases:
        ...
        require(
            isinstance(tags, list) and all(isinstance(tag, str) and tag in allowed_tags for tag in tags),
            "PROPOSAL_TAG_INVALID",
        )
...
def build_receipts(
    *,
    ...
    labels, tags = (
        set(_unique_strings(list(allowed_labels), "allowed labels")),
        set(_unique_strings(list(allowed_tags), "allowed tags")),
    )
    ...
    for ordinal, ballot in enumerate(ballots):
        if not isinstance(ballot, Mapping):
            ...
            continue
        voter, target, case_id, label = (
            ballot.get("voter_candidate_id"),
            ballot.get("candidate_id"),
            ballot.get("case_id"),
            ballot.get("label"),
        )
        ...
        if key in ballot_keys:
            residuals.append(
                _residual("DUPLICATE_CASE_BALLOT", ballot_index=ordinal, candidate_id=str(voter), case_id=str(case_id))
            )
            continue
        ballot_keys.add(key)
        valid_ballots.append(
            {
                "voter_candidate_id": str(voter),
        ...
        for target in sorted(candidates):
            ...
            for case_id in frozen_cases:
                if (voter, target, case_id) not in ballot_keys:
                    residuals.append(
                        _residual(
                            "MISSING_CASE_BALLOT", candidate_id=voter, target_candidate_id=target, case_id=case_id
                        )
                    )
    ...
    for index, case_id in enumerate(frozen_cases):
        outputs = [
            {"candidate_id": candidate_id, **proposals[candidate_id][index]} for candidate_id in sorted(proposals)
        ]
        ...
        for target_candidate_id in sorted(candidates):
            ...
            consensus_label = (
                ranked[0][0] if len(ranked) == 1 or (len(ranked) > 1 and ranked[0][1] > ranked[1][1]) else None
            )
    ...
    public = {
        ...
        "counts": {
            ...
            "valid_routes": len(proposals),
            "invalid_routes": len(routes) - len(proposals),
            "exact_agreement_cases": sum(case["agreement_status"] == "exact_agreement" for case in per_case),
            "disputed_cases": sum(case["agreement_status"] == "disagreement" for case in per_case),
        },
...
def verify_receipt(receipt: Mapping[str, Any]) -> dict[str, Any]:
    ...
    cases, routes = (
        _unique_strings(binding.get("case_ids"), "receipt case IDs"),
        _unique_strings(binding.get("route_denominator"), "receipt route denominator"),
    )
    ...
    require(
        {item.get("route_id") for item in statuses if isinstance(item, Mapping)} == set(routes),
        "receipt route denominator drift",
    )
    if visibility == "public_safe_text_free":
        ...
        require(
            ...
            and [item.get("case_id") if isinstance(item, Mapping) else None for item in case_rows] == cases,
            "receipt case order drift",
        )
        require(
            all(isinstance(item, Mapping) and item.get("disposition") == QUARANTINE for item in case_rows),
            "arena quarantine drift",
        )
    return dict(receipt)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--input", required=True, type=Path, help="JSON fixture containing build_receipts keyword arguments"
    )
    ...
    print(
        canonical_json(
            {
                "private_receipt_sha256": receipts["private"]["receipt_sha256"],
                "public_receipt_sha256": receipts["public"]["receipt_sha256"],
            }
        )
    )
    return 0
