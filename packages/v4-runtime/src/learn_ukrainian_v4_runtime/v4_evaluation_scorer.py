...
def verify_receipt(receipt: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        set(body) != {"schema_version", "visibility", "outcome_sha256", "scores", "counts"}
    ...
    for item in scores:
        ...
        if (
            item.get("scored") is not False
            or item.get("score") is not None
            or item.get("residual_code") != UNSCORABLE_RESIDUAL_CODE
        ):
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--input", required=True, type=Path, help="versioned JSON object containing outcome_sha256 and admitted_rows"
    )
    ...
    try:
        ...
        if (
            ...
            or set(payload) != {"schema_version", "outcome_sha256", "admitted_rows"}
