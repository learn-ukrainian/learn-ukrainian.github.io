...
def distinct_supporting_family_ids(supporting_source_unit_ids: list[str], family_registry: dict[str, Any]) -> set[str]:
    ...
    for unit_id in supporting_source_unit_ids:
        require(
            unit_id in mapping,
            f"supporting_existing_source_unit_id {unit_id!r} is not a member of any registered family -- refusing",
        )
        families.add(mapping[unit_id])
    return families


def candidate_family_floor_met(
    supporting_source_unit_ids: list[str], family_registry: dict[str, Any], heldout_count: int
) -> bool:
...
def validate_candidate_family_floor(
    stratum: str, coverage_entry: dict[str, Any], family_registry: dict[str, Any], heldout_count: int
) -> None:
    ...
    require(
        coverage_entry["stratum"] == stratum,
        f"coverage_entry stratum {coverage_entry.get('stratum')!r} does not match {stratum!r} -- refusing",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--coverage-entry", required=True, type=Path, help="JSON file: one A2 stratum_coverage_map entry"
    )
    parser.add_argument(
        "--family-registry", required=True, type=Path, help="JSON file: an A3 seal receipt's source_family_registry"
    )
