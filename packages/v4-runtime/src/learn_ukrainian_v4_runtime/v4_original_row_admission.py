...
def _basis_is_model_only(value: Any) -> bool:
    if not isinstance(value, Mapping):
        return False
    return (
        value.get("basis") in MODEL_ONLY_BASES
        or value.get("model_agreement") is True
        or value.get("arena_vote") is True
        or value.get("model_vote") is True
    )
...
def evaluate_row(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if _basis_is_model_only(evidence):
        ...
        or evidence.get("grade") != "verified"
        or evidence.get("uncertainty") not in {"resolved", "bounded"}
        or evidence.get("disposition") not in {"supported", "admitted"}
    ):
    ...
    if (
        ...
        or not isinstance(split.get("receipt_id"), str)
        or not split.get("receipt_id")
    ):
    ...
    if tier == "gold":
        ...
        if (
            not isinstance(gold_basis, Mapping)
            or gold_basis.get("kind") not in {"authoritative_deterministic", "independent_qualified_adjudication"}
            or not isinstance(gold_basis.get("receipt_id"), str)
            or not gold_basis.get("receipt_id")
            or _basis_is_model_only(gold_basis)
        ):
    ...
    result = {
        ...
        "eligibility": {
            "gold": tier == "gold" and not residuals,
            "training": not residuals,
            "evaluation": False,
            "teaching": False,
            "coverage": False,
        },
...
def admit_rows(*, outcome_sha256: str, rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    result = {
        ...
        "counts": {
            ...
            "admitted_rows": sum(item["disposition"] == "admitted" for item in receipts),
            "rejected_rows": sum(item["disposition"] == "rejected" for item in receipts),
        },
    }
    result["receipt_sha256"] = sha256_value(result)
    return result
...
def assemble_receipt_from_row_receipts(
    *, outcome_sha256: str, row_receipts: Sequence[Mapping[str, Any]]
) -> dict[str, Any]:
...
def verify_receipt(receipt: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        set(body) != {"schema_version", "visibility", "outcome_sha256", "rows", "counts"}
        or body.get("visibility") != "machine_receipt_text_free"
    ):
        raise OriginalRowAdmissionError("receipt schema drift")
    ...
    if counts != {
        "input_rows": len(rows),
        "admitted_rows": sum(item.get("disposition") == "admitted" for item in rows if isinstance(item, Mapping)),
        "rejected_rows": sum(item.get("disposition") == "rejected" for item in rows if isinstance(item, Mapping)),
    }:
    ...
    for row in rows:
        ...
        if row_content_sha256 is not None and (
            not isinstance(row_content_sha256, str) or SHA256_RE.fullmatch(row_content_sha256) is None
        ):
        ...
        if row.get("disposition") == "admitted" and row_content_sha256 is None:
            raise OriginalRowAdmissionError("row receipt content SHA-256 drift")
        if row.get("disposition") not in {"admitted", "rejected"} or row.get("training_eligible") != (
            row.get("disposition") == "admitted"
        ):
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--input", required=True, type=Path, help="versioned JSON object containing outcome_sha256 and rows"
    )
    ...
    try:
        ...
        if (
            ...
            or set(payload) != {"schema_version", "outcome_sha256", "rows"}
