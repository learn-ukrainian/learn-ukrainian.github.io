...
from learn_ukrainian_v4_runtime import v4_a8_admission_assembly as a8
from learn_ukrainian_v4_runtime import v4_evaluation_scorer as scorer
...
def check_evaluation_gate(root: Path = ROOT) -> dict[str, Any]:
    ...
    a8_completion_ids = (
        ev.completion_slot_ids(
            a8_receipt.get("a8_completions", []), stage="A8", total_slot_ids=total_ids, error_cls=EvaluationPackageError
        )
        if a8_valid
        else set()
    )
    ...
    a9_completion_ids = ev.completion_slot_ids(
        a9_completions, stage="A9", total_slot_ids=total_ids, error_cls=EvaluationPackageError
    )
    ...
    ev.validate_subset(
        a9_completion_ids,
        eligible_ids,
        label="A9 completions vs prerequisite-eligible slots",
        error_cls=EvaluationPackageError,
    )
    ev.validate_subset(
        a9_completion_ids,
        a8_completion_ids,
        label="A9 completions vs A8 completions (upstream subset)",
        error_cls=EvaluationPackageError,
    )
...
def build_consumer_reproduction_view(
    manifest: dict[str, Any], a8_receipt: dict[str, Any], a9_residuals: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    admitted_rows_by_id = {
        row["row_id"]: row
        for row in a8_receipt["engine_wiring"]["admission_receipt"]["rows"]
    ...
    require(
        ...
        "A8 admitted_slice_view does not cover exactly the frozen slot manifest -- refusing consumer reproduction",
    )
    ...
    for entry in a8_receipt["admitted_slice_view"]:
        ...
        if row_admitted:
            ...
            require(
                row_id is None,
                f"A8 admitted_slice_view carries a row_id for a non-admitted slot {slot_id!r} -- refusing",
            )
        view.append(
            {
                "slot_id": slot_id,
...
def validate_scorer_wiring(receipt: dict[str, Any]) -> None:
    ...
    require(
        wiring["scorer_schema_version"] == scorer.SCHEMA_VERSION
        and wiring["scorer_input_schema_version"] == scorer.INPUT_SCHEMA_VERSION,
    ...
    require(
        wiring["scoring_receipt"] == recomputed,
        "scorer_wiring.scoring_receipt does not reproduce from a live, zero-row v4_evaluation_scorer.score_rows call -- refusing",
    )
...
def validate_eligibility_and_completion_and_consumer_view(receipt: dict[str, Any], root: Path) -> None:
    ...
    expected_eligibility = ev.public_eligibility(
        ev.stratum_eligibility(manifest, a2_receipt, error_cls=EvaluationPackageError)
    )
    require(
    ...
    completion_ids = ev.completion_slot_ids(
        receipt["a9_completions"], stage="A9", total_slot_ids=total_ids, error_cls=EvaluationPackageError
    )
    ev.validate_subset(
        completion_ids,
        eligible_ids,
    ...
    require(
        ...
        "a9_completions count does not match the gate's slots_stage_complete -- refusing",
    )
    ...
    require(
        ...
        "a9_residuals does not exactly cover the complement of a9_completions over the frozen denominator -- refusing",
    )
    ...
    require(
        receipt["a9_residuals"] == expected_a9_residuals,
        "a9_residuals does not reproduce from the live slot manifest, A2 receipt, and gate -- refusing",
    )

    expected_view = build_consumer_reproduction_view(manifest, a8_receipt, expected_a9_residuals)
    require(
        receipt["consumer_reproduction_view"] == expected_view,
        "consumer_reproduction_view does not reproduce from the live A8 receipt and a9_residuals -- refusing",
    )
    ...
    require(
        all(
            (entry["row_admitted"] and isinstance(entry["row_id"], str))
            or (not entry["row_admitted"] and entry["row_id"] is None)
...
def validate_eligibility_and_safety_all_false(receipt: dict[str, Any]) -> None:
    require(
        receipt["eligibility"] == EVALUATION_ELIGIBILITY,
        "receipt eligibility does not equal the frozen all-false evaluation eligibility -- refusing",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--receipt",
        type=Path,
        default=A9_RECEIPT_PATH,
        help="A9 receipt JSON to verify (default: the tracked V4 A9 evaluation package receipt).",
    )
    parser.add_argument(
