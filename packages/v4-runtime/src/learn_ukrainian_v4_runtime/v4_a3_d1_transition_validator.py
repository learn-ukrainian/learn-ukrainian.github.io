...
def validate_manifest_meets_d1(
    manifest: dict[str, Any], a2_receipt: dict[str, Any], seal_receipt: dict[str, Any]
) -> None:
    ...
    require(
        isinstance(manifest, dict) and isinstance(manifest.get("slot_series"), list),
        "manifest is malformed -- refusing",
    )
    require(
        isinstance(a2_receipt, dict) and isinstance(a2_receipt.get("stratum_coverage_map"), list),
        "A2 receipt is malformed -- refusing",
    )
    ...
    for series in manifest["slot_series"]:
        require(
            isinstance(series.get("stratum"), str) and series["stratum"],
            "manifest slot_series entry is missing a stratum -- refusing (malformed slot mapping)",
        )
        ...
        require(
            coverage_entry is not None,
            f"manifest stratum {stratum!r} is ASSIGNED but has no matching A2 stratum_coverage_map entry -- refusing",
        )
        try:
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--seal-receipt", required=True, type=Path, help="the public A3 heldout-source-family seal receipt JSON"
    )
