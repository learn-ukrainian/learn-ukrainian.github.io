...
from learn_ukrainian_v4_runtime import v4_a3_candidate_family_floor as floor
...
def validate_assigned_strata_meet_candidate_family_floor(
    manifest: dict[str, Any], a2_receipt: dict[str, Any], new_receipt: dict[str, Any]
) -> None:
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--old-receipt",
        type=Path,
        default=heldout.DEFAULT_RECEIPT,
        help="the currently sealed receipt JSON (read-only)",
    )
    parser.add_argument(
        "--new-receipt",
        type=Path,
        required=True,
        help="the new sealed receipt JSON to reissue the private artifact's binding against",
    )
    parser.add_argument(
        "--private-dir",
        type=Path,
        default=heldout.DEFAULT_PRIVATE_DIR,
        help=f"private artifact directory; must be inside {heldout.PRIVATE_ROOT}",
    )
    parser.add_argument(
        "--a2-receipt",
        type=Path,
        required=True,
        help="A2's public stratum_coverage_map receipt JSON (for the candidate-family floor check)",
    )
    parser.add_argument(
        "--manifest",
        type=Path,
        required=True,
        help="the frozen 100-slot pilot manifest JSON (for the candidate-family floor check)",
    )
    parser.add_argument(
        "--expect-salt-commitment", help="assert the new receipt's salt_commitment_sha256 equals this value"
    )
    parser.add_argument(
        "--expect-assignment-commitment", help="assert the new receipt's assignment_commitment_sha256 equals this value"
    )
