...
from learn_ukrainian_v4_runtime import v4_sources_authority as sources_authority
...
def _require_verifier_receipt_body_shape(body: dict[str, Any]) -> None:
    require(
        verify_identifier_shape(body.get("identifier")),
        f"verifier receipt identifier does not match the pinned VESUM/sources shape: {body.get('identifier')!r}",
    )
    require(
    ...
    require(
        isinstance(body.get("tool_version"), str) and body["tool_version"],
        "verifier receipt tool_version must be a nonempty string -- refusing",
    )
    require(
        isinstance(body.get("row_content_sha256"), str) and bool(SHA256_HEX_RE.match(body["row_content_sha256"])),
        "verifier receipt row_content_sha256 is not a well-formed sha256 -- refusing",
    )
    require(
        isinstance(body.get("tool_result_sha256"), str) and bool(SHA256_HEX_RE.match(body["tool_result_sha256"])),
        "verifier receipt tool_result_sha256 is not a well-formed sha256 -- refusing",
    )
    ...
    require(
        ...
        "verifier receipt lookup_ids must be a nonempty list of immutable identifiers -- refusing",
    )


def build_verifier_receipt(*, attestation: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        sources_authority.verify_verifier_attestation(
            attestation, trust_policy=trust_policy, outcome_sha256=V4_SHA256, row_content_sha256=row_content_sha256
        )
    ...
    return {**body, "signature_hex": signature_hex, "receipt_id": receipt_id, "trust_policy_sha256": policy_digest}
...
def validate_verifier_receipt_integrity(receipt: dict[str, Any]) -> None:
    ...
    require(
        recomputed == receipt.get("receipt_id"),
        "verifier receipt fails its own integrity recheck (tampered or hand-fabricated) -- refusing",
    )
    require(
        receipt.get("trust_policy_sha256") == policy_digest,
        "verifier receipt trust_policy_sha256 does not match the active production policy -- refusing",
    )
    try:
        sources_authority.verify_verifier_attestation(
            {**body, "signature_hex": signature_hex},
            trust_policy=trust_policy,
            outcome_sha256=V4_SHA256,
            row_content_sha256=body.get("row_content_sha256"),
        )
    except sources_authority.SourcesAuthorityError as exc:
...
def build_evidence_receipt(
    row_content_sha256: str, verifier_receipts: list[dict[str, Any]], *, uncertainty: str = "resolved"
) -> dict[str, Any]:
    ...
    require(
        isinstance(verifier_receipts, list) and verifier_receipts,
        "verifier_receipts must be a nonempty list of real verifier receipts (see build_verifier_receipt)",
    )
    ...
    for verifier_receipt in verifier_receipts:
        ...
        require(
            verifier_receipt["row_content_sha256"] == row_content_sha256,
            "verifier receipt is not bound to this row's content hash -- refusing",
        )
...
def validate_evidence_receipt_integrity(evidence_receipt: dict[str, Any]) -> None:
    ...
    require(
        source in {"verifier_receipt", "synthetic_fixture"},
        "evidence_receipt.evidence_source must be verifier_receipt or synthetic_fixture -- refusing",
    )
    ...
    require(
        recomputed == evidence_receipt.get("receipt_id"),
        "evidence_receipt fails its own integrity recheck (tampered or hand-fabricated) -- refusing",
    )
    ...
    if source == "verifier_receipt":
        ...
        require(
            ...
            "verifier-backed evidence_receipt must declare production_capable true -- refusing",
        )
        _, policy_digest = trust.load_production_trust_policy()
        require(
            evidence_receipt.get("trust_policy_sha256") == policy_digest,
            "evidence_receipt trust_policy_sha256 does not match the active production policy -- refusing",
        )
        verifier_receipts = evidence_receipt.get("verifier_receipts")
        require(
            isinstance(verifier_receipts, list) and verifier_receipts,
            "verifier-backed evidence_receipt must carry a nonempty verifier_receipts list -- refusing",
        )
        for verifier_receipt in verifier_receipts:
            validate_verifier_receipt_integrity(verifier_receipt)
            require(
                verifier_receipt["row_content_sha256"] == evidence_receipt["row_content_sha256"],
                "verifier receipt is not bound to this evidence receipt's row_content_sha256 -- refusing",
            )
    else:
        require(
            evidence_receipt.get("production_capable") is False,
            "synthetic-fixture evidence_receipt must declare production_capable false -- refusing",
        )
