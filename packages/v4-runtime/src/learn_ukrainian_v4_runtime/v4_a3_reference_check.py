...
from learn_ukrainian_v4_runtime import v4_a3_split_duplicate_check as split_check
...
RECEIPT_SIGNATURE_KEYS = frozenset(
    {"schema_version", "outcome_sha256", "reference_check_receipt_sha256", "signer_key_id", "trust_policy_sha256"}
)
REPLAY_ATTESTATION_KEYS = frozenset(
    {
        ...
        "outcome_sha256",
        "row_content_sha256",
        "reference_check_receipt_sha256",
        "policy_sha256",
...
def _structural_pass(candidate: str, reference_texts: list[str], policy: dict[str, Any]) -> bool:
    for reference in reference_texts:
        ...
        if (
            result.token_jaccard >= STRUCTURAL_SIMILARITY_THRESHOLD
            or result.normalized_edit_similarity >= STRUCTURAL_SIMILARITY_THRESHOLD
        ):
            return False
    return True


def _run_reconstruction_gates(
    candidate_text: str, reference_texts: dict[str, str], policy: dict[str, Any]
    ...
    exact_pass = all(
        near_duplicate.classify_texts(candidate_text, reference, scope="span", policy=policy).classification != "exact"
        for reference in references
    )
    fuzzy_pass = all(
        not near_duplicate.classify_texts(candidate_text, reference, scope="span", policy=policy).duplicate
        for reference in references
    )
    structural_pass = _structural_pass(candidate_text, references, policy)
    cumulative_reference = "\n".join(references)
    cumulative_pass = not near_duplicate.classify_texts(
        candidate_text, cumulative_reference, scope="span", policy=policy
    ).duplicate
    normalized_candidate = near_duplicate.normalize(candidate_text)
    no_verbatim_containment = all(
        normalized_candidate not in near_duplicate.normalize(reference)
        and near_duplicate.normalize(reference) not in normalized_candidate
        for reference in references
    )
...
def build_reference_check_receipt(
    candidate_text: str, reference_texts: dict[str, str], salt: bytes, *, policy: dict[str, Any] | None = None
) -> dict[str, Any]:
...
def validate_reference_check_receipt_integrity(receipt: dict[str, Any]) -> None:
    ...
    require(
        recompute_receipt_id(receipt) == receipt.get("receipt_id"),
        "reference_check_receipt fails its own integrity recheck (tampered or hand-fabricated) -- refusing",
    )
    gates = receipt.get("reconstruction_gates")
    require(
        isinstance(gates, dict) and set(gates) == set(RECONSTRUCTION_GATES),
        "reference_check_receipt.reconstruction_gates does not cover exactly the five reconstruction gates -- refusing",
    )
    split_passed = receipt.get("split_duplicate", {}).get("passed")
    all_gates_passed = all(
        isinstance(gates[gate], dict) and gates[gate].get("passed") is True for gate in RECONSTRUCTION_GATES
    )
    expected_passed = split_passed is True and all_gates_passed
    require(
        receipt.get("passed") == expected_passed,
        "reference_check_receipt.passed is not the logical AND of its own declared split/gate results -- refusing",
    )


def verify_reference_check_receipt(
    ...
    *,
    ...
    require(
        recomputed == receipt,
        "reference_check_receipt does not reproduce from the candidate text, reference-text set, and salt -- refusing",
    )
...
def _sign_reference_check_receipt_from_evidence(
    *, signing_key_hex: str, signer_key_id: str, receipt: dict[str, Any], outcome_sha256: str, trust_policy_sha256: str
) -> dict[str, Any]:
...
def verify_reference_check_receipt_signature(
    signature: dict[str, Any], *, receipt: dict[str, Any], trust_policy: dict[str, Any], outcome_sha256: str
) -> None:
    ...
    require(
        set(body) == RECEIPT_SIGNATURE_KEYS,
        f"reference-check receipt signature must declare exactly {sorted(RECEIPT_SIGNATURE_KEYS)} -- refusing (unexpected or missing key)",
    )
    require(
        body.get("schema_version") == RECEIPT_SIGNATURE_SCHEMA_VERSION,
        "reference-check receipt signature schema_version mismatch -- refusing",
    )
    require(
        body.get("outcome_sha256") == outcome_sha256,
        "reference-check receipt signature is bound to a different outcome -- refusing",
    )
    require(
        body.get("reference_check_receipt_sha256") == _sha256_text(_canonical_json(receipt)),
        "reference-check receipt signature does not bind this exact receipt -- refusing (stale or altered receipt)",
    )
    signature_hex = signature.get("signature_hex")
    require(
        isinstance(signature_hex, str) and bool(signature_hex),
        "reference-check receipt signature carries no signature_hex -- refusing",
    )
    try:
        trust.verify_with_policy(
            trust_policy, "a3", body.get("signer_key_id"), RECEIPT_SIGNATURE_DOMAIN, body, signature_hex
        )
...
def _issue_replay_attestation_from_evidence(
    *,
    ...
    attestation`` below."""
    require(
        isinstance(replay_invocation_id, str) and replay_invocation_id,
        "replay_invocation_id must be a nonempty string -- refusing",
    )
    trust.require_sha256_hex(trust_policy_sha256, "trust_policy_sha256", error_cls=ReferenceCheckError)
    recomputed = build_reference_check_receipt(candidate_text, reference_texts, salt, policy=policy)
    require(
        recomputed == receipt,
        "reference_check_receipt does not reproduce from the candidate text, reference-text set, and salt -- refusing to attest replay",
    )
    body = {
...
def verify_replay_attestation(
    ...
    *,
    ...
    require(
        set(body) == REPLAY_ATTESTATION_KEYS,
        f"replay attestation must declare exactly {sorted(REPLAY_ATTESTATION_KEYS)} -- refusing (unexpected or missing key)",
    )
    require(
        body.get("schema_version") == REPLAY_ATTESTATION_SCHEMA_VERSION,
        "replay attestation schema_version mismatch -- refusing",
    )
    require(
        body.get("outcome_sha256") == outcome_sha256, "replay attestation is bound to a different outcome -- refusing"
    )
    require(
        body.get("row_content_sha256") == row_content_sha256,
        "replay attestation is not bound to this row's content hash -- refusing",
    )
    require(
        body.get("reference_check_receipt_sha256") == _sha256_text(_canonical_json(receipt)),
        "replay attestation does not bind this exact receipt -- refusing (stale or altered receipt)",
    )
    require(
        body.get("policy_sha256") == receipt.get("policy_sha256"),
        "replay attestation policy_sha256 does not match the receipt's own policy_sha256 -- refusing",
    )
    signature_hex = attestation.get("signature_hex")
    require(
        isinstance(signature_hex, str) and bool(signature_hex),
        "replay attestation carries no signature_hex -- refusing",
    )
    try:
        trust.verify_with_policy(
            trust_policy, "a3", body.get("signer_key_id"), REPLAY_ATTESTATION_DOMAIN, body, signature_hex
        )
    except trust.TrustAuthorityError as exc:
