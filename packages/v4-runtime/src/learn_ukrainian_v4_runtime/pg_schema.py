...
MIGRATIONS: tuple[PgMigration, ...] = (
    ...
    PgMigration(
        version=6,
        name="fleet-comms-pg-v6-v4-operation-ownership",
        statements=((resource_root() / "sql/operation_authority_v1.sql").read_text(),),
    ),
)
...
def apply_pg_schema(conn: Any) -> int:
    ...
    with conn.transaction():
        ...
        for migration in MIGRATIONS:
            ...
            conn.execute(
                f"INSERT INTO {PG_MIGRATION_TABLE}(version, name, checksum, applied_at) VALUES (%s, %s, %s, %s)",
                (
                    migration.version,
                    migration.name,
                    migration.checksum,
