...
from learn_ukrainian_v4_runtime import v4_a12_gold_overlay_gate as a12
...
def check_cleanup_recovery_state(root: Path = ROOT) -> dict[str, Any]:
    ...
    a12_path = (
        root / "data/projects/open_model_data/admission/dataset_v4_a12_gold_overlay_gate_receipt_v1.json"
    ).resolve()
    ...
    all_ids = [
        slot_id for stratum in a12.a11.a10.a9.a8.a7.a6.frozen_slot_strata(manifest) for slot_id in stratum["slot_ids"]
    ]
...
def derive_a13_slot_residuals(
    manifest: dict[str, Any], a2_receipt: dict[str, Any], state: dict[str, Any]
) -> list[dict[str, Any]]:
...
def build_receipt(root: Path = ROOT) -> dict[str, Any]:
    ...
    carried_by_stage = {
        stage: [
            {"residual_id": entry["residual_id"], "origin_stage": stage, "status": "unresolved_carried_to_a13"}
...
def validate_recovery_state_matches_receipt(
    receipt: dict[str, Any], root: Path, state: dict[str, Any] | None = None
) -> None:
    ...
    require(
        declared["epic_closed"] is False,
        "receipt recovery_state claims the epic is closed -- refusing (never a valid A13 claim)",
    )
...
def validate_cleanup_policy(receipt: dict[str, Any]) -> None:
    ...
    receipt."""
    require(
        receipt["cleanup_policy"] == CLEANUP_POLICY,
        "cleanup_policy does not equal the frozen cleanup-policy contract -- refusing",
    )
...
def validate_named_residual(receipt: dict[str, Any], root: Path, state: dict[str, Any] | None = None) -> None:
    state = check_cleanup_recovery_state(root) if state is None else state
    require(
        receipt["named_residual"] == build_named_residual(state),
        "named_residual does not reproduce from the live recovery state -- refusing",
    )
    require(
        receipt["named_residual"]["owner_role"] == "A2_A3_PRIVATE_ARTIFACT",
        "named_residual owner_role is not A2_A3_PRIVATE_ARTIFACT -- refusing",
    )
...
def validate_residuals_carried_forward(
    receipt: dict[str, Any], root: Path, state: dict[str, Any] | None = None
) -> None:
    a2_receipt = _load(A2_RECEIPT_PATH)
    a4_receipt = _load(A4_RECEIPT_PATH)
    ...
    require(
        receipt["a13_residuals"] == expected_a13_residuals,
        "a13_residuals does not reproduce from the live slot manifest, A2 receipt, and recovery state -- refusing",
    )
...
def validate_eligibility_and_safety_all_false(receipt: dict[str, Any]) -> None:
    require(
        receipt["eligibility"] == CLEANUP_RECOVERY_ELIGIBILITY,
        "receipt eligibility does not equal the frozen all-false cleanup-recovery eligibility -- refusing",
    )
    ...
    require(
        receipt["execution_counters"]["temp_outputs_reaped"] == 0,
        "receipt execution_counters.temp_outputs_reaped is not 0 -- refusing (this module declares policy, it never executes cleanup)",
    )
    require(
        receipt["execution_counters"]["forbidden_paths_touched"] == 0,
        "receipt execution_counters.forbidden_paths_touched is not 0 -- refusing",
    )
    require(
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--receipt",
        type=Path,
        default=A13_RECEIPT_PATH,
        help="A13 receipt JSON to verify (default: the tracked V4 A13 cleanup/recovery receipt).",
    )
    parser.add_argument(
