...
FORBIDDEN_COMPLETION_CLAIMS = tuple(sorted({*a9.FORBIDDEN_COMPLETION_CLAIMS, "EVAL_ARTIFACT_READY"} - {"PILOT_REVIEW_PASSED"}))
...
def check_pilot_review_gate(root: Path = ROOT) -> dict[str, Any]:
    ...
    a9_path = (root / "data/projects/open_model_data/admission/dataset_v4_a9_evaluation_package_receipt_v1.json").resolve()
    ...
    require(REQUIRED_GATE_ID in manifest.get("required_gate_ids", []), "slot manifest no longer lists this stage's required gate ID -- refusing")
    ...
    pilot_review_slice_ready = rights_resolved and all_assigned and a9_valid and upstream_evaluation_slice_ready and independent_review_recorded
...
def derive_a10_slot_residuals(manifest: dict[str, Any], a2_receipt: dict[str, Any], gate: dict[str, Any]) -> list[dict[str, Any]]:
...
def build_review_readiness_view(manifest: dict[str, Any], a9_receipt: dict[str, Any], a10_residuals: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    require(seen_slot_ids == expected_slot_ids, "A9 consumer_reproduction_view does not cover exactly the frozen slot manifest -- refusing pilot review readiness view")
...
def validate_gate_matches_receipt(receipt: dict[str, Any], root: Path) -> None:
    ...
    require(declared["independent_review_recorded"] is False, "receipt review_gate claims a review was recorded, but no execution mechanism exists -- refusing")
...
def validate_review_packet(receipt: dict[str, Any]) -> None:
    ...
    require(receipt["review_packet"] == REVIEW_PACKET_REQUIREMENTS, "review_packet does not equal the frozen review-packet contract -- refusing")
...
def validate_residuals_and_review_view(receipt: dict[str, Any], root: Path) -> None:
    ...
    require(receipt["a10_residuals"] == expected_a10_residuals, "a10_residuals does not reproduce from the live slot manifest, A2 receipt, and gate -- refusing")

    expected_view = build_review_readiness_view(manifest, a9_receipt, expected_a10_residuals)
    require(receipt["review_readiness_view"] == expected_view, "review_readiness_view does not reproduce from the live A9 receipt and a10_residuals -- refusing")
    require(
        all(entry["review_executed"] is False and entry["reviewer_family"] is None and entry["cf_review_of_record_passed"] is False for entry in receipt["review_readiness_view"]),
...
def validate_eligibility_and_safety_all_false(receipt: dict[str, Any]) -> None:
    require(receipt["eligibility"] == PILOT_REVIEW_ELIGIBILITY, "receipt eligibility does not equal the frozen all-false pilot-review eligibility -- refusing")
    ...
    require(receipt["execution_counters"]["rows_reviewed"] == 0, "receipt execution_counters.rows_reviewed is not 0 -- refusing (no review-execution mechanism exists yet)")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--receipt", type=Path, default=A10_RECEIPT_PATH, help="A10 receipt JSON to verify (default: the tracked V4 A10 pilot review gate receipt).")
