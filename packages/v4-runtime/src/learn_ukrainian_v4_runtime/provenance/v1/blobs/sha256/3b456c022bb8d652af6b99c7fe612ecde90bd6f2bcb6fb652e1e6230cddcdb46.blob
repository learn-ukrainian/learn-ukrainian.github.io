...
FORBIDDEN_COMPLETION_CLAIMS = tuple(sorted({*a10.FORBIDDEN_COMPLETION_CLAIMS, "PILOT_REVIEW_PASSED"} - {"TRAINING_READY_SILVER"}))

SILVER_RELEASE_ELIGIBILITY = {"gold": False, "training": False, "evaluation": False, "teaching": False, "coverage": False}
...
def check_silver_release_gate(root: Path = ROOT) -> dict[str, Any]:
    ...
    *and* a silver release has actually been executed -- and that last
    ...
    a10_path = (root / "data/projects/open_model_data/admission/dataset_v4_a10_pilot_review_gate_receipt_v1.json").resolve()
    ...
    for gate_id in REQUIRED_GATE_IDS:
        require(gate_id in manifest.get("required_gate_ids", []), f"slot manifest no longer lists this stage's required gate ID {gate_id!r} -- refusing")
...
def derive_a11_slot_residuals(manifest: dict[str, Any], a2_receipt: dict[str, Any], gate: dict[str, Any]) -> list[dict[str, Any]]:
...
def build_silver_release_view(manifest: dict[str, Any], a10_receipt: dict[str, Any], a11_residuals: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    require(seen_slot_ids == expected_slot_ids, "A10 review_readiness_view does not cover exactly the frozen slot manifest -- refusing silver release view")
...
def validate_gate_matches_receipt(receipt: dict[str, Any], root: Path) -> None:
    ...
    require(declared["silver_release_executed"] is False, "receipt release_gate claims a silver release was executed, but no execution mechanism exists -- refusing")
...
def validate_release_packet(receipt: dict[str, Any]) -> None:
    ...
    require(receipt["release_packet"] == RELEASE_PACKET_REQUIREMENTS, "release_packet does not equal the frozen release-packet contract -- refusing")
...
def validate_residuals_and_silver_view(receipt: dict[str, Any], root: Path) -> None:
    ...
    require(receipt["a11_residuals"] == expected_a11_residuals, "a11_residuals does not reproduce from the live slot manifest, A2 receipt, and gate -- refusing")

    expected_view = build_silver_release_view(manifest, a10_receipt, expected_a11_residuals)
    require(receipt["silver_release_view"] == expected_view, "silver_release_view does not reproduce from the live A10 receipt and a11_residuals -- refusing")
    require(
        all(
            entry["release_executed"] is False and entry["label_tier"] is None and entry["row_admitted"] is False and entry["pilot_review_passed"] is False
...
def validate_eligibility_and_safety_all_false(receipt: dict[str, Any]) -> None:
    require(receipt["eligibility"] == SILVER_RELEASE_ELIGIBILITY, "receipt eligibility does not equal the frozen all-false silver-release eligibility -- refusing")
    ...
    require(receipt["execution_counters"]["rows_released_as_silver"] == 0, "receipt execution_counters.rows_released_as_silver is not 0 -- refusing (no release-execution mechanism exists yet)")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--receipt", type=Path, default=A11_RECEIPT_PATH, help="A11 receipt JSON to verify (default: the tracked V4 A11 silver release gate receipt).")
