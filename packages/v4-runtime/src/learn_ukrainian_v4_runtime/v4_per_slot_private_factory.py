...
def check_per_slot_gate(root: Path = ROOT) -> dict[str, Any]:
    ...
    a7_completion_ids = (
        ev.completion_slot_ids(
            a7_receipt.get("a7_completions", []), stage="A7", total_slot_ids=total_ids, error_cls=PrivateFactoryError
        )
        if a7_valid
        else set()
    )
...
def validate_gate_matches_receipt(receipt: dict[str, Any], root: Path) -> None:
    gate = check_per_slot_gate(root)
    require(
        receipt["per_slot_gate"] == gate,
        "receipt per_slot_gate does not match the state independently re-derived from the live public artifacts -- refusing (re-verify/regenerate required)",
    )
...
def validate_eligibility_field(receipt: dict[str, Any], root: Path) -> None:
    ...
    require(
        receipt["prerequisite_eligibility"] == expected,
...
def validate_reason_code_totals(receipt: dict[str, Any], root: Path) -> None:
    ...
    require(
        receipt["reason_code_totals"] == expected,
        "reason_code_totals does not reproduce from A7's own public a7_residuals -- refusing",
    )
    require(
        sum(expected.values()) == 100, "reason_code_totals does not sum to the frozen 100-slot denominator -- refusing"
    )
...
def validate_private_rows_constructed(receipt: dict[str, Any], root: Path) -> None:
    ...
    require(
        receipt["execution_counters"]["private_rows_constructed"] == 0,
        "execution_counters.private_rows_constructed is not 0 -- refusing (no independently authored candidate row exists yet)",
    )
    require(
        receipt["execution_counters"]["dataset_rows_emitted"] == 0,
        "execution_counters.dataset_rows_emitted is not 0 -- refusing",
    )
...
def validate_no_forbidden_keys(receipt: dict[str, Any]) -> None:
    ...
    require(
        not leaked_hmac_keys,
        f"receipt carries a forbidden slot->HMAC/source-unit key: {sorted(leaked_hmac_keys)} -- refusing",
    )
...
def validate_eligibility_and_safety_all_false(receipt: dict[str, Any]) -> None:
    require(
        ...
        "receipt eligibility does not equal the frozen all-false eligibility -- refusing",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        ...
        help="Public receipt JSON to verify (default: the tracked V4 per-slot private factory receipt).",
    )
    parser.add_argument(
        "--write-receipt",
        action="store_true",
        help="Assemble and persist a freshly computed public receipt, and refresh the private ledger under batch_state/ (0700/0600, never committed).",
    )
            ...
            canonical_json(
                {
                    ...
                    "per_slot_gate": receipt["per_slot_gate"],
