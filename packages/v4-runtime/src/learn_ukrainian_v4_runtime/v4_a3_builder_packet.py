...
DEFAULT_PACKET_RECEIPT = ROOT / "data/projects/open_model_data/admission/dataset_v4_a3_builder_packet_receipt_v1.json"
...
def builder_eligible_source_unit_ids(seal_receipt: dict[str, Any], builder_eligible_family_ids: list[str]) -> list[str]:
...
def public_commitment_summary(salt: bytes, packet_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "eligible_units_commitment_sha256": eligible_units_commitment_sha256(
            salt, packet_payload["builder_eligible_source_unit_ids"]
        ),
...
def verify_and_load_eligible_units(
    ...
    require(
        len(eligible_unit_ids) == len(set(eligible_unit_ids)),
        "builder-eligible source_unit_ids carries a duplicate -- refusing",
    )
...
def validate_receipt_schema(receipt: dict[str, Any]) -> None:
    errors = sorted(
        Draft202012Validator(_load_packet_receipt_schema()).iter_errors(receipt), key=lambda e: list(e.path)
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--seal-receipt", type=Path, default=DEFAULT_SEAL_RECEIPT, help="sealed A3 receipt JSON (read-only)"
    )
    parser.add_argument(
        "--membership-dir", type=Path, default=DEFAULT_MEMBERSHIP_DIR, help="private membership artifact directory"
    )
    parser.add_argument(
        "--packet-dir", type=Path, default=DEFAULT_MEMBERSHIP_DIR, help="private packet artifact directory"
    )
    parser.add_argument(
        "--packet-receipt",
        type=Path,
        default=DEFAULT_PACKET_RECEIPT,
        help="public packet receipt JSON (read, or written with --write-receipt)",
    )
    parser.add_argument(
        "--issue",
        action="store_true",
    ...
    parser.add_argument(
        "--new-seal-receipt",
        type=Path,
        help="the new sealed A3 receipt JSON to reissue against (required with --reissue).",
    )
    parser.add_argument(
        "--expect-eligible-units-commitment",
        help="assert the recomputed eligible_units_commitment_sha256 equals this value (reissue only).",
    )
