...
if [ -n "${SESSION_EPIC:-}" ] && [ "$_has_prompt" -eq 0 ]; then
  ...
  # Operator 2026-07-27: many paid seats on purpose — Grok is not the hero soloist.
  # Demotion trigger if ignored. Appended to every epic cold-start.
  _fleet_first="FLEET-FIRST / NO SOLO (operator 2026-07-27, demotion trigger): You are ONE seat among many paid models — the operator does NOT trust a single AI and will demote seats that ego-solo. You DRIVE only: prioritize, scripts/delegate.py dispatch --agent <lane> --worktree, settle-loop, formal review-pr (cross-family — never self-seal), merge, dual-write handoff. FORBIDDEN: multi-file implementation yourself, quick fixes in this TUI, dictionary/morphology rabbit holes, heroics, treating other seats as optional. Idle free lane while open work remains = utilization failure. Prefer codex/claude/agy/kimi workers. Other seats exist because one model is not enough."
  case "${SESSION_EPIC}" in
    atlas|practice|practice-hub)
      _cold_prompt="You are the interim ATLAS lane driver (stream ${SESSION_STREAM_ID:-epic:4387}). The launcher has already claimed the stream lease; do NOT open or resume it yourself. Immediately: (1) read/execute .claude/atlas-epic/TAKEOVER-PROMPT.md — tail stream, reconcile board, dual-write INTERIM-DRIVER-HANDOFF.md in every plane mode; (2) mint the session canary: .venv/bin/python -m scripts.session_canary.grok_lane mint --epic atlas --stream ${SESSION_STREAM_ID:-epic:4387} (docs/runbooks/grok-session-canary.md). ${_fc} ${_fleet_first} Drive the next unblocked action without a menu — by DISPATCH, not by coding. End session on canary FAIL-HANDOFF (<8/10), not on compact count; after any auto-compact re-score from memory then AUTO-HYDRATE on PASS — never ask the operator to restart. Primary checkout is read-only — writes via dispatched worker worktrees only."
      ;;
    harness|infra)
      _cold_prompt="You are the INFRA / harness lane driver (stream ${SESSION_STREAM_ID:-epic:4707}). The launcher has already claimed the stream lease; do NOT open or resume it yourself. Cold-start from the infra handoff dual-write (.claude/harness-epic/*-DRIVER-HANDOFF.md) and stream tail; mint canary (.venv/bin/python -m scripts.session_canary.grok_lane mint --epic harness); reconcile in-flight work; drive the next unblocked infra action. ${_fc} ${_fleet_first} End on canary FAIL-HANDOFF not compact count (docs/runbooks/grok-session-canary.md). After auto-compact you own recovery: score canary FROM MEMORY then read the AUTO-HYDRATE capsule score prints on PASS — never ask the operator to restart or re-load the diary. Do not claim curriculum content lanes. Primary checkout is read-only — writes via dispatched worker worktrees only."
      ;;
    hramatka)
      _cold_prompt="You are the hramatka lane driver (stream ${SESSION_STREAM_ID:-epic:4542}). The launcher has already claimed the stream lease; do NOT open or resume it yourself. Cold-start from the epic handoff dual-write and stream if present; mint canary (.venv/bin/python -m scripts.session_canary.grok_lane mint --epic hramatka); reconcile and drive the next unblocked action. ${_fc} ${_fleet_first} End on canary FAIL-HANDOFF (docs/runbooks/grok-session-canary.md). After auto-compact you own recovery: score FROM MEMORY then AUTO-HYDRATE from score PASS output — never ask the operator to restart/hydrate/diary. Primary checkout is read-only — writes via dispatched worker worktrees only."
      ;;
    *)
      _cold_prompt="You are the ${SESSION_EPIC} lane driver (SESSION_STREAM_ID=${SESSION_STREAM_ID:-unset}). You are NOT the main orchestrator. The launcher has already claimed the stream lease; do NOT open or resume it yourself. Cold-start: load the epic handoff dual-write under .claude/${SESSION_EPIC}-epic/ (or stream tail if SESSION_STREAM_ID is set), mint canary via .venv/bin/python -m scripts.session_canary.grok_lane mint --epic ${SESSION_EPIC}, reconcile reality, and drive the next unblocked action without a menu. ${_fc} ${_fleet_first} End on canary FAIL-HANDOFF not compact count (docs/runbooks/grok-session-canary.md). Primary checkout is read-only — writes via dispatched worker worktrees only."
      ;;
  esac
  # Wire the drive-epic playbook into every epic's cold prompt (Sol review #5632 F005).
  _cold_prompt="${_cold_prompt} Your orchestration playbook is agents_extensions/shared/skills/drive-epic/SKILL.md (invoke \$drive-epic if your harness exposes skills) — load it before acting; it defines the topology→route→dispatch→cross-family-review→merge→handoff loop and the escalation triggers. Grok never self-seals formal CF."
  _forward+=("$_cold_prompt")
  unset _cold_prompt _fc _fleet_first
fi
