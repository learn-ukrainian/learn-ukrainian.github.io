#!/usr/bin/env python3
"""Text-free fail-stop controller for the frozen Cycle-006 stage sequence.
...
REQUIRED_CODE_PATHS = {
    ...
    "controller": HERE / "phase3-run-cycle006-controller-v2.py",
    "public_canary": HERE / "phase3-test-cycle006-gemini-public-canary-v2.py",
    "grok_runner": HERE / "phase3-run-cycle006-grok-label-provider-batch-v2.py",
    "compare_runner": HERE / "phase3-compare-cycle006-dual-labels-v2.py",
    "adjudicate_runner": HERE / "phase3-run-cycle006-dual-label-adjudication-v2.py",
    "resolve_runner": HERE / "phase3-apply-cycle006-operator-resolutions-v2.py",
    "certify_runner": HERE / "phase3-verify-cycle006-label-completion-v2.py",
}
STAGE_RUNNER_LABELS = {
    "grok": "grok_runner",
...
def _validate_real_public_canary(receipt_path: Path) -> tuple[str, str]:
    """Accept only a receipt bound to the current AGY executable and documented init/result fields."""
    ...
    if (
        ...
        or not events
        or events[0] is not init_events[0]
        or events[-1] is not result_events[0]
    ):
        raise ControllerError("preflight_binding_drift")
    init, result = init_events[0].get("init"), result_events[0].get("result")
    init_id, result_id = (
        init_events[0].get("conversation_id"),
    ...
    challenge = output.get("liveness_challenge") if isinstance(output, dict) else None
    if not isinstance(init, dict) or not isinstance(result, dict) or not isinstance(challenge, str) or not challenge:
        raise ControllerError("preflight_binding_drift")
    provenance_basis = {
        "agy_executable_sha256": agy_sha256,
        "init_model": GEMINI_MODEL,
        "result_status": "SUCCESS",
        "init_conversation_id": init_id,
        "result_conversation_id": result_id,
        "challenge_sha256": digest(challenge.encode()),
        "raw_stream_sha256": sha256(raw_path),
    }
    if (
        init.get("model") != GEMINI_MODEL
    ...
    expected = {
        ...
        "evaluation_cycle_id": "phase3-v2-1-evaluation-cycle-006",
        "ok": True,
        "execution_mode": "real",
        "public_position_count": 20,
        "provider_calls": 1,
        "exact_model": GEMINI_MODEL,
        "harness": "agy",
...
def preflight(
    ...
    if not package.is_dir() or package.is_symlink() or os.stat(package).st_mode & 0o777 != 0o700:
        raise ControllerError("preflight_binding_drift")
    receipt = _read_json(receipt_path)
    if any(code_paths.get(label) != path.resolve() for label, path in REQUIRED_CODE_PATHS.items()):
        raise ControllerError("preflight_binding_drift")
    ...
    _mode(package / "label-manifest.json", 0o600)
    custody_sha256 = sha256(package / "custody-receipt.json")
    manifest_sha256 = sha256(package / "label-manifest.json")
    hashes = {label: sha256(path) for label, path in code_paths.items()}
    expected = {
        "schema_version": "phase3_cycle006_preflight_receipt_v2",
...
def _stage_stop_paths(package: Path) -> tuple[Path, ...]:
    return (
        package / "label-output-gemini-v2" / "provider-stop.json",
        package / "label-output-grok-cycle006-v2" / "provider-stop.json",
        package / "dual-label-adjudication-cycle006-v2" / "provider-stop.json",
    )
...
def status(package: Path) -> dict[str, Any]:
    ...
    runtime_directories = [
        path.name for path in package.iterdir() if path.is_dir() and path.name.startswith(".cycle006-")
    ]
    return {
        "schema_version": "phase3_cycle006_controller_status_v2",
        "evaluation_cycle_id": "phase3-v2-1-evaluation-cycle-006",
...
def _gemini_runner(
    expected_custody_sha256: str | None = None, expected_label_manifest_sha256: str | None = None
) -> Any:
    path = HERE / "phase3-run-cycle006-gemini-label-provider-batch-v2.py"
    spec = importlib.util.spec_from_file_location("cycle006_controller_gemini", path)
...
def revalidate_full_packets(package: Path, expected_custody_sha256: str, expected_label_manifest_sha256: str) -> None:
    """Revalidate every reassembled Gemini packet before its stage can seal."""
    runner = _gemini_runner(expected_custody_sha256, expected_label_manifest_sha256)
...
def _load_bound_runner(label: str, code_paths: dict[str, Path]) -> Any:
    ...
    if path is None or path.is_symlink() or not path.is_file():
        raise ControllerError("preflight_binding_drift")
    spec = importlib.util.spec_from_file_location(f"cycle006_controller_{label}", path)
    if spec is None or spec.loader is None:
        raise ControllerError("preflight_binding_drift")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
...
def grok_missing_ranges(package: Path, runner: Any) -> dict[str, list[tuple[int, int]]]:
    ...
    for lane, count in LANES.items():
        ...
        for index in range(1, count + 1):
            ...
            if all(present):
                try:
                    packet_path, packet_value = runner.packet(package, lane, index)
                    runner._verify_sealed(package, lane, index, packet_path, packet_value)
                except Exception as exc:
                    raise ControllerError("partial_or_invalid_seal") from exc
            else:
                missing.append(index)
...
def _verify_certification_receipt(package: Path, operator_inspected_count: int) -> None:
    receipt = _read_json(package / "control" / "certification-receipt-v2.json")
    unsigned = {key: value for key, value in receipt.items() if key != "receipt_sha256"}
    if (
        receipt.get("schema_version") != "phase3_cycle006_label_completion_receipt_v2"
...
def _commands_for_stage(
    ...
    *,
    ...
    if stage == "gemini":
        ranges = gemini_missing_ranges(package, expected_custody_sha256 or "", expected_label_manifest_sha256 or "")
        return (
            [
                [
                    str(PRIMARY_PYTHON),
...
def _seal(package: Path, stage: str, preflight_receipt_sha256: str) -> None:
    _atomic(
        ...
        {
            "schema_version": "phase3_cycle006_stage_complete_v2",
            "evaluation_cycle_id": "phase3-v2-1-evaluation-cycle-006",
            "stage": stage,
...
def run_stage(
    ...
    *,
    ...
    if stage == "gemini":
        if not isinstance(expected_agy_executable_sha256, str) or len(expected_agy_executable_sha256) != 64:
            raise ControllerError("preflight_binding_drift")
        if not isinstance(expected_custody_sha256, str) or not isinstance(expected_label_manifest_sha256, str):
            raise ControllerError("preflight_binding_drift")
    else:
        ...
        if stage == "adjudicate" and (
            not isinstance(expected_agy_executable_sha256, str) or len(expected_agy_executable_sha256) != 64
        ):
            raise ControllerError("preflight_binding_drift")
    ...
    if dry_run:
        return {
            ...
            "concurrency": 4 if stage == "grok" else 1,
            "text_free": True,
        }
    for command in commands:
        ...
        if any(path.exists() for path in _stage_stop_paths(package)):
            raise ControllerError("provider_stop_present")
    if stage == "gemini":
        revalidate_full_packets(package, expected_custody_sha256 or "", expected_label_manifest_sha256 or "")
    elif stage == "grok":
