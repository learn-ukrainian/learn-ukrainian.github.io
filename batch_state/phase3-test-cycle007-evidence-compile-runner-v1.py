...
def _arguments(
    ...
    *,
    endpoint: str = "http://127.0.0.1:18476/mcp",
) -> list[str]:
    return [
        "--package",
        str(package),
...
def _package(tmp_path: Path) -> tuple[Path, Path]:
    ...
    source_root = tmp_path / "frozen-cycle005-source"
    source_root.mkdir(mode=0o700)
    os.chmod(source_root, 0o700)
    source_manifest = source_root / "label-manifest.json"
    _private_file(source_manifest)
    return package, source_manifest
...
def test_rejects_path_and_mode_drift_without_echoing_private_paths(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    package, source_manifest = _package(tmp_path)
    package.chmod(0o755)
    sentinel = "PRIVATE-PATH-SENTINEL"
    code = RUNNER.main([*_arguments(package, source_manifest), "--ignored", sentinel])
    payload = _result(capsys)
    assert code == 1
    assert payload == {"failure_code": "argument_invalid", "ok": False, "text_free": True}
    assert sentinel not in json.dumps(payload)

    code = RUNNER.main(_arguments(package, source_manifest))
    payload = _result(capsys)
    assert code == 1
...
def test_rejects_nonexact_and_occupied_endpoints_before_starting_server(
    ...
    package, source_manifest = _package(tmp_path)
    code = RUNNER.main(_arguments(package, source_manifest, endpoint="http://localhost:18476/mcp"))
    assert code == 1
...
def test_managed_server_cleanup_and_text_free_stdout_on_success_and_compile_failure(
    ...
    assert RUNNER.main(_arguments(package, source_manifest)) == 0
    payload = _result(capsys)
    ...
    assert RUNNER.main(_arguments(package, source_manifest)) == 1
    payload = _result(capsys)
...
def test_requires_frozen_external_source_manifest_and_classifies_contract_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    package, source_manifest = _package(tmp_path)
    code = RUNNER.main(_arguments(package, package / "manifest.json"))
    assert code == 1
    assert _result(capsys) == {"failure_code": "source_manifest_path_invalid", "ok": False, "text_free": True}

    source_manifest.parent.chmod(0o755)
    code = RUNNER.main(_arguments(package, source_manifest))
    assert code == 1
    assert _result(capsys) == {"failure_code": "source_manifest_parent_invalid", "ok": False, "text_free": True}
    source_manifest.parent.chmod(0o700)

    process = _Process()
    monkeypatch.setattr(RUNNER, "_start_reviewed_sources_server", lambda _endpoint: process)
    monkeypatch.setattr(RUNNER.compiler, "LocalMcpSourcesClient", lambda **_kwargs: _Client())

    sentinel = "PRIVATE-CONTRACT-SENTINEL"

    def fail_contract(*_args: object, **_kwargs: object) -> dict[str, Any]:
        raise RUNNER.evidence_contract.EvidenceContractError("source_binding_drift") from RuntimeError(sentinel)

    monkeypatch.setattr(RUNNER.compiler, "compile_cycle007_package", fail_contract)
    assert RUNNER.main(_arguments(package, source_manifest)) == 1
    payload = _result(capsys)
    assert payload == {"failure_code": "compile_source_binding_drift", "ok": False, "text_free": True}
    assert sentinel not in json.dumps(payload)
