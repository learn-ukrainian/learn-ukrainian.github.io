...
def label_prompt_hash(package: Path, lane: str) -> str:
    """The independently supplied reviewed labeling-prompt hash for a lane."""
...
def test_batch_requires_label_prompt_hash_before_provider(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RUN.Error, match="label_count_or_envelope_drift"):
        RUN.batch(pkg, "clean_label", 1, 1, fake_bin, synthetic_provider=True)

    assert not state_file.exists()


def test_gemini_public_canary_prompt_hash_is_not_a_labeling_prompt_binding(
    ...
    monkeypatch.setenv("FAKE_STATE", str(state_file))
    monkeypatch.setenv("FAKE_MODE", "valid")
    gemini_canary_hash = CANARY.static_verify("gemini")["prompt_sha256"]

    assert gemini_canary_hash != label_prompt_hash(pkg, "clean_label")
    with pytest.raises(RUN.Error, match="label_count_or_envelope_drift"):
        RUN.run_packet(
            pkg,
            "clean_label",
            1,
            fake_bin,
            expected_label_prompt_sha=gemini_canary_hash,
            synthetic_provider=True,
        )
...
def main() -> int:
    ...
    try:
        ...
        test_gemini_public_canary_prompt_hash_is_not_a_labeling_prompt_binding(tmp / "t4", mp)
        test_valid_residual_packet(tmp / "t5", mp)
        test_resume_sealed_packet(tmp / "t6", mp)
        test_retryable_structural_failure_recovers(tmp / "t7", mp)
        test_two_structural_failures_writes_stop(tmp / "t8", mp)
        test_semantic_failure_immediately_stops(tmp / "t9", mp)
        test_stop_idempotence(tmp / "t10", mp)
        test_concurrency_must_be_one(tmp / "t11")
        test_partial_seal_rejected(tmp / "t12", mp)
        test_no_private_disclosure_in_receipts(tmp / "t13", mp)
        test_wrong_evidence_manifest_hash_rejected(tmp / "t14", mp)
        test_missing_evidence_manifest_hash_rejected(tmp / "t15", mp)
        test_sidecar_path_drift_rejected(tmp / "t16", mp)
        test_duplicate_or_foreign_sidecar_manifest_entry_rejected(tmp / "t17", mp)
        test_prompt_tamper_rejected(tmp / "t18", mp)
        print(
            json.dumps(
