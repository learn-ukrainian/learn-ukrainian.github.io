...
The `MAX_SCAN_BYTES` cap bounds **total physical bytes read**, not just bytes that make it into a parsed event: bytes discarded while draining an oversized (no-newline) line are charged against the same shared budget as ordinary lines, so a pathological multi-GB no-newline file cannot be read start-to-finish while `bytes_scanned` stays near zero — the scan stops mid-drain, safely, the moment the budget is exhausted. Separately, the oversized check itself compares physical line *content* length against `MAX_LINE_CHARS`, not `readline()`'s raw return length: a line whose content is exactly `MAX_LINE_CHARS` chars plus its own trailing newline fits in one read and is never treated as needing a drain (PR #4998 final review).
