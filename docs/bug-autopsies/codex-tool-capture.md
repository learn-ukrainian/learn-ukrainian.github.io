...
**Category:** codex-tool-capture · **Severity:** build-breaking (HARD-fail) / QG silent-zero · **Recurrences:** 7
...
6. **(this, 2026-07-08) — QG grounding exact-query match mismatch.** The grounding gate's query matching (`_event_input_matches_query`) required exact string equality between the model's cited query and the telemetry event's query. But models decorate cited queries with parameters/context (e.g. `"Сковорода Григорій Савич mode=section 3 (Придворна капела)"` vs event query `"Сковорода Григорій Савич"`). This exact match check silently rejected legitimate confirms (neutralizing correct confirmations, reducing Claude live confirms from 880 to 410). Fixed by changing matching to containment (`event_query in cited_query`) in one direction with a min-length guard (>= 3 chars) to prevent trivially short query wildcards.
7. **(this, 2026-08-15) — QG grounding excerpt ellipsis/abridgment brittleness.** The 4th #4797 axis: models abridge quoted evidence with ellipsis spellings and formatting the literal matcher rejected — live-observed «Весня́нки**...** назва…» (markdown-bold ellipsis; emphasis glued to split segments, 4/7 legit groundings failed in the «Веснянки» run-2 proof), spaced dot runs `. . .`, loosely-bracketed `[ ... ]`, and light truncation where the model closes a quoted clause with `.` the source lacks at that position («пісень.» vs «пісень, що») or wraps the quote in «». Fixed by treating markdown emphasis (`*`/`` ` ``) and segment-edge quote/sentence punctuation as formatting variance (symmetric — never fabricates tokens) and widening the ellipsis regex; the anti-fabrication guards (every retained fragment verbatim, in order, same captured output, ≥4-char segments, ≥12 nonspace evidence mass) are unchanged, so number-swap/splice fabrications still reject. Deliberately NOT relaxed: an unmarked sentence restart («пісень. Вони» eliding «, що … .») still fails closed — semantic re-adjunction is the v2 entailment gate's job.
...
- Fix (exact-query mismatch): `fix(qg): match grounding query by containment, not exact equality` — `_event_input_matches_query` checks normalized candidate containment in cited query with a 3-character min-length guard; regression test in `test_llm_reviewer_dispatch.py`.
- Fix (excerpt ellipsis/abridgment): `fix(qg): accept ellipsis-abridged excerpts in grounding matcher` — `_output_contains_excerpt` now treats markdown emphasis and segment-edge quote/sentence punctuation as formatting variance and recognizes spaced/bracketed ellipsis spellings; anti-fabrication guards (verbatim fragments, in-order, same output) unchanged; regression tests in `test_llm_reviewer_dispatch.py`.
