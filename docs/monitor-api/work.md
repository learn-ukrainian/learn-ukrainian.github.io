...
Delegate class-4 inventory is filtered by the authoritative task-state fields `repository` / `repository_id` (both must agree when present) **before** sort/limit/total so private volume cannot inflate public counts or starve public rows. Generic `/api/delegate/tasks` and `/api/delegate/active` summaries never re-emit those attribution fields (legacy public shape only). Work's production collectors pass the already-admitted public singleton into the internal Python loaders, then stamp Work's own admitted repository metadata under the trusted scoped-loader contract; present foreign claims are still dropped defense-in-depth. Mixed/injected section payloads without that scoped contract still require an exact public claim at normalize. The public HTTP `/api/delegate/*` routes remain unscoped for other Monitor consumers and do not expose a free-form repository selector. Bodies and result files are never read.
