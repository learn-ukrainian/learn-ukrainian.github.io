# Curriculum Tracks: Master Scorecard

**Generated:** 2026-01-31
**Total Tracks:** 6 specialized tracks

---

## Track-Specific Scoring Systems

Each track type uses different weights based on its pedagogical focus:

### Core Level Weights (A1-C2)
| Criterion | Weight |
|-----------|--------|
| Grammar/Content coverage | 15% |
| Vocabulary coverage | 10% |
| Skills balance | 15% |
| State Standard compliance | 15% |
| CEFR alignment | 10% |
| Internal consistency | 10% |
| Checkpoint structure | 10% |
| Audit pass rate | 15% |

### LIT Track Weights
| Criterion | Weight |
|-----------|--------|
| Literary depth/analysis | 20% |
| Authentic text engagement | 15% |
| Archaic/literary vocabulary | 15% |
| Cultural significance | 10% |
| CEFR alignment | 10% |
| Activity coverage | 10% |
| Audit pass rate | 15% |
| Internal consistency | 5% |

### HIST Track Weights
| Criterion | Weight |
|-----------|--------|
| Historical accuracy | 15% |
| Primary source integration | 15% |
| Decolonization perspective | 10% |
| Era-appropriate vocabulary | 10% |
| Chronological coherence | 10% |
| Critical analysis skills | 10% |
| Activity coverage | 10% |
| Audit pass rate | 15% |
| Internal consistency | 5% |

### BIO Track Weights
| Criterion | Weight |
|-----------|--------|
| Biographical accuracy | 15% |
| Cultural/historical context | 15% |
| Era-appropriate language | 10% |
| Significance assessment | 10% |
| Source reliability | 10% |
| Activity coverage | 10% |
| Vocabulary coverage | 10% |
| Audit pass rate | 15% |
| Internal consistency | 5% |

### PRO Track Weights
| Criterion | Weight |
|-----------|--------|
| Practical application | 20% |
| Domain vocabulary | 15% |
| Professional register | 15% |
| Real-world scenarios | 10% |
| Skills balance | 10% |
| Activity coverage | 10% |
| Audit pass rate | 15% |
| Internal consistency | 5% |

---

## Track Scores Summary

| Track | Type | Modules | Score | Pass Rate | Status |
|-------|------|---------|-------|-----------|--------|
| **HIST** | History | 140 | 7.2/10 | 77% (109/140) | ⚠️ Good |
| **LIT** | Literature | 30 | 3.2/10 | 0% (0/30) | 🔴 Vocab block |
| **C1-BIO** | Biography | 128 | 2.8/10 | 3% (4/128) | 🔴 Many gaps |
| **B2-PRO** | Professional | 40 | 1.0/10 | 0% (0/40) | 🔴 Greenfield |
| **C1-PRO** | Professional | 50 | 1.0/10 | 0% (0/50) | 🔴 Greenfield |
| **C1-HIST** | Historiography | 135 | 0.9/10 | 1% (1/135) | 🔴 Greenfield |

---

## Implementation Priority (ROI-Based)

Priority ranking based on effort-to-impact ratio:

| Priority | Track | Score | Effort | ROI Rationale |
|----------|-------|-------|--------|---------------|
| **1** | LIT | 3.2→10 | 40h | Content exists, vocab only |
| **2** | HIST | 7.2→10 | 77h | 77% done, finish strong |
| **3** | C1-BIO | 2.8→10 | 319h | 87% content, needs vocab |
| **4** | B2-PRO | 1.0→10 | 160h | Practical, B2 audience |
| **5** | C1-PRO | 1.0→10 | 200h | Advanced professional |
| **6** | C1-HIST | 0.9→10 | 540h | Largest greenfield |

---

## Current State by Track

### HIST (Most Complete)
```
✅ 109 modules passing (77%)
❌ 23 modules failing
📝 8 stub modules (<100 words)
```
**Primary issue:** Stub content for recent history (M118-140)

### LIT (Content Complete)
```
✅ 0 modules passing (0%)
❌ 30 modules failing
📝 0 stubs
```
**Primary issue:** All 30 missing vocabulary sidecars

### C1-BIO (Partially Complete)
```
✅ 4 modules passing (3%)
❌ 107 modules failing
⚠️ 17 modules missing files
```
**Primary issues:** 17 missing files, 111 missing vocabulary

### C1-HIST (Greenfield)
```
✅ 1 module passing (1%)
⚠️ 134 modules missing files
```
**Primary issue:** Only M01 exists

### B2-PRO (Plans Only)
```
✅ 0 modules (0%)
⚠️ 40 modules need creation
```
**Primary issue:** No content, activities, or vocabulary exist

### C1-PRO (Plans Only)
```
✅ 0 modules (0%)
⚠️ 50 modules need creation
```
**Primary issue:** No content, activities, or vocabulary exist

---

## Effort Estimates (Hours)

| Track | Content | Vocab | Activities | Other | Total |
|-------|---------|-------|------------|-------|-------|
| LIT | 10 | 20 | 3 | 7 | **40** |
| HIST | 56 | 8 | 8 | 5 | **77** |
| C1-BIO | 228 | 37 | 17 | 37 | **319** |
| B2-PRO | 120 | 20 | 15 | 5 | **160** |
| C1-PRO | 160 | 25 | 10 | 5 | **200** |
| C1-HIST | 400 | 60 | 60 | 20 | **540** |
| **Total** | **974** | **170** | **113** | **79** | **1,336** |

---

## Quarterly Roadmap (40h/week)

### Q1 2026: Quick Wins
| Month | Focus | Hours |
|-------|-------|-------|
| Feb | LIT vocabulary | 40h |
| Mar | HIST stubs | 77h |
| Apr | HIST completion | — |

**Q1 Result:** LIT 10/10, HIST 10/10

### Q2 2026: C1-BIO Push
| Month | Focus | Hours |
|-------|-------|-------|
| May | C1-BIO missing files | 68h |
| Jun | C1-BIO vocabulary | 100h |
| Jul | C1-BIO expansion | 100h |

**Q2 Result:** C1-BIO ~7/10

### Q3 2026: C1-BIO Completion + B2-PRO Start
| Month | Focus | Hours |
|-------|-------|-------|
| Aug | C1-BIO completion | 51h |
| Sep | B2-PRO Phase 1 | 60h |
| Oct | B2-PRO Phase 2 | 72h |

**Q3 Result:** C1-BIO 10/10, B2-PRO 4/10

### Q4 2026: B2-PRO + C1-PRO Start
| Month | Focus | Hours |
|-------|-------|-------|
| Nov | B2-PRO completion | 28h |
| Dec | C1-PRO Phase 1 | 60h |

**Q4 Result:** B2-PRO 10/10, C1-PRO 3/10

### 2027: C1-PRO + C1-HIST
- Q1: C1-PRO completion (140h)
- Q2-Q4: C1-HIST (540h)

---

## Track Improvement Plans

| Track | Improvement Plan |
|-------|------------------|
| LIT | `docs/l2-uk-en/LIT-10-10-IMPROVEMENT-PLAN.md` |
| HIST | `docs/l2-uk-en/HIST-10-10-IMPROVEMENT-PLAN.md` |
| C1-BIO | `docs/l2-uk-en/C1-BIO-10-10-IMPROVEMENT-PLAN.md` |
| C1-HIST | `docs/l2-uk-en/C1-HIST-10-10-IMPROVEMENT-PLAN.md` |
| B2-PRO, C1-PRO | `docs/l2-uk-en/PRO-TRACKS-10-10-IMPROVEMENT-PLAN.md` |

---

## Combined with Core Levels

### Full Curriculum Priority Order

| Priority | Level/Track | Score | Effort | Type |
|----------|-------------|-------|--------|------|
| 1 | A1 | 9.7/10 | 5h | Core |
| 2 | A2 | 9.6/10 | 11h | Core |
| 3 | LIT | 3.2/10 | 40h | Track |
| 4 | B1 | 7.0/10 | 21h | Core |
| 5 | HIST | 7.2/10 | 77h | Track |
| 6 | B2 | 4.9/10 | 79h | Core |
| 7 | C1-BIO | 2.8/10 | 319h | Track |
| 8 | C1 | 4.7/10 | 99h | Core |
| 9 | B2-PRO | 1.0/10 | 160h | Track |
| 10 | C1-PRO | 1.0/10 | 200h | Track |
| 11 | C2 | 4.9/10 | 420h | Core |
| 12 | C1-HIST | 0.9/10 | 540h | Track |

### Total Curriculum Effort

| Category | Effort |
|----------|--------|
| Core Levels (A1-C2) | 635 hours |
| Specialized Tracks | 1,336 hours |
| **Total** | **1,971 hours** |

---

## References

- Core level plans: `docs/l2-uk-en/{LEVEL}-10-10-IMPROVEMENT-PLAN.md`
- Track plans: Listed above
- Status files: `docs/{TRACK}-STATUS.md`
