...
2. **Regenerate the site MDX** — folk uses `assemble_mdx` (NOT `generate_mdx.py`, which is core-only).
   ...
   forward-drift gate).
3. **Independent content review** — **LANGUAGE-LANES only** (agy / codex / claude / grok-4.6),
   VESUM-backed via `sources` MCP, off-seat. Example:
   `delegate.py dispatch --agent agy --mode read-only` (or codex/claude/grok-4.6).
...
- [ ] Independent language-lane content review clean (agy/codex/claude/grok-4.6; or deltas applied)
