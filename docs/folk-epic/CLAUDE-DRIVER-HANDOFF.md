...
**This session — Phase 1c core: clean-text + free-full-text-link resolver (PR #3731).** New `scripts/readings/clean_text_resolver.py` (standalone composable, like 0/1a/1b — engine wiring deferred to Phase 2). Read-only over `literary_texts` (137,723 rows; 134 authors / 3,310 works). For a demanded (work, author): assembles clean text (multi-chunk, ordered), classifies rights via `classify_rights`, emits PD+in_corpus → `hostable_full=True` + clean_text_source ref; in-copyright OR not-in-corpus → `hostable_full=False` + `free_full_text_link` candidate (source_url → ukrlib → wikisource). **Link invariant: every emitted link is `verified=False`** — this module NEVER verifies; a browser-verify step (my lane, no-unchecked-links order) must confirm before any link renders. REAL-DATA verified (Claude independent): Заповіт/Шевченко PD-hostable; Кавказ 3-chunk assembly 4653 chars; Стус in-copyright→ukrlib candidate verified=false; non-corpus→in_corpus false. 7 tests + ruff green (re-run by me). Manifest: 50/1882 in-corpus, 25 hostable_full, 1857 need-link.

**FOLLOW-UP #1 (Phase 0 concern):** demand manifest carries author-spelling-variant duplicates (Лісова пісня ×3, Тіні забутих предків ×4). Consider author-canonicalization in `primary_text_demand.py`.
**✅ SHIPPED — rights in-copyright SAFETY fix (PR #3733).** Diagnosed via 1c real-data: 105/134 corpus authors missed `authors_rights.yaml` and **53 were wrongly classified PD** by a `year<=1928→PD` fallback that trusts `literary_texts.year` (often the author's BIRTH year). Executed-Renaissance authors (Драй-Хмара d.1939, Маланюк d.1968, Малишко d.1970, Остап Вишня d.1956) all resolved PD — legal-exposure bug. Fix in `rights_classifier.py` (logic only): (a) robust author-key resolution via a canonical-key index (surname+first-initial) so "Шевченко Т." / "Шевченко Т.Г." / "Тарас Шевченко" all hit the table, WITH an over-merge guard (ambiguous canonical keys popped + WARNING — no silent merge of distinct authors); (b) REMOVED the poisoned `year<=1928→PD` fallback → unknown author now defaults conservatively to `in_copyright`. VERIFIED (Claude independent, current_year=2026): risk 53→**0**; Драй-Хмара/Маланюк/Малишко→in_copyright; all Shevchenko formats + Франко→public_domain via table; folk + pre-modern paths preserved. 27 tests + ruff green (re-run by me). NO live regression (only folk hosted today; folk PD via its own path).

**FOLLOW-UP #2-RESIDUAL (PR B — PRECISION, Phase-2 PREREQUISITE, NOT yet done):** the conservative fallback now makes not-yet-tabled PD classics (Панас Мирний d.1920, Карпенко-Карий, Вовчок, Костомаров…) resolve `in_copyright` (excerpt-only) instead of hostable-full. To restore MAX-access for genuine PD before Phase 2 hosts lit, EXPAND `authors_rights.yaml` with SOURCED death/rehab years for the ~100 still-missing corpus authors (Ukrainian Wikipedia; #M-4 — omit, don't guess; rehabilitation+70 for Executed Renaissance). Research+data task; verify a sample independently.

**IN-FLIGHT:** none after PR #3733 merges.

**NEXT ACTION:** (1) PR B — sourced `authors_rights.yaml` expansion for the ~100 missing corpus authors (Phase-2 prereq so PD classics host full, not excerpt). (2) Phase 2 — generalize `generate_readings.py` (folk-only, 819 LOC) + rendering across tracks, wiring demand+rights+curation+clean-text resolvers; host first real PD lit batch. (3) Browser-verify candidate free-full-text links before any render (no-unchecked-links). Folk 4/6 zero-reading modules (kalendarna, narodni-viruvannia, narodna-kultura, zamovliannia) get readings via this demand engine. FOLLOW-UP #1 (demand-manifest author-dup canonicalization) still open.
...
- **✅ DEFAULT-LANGUAGE DECIDED (user, 2026-06-21):** chrome **follows the user's own locale** (client-side `navigator.language`, primary subtag `uk`→Ukrainian chrome, else **English fallback**); explicit toggle (localStorage) overrides; no-JS = English. Rationale: this is the *English-speakers* track — Ukrainian-default chrome would strand A1 beginners who can't read the nav. **#M-13 governs learning CONTENT, not nav chrome** (content stays 100% Ukrainian for all). My first-draft "Ukrainian-default" was wrong (conflated chrome with content); user caught it; Codex confirmed the correction (msg #1338). ADR updated.
- **NEXT ACTION:** no user gate remains — hand the ADR spec to **infra Claude** for the sequenced build (centralize strings behind `t()` + lint → expose toggle once chrome strings stable). Other open driver work unchanged (PRs #3657 template-rewrite, #3653 agent-def→user; parked branches `folk-wikis-wave8`, `folk-retest2`).
...
### 🎯 NEXT ACTION = module corpus-hammer (the ONLY substantive step before these 6 count as truly shipped). Folk-module "certification" is **INFORMAL**: no `folk/status/*.json` flip (that's core-track only) → certified = `verify_shippable` GREEN **+** corpus-hammer (#M-11: read content + independently `verify_quote` every embedded fragment) **+** recorded here. Run from the **DATA-BEARING MAIN TREE** (worktrees sparse-exclude `data/` → vesum/quote tools fail there): `verify_shippable folk <slug> --module-dir <wt>/curriculum/l2-uk-en/folk/<slug>`. Lane = **DeepSeek-pro** content review (#M0; sources MCP verify_quote×N + verify_words + russian_shadow) OR a fresh focused session — do NOT cram 6×~59KB reads into a tail-of-session. **My lane confirmed** (S60 role grant: I own folk+seminar MODULES; the `phase-folk-queue.md` "Claude: no modules" Stage-0 boundary is SUPERSEDED).

### ⛔ USER-GATED — un-hide folk nav (`site/src/components/LevelLanding.tsx:45` + `site/astro.config.mjs:17`) needs explicit user GO: reverses orchestrator #3027 + user "deliver e2e first" gate. Do NOT touch without it.

### ⚠️ INFRA carried (#0.2): `render_canonical_formulas_block` unwired in wiki prompt; wiki `index.md` batch-regen owed (+ drop phantom `folk/genres/charivni-kazky.md`); `--writer` help vs "Gemini-family always" policy (gpt-5.5 now a proven folk wiki writer → update both). **REMOVED (was a phantom blocker):** `search_images` "Phase-3 gate" — it is an optional 1-of-3 multimedia tools (`query_wikipedia`/`search_external` satisfy `resources_search_attempted`); Qdrant:6334 just down locally, modules carry no required image slots.

### 📋 REMAINING WIKIS (20, all dossier-backed): dytiachyi-folklor-kolyskovi, istorychni-perekazy, narodna-kukhnia-obriadova-yizha, narodna-kultura-ta-vysoka-kultura-mistky, narodna-vyshyvka-rushnyk-strii, narodne-zhytlo-sadyba-hospodarstvo, narodni-anekdoty, narodni-lehendy, narodni-muzychni-instrumenty, narodni-opovidannia-buvalshchyny-memoraty, narodni-remesla-ta-khudozhni-promysly, narodni-tantsi, prykazky-ta-pryslivia, pysankarstvo, rehionalni-etnokulturni-tradytsii, sotsialno-pobutovi-kazky, striletski-povstanski-pisni, suspilno-pobutovi-pisni, vertep-narodna-drama, zahadky. Recipe = S61/S56 (gpt-5.5 default, claude fallback <8; compile MAIN tree → corpus-hammer → copy to `claude/folk-wikis-waveN` → batch 3-4/PR → no index.md).

### IN-FLIGHT: 0. Merge grant LIVE; worktree-only; never main.

## ▶▶▶ SESSION 61 HANDOFF (2026-06-18 — ✅ WIKI WAVE 7: 4 wikis (gpt-5.5), ALL corpus-hammered + pisni re-grounded at root → 22/42. 🔍 MODULE COUNT CORRECTION: 5 machine-pass DoD, not 1. 🧹 search_images "blocker" = phantom.)
