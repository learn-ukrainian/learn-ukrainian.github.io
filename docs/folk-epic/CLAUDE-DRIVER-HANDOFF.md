...
**FOLLOW-UP #2 (Phase 1a rights-DATA gap — surfaced by 1c real-data, IN-COPYRIGHT SAFETY):** corpus stores authors as "Surname І." but `authors_rights.yaml` keys are inconsistent — Shevchenko has "Шевченко Т.Г." NOT "Шевченко Т.", so "Шевченко Т." MISSES the author table and falls to the weaker `year<=1928` fallback (harmless for PD Shevchenko, but an author stored "Surname І." missing from table + corpus year ≤1928 could MISCLASSIFY in-copyright as PD). ROOT-CAUSE fix = canonical author-key normalization in `rights_classifier.py` (surname+first-initial canonical form), not per-alias whack-a-mole. **This is my NEXT infra task** (own folk-adjacent infra, #0.2).

**IN-FLIGHT:** none after PR #3731 merges.

**NEXT ACTION:** (1) Fix FOLLOW-UP #2 (rights author-key normalization — in-copyright safety) via focused dispatch + tests + PR. (2) Then Phase 2 — generalize `generate_readings.py` (currently folk-only, 819 LOC) + rendering across tracks, wiring demand+rights+curation+clean-text resolvers; host first real PD lit batch. (3) Browser-verify candidate free-full-text links before any render (no-unchecked-links). Folk 4/6 zero-reading modules (kalendarna, narodni-viruvannia, narodna-kultura, zamovliannia) get readings via this demand engine.
