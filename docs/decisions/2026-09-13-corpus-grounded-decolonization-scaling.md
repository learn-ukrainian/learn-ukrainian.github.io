...
### Pillar 7: Absolute Anti-Surzhyk Invariant & 5-Tier AI Lab Stack Expansion

- **Anti-Surzhyk Invariant:** Surzhyk is not an authentic dialect or acceptable informal register. It is toxic colonial contamination and imperial Russian linguistic poisoning designed to ruin Ukrainian language and culture. Zero promotion, generation, or validation is tolerated in datasets or model completions. Any model output normalizing Surzhyk is a critical quality failure. Surzhyk inputs are strictly diagnosed and eradicated into pristine modern standard Ukrainian.
- **Dialect Protection:** Authentic Ukrainian regional dialects (Hutsul, Boyko, Lemko, Polissian, Slobozhan) are protected cultural heritage and must never be conflated with or misclassified as Surzhyk.
- **5-Tier AI Lab Suite:** Expansion to supply frontier AI labs with a full lifecycle reinforcement stack: Tier 1 (Clean CPT pretraining stream from 150K+ textbook/literary chunks in `sources.db`), Tier 2 (General GEC, syntax, and valency via full UA-GEC, ZMO, and ULP), Tier 3 (ULDR Decolonization & Reasoning), Tier 4 (Dialects & Historical Monuments), and Tier 5 (Turnkey Leaderboard Benchmark).
