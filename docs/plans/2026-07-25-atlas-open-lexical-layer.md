...
Local dual-write (gitignored) retains full broker dumps under `.claude/atlas-epic/plans/curated-seed/`.
...
| **DC-D** | Curated private teacher-lesson / teacher seed privacy | **Private/local by default** until operator grants publish consent; Example deck may be local-first |
| **DC-E** | Curated private teacher-lesson input SSOT | **Document curated table (1018)** is Practice seed vocabulary; raw two-table extract + problem log remain audit trail; sentence file is **evidence**, not silent product truth |
...
| **1** | Curated private teacher-lesson reconcile + rights audit of sentence candidates | 1018 conservation; redistributable flags; quarantine weak/no_hit |
...
## 5. Curated private teacher-lesson / Practice vertical slice (first human+machine test)

**Input evidence (local):**

- Curated: `.claude/atlas-epic/plans/curated-seed/v2-curated.jsonl` (1018)
...
Tracked in #5790 (Curated private teacher-lesson seed reconcile + sentence rights audit).
...
| **#5790** | Curated private teacher-lesson seed + sentence rights audit |
...
3. Curated private teacher-lesson rights audit issue.
