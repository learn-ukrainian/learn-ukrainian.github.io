...
- **Ukrainian first, gloss after.** A Ukrainian term is met in Ukrainian before its English gloss, never "the word for X is Y". English narration embeds Ukrainian as quoted-term spans `{{uk:…}}` — Ukrainian first, gloss after (ULP practice 1); the span is counted as Ukrainian and resolved by the engine.
...
- An `explanation` for every item — required by `schemas/activities-<level>.schema.json` (E1.2). A1 register: scaffolded patterns with English support where the immersion rule allows; no grammatical terminology in an activity explanation (R-19 decision card, point 3).
...
Narration is in the language the immersion payload gives for the position and the field role. English narration embeds Ukrainian as quoted-term spans `{{uk:…}}` — Ukrainian first, gloss after (ULP practice 1); the span is counted as Ukrainian and resolved by the engine.
