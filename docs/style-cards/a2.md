...
- **Ukrainian first, gloss after.** A Ukrainian term is met in Ukrainian before its English gloss, never "the word for X is Y". Where English narration still occurs it embeds Ukrainian only as quoted-term spans `{{uk:…}}` — Ukrainian first, gloss after; a Ukrainian sentence never embeds English except a gloss reference.
- **Passages.** A `bilingual` block only where the payload allows English for the narration role; by the ramp's end it is gone.
- **Dialogue.** The dialogue is Ukrainian only, presented as the artifact the learner meets; its breakdown comes after it in the following blocks. `dialogue.translation_en` only where the payload allows English for the dialogue role, one entry per line, after the dialogue; otherwise absent as a whole.
- **Comprehension in Ukrainian.** Activity instructions are Ukrainian unless the payload says otherwise for the instruction role; comprehension questions and options are Ukrainian.
...
- **Band posture.** From A2 the English share is never raised (operator contract). R-30: 75–100 % Ukrainian at the bridge, 85–100 % through the ramp and the first 20 modules, 90–100 % to module 50, 95–100 % to the end, with a little English support. Narration is Ukrainian-primary; where the payload allows English it comes as a repeat block after the Ukrainian, never interleaved sentence by sentence (ULP S2→S3). Section labels are Ukrainian. The payload binds; these numbers orient.
...
- An `explanation` for every item — required by `schemas/activities-<level>.schema.json` (E1.2). A2 register: scaffolded patterns, English support only where the immersion rule allows; no grammatical terminology in an activity explanation (R-19 decision card, point 3).
...
- Where a term is needed it is the Ukrainian term (звук / літера, відмінок, наголос — contract §5), introduced as a quoted term with its gloss where the payload still allows English.
...
Narration is in the language the immersion payload gives for the position and the field role. Where English narration still occurs it embeds Ukrainian only as quoted-term spans `{{uk:…}}` — Ukrainian first, gloss after; a Ukrainian sentence never embeds English except a gloss reference.
...
touches (checked against `docs/epics/fresh-build-a2-arc.md`); a gate fails a draft that reuses an exemplar's sentences. Whitespace is
