...
> **Status:** v1.8 — updated 2026-09-22 (Cursor first pick: prefer `--agent cursor --model grok-4.7-high` for mechanical + ordinary infra/code implement per operator 2026-09-22, #8493/#8498 merged; DeepSeek Flash everyday / Pro @ high = hard implement only per operator GO 2026-08-13; #5737 agy/gemini-flash worker-brief boundary). K3 is an active consequential-work and cross-family review candidate, while remaining excluded from the Ukrainian factual/folk gate. The per-task bakeoff SCORE cells (§3–§7) are historical evidence; the current machine-enforced quality floors and ordered peer tiers live in `scripts/config/model_catalog.yaml`. v1.4 added the native Kimi Code OAuth lane. See §10 Provenance for version history.
...
| **Codex** ⭐ | GPT-6 Sol (orchestrator high, reviewer high, advisor high), Astra (workhorse low), Luna (scout max); Terra fallback | OpenAI via Codex CLI | $1000/wk bucket (metered) | **TOP-PRIORITY lane.** Novel impl, cross-file patterns, hard debug, primary V7 reviewer + novel-architecture code review. Cap 2 in-flight. |
...
| **cursor** | **`grok-4.7-high`** (first pick worker; operator 2026-09-22; #8493/#8498) · Auto · Composer 2.5 (pinned when family independence matters) | `scripts/delegate.py dispatch --agent cursor --model grok-4.7-high` / `ask-cursor` | Metered | **First-class code/infra worker.** **First pick** (`--model grok-4.7-high`) for mechanical + ordinary infra/code implement when fit allows; never `auto` as formal CF identity. `concurrency_limit: 1`. Not a language seat. |
...
- **Cursor first-pick (operator 2026-09-22; #8493/#8498 merged):** Prefer **`--agent cursor --model grok-4.7-high`** as the **first pick** for mechanical **and** ordinary infra/code implement when fit allows (not LANGUAGE-LANES, not advisor/authority). Spread remains — Cursor is not the only seat — but leaving Cursor idle while burning Codex/Kimi/DeepSeek on those jobs is waste. `cursor:auto` still never CF-of-record. DeepSeek Flash everyday; **Pro @ high = hard implement only**.
...
- **When a lane frees up, pull the next item that FITS it — don't idle, don't make-work:**
  ...
  - **DeepSeek Flash (default) / Pro (hard implement only):** code/infra PR diffs and CF volume — `deepseek-v4-flash`; Pro @ high only for hard implement tasks (complex multi-file, hard lookup — 2026-08-13). Not language/VESUM seats (LANGUAGE-LANES). Don't review inline on the author seat.
  - **Cursor (`--model grok-4.7-high` first pick):** prefer for mechanical + ordinary infra/code implement when fit allows — do not leave Cursor idle while Codex/Kimi/DeepSeek burn those jobs; still not language/advisor/CF identity.
...
| Adversarial review (post-June-15) | `.venv/bin/python scripts/delegate.py dispatch --agent codex --model gpt-6-sol --effort high --mode read-only ...` per substitutions YAML |
...
- v1.8: 2026-09-22 (Cursor grok-4.7-high first pick + DeepSeek Pro hard-implement pin). Prefer `--agent cursor --model grok-4.7-high` for mechanical + ordinary infra/code implement (#8493/#8498); Flash everyday / Pro @ high = hard implement only; roster + §2b updated; CF/`auto` and LANGUAGE-LANES exclusions unchanged.
