...
| **DeepSeek** | **`deepseek-v4-flash` only** @ high (frontier-practical code/infra). **`deepseek-v4-pro` DO NOT USE** (older; Flash better for current workloads — operator 2026-08-08) | Review/research: `opencode run --model deepseek-direct/deepseek-v4-flash --variant high` (native Entire capture). Tool-heavy: `delegate.py --agent deepseek --model deepseek-v4-flash`. **Never Pro.** | Cheap / often idle paid balance | **CODE + infra CF volume.** Prefer when Codex/Claude hot. Not language/folk/authority. First-party only (`openrouter/deepseek/*` refused). |
...
| **cursor** | **Auto** (default worker) · Composer 2.5 (pinned when family independence matters) | `scripts/delegate.py dispatch --agent cursor` / `ask-cursor` (default model `auto`) | Metered; Auto often **underused** — check CodexBar Total/Auto | **First-class code/infra worker.** Prefer Auto for CI/mechanical when free; never `auto` as formal CF identity. `concurrency_limit: 1`. Not a language seat. |
...
| **glm / Z.AI** | Zhipu `glm-5.2` | `ab ask-glm` (Z.AI Coding Plan; opencode) | Z.AI sub (CodexBar provider `zai`: watch weekly vs 5h) | Differentiator = deep code/security + **large-context coherence**. ⚠️ **China-hosted → LOCAL-ONLY, never CI/sensitive**. NOT Ukrainian/prose. Prefer 5h window when weekly hot. Not a general OpenRouter bus. |
...
- **When a lane frees up, pull the next item that FITS it — don't idle, don't make-work:**
  ...
  - **DeepSeek Flash only:** code/infra PR diffs and CF volume — `deepseek-v4-flash`; **never Pro**. Not language/VESUM seats (LANGUAGE-LANES). Don't review inline on the author seat.
  - **Cursor Auto:** pull code/infra CI and mechanical-with-judgment when Auto pace is behind/farBehind — do not leave Auto idle while Codex is near_cap.
  - **Z.AI / GLM-5.2:** deep security / large-context coherence (LOCAL-ONLY); use free 5h when weekly is hot.
...
| Historical only | DeepSeek | deepseek-v4-pro history (PR #2107/#2112) — **not a live route** | 2026-05-17 | bakeoff archive | **Operational (2026-08-08):** use **Flash only**; Pro DO NOT USE; content/VESUM seats are LANGUAGE-LANES (no deepseek). |
...
| Content review (load-bearing, VESUM) | **LANGUAGE-LANES only:** agy / codex / claude / grok-4.5 — **not** deepseek. ~~deepseek-v4-pro example retired~~ (Pro DO NOT USE; language seats exclude deepseek). |
