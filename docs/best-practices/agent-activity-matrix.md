...
| **glm / Z.AI** | Zhipu `glm-5.3` (default high; advisory max) | `ab ask-glm` (Z.AI Coding Plan; opencode) | Z.AI sub (CodexBar provider `zai`: watch weekly vs 5h) | Differentiator = deep code/security + **large-context coherence**. ⚠️ **China-hosted → LOCAL-ONLY, never CI/sensitive**. NOT Ukrainian/prose. Prefer 5h window when weekly hot. Not a general OpenRouter bus. |
| **ox-alpha (OpenRouter stealth)** | `openrouter/stealth/ox-alpha` (aliases `ox-alpha`, `0x-alpha`, `stealth/ox-alpha`) | OpenCode only (`opencode run --model openrouter/stealth/ox-alpha`) | Free preview (~through 2026-08-27; OpenRouter $0 / 1M) | **Hold** — LOCAL-ONLY local shadow/compare seat (#6943). Workhorse tier (`frontier_practical`); not automatic routing, not LANGUAGE-LANES, not CF/review ladder. Provider retains prompts (stealth terms); likely China egress; identity unconfirmed (`family: zhipu` is not proof). |
...
- **When a lane frees up, pull the next item that FITS it — don't idle, don't make-work:**
  ...
  - **Z.AI / GLM-5.3:** deep security / large-context coherence (LOCAL-ONLY); use free 5h when weekly is hot.
