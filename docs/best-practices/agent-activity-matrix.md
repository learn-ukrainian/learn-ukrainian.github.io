...
| **DeepSeek** | deepseek-v4-flash @ high (frontier-practical frontend/code execution and review); deepseek-v4-pro retained for explicit long-context or critical fallback | `delegate.py --agent deepseek --model deepseek-v4-flash --effort high` (Hermes adapter; effort is shared/config-scoped) | Cheap | **Off-seat CODE lane.** Flash precedes Pro on practical code ladders but remains behind Pro on critical fallback; neither is a Ukrainian-language or approval-authority seat. |
...
| Current practical seat | DeepSeek V4 Flash High | Arena Code/WebDev rank 7 (preliminary) plus the historical E:A+ 15s repo probe | 2026-08-02 | `scripts/config/model_catalog.yaml` · Arena Code leaderboard | Frontier-practical frontend/code lane at Hermes high; not critical authority. |
