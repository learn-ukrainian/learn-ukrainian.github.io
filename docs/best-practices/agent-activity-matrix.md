...
- **Eligible harnesses:** `claude` (Claude Code accepts `--strict-mcp-config --mcp-config` to isolate the attempt's stdio config), `grok` (native Grok CLI receives strict per-attempt MCP config), and `cursor` (mirrors the per-attempt stdio config into the worktree's `.cursor/mcp.json` without daemon URL fallback).
- **Ineligible harnesses:**
  ...
  - `codex`: refused because its internal read-only sandbox cancels unapproved stdio MCP calls and cannot configure per-attempt stdio servers with environment variables.
