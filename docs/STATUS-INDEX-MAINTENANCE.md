...
```json
{
  "scripts": {
    ...
    "status:bio": "python scripts/generate_level_status.py bio",
...
```markdown
MODULE DEPLOYED: hist/scythians-sarmatians ✅

...

Next steps:
1. Run `npm run status:hist` to update level overview
2. Commit changes: `git add docs/HIST-STATUS.md`
```
...
# Update overview
npm run status:hist
git add docs/HIST-STATUS.md
git commit -m "fix: resolve 10 hydration errors in HIST"
```
...
- HIST progress: [docs/HIST-STATUS.md](docs/HIST-STATUS.md)
```

**Issue #463 (HIST upgrade):**
Link to `docs/HIST-STATUS.md` in issue description for live progress tracking.
...
**Scenario: Working on HIST modules**

```bash
# 1. Check current status
npm run status:hist
cat docs/HIST-STATUS.md  # See 21 failing modules
...
# 4. Commit changes
git add docs/HIST-STATUS.md \
  curriculum/l2-uk-en/hist/meta/*.yaml \
  curriculum/l2-uk-en/hist/*.md
git commit -m "fix: resolve issues in HIST M05, M08, M10"
```
