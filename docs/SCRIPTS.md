...
- `scripts/benchmarks/generate_synthetic_manifest.py` - generate a deterministic, synthetic 410k-scale Atlas manifest fixture by resampling real current public manifest entries (#8307); outputs a clearly marked synthetic JSON manifest accepted by `scripts.atlas.atlas_db`
...
| `scripts/ci/check_issue_task_quality.py` | Advisory DoR/DoD field check for issue bodies (#7854) | `.venv/bin/python scripts/ci/check_issue_task_quality.py --issue N` |
...
| `scripts/benchmarks/generate_synthetic_manifest.py` | Generate deterministic synthetic 410k manifest fixture from current Atlas entries (#8307) | `.venv/bin/python -m scripts.benchmarks.generate_synthetic_manifest --output /tmp/synthetic-manifest.json` |
