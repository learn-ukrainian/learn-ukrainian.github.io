...
Latest-Brief: docs/session-state/2026-05-12-cold-start-followups-brief.md

> **Repo state (late night 2026-05-11 → 2026-05-12):** **4 PRs merged this session** (#1889 Gemini warm-cache · #1890 Claude rule autoload dedupe · #1892 act local CI · #1893 act doc follow-up). **`.claude/rules/` 47.5 KB → 17.4 KB verified post-deploy** (−63%, ~7.5 K tokens off every Claude cold-start) — addressed user's "65k cold-start" complaint by extending `/api/rules?format=markdown` to serve all 6 unscoped rule files + skipping them from the `.claude/` deploy target only (Codex/Gemini rules unchanged). **act > Dagger pivot:** orchestrator pushed back on user's "we urgently need Dagger" framing (Dagger requires pipeline-as-code rewrites; months); user accepted "ok if act is better do act"; shipped `.actrc` + `scripts/local-ci.sh` (OrbStack-aware) + docs. Verified `lint` 38s with --reuse; pytest 7m45s first run (slower than GHA 4m17s — emulation overhead + no actions/cache honor); 7 known-act failures documented (4× rsync missing + 3× perf budgets under QEMU). **Codex bridge resume queued** (#1894) — user shared `codex resume` CLI help; mirror Gemini #1887 pattern. **Codex dispatch silent-death + orchestrator rescue:** Codex #1888 dispatch exited without committing; orchestrator verified work + committed inline with `X-Agent: claude-inline/1888-rescue` trailer. Working tree clean, 0 PRs open, 0 active dispatches, 0 worktrees open. Main at `e76c152f36`. **P0 next session:** dispatch #1894 (Codex bridge resume, ~50 LOC + tests, mirror #1888 brief shape).
...
|---|---|---|
| **Late night 2026-05-11 → 2026-05-12 — cold-start rule dedupe + act local CI + Codex resume queued** | **Brief (read first):** `docs/session-state/2026-05-12-cold-start-followups-brief.md`<br>**Detail (human-read):** `docs/session-state/2026-05-12-cold-start-followups.html` *(HTML, thirteenth under #M-2)* | **4 PRs merged (#1889, #1890, #1892, #1893), 3 issues filed (#1888/1891/1894), 3 closed (#1887/1888/1891). `.claude/rules/` 47.5 KB → 17.4 KB verified post-deploy (~7.5 K tokens off Claude cold-start) — addresses user's "65k cold-start" complaint by extending `/api/rules?format=markdown` to all 6 unscoped rules + skipping them from `.claude/` deploy target only. Codex/Gemini rules unchanged. act local CI shipped (`scripts/local-ci.sh` OrbStack-aware + `.actrc` + docs); pushback on Dagger was right (user accepted "ok if act is better do act"). Pytest first-run 7m45s via act = slower than GHA 4m17s; doc reframes value prop as no-queue-wait + local debug, not raw speed. 7 known-act failures cataloged (4× rsync missing in `:act-latest` image + 3× perf budgets under QEMU emulation). Codex #1888 dispatch died silently; orchestrator rescued the uncommitted worktree work with `X-Agent: claude-inline/1888-rescue`. Codex's actual code was high quality (deploy idempotency test went beyond brief). User shared `codex resume` CLI help — filed as #1894 (mirror Gemini #1887 pattern, dispatch deferred to next session). Main at `e76c152f36`, green. **P0 next session:** dispatch #1894. Then `claude agents` view (#2), `/goal` rule #1884 (#3), #1885 mkdir gap (#4), act follow-ups rsync/perf-tags (#5), Dagger de-prioritized (#6).** |
