...
| 6 | 20260523-220712 | between writer-emit and python_qg | Writer emitted all 5 artifacts (module.md, activities/vocabulary/resources/implementation_map yamls) but no `writer_tool_calls.json` saved and python_qg.json never created. **resources.yaml STILL cites Захарійчук Grade 4 pp.162/163** (plan's references are Grade 1 pp.24/52). Build branch `build/a1/my-morning-20260523-220712` exists but artifacts not auto-committed (_persist_build_artifacts didn't run or found nothing). |
