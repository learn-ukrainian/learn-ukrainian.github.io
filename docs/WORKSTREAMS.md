...
| open-model-data | #6321 (successor to closed #6164 and #6056) | Build evidence-bearing prepared Ukrainian data products on the completed Foundry engine. Community usefulness is primary; adoption is secondary evidence, not a gate. Every shipped artifact names a concrete consumer decision/use case. Current truth: #6375 is the active Phase 3 v2 rebinding task; #6333 is historical `ENGINE_READY` evidence only and establishes no linguistic, source, consumer, or completion status; Phase 4 remains blocked. |
...
| open-model-data | ACTIVE | Execute #6375: bind Phase 3 packets, runs, and compatibility evidence to the operator-pinned v2 contract; retain #6333 only as historical `ENGINE_READY` provenance. | The v2 compatibility matrix is hash-complete and fail-closed, all pre-v2 semantic/source/consumer/completion claims are invalidated, and Phase 4 remains blocked pending a new v2 build, independent review, and completion evidence. |
