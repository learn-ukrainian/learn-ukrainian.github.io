...
| open-model-data | ACTIVE | Execute #6273: run the operator-approved official Gemma 4 QAT Q4_0 Hugging Face Jobs baseline across the frozen 4,000 cases after its 100-case canary and 25%-buffered USD 6.00 gate; publish complete reproducible results; create a linked Lapa-baseline follow-up without outreach. #6171 and #6164 remain completed implementation history. | The GitHub release and exact hashes are public; the Gemma 4 result dataset is anonymously reproducible with fourteen separate track reports, exact runtime/cost evidence, and no model weights; a uniquely linked Lapa follow-up owns later execution; and an independently produced receipt exists before adoption is claimed. No training, global quality score, researcher contact, or evaluation-to-learning leakage occurs. |
