...
> hand-over of the design phase. Status: **design complete for A1 build start; the operator's three decisions
> of §6 are taken (2026-09-22).** Everything below is a pointer to a frozen brief or an accepted document; this file
...
| 07a | Learner state, Part 1: planned state, base-layer contract, lesson immersion band (#8414) | #8414 comment 5773261200 (r4, sha256 `1f701dbef7b36280`), Part 1 | 01, 03, 05, plan schema r9 | language | config pin test; seven-knee band test |
| 08 | Token resolver: classification, narrowing, constrained questions, receipts (#8413 Brief B) | #8413 comment 5773128486 (r2, sha256 `dbd14390877cf7de`) | 03, 07a | language | the ULP lesson 10 replay numbers reproduced |
| 07b | Learner state, Part 2: observed index, inventory gate (#8414) | #8414 comment 5773261200 (r4, sha256 `1f701dbef7b36280`), Part 2 | 07a, 08 | language | gate fixtures; observed index byte-stable |
...
| 10 | Base-layer request file `evidence/a1/_base.request.yaml` | #8414 r4 (comment 5773261200) §"The base layer" — the rule; the lane writes the file from it | 03 | language (writes planning data, reviewed cross-family like a plan) | built by `build-words`; every line has its closed-class note |
...
| 12 | Engine E2: prompt, rendered-prompt check, preflight, immersion payload, writer call | same brief | 11, 07a | language | preflight gap report; rendered prompt with hash |
| 13 | Engine E3: assembler, expanded document, check runner, state files, module build | same brief | 12, 07b, 08, 09 | language | check runner on a fixture module; `verify_shippable --astro-build` |
...
| 15 | Review R2: reviewer prompts, module digest, layer assignment, budgets, settle task | same brief | 14, 07b, 13 | language | fix-loop report with a terminal transition |
...
| 23 | R-19 in place: `fill-in` (required 2–4 store-generated form options, `mode: form-choice | orthography`, no free typing inline at A1–A2) and `true-false` (post-text only, required text-citing explanation); `explanation` required on both; explanation register by level; `FillIn.tsx` chips as tap buttons, `TrueFalse.tsx` shows the explanation; `activity-pedagogy.md` rows; `linear_pipeline.py` explanation set | #8397 comment 5773129143 (the decision card, r2 — its "Recommendation" section is the spec; operator: "improve them") | 11 (E1 schema additions) | any (schemas, components); language for the pedagogy-doc wording | schema fixtures; component tests; `plan-validate` rejects `true-false` outside a post-text step |
| 24 | `lesson-review` skill for fresh-build lessons, replacing `content-review` for them (v1 and seminars keep the old skill) — the skill is a thin wrapper over WP 14–15 (manifest, prompt, validator, ledger); source `agents_extensions/shared/skills/` only | review contracts, deliverable (h); operator go 2026-09-22 | 14, 15 | any | the skill runs the validator and refuses a review without receipts |
| 22 | Position 1 pilot: pack, word store, plan, plan review, measurement, canary, lessons, recap, review, LU QA sweep (#8425) | §5 of this file (the runbook) | 01–16 (07a, 07b), 20, 21, 24 | driver + language lanes; LU QA observer once the module is live | `/a1/sounds-letters-and-hello/` built, gates green, module verdict APPROVE, settle items counted, LU QA sweep triaged |

Not packages — follow-ups: Q10 "one wiki packet per module rendered from the pack" (R-25: a human-readable page may be rendered from the pack; not an input — a small brief for after the pilot), the grammar checker of writer contract §7 row 10 (an implementing brief after real drafts exist), lesson-level structural minimums (calibrated from the pilot, #8414), the arc's structured grammar/vocabulary columns (#8412 comment of 2026-09-21: option (b), after the first two or three plans).
...
10. **LU QA sweep of the published module** (operator 2026-09-24: Grok Bot "LU QA" tests pages and activities, as it already does for Atlas and practice). After the Pages GO, once the deployment has succeeded and the public module URL is verified live (§3 rule 6), the driver drafts a `[qa-request][LU QA]` issue (labels `task`, `qa`, `observer:lu-qa`; template: #8686) and the operator assigns LU QA to it. Scope: `/a1/` landing, the module page, every lesson route and all four tabs (Урок, Словник, Вправи, Ресурси), every activity exercised start → correct and wrong answer → feedback/explanation → result → retry, audio and resources links, desktop, narrow mobile and keyboard. Denominator: lessons × tabs and activities × items inspected, with untested surfaces listed. Each finding is a linked, labeled issue; Ukrainian word, stress and morphology findings stay **suspected** until a language lane checks them against VESUM and the `sources` tools. The driver triages each finding to its layer (renderer, engine gate, pack, word store, plan, or writer prose — the provenance file names it), and a defect a gate could have caught becomes a gate issue. LU QA is an external observer: not a review of record, not a fleet seat, no merges or deploys (`docs/runbooks/grok-bot-qa-observer.md`). The module is complete only after the sweep report is triaged and every finding is fixed or explicitly accepted by the operator on its issue.
