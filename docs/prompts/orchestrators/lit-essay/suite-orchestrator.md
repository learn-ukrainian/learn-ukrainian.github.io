...
- `lit-essay` is an active LIT specialization track for essays, criticism, public intellectual prose, and nonfiction argument.
- Treat essays, reviews, manifestos, letters, and critical prose as reading candidates where rights allow.
- Most modern essays may be copyrighted; default to linked-only or excerpt-only unless hostable rights are explicit.
- This suite covers preflight, production, quality audit, and remediation. Use only the stage that matches the task.

## Goal

Run preflight, production, quality audit, and remediation scoped to `lit-essay` modules without touching B2 or inactive LIT remnants.
...
- Preflight / quality audit reports under `docs/audits/lit-essay-<scope>-<date>.md` when explicitly scoped.
- Scoped current-layout module files under `curriculum/l2-uk-en/lit-essay/`.
- Scoped sidecars under `curriculum/l2-uk-en/lit-essay/{meta,activities,vocabulary}/` when that layout is used.
- Generated learner page `site/src/content/docs/lit-essay/<slug>.mdx` for scoped built modules.
...
- Teach argument structure, register, voice, evidence, and public context.
- Separate authorial stance from later reception.
- Do not turn essay modules into generic opinion-writing worksheets.
