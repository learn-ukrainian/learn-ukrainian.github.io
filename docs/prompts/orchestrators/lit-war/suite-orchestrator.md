...
- Most texts are recent and copyrighted. Default to linked-only or excerpt-only unless hostable rights are explicit.
- This suite covers preflight, production, quality audit, and remediation. Use only the stage that matches the task.

## Goal

Run preflight, production, quality audit, and remediation scoped to `lit-war` modules, preserving Ukrainian agency, testimony, rights, and learner-safe framing.
...
- Preflight / quality audit reports under `docs/audits/lit-war-<scope>-<date>.md` when explicitly scoped.
- Scoped current-layout module files under `curriculum/l2-uk-en/lit-war/`.
- Scoped sidecars under `curriculum/l2-uk-en/lit-war/{meta,activities,vocabulary}/` when that layout is used.
- Generated learner page `site/src/content/docs/lit-war/<slug>.mdx` for scoped built modules.
...
- Do not use non-hostable full contemporary texts, captivity images, or propaganda-origin media.
...
- Frame literature as resistance, testimony, mourning, witness, and cultural endurance, not neutral `conflict literature`.
- Avoid tragedy-on-both-sides prose and Russian current-source laundering.
- Verify text identity, author context, rights, sensitivity risks, and current-event claims.
