...
Use read-only helpers for reading/copyright discovery, quote/source verification, rendered-page checking, and exemplar comparison when useful. Summarize long helper outputs; push bulky evidence behind a file path or PR link. The main orchestrator owns edits, review routing, PR creation, and merge decisions.
