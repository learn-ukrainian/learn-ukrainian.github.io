...
- `lit-humor` is an active LIT specialization track for satire, humor, comic prose, parody, and public wit.
- Treat sketches, prose excerpts, feuilletons, performances, author commentary, and reviews as reading candidates where rights allow.
- Modern humorous texts may be copyrighted; default to linked-only or excerpt-only unless hostable rights are explicit.
- This suite covers preflight, production, quality audit, and remediation. Use only the stage that matches the task.

## Goal

Run preflight, production, quality audit, and remediation scoped to `lit-humor` modules without touching B2 or inactive LIT remnants.
...
- Target plan: `curriculum/l2-uk-en/plans/lit-humor/<slug>.yaml`
- Existing target source, sidecars, readings, and `site/src/content/docs/lit-humor/<slug>.mdx` when present.

## Allowed Writes

- Preflight / quality audit reports under `docs/audits/lit-humor-<scope>-<date>.md` when explicitly scoped.
- Scoped current-layout module files under `curriculum/l2-uk-en/lit-humor/`.
- Scoped sidecars under `curriculum/l2-uk-en/lit-humor/{meta,activities,vocabulary}/` when that layout is used.
- Generated learner page `site/src/content/docs/lit-humor/<slug>.mdx` for scoped built modules.
...
- Teach comic mechanism, register shift, irony, parody target, and historical/cultural context.
- Do not flatten satire into generic jokes or quote humor without context.
- Check ethnic, regional, gender, and wartime sensitivity where relevant.
