...
> **Document Status:** Authoritative Operational Plan & Roadmap (Amended Post Tri-Family Deliberation: Gemini, Claude Fable 5.1, Codex Sol)
...
## 1. Executive Strategy: The Zero-Hallucination Human Source Mandate

Pre-trained foundation models systematically confuse Soviet lexical leveling with authentic Ukrainian standards. Expanding from the **150 Human Gold Seeds (Phase 2)** to a **production-scale dataset (6,000 SFT trajectories + 3,000 DPO preference pairs)** requires avoiding synthetic LLM "hallucinated errors".
...
1. **100% Human-Authored Contexts & Error Spans:** Every underlying sentence, attested error span, distractor, and candidate correction originates directly from verified human-authored Ukrainian texts residing in local storage (`sources.db`, `ua_gec_errors`, `zno_tasks`, `textbooks`, `style_guide`). No sentences or errors may be invented by an LLM.
2. **Automated Claim-Verification of Reasoning Prose:** Explanatory reasoning and Chain-of-Thought (CoT) trajectories must pass an automated claim-verifier that confirms every dictionary citation (СУМ-11, R2U), inflectional claim (VESUM), and register qualifier (ULIF) against local databases. Any ungrounded claim triggers record rejection.
3. **Pinned Target Norm:** The target standard is explicitly pinned to **Modern Standard Ukrainian per the 2019 Orthography (Український правопис 2019) and current MESU school curricula**, cleansed of Soviet leveling and Russian calques. Historical 1920s dictionaries and 1930s purge bulletins serve as *candidate discovery heuristics*, never as an uncritical replacement blueprint (preventing archaisms like *рівнобіжник* instead of standard *паралелограм*).

---

## 2. Complete Inventory of Local Corpus Assets

| Corpus Asset | Storage Location | Exact Volume | Primary Decolonization Role |
| :--- | :--- | :--- | :--- |
| **STEM Textbooks (Gr 1–11)** | `sources.db` $\rightarrow$ `textbooks` | **15,563 chunks** (65 books across 10 subjects) | Scientific terminology decolonization, polysemy boundaries (*об'єм*, *рахувати*, *відношення*), and vetted `PRESERVE` negative controls |
| **Language & Lit Textbooks** | `sources.db` $\rightarrow$ `textbooks` | **16,872 chunks** (127 books) | Explicit contrastive tables (❌ НЕПРАВИЛЬНО $\rightarrow$ ✅ ПРАВИЛЬНО), speech culture notes |
| **ZNO / NMT Examination Tasks** | `sources.db` $\rightarrow$ `zno_tasks` | **1,646 official tasks** | Real state-exam distractors (authentic calques) paired with validated correct keys (Train-only to prevent pre-training eval contamination) |
| **UA-GEC Error Corpus** | `sources.db` $\rightarrow$ `ua_gec_errors` | **8,937 annotated error spans** | 2,856 human-annotated calques (`F/Calque`) and collocations (`F/Collocation`) from real writing with author metadata |
| **Antonenko-Davydovych *«Як ми говоримо»*** | `sources.db` $\rightarrow$ `style_guide` | **342 full chapters** | Morphemic, etymological, and literary reasoning steps |
| **Pre-Soviet & 1920s Dictionaries (R2U)** | `scripts.rag.source_query.r2u_translate` | **Full indexed endpoints** | Pre-1933 authentic equivalents (Krymskyi-Yefremov 1924–33, Pidmohylnyi-Pluzhnyk 1926–27) used as historical discovery heuristics |
| **Soviet СУМ-11 (Differential Mirror)** | `sources.db` $\rightarrow$ `sum11` | **127,069 entries** | Negative differential mirror: detecting Soviet ideological elevation and artificial convergence |
| **ULIF (Словники України on-line)** | `data/ulif_dump_all.db` $\rightarrow$ `ulif_entries` | **13,392 entries** | Register qualifiers (`діал.`, `розм.`, `книжн.`, `заст.`), stress, and synonym spectrum to bound purism |
| **VESUM Morphological Engine** | `data/vesum.db` $\rightarrow$ `forms` / `forms_all` | **409K lemmas, 6.7M forms** | Paradigm verification, inflectional consistency, and non-standard marker detection (`:bad`, `:subst`) |
...
2. **`рахувати` vs `обчислювати` vs `вважати`:**
   - `PRESERVE`: Discrete counting of concrete objects or elements (*рахувати дні*, *порахувати предмети*).
...
- **Register Preservation:** Include vetted conversational and colloquial prose (`розм.`) where informal Ukrainian is preserved as-is, teaching the model that colloquial register does not equal Russian interference.
...
2. **PRESERVE DPO Pairs (900 pairs):** For clean `PRESERVE` passages, the rejected response represents **plausible hyper-puristic over-correction** (e.g. wrongly modifying *об'єм конуса* $\rightarrow$ *обсяг конуса* in geometry). This directly trains the model that "more changes $\neq$ better".
...
4. **Error Density Bounding:** Cap rejected completions to realistic error densities (matching the empirical UA-GEC distribution of 1–2 errors per sentence), preventing exaggerated caricatures.

### 3.4 Pre-Extraction Partition Firewall
...
4. **Pre-Training Contamination Defense:** Verbatim ZNO/NMT exam tasks and classic Antonenko-Davydovych sentences are widespread on the public web and memorized in Gemma pre-training. They are restricted to **Train-only**. The 1,000-case Held-Out Evaluation Suite is constructed from low-web-visibility sources, author-disjoint partitions, and fresh human-authored gold cases.
5. **Evaluation Allocation & Statistical Power:** The 1,000 held-out cases are allocated as **600 PRESERVE cases + 400 CORRECT cases**. On 600 clean cases, observing $\le 1$ harmful edit yields an exact one-sided 95% binomial upper bound of $0.788\%$, statistically proving compliance with the $\le 1.0\%$ Harmful-Edit Rate gate.
...
| :--- | :--- | :--- | :--- |
| **Phase 3.0** | Source Custody & Partition Firewall | `phase3_heldout_partition.py` | Phenomenon-level split; ZNO/style-guide to Train; 600/400 eval split |
...
| **Phase 3.2** | UA-GEC Context Extraction | `v4_mine_uagec_calques.py` | 2,856 `F/Calque` and `F/Collocation` pairs in full context; author-disjoint |
...
| **Phase 3.4** | Differential Soviet Candidate Miner | `v4_differential_soviet_miner.py` | СУМ-11 `sovietization_risk` vs. R2U; human/expert adjudication filter |
| **Phase 3.5** | Automated CoT Claim-Verifier | `v4_verify_trajectory_claims.py` | Verifies every cited lemma, form, dictionary claim, and historical date |
...
1. **Zero Schema Violations:** All generated records must validate against `v1_decolonization_trajectory.schema.json` and `v1_decolonization_dpo_pair.schema.json`, with conditional schema rules separating `CORRECT` (requiring suppression fields) from `PRESERVE` (omitting historical suppression).
...
3. **100% CoT Claim-Verification:** Zero unverified dictionary or historical assertions in reasoning trajectories.
...
6. **No Synthetic Sentence Hallucination:** 100% of underlying sentence contexts must originate from local human-authored corpus files.
