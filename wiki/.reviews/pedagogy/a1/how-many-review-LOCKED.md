...
- **Reviewer:** claude-opus-4-7-xhigh-1446 (scale batch 1, review-and-lock pass)
...
|---|-----------|-------|----------|
| 1 | Factual accuracy | **9/10** | Existing source-keyed claims ([S1]–[S7] via the `how-many.sources.yaml` sidecar) were left intact. The new Step 7 (age / prices / phone) and the L2-errors table do not introduce new unsourced lexical claims — every right-column form was checked in VESUM (batch verification performed via `mcp__sources__verify_words`: 25+25+12+6+2 = 70 form checks, all right-column forms found, all named Russianisms absent); every left-column Russianism is named as such and its absence from VESUM stated inline in the table's introductory paragraph. The `год` entry cites СУМ-11's own label "розм., рідко" verbatim; the `дев'ятдесят` entry carries both VESUM's `arch` tag and the `діал.` label from orthographic dictionaries so the reader can see the primary source. The one soft spot is that Step 7's phone-number pronunciation convention is native-speaker usage rather than a textbook cite — this is intentional and mirrors the at-the-cafe precedent for modern pragmatics that the L1 textbook corpus does not cover. |
...
- **`кі́лько`** is attested in VESUM as a numeral and is flagged in the L2-errors table as dialectal (per the standard dictionary label). A learner who hears it from a dialect speaker is not hearing "bad Ukrainian" — they are hearing a regional variant. The table note is accurate on this; framing is the writer's problem at module build time, not a wiki defect. Etymological attribution (Polish influence, Western-Ukrainian core) was intentionally trimmed from the wiki note pending a citeable source.
