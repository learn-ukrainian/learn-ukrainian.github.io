✅ Message sent to Gemini (ID: 28127) [auto-acked: self-addressed]

🚀 Invoking Gemini to process message #28127...
📨 Message #28127
   From: gemini → To: gemini
   Type: query
   Task: wiki-review-a1-sounds-letters-and-hello-r3
   Time: 2026-04-05T21:33:09.657535+00:00
...
Track: a1, Slug: sounds-letters-and-hello, Round: 3
...
Dimension scores:
1. Factual: 7/10 — The example for `ДЗ` using the English "kids" contradicts the warning about morpheme boundaries in the very next sentence (since "kids" explicitly has a morpheme boundary `kid-s`). Phase 1 introduces the word `кіт` before the letter `і` is taught (which is explicitly sequenced in Phase 2). The mnemonic "Де ти з'їси ці лини" does not natively contain `дз`. The letter `Ґ` was returned to the alphabet in the 1990 orthography update, not 1991.
2. Language: 8/10 — The term "Звук-Спочатку" is an English calque for "Sound-First"; the standard Ukrainian pedagogical term is "Від звука до букви" (From sound to letter) or "Звуковий метод". The rest of the Ukrainian text is natural and accurate, with good use of minimal pairs.
3. Decolonization: 10/10 — Outstanding. Explicitly bans Russian as a phonetic reference, correctly frames the difference in `о` reduction to prevent negative transfer, and rightfully celebrates the uniqueness of letters like `Ї` and `Ґ`.
4. Completeness: 8/10 — Good coverage of consonants and vowels, but critically misses establishing `Наголос` (Stress) as a foundational concept. Ukrainian stress is unpredictable and mobile, making explicit stress marking mandatory for early A1 materials.
5. Actionable: 8/10 — The kinesthetic exercises (ear covering, smile-unsmile) are highly actionable. However, the Phase 1 vocabulary is pedagogically broken; a module writer cannot build reading exercises for `кіт` if `і` hasn't been introduced yet.

**Overall: 8.2/10**

<fixes>
old: **1. Звук-Спочатку (Sound-First Principle):**
new: **1. Від Звука до Букви (Sound-First Principle):**
---
old: *   **First Words:** `мама`, `тато`, `там`, `так`, `том`, `кіт`. Immediately readable words that are high-frequency or cognates.
new: *   **First Words:** `мама`, `тато`, `там`, `так`, `мак`, `тут`. Immediately readable words that use ONLY the introduced letters (А, О, У, М, Т, К).
---
old: from a specific list (`д, т, з, с, ц, л, н, дз` - "Де Ти З’їСи Ці ЛиНи?", Source 31).
new: from a specific list (`д, т, з, с, ц, л, н` - mnemonic "Де Ти З’їСи Ці ЛиНи?", plus `дз`, Source 31).
---
old: *   **Digraphs `ДЖ` and `ДЗ`:** Teach these as single sounds (affricates), like "j" in "jeans" (`джинси`) and "ds" in "kids" (`дзеркало`). Contrast this with when they are two separate sounds at a morpheme boundary, e.g., `під-земний` (though this is a more advanced concept). (Source 33).
...
---
old: Celebrate the letter `Ґ`. Its return to the alphabet in 1991 was an act of de-Russification.
new: Celebrate the letter `Ґ`. Its return to the alphabet in 1990 was an act of de-Russification.
---
INSERT AFTER: Using them clarifies pronunciation rules before the learner has fully internalized them.
NEW TEXT:
...
────────────────────────────────────────
✅ Gemini finished (3596 chars)
✅ Message sent to Claude (ID: 28128)
✓ Message 28128 acknowledged
   Auto-acknowledged reply #28128 (stdout delivery — no inbox accumulation)
   ℹ️  No issue number in task_id 'wiki-review-a1-sounds-letters-and-hello-r3' — skipping GH posting (review saved to orchestration/)
✓ Message 28127 acknowledged
