...
|---|-----------|-------|----------|
| 1 | Factual accuracy | **9/10** | Every claim sourced via inline `[S1]–[S6]` keyed to the sidecar `what-is-it-like.sources.yaml`. The new `Типові помилки L2` table is the heaviest factual addition; every right-column item is VESUM-verified (batch `mcp__sources__verify_words` — `смачний`, `жовтий`, `чорний`, `червоний`, `правильний`, `поганий`, `розумний`, `лінивий` all attested) and every left-column item is either VESUM-absent (`вкусний`, `жолтий`, `черний`, `ленивий`), СУМ-11 розм. (`плохий`), or a paronym with documented different semantics in Ukrainian (`красний` = «гарний, прекрасний» нар.-поет. per СУМ-11; `вірний` = «loyal/faithful»; `любий` = «дорогий, милий»). The remaining soft spot is that the `розм.` mark on `плохий` makes it a "preferred form" recommendation rather than a hard wrong-form deduction — this is intentional and stated in the table's note. |
...
- The `Лінивий` row of the L2 table is shaped slightly differently (`лінивий vs ленивий` — ordering swapped, no full-sentence pair) because both forms surface in colloquial Surzhyk and the contrast is the surface form, not a sentence-level confusion. This is intentional but cosmetically inconsistent with the other rows; not worth a deduction.
...
- The source sidecar `wiki/pedagogy/a1/what-is-it-like.sources.yaml` still uses the generic `type: unknown` convention, like many other wiki sidecars in the repo (including the locked at-the-cafe sidecar). This is corpus-hygiene debt, not a pedagogical blocker.
