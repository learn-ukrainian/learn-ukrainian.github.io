...
|---|-----------|-------|----------|
| 1 | Factual accuracy | **9/10** | Every claim in the new sections (Крок 6 patronymics, "Типові помилки L2" table, writer-note) is traceable. Patronymic suffix inventory `-ович/-йович/-івна/-ївна` verified against VESUM (`Іванович`, `Сергійович`, `Петрівна`, `Сергіївна` all present). The eight Surzhyk/calque pairs are each annotated with a VESUM-derived verification note (the right-hand lemma verified present; the left-hand form verified absent or marked `slang`/restricted). One soft spot: Крок 6's sociolinguistic claim about "always `ім'я + по батькові + Ви` at work/school/state offices" is general cultural knowledge, not tied to a specific S-source — this is intentional and acknowledged in the same prose. |
| 2 | Ukrainian language quality | **9/10** | Zero Russianisms in instructional text. Every right-column token in the new "Типові помилки L2" table verified in VESUM (`data/vesum.db`); every left-column token verified **absent** from VESUM, OR present only with `slang`/archaic tagging (`дєдушка: slang`, `муж`: single form without full paradigm). The `папа`/`мама і папа → тато`/`мама і тато` row explicitly acknowledges that `папа` is in VESUM but flags it as stylistically dispreferred in A1 teaching materials — no claim is overreached. Four separate checks per rule #2 of `ukrainian-linguistics.md`: no Russianisms in prose, no Surzhyk (shown as the *object* of prevention, not embedded in the text), no untagged calques, no paronym confusion. |
...
- `папа` row in the Surzhyk table is weaker than the other seven (the form IS in VESUM as an untagged standard noun). The row is kept because in the context of modern Ukrainian pedagogy for children (Большакова, Вашуленко) `тато` is overwhelmingly preferred, and learners of Russian background default to `папа`. A stricter reviewer could argue this row undermines the table's consistency; a softer reviewer could argue it does exactly the work of nudging the learner to a preferred-in-pedagogy register. Kept as row 8 (least-weight position).
- `дядя` / `тьотя` are not included in the table even though they were considered. Reason: VESUM treats `дядя` as a valid lemma (`дядьо`) and `тьотя` as a valid lemma, both without restrictive tags. Framing them as Russianisms without a style-guide flag would contradict the rubric's "do NOT add if not verifiable in VESUM + СУМ-11" rule applied in reverse. Writer should steer toward `дядько` / `тітка` through positive modeling, not through a pair entry here.
