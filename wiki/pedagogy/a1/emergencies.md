...
2.  **Step 2: Key Emergency Services & Numbers.** Learners must memorize the three primary emergency numbers in Ukraine.
    ...
    *   `102` — `Поліція` (Police)
    *   `103` — `Швидка допомога` (Ambulance, lit. "fast help") [S44]
...
3.  **Step 3: Stating the Location (The Address).** Teach the formula for giving a location. The order is critical: Street -> Building -> Apartment.
    ...
    *   Model sentence: `Моя адреса: вулиця Квіткова, будинок 3` (My address is: Kvitkova street, building 3) [S45].
...
4.  **Step 4: Describing a Medical Problem (The Body).** Introduce the core structure for expressing pain. This is one of the most common and necessary functions for a beginner.
    ...
    *   Example: `У мене болить голова і горло` (I have a headache and a sore throat) [S46].
    *   Introduce `температура`. Example: `У мене температура 38,2` (I have a temperature of 38.2) [S46].
...
5.  **Step 5: Making a Call & Requesting Action.** Combine the previous steps into a functional phone call.
    ...
    *   Example: `Я хочу записатися на прийом до лікаря` (I want to make an appointment with a doctor) [S47].
    *   Stating the need for help: `Треба допомогти` (Help is needed) [S48].
    *   Using the imperative: `Зателефонуйте за номером 101` (Call number 101) [S49].
...
| :--- | :--- | :--- |
| Я маю головний біль. | **У мене болить голова.** | Direct translation from "I have a headache". Ukrainian uses a different structure, literally "At me aches the head". This is a core pattern to drill [S46][S47]. |
...
| Мій адрес... | **Моя адреса...** | `Адрес` and `адреса` are false friends. `Адреса` (feminine) is a physical location. `Адрес` (masculine) is a formal written greeting/tribute, which an A1 learner will almost never need [S50][S51]. |
| Я є хворий. | **Я захворів / Я захворіла.** | Direct translation of "I am sick". While grammatically understandable, the natural way to express the onset of illness is with the perfective verb `захворіти` (to get sick) [S46]. |
| Мені потрібен паспорт і код. | **Мені потрібен ваш паспорт та ідентифікаційний код.** | Learners often simplify "identification code" to just "code". The full term `ідентифікаційний код` is standard in official contexts like banks or clinics and should be learned as a chunk [S52][S53]. |
| Прийдіть раніше і візьміть **ваш** паспорт. | **Прийдіть... раніше і візьміть паспорт.** | English relies heavily on possessive pronouns ("your passport"). In Ukrainian imperatives, when the context is clear, the possessive pronoun is often dropped as redundant. The phrase `Візьміть паспорт` is more natural than `Візьміть ваш паспорт` in this context [S47]. |
...
3.  **Vocabulary Purity:** Use exclusively Ukrainian vocabulary. For instance, use `лікарня` for hospital, not a calque. The term `швидка допомога` (or just `швидка`) is the correct and natural term for an ambulance [S44]. Avoid any regionalisms that might have Russian influence.
...
*   `Що вас турбує?` ★★☆ (What's bothering you?) [S47]
*   `Будь ласка` ★★★ (Please)
