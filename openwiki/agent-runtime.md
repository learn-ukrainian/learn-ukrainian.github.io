...
> [!NOTE]
> OpenWiki is a non-authoritative locator. Authoritative multi-agent harness policies are maintained in [`agents_extensions/shared/rules/`](../agents_extensions/shared/rules/) and executable harness scripts in [`scripts/ai_agent_bridge/`](../scripts/ai_agent_bridge/).
...
Per [ADR-013](../docs/architecture/adr/adr-013-docs-knowledge-openwiki.md), tasks touching Ukrainian pedagogy are language-adjacent. Therefore, LANGUAGE-LANES seating applies, and paired cross-family review (e.g., AGY executes, Codex Astra reviews) is mandatory.
...
As validated in [`tests/test_agent_runtime_env_sanitize.py`](../tests/test_agent_runtime_env_sanitize.py):
...
Following [`docs/bug-autopsies/secret-leakage.md`](../docs/bug-autopsies/secret-leakage.md):
