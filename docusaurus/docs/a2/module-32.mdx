...
<FillIn>
  <FillInQuestion
    sentence={`1. Я подзвоню, ___ приїду. (Sequence)`}
    answer={`коли`}
...
<Quiz>
  <QuizQuestion
    question={`1. "Поки" usually goes with which Aspect?`}
    options={[`Imperfective (Process)`, `Perfective (Result)`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Коли" usually goes with which Aspect for sequence?`}
    options={[`Perfective (Result)`, `Imperfective (Process)`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Після того як" implies:`}
    options={[`One action strictly follows another`, `Actions happen together`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`Translate: "Before I go..."`}
    options={[`Перед тим як я піду...`, `Перед я піду...`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Поки не" implies:`}
    options={[`Until`, `While`, `Because`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
  ...
  <QuizQuestion
    ...
    options={[`Instant reaction`, `Long wait`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Перш ніж" is a synonym for:`}
    options={[`Перед тим як`, `Після того як`]}
    correctIndex={0}
    explanation={``}
  />
</Quiz>
...
<Unjumble>
  <UnjumbleQuestion
    words={`кожного / разу / коли / я / дзвоню / він / зайнятий`}
    answer={`Кожного разу, коли я дзвоню, він зайнятий.`}
  />
  <UnjumbleQuestion
    words={`ми / гуляли / тим / часом / як / йшов / дощ`}
    answer={`Ми гуляли, тим часом як йшов дощ.`}
  />
  <UnjumbleQuestion
    words={`я / заснув / після / як / того / прочитав`}
    answer={`Я заснув після того, як прочитав.`}
  />
  <UnjumbleQuestion
    words={`подзвони / як / тільки / будеш / вдома`}
    answer={`Подзвони, як тільки будеш вдома.`}
  />
  <UnjumbleQuestion
    words={`перед / тим / як / сказати / подумай`}
    answer={`Перед тим як сказати, подумай.`}
  />
  <UnjumbleQuestion
    words={`я / чекав / поки / ти / не / прийшов`}
...
<DialogueReorder>
  ...
  {
    "speaker": "",
    "line": "Коли ти прийдеш?"
  },
  {
    "speaker": "",
    "line": "Я прийду, як тільки закінчу роботу."
  },
  {
    "speaker": "",
    "line": "А коли ти закінчиш?"
  },
  {
    "speaker": "",
    "line": "Я закінчу, коли напишу звіт."
  },
  {
    "speaker": "",
    "line": "Добре, подзвони мені перед тим як вийти."
  },
  {
    "speaker": "",
    "line": "Гаразд. А що ти будеш робити, поки чекаєш?"
  },
  {
    "speaker": "",
    "line": "Я буду пити каву, поки ти працюєш."
  },
  {
    "speaker": "",
    "line": "Ти як завжди!"
...
<MarkTheWords>
  <MarkTheWordsActivity
    text={`Ми пішли гуляти, коли закінчився дощ. Я читав книгу, поки їхав у метро. Як тільки я побачив тебе, я зрадів. Перш ніж купити це, подумай. Ми їли торт після того як задули свічки. Він мій брат(incorrect). Це гарний дім(incorrect). Я люблю каву(incorrect).`}
    correctWords={[`коли`, `поки`, `Як тільки`, `Перш ніж`, `після того як`, `мій брат`, `гарний дім`, `люблю каву`]}
  />
</MarkTheWords>
...
<ErrorCorrection>
  <ErrorCorrectionItem
    sentence={`1. Я спав, **коли** я читав. (While)`}
    error={`коли`}
    answer={`поки`}
    options={[`поки`, `коли`]}
...
<Translate>
  <TranslateItem
    source={`1. While I sleep, you work.`}
    answer={`Поки я сплю, ти працюєш.`}
    options={[`Поки я сплю, ти працюєш.`, `Коли я сплю, ти працюєш.`]}
    explanation={``}
  />
  <TranslateItem
    source={`Before I ask...`}
    answer={`Перед тим як я запитаю...`}
    options={[`Перед тим як я запитаю...`, `Перед я запитаю...`]}
    explanation={``}
  />
  <TranslateItem
    source={`After I finish...`}
    answer={`Після того як я закінчу...`}
    options={[`Після того як я закінчу...`, `Після закінчу...`]}
    explanation={``}
  />
  <TranslateItem
    source={`As soon as I see him.`}
    answer={`Як тільки я побачу його.`}
    options={[`Як тільки я побачу його.`, `Поки я побачу його.`]}
    explanation={``}
  />
  <TranslateItem
    source={`Whenever I see him...`}
    answer={`Кожного разу, коли я бачу його...`}
    options={[`Кожного разу, коли я бачу його...`, `Поки я бачу його...`]}
    explanation={``}
  />
  <TranslateItem
    source={`Wait until I return.`}
    answer={`Чекай, поки я не повернуся.`}
    options={[`Чекай, поки я не повернуся.`, `Чекай, як я повернуся.`]}
    explanation={``}
  />
</Translate>
