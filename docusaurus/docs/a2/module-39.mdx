...
<FillIn>
  <FillInQuestion
    sentence={`1. Музика -> ___ [Hint: Musical]`}
    answer={`музичний`}
...
<Unjumble>
  <UnjumbleQuestion
    words={`експеримент / це / хімічний`}
    answer={`Це хімічний експеримент.`}
  />
  <UnjumbleQuestion
    words={`проблема / це / фізична`}
    answer={`Це фізична проблема.`}
  />
  <UnjumbleQuestion
    words={`мапа / де / географічна`}
    answer={`Де географічна мапа?`}
  />
  <UnjumbleQuestion
    words={`факт / це / історичний`}
    answer={`Це історичний факт.`}
  />
  <UnjumbleQuestion
    words={`рівняння / це / математичне`}
    answer={`Це математичне рівняння.`}
  />
  <UnjumbleQuestion
    words={`журнал / це / науковий`}
...
<FillIn>
  <FillInQuestion
    sentence={`1. Київський ___. [Hint: Cake]`}
    answer={`торт`}
...
<Quiz>
  <QuizQuestion
    question={`1. Which suffix is used for "Urban/Rural"?`}
    options={[`-ський`, `-овий`, `-ний`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`Which suffix is used for "Scientific"?`}
    options={[`-овий`, `-ський`, `-чий`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Гарячий" describes:`}
    options={[`Temperature (Hot)`, `Color (Red)`, `Taste (Spicy)`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Маленький" uses which suffix?`}
    options={[`-еньк- (Diminutive)`, `-ськ- (Origin)`, `-ов- (Material)`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Жіночий" means:`}
    options={[`Women's/Female`, `Men's/Male`, `Child's`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Сільський" relates to:`}
    options={[`Village`, `Salt`, `Sun`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Український" matches "people" (люди) in plural as:`}
    options={[`Українські`, `Українська`, `Український`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
...
<ErrorCorrection>
  <ErrorCorrectionItem
    sentence={`1. Це **наукний** журнал. (Wrong suffix)`}
    error={`наукний`}
    answer={`науковий`}
