...
<FillIn>
  <FillInQuestion
    sentence={`1. Кава [___] цукру, будь ласка. [Hint: without]`}
    answer={`без`}
    options={[`без`, `для`, `з`]}
  />
  <FillInQuestion
...
<Quiz>
  <QuizQuestion
    question={`1. "Для" requires which case?`}
    options={[`Genitive`, `Accusative`, `Instrumental`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Через дощ" means:`}
    options={[`Because of rain`, `Through the rain`, `About rain`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Від мами" means:`}
    options={[`From mom`, `For mom`, `With mom`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
  ...
  <QuizQuestion
    ...
    options={[`Without sugar`, `With sugar`, `For sugar`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"О третій" refers to:`}
    options={[`Time (at 3 o'clock)`, `Place (at the third)`, `Person (about the third)`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
    question={`"Замість чаю" means:`}
    options={[`Instead of tea`, `About tea`, `Without tea`]}
    correctIndex={0}
    explanation={``}
  />
  <QuizQuestion
...
<Unjumble>
  ...
  />
  <UnjumbleQuestion
    words={`дощ / залишився / Через / я / вдома`}
    answer={`Через дощ я залишився вдома.`}
  />
  <UnjumbleQuestion
    words={`про / говорили / Ми / тебе`}
    answer={`Ми говорили про тебе.`}
  />
  <UnjumbleQuestion
    words={`молоком / Кава / з`}
    answer={`Кава з молоком.`}
  />
  <UnjumbleQuestion
    words={`третій / о / Зустрінемось`}
    answer={`Зустрінемось о третій.`}
  />
  <UnjumbleQuestion
    words={`кави / Чай / замість`}
...
<ErrorCorrection>
  <ErrorCorrectionItem
    sentence={`1. Подарунок **від** мами. (Intended: for mom)`}
    error={`від`}
    answer={`для`}
    options={[`для`, `від`]}
