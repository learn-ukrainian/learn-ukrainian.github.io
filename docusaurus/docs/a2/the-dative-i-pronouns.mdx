...
<GroupSort groups={JSON.parse(`{"1 особа (Мені/Нам)": ["Мені холодно", "Нам весело", "Мені треба", "Нам подобається"], "2 особа (Тобі/Вам)": ["Тобі сумно?", "Вам зручно?", "Тобі допомогти?", "Вам потрібно"], "3 особа (Йому/Їй/Їм)": ["Їй цікаво", "Йому боляче", "Їм байдуже", "Їй пасує"]}`)} />
