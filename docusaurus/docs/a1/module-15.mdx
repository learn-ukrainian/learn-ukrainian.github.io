...
<Quiz>
  <QuizQuestion
    question={`1. You're at an intersection and need to turn left. Which word do you use?`}
    options={[`прямо`, `наліво`, `направо`, `назад`]}
    correctIndex={1}
    explanation={`наліво = to the left`}
  />
  <QuizQuestion
    question={`What does "Це далеко?" mean?`}
    options={[`Is it nearby?`, `Is it far?`, `Where is it?`, `How do I get there?`]}
    correctIndex={1}
    explanation={`далеко = far`}
  />
  <QuizQuestion
    question={`Which preposition means "opposite"?`}
    options={[`біля`, `за`, `навпроти`, `перед`]}
    correctIndex={2}
    explanation={`навпроти = opposite, across from`}
  />
  <QuizQuestion
    question={`How do you say "by bus" in Ukrainian?`}
    options={[`автобус`, `автобуса`, `на автобусі`, `в автобусі`]}
    correctIndex={2}
    explanation={`на автобусі (on the bus) = by bus`}
  />
  <QuizQuestion
    question={`What is a "провулок"?`}
    options={[`A square`, `A lane or alley`, `A bridge`, `A traffic light`]}
    correctIndex={1}
    explanation={`провулок = narrow side street, alley`}
  />
  <QuizQuestion
    question={`If someone says "Прямо, потім направо," what should you do?`}
    options={[`Turn left, then go straight`, `Go back, then turn right`, `Go straight, then turn right`, `Turn right, then turn left`]}
    correctIndex={2}
    explanation={`прямо = straight, направо = to the right`}
  />
  <QuizQuestion
    question={`What does "пішки" mean?`}
    options={[`by bicycle`, `on foot`, `by car`, `by metro`]}
    correctIndex={1}
    explanation={`пішки = walking, on foot`}
  />
  <QuizQuestion
    question={`Which is NOT a type of urban street in Ukrainian?`}
    options={[`вулиця`, `проспект`, `бульвар`, `хмарочос`]}
    correctIndex={3}
    explanation={`хмарочос = skyscraper (building, not a street type)`}
  />
  <QuizQuestion
    question={`Where would you find a "зупинка"?`}
    options={[`In a restaurant`, `At a bus or tram stop`, `At the post office`, `In a park`]}
    correctIndex={1}
    explanation={`зупинка = stop (for public transport)`}
  />
  <QuizQuestion
    question={`What does "станція метро" mean?`}
    options={[`bus station`, `train platform`, `metro station`, `taxi stand`]}
    correctIndex={2}
    explanation={`станція метро = metro/subway station`}
  />
  <QuizQuestion
    question={`If something is "поруч," it is:`}
    options={[`far away`, `behind you`, `nearby`, `in front`]}
    correctIndex={2}
    explanation={`поруч = nearby, next to`}
  />
  <QuizQuestion
    question={`Which city has Ukraine's oldest metro system?`}
    options={[`Lviv`, `Kyiv`, `Kharkiv`, `Odesa`]}
    correctIndex={1}
    explanation={`Kyiv metro opened in 1960, making it the oldest`}
  />
</Quiz>
...
<FillIn>
  <FillInQuestion
    sentence={`1. Іди ___, потім поверни направо. (straight)`}
    answer={`прямо`}
    options={[`прямо`, `наліво`, `направо`, `назад`]}
  />
  <FillInQuestion
...
<FillIn>
  <FillInQuestion
    sentence={`1. Ми їдемо ___ до центру. (by bus)`}
    answer={`на автобусі`}
...
<Unjumble>
  <UnjumbleQuestion
    words={`прямо / Іди / наліво / потім / поверни`}
    answer={`Іди прямо, потім поверни наліво.`}
  />
  <UnjumbleQuestion
    words={`Де / станція / метро`}
    answer={`Де станція метро?`}
  />
  <UnjumbleQuestion
    words={`навпроти / Аптека / банку`}
    answer={`Аптека навпроти банку.`}
  />
  <UnjumbleQuestion
    words={`дістатися / Як / центру / до`}
    answer={`Як дістатися до центру?`}
  />
  <UnjumbleQuestion
    words={`автобусі / Я / на / їду / роботу / на`}
    answer={`Я їду на роботу на автобусі.`}
  />
  <UnjumbleQuestion
    words={`біля / Зупинка / світлофора`}
    answer={`Зупинка біля світлофора.`}
  />
  <UnjumbleQuestion
    words={`вулиці / вздовж / Іди / Шевченка`}
    answer={`Іди вздовж вулиці Шевченка.`}
  />
  <UnjumbleQuestion
    words={`Це / далеко / дуже`}
    answer={`Це дуже далеко.`}
  />
  <UnjumbleQuestion
    words={`перехресті / направо / На / поверніть`}
    answer={`На перехресті поверніть направо.`}
  />
  <UnjumbleQuestion
    words={`пішки / Я / університет / до / ходжу`}
    answer={`Я ходжу до університету пішки.`}
  />
  <UnjumbleQuestion
    words={`між / Музей / театром / парком / і`}
...
<Unjumble>
  <UnjumbleQuestion
    words={`їду / Я / автобусі / на / щодня`}
    answer={`Я їду на автобусі щодня.`}
  />
  <UnjumbleQuestion
    words={`трамваї / вона / на / дістається / роботу / На`}
    answer={`Вона дістається на роботу на трамваї.`}
  />
  <UnjumbleQuestion
    words={`метро / Ми / їдемо / центру / до`}
    answer={`Ми їдемо метро до центру.`}
  />
  <UnjumbleQuestion
    words={`велосипеді / він / на / щодня / Їздить`}
    answer={`Він їздить на велосипеді щодня.`}
  />
  <UnjumbleQuestion
    words={`машині / Вони / на / приїхали`}
    answer={`Вони приїхали на машині.`}
  />
  <UnjumbleQuestion
    words={`університету / до / пішки / Я / ходжу`}
    answer={`Я ходжу до університету пішки.`}
  />
  <UnjumbleQuestion
    words={`таксі / на / додому / Він / їде`}
    answer={`Він їде додому на таксі.`}
  />
  <UnjumbleQuestion
    words={`зупинка / Де / автобуса`}
    answer={`Де зупинка автобуса?`}
  />
  <UnjumbleQuestion
    words={`станція / там / Метро / прямо`}
    answer={`Метро станція там, прямо.`}
  />
  <UnjumbleQuestion
    words={`трамваї / їду / Зазвичай / на / я`}
    answer={`Зазвичай я їду на трамваї.`}
  />
  <UnjumbleQuestion
    words={`світлофора / Поверніть / направо / біля`}
    answer={`Поверніть направо біля світлофора.`}
  />
  <UnjumbleQuestion
    words={`перехресті / На / наліво / ідіть`}
