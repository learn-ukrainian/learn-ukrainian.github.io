"""Hermetic tests for the text-free UzhNU 2023 morphemology/derivatology source intake."""
...
from scripts.projects.open_model_data import phase3_uzhnu_2023_morphemology_derivatology_intake as intake
...
def test_resealed_mutations_fail(path: tuple[object, ...], value: object) -> None:
    ...
    with pytest.raises(intake.Uzhnu2023MorphemologyDerivatologyIntakeError, match=r"body drift|schema violation"):
...
def test_private_audit_rejects_modes_symlink_and_hash_drift(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(intake.Uzhnu2023MorphemologyDerivatologyIntakeError, match="0700"):
        intake.private_audit(stage)
    os.chmod(stage, 0o700)
    os.chmod(stage / first, 0o400)
    with pytest.raises(intake.Uzhnu2023MorphemologyDerivatologyIntakeError, match="0600"):
        intake.private_audit(stage)
    os.chmod(stage / first, 0o600)
    (stage / first).write_bytes(b"drift")
    with pytest.raises(intake.Uzhnu2023MorphemologyDerivatologyIntakeError, match=r"byte drift|hash drift"):
        intake.private_audit(stage)
    ...
    with pytest.raises(intake.Uzhnu2023MorphemologyDerivatologyIntakeError, match=r"symbolic-link|regular file"):
        intake.private_audit(stage)
...
def test_private_audit_rejects_relative_ancestor_symlink(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(intake.Uzhnu2023MorphemologyDerivatologyIntakeError, match="symbolic-link path component"):
...
def test_private_audit_rejects_provider_identity_drift(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(intake.Uzhnu2023MorphemologyDerivatologyIntakeError, match="provider identity drift"):
        intake.private_audit(stage)
...
def test_immutable_public_receipt_and_no_db_surface(tmp_path: Path) -> None:
    ...
    with pytest.raises(intake.Uzhnu2023MorphemologyDerivatologyIntakeError, match="immutable"):
...
def test_optional_live_drive_audit() -> None:
    ...
    except intake.Uzhnu2023MorphemologyDerivatologyIntakeError:
