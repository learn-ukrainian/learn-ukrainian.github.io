...
class TestDiscovery:
    ...
    def test_list_discovery_slugs_reconciles_new_plans(self, tmp_path):
        """Regression (#2535): a track seeded with SOME discovery files must
        still surface plans added later.

        Before the fix, ``list_discovery_slugs`` returned early on the first
        non-empty discovery glob, so plans added after the discovery dir was
        seeded (e.g. bio-181..310 vs the original 180) never surfaced and the
        wiki pipeline could not compile them. The function now reconciles:
        every plan missing a discovery file gets one auto-generated, while
        pre-existing discovery files are left untouched.
        """
        from wiki.sources import list_discovery_slugs
        ...
        discovery_dir = curriculum / track / "discovery"
        discovery_dir.mkdir(parents=True)

        # Three plans exist on disk...
        ...
        # ...but only one was seeded with a discovery file.
        seeded = {
            "discovered_at": "2026-01-01T00:00:00+00:00",
            "query_keywords": ["seeded-alpha"],
            "error": None,
            "rag_chunks": [],
            "rag_literary": [],
        }
        (discovery_dir / "alpha.yaml").write_text(
            yaml.dump(seeded, allow_unicode=True), encoding="utf-8"
        )

        with patch("wiki.sources.CURRICULUM_DIR", curriculum):
            slugs = list_discovery_slugs(track)

        # Every plan slug surfaces — not just the pre-seeded one.
        assert slugs == ["alpha", "beta", "gamma"]
        # The missing discovery files were generated by reconciliation.
        assert (discovery_dir / "beta.yaml").exists()
        assert (discovery_dir / "gamma.yaml").exists()
        # The pre-existing discovery file was preserved, not regenerated.
        kept = yaml.safe_load((discovery_dir / "alpha.yaml").read_text("utf-8"))
        assert kept["query_keywords"] == ["seeded-alpha"]
