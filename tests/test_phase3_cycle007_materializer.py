...
def test_materializer_real_mode_rejects_a_world_readable_source_package(
    ...
    source, output, _rows = _fixture(tmp_path)
    os.chmod(source / "custody-receipt.json", 0o644)  # codeql[py/overly-permissive-file] — intentional world-readable fixture; production path must refuse
...
def test_main_real_mode_config_file_must_be_mode_0600(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    os.chmod(config_path, 0o644)  # codeql[py/overly-permissive-file] — intentional world-readable fixture; production path must refuse
