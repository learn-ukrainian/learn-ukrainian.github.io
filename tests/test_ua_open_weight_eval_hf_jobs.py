...
def test_config_freezes_official_qat_artifact_runtime_and_budget() -> None:
    ...
    assert config["authorization"]["prior_provider_cost_usd"] == 1.530167
    assert config["authorization"]["incurred_provider_costs"][-1] == {
        "job_id": "6a6ff31d6b79c09949c2000b",
        "mode": "canary",
        "provider_billed_minutes": 6,
        "provider_derived_cost_usd": 0.18,
        "provider_running_seconds": 351,
        "stage": "ERROR",
    }
    ...
    assert config["runner"]["structured_outputs_strip_terminal_control_tokens"] is True
...
def test_operator_canary_gate_binds_superseding_cpu_evidence_and_exact_gpu_bundle() -> None:
    ...
    assert gate["prior_provider_cost_usd"] == 1.530167
