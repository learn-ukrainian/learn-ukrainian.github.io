...
def _make_remote_fixture(tmp_path: Path) -> tuple[Path, Path, str, str]:
    """Return a primary clone lagging behind its canonical origin/main."""
...
def _patch_worker_spawn(monkeypatch) -> list[list[str]]:
    """Record non-git worker spawns without breaking subprocess.run git calls."""
...
def _dispatch_args(
    *, task_id: str, worktree: Path, receipt_id: str | None = None, branch: str | None = None
    ...
    command = [
        "dispatch", "--agent", "codex", "--task-id", task_id,
        "--prompt", "fixture", "--mode", "workspace-write",
        "--worktree", str(worktree), "--research-owned-path", RAIL_PATH,
    ]
    if receipt_id:
...
def test_dispatch_fresh_worktree_admits_receipt_bound_to_fetched_origin_tip(
    ...
    """cmd_dispatch must use origin/main, not the lagging primary HEAD."""
    ...
    assert delegate.cmd_dispatch(_dispatch_args(task_id=task_id, worktree=worktree, receipt_id=receipt["receipt_id"])) == 0
...
def test_dispatch_branch_admits_receipt_bound_to_branch_tip_not_primary(
    ...
    """A follow-up branch remains admissible while primary points elsewhere."""
    ...
    assert delegate.cmd_dispatch(
        _dispatch_args(task_id=task_id, worktree=worktree, receipt_id=receipt["receipt_id"], branch=branch),
    ) == 0
...
def test_dispatch_refuses_receipt_bound_to_wrong_base_before_worker_spawn(
    ...
    assert delegate.cmd_dispatch(_dispatch_args(task_id=task_id, worktree=worktree, receipt_id=receipt["receipt_id"])) == 2
...
def test_dispatch_refuses_worker_spawn_when_created_head_differs_from_resolved_base(
    ...
    """The post-create check must run before provisioning or Popen."""
    calls: list[list[str]] = []
    resolved_sha = "a" * 40
    created_sha = "b" * 40
    ...
    args = delegate.build_parser().parse_args(
        [
            "dispatch", "--agent", "codex", "--task-id", "head-verify", "--prompt", "fixture",
            "--mode", "workspace-write", "--worktree", str(worktree),
        ],
    )
...
def test_reused_stale_worktree_refuses_without_rebase_before_admission(
    ...
    """Receipt rejection can never be preceded by a reuse rebase side effect."""
    ...
    with pytest.raises(delegate.WorktreeStaleBase):
        delegate._resolve_worktree_base_sha(
            agent="codex", task_id="stale", raw_path=str(worktree), base="main", branch=None,
        )
...
def test_worktree_creation_uses_resolved_sha_after_remote_ref_moves(
    ...
    """A second remote push after resolution cannot change the created base."""
    ...
    resolved_sha = delegate._resolve_worktree_base_sha(
        agent="codex", task_id="frozen-base", raw_path=str(worktree), base="main", branch=None,
    )
    ...
    created, _branch, telemetry = delegate._ensure_worktree(
        agent="codex", task_id="frozen-base", raw_path=str(worktree), base="main", resolved_base_sha=resolved_sha,
    )
