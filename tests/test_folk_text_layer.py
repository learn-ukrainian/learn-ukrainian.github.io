...
def test_writer_and_archetype_require_folk_text_layer():
    # The folk text-layer rules are gated into SEMINAR_LEVELS prompts via the
    # {SEMINAR_FOLK_WRITER_RULES} token in linear-write.md, sourced from the
    # linear-write-seminar-folk-rules.md partial (kept out of core a1-c2
    # prompts for size). For a folk build the partial is injected, so the
    # composed writer prompt = base template + the injected partial.
    base_prompt = (PROJECT_ROOT / "scripts/build/phases/linear-write.md").read_text(encoding="utf-8")
    assert "{SEMINAR_FOLK_WRITER_RULES}" in base_prompt
    seminar_folk_rules = (
