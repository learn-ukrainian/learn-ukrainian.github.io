...
def test_argument_limit_failure_is_retained_with_portable_guidance(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    invocation_parent = tmp_path / "invocations"
    invocation_parent.mkdir()
    monkeypatch.setattr(runner, "_temporary_parent", lambda: invocation_parent)
...
def test_runner_metadata_imports_and_saved_response_validates(tmp_path: Path) -> None:
    ...
    config = _config(
        tmp_path,
        auth_environment=["FAKE_LOG", "FAKE_MODE", "FAKE_STDIN"],
    )
    script = _fake_script(tmp_path)
    log = tmp_path / "calls.log"
    previous = {
        "FAKE_LOG": os.environ.get("FAKE_LOG"),
        "FAKE_STDIN": os.environ.get("FAKE_STDIN"),
    }
    os.environ.update({"FAKE_LOG": str(log), "FAKE_STDIN": "1"})
    raw = tmp_path / "raw.jsonl"
    output = tmp_path / "output.jsonl"
    metadata = tmp_path / "metadata.json"
    state = tmp_path / "state"
