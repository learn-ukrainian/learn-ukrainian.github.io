...
def test_a10_gate_closed_when_a9_receipt_is_invalid(tmp_path: Path) -> None:
    forged = copy.deepcopy(_cached_json(A9_RECEIPT_PATH))
...
def test_a10_gate_carries_the_upstream_a9_blocked_reason_once_rights_and_slots_clear(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert gate["a9_receipt_valid"] is True
    # A9's own evaluation gate is still closed in _cached_json(A9_RECEIPT_PATH).
    assert gate["upstream_evaluation_slice_ready"] is False
    assert gate["pilot_review_slice_ready"] is False
...
def test_a10_residuals_are_one_typed_entry_per_frozen_slot_never_a_silent_drop() -> None:
    ...
    assert {r["subject_id"] for r in residuals} == set(a10.a9.a8.a7.a6.all_frozen_slot_ids(_cached_json(MANIFEST_PATH)))
...
def test_a10_review_readiness_view_is_unreviewed_plus_residuals_never_a_fabricated_verdict() -> None:
    gate = a10.check_pilot_review_gate()
    residuals = a10.derive_a10_slot_residuals(_cached_json(MANIFEST_PATH), _cached_json(A2_RECEIPT_PATH), gate)
    view = a10.build_review_readiness_view(_cached_json(MANIFEST_PATH), _cached_json(A9_RECEIPT_PATH), residuals)
    assert len(view) == 100
    assert {entry["slot_id"] for entry in view} == set(a10.a9.a8.a7.a6.all_frozen_slot_ids(_cached_json(MANIFEST_PATH)))
...
def test_a10_review_readiness_view_fails_closed_on_a_dropped_slot() -> None:
    ...
    residuals = a10.derive_a10_slot_residuals(_cached_json(MANIFEST_PATH), _cached_json(A2_RECEIPT_PATH), gate)
    with pytest.raises(a10.PilotReviewGateError):
        a10.build_review_readiness_view(_cached_json(MANIFEST_PATH), forged_a9, residuals)
...
def test_a10_required_gate_id_is_the_manifests_own_gate_id_never_invented() -> None:
    assert a10.REQUIRED_GATE_ID in _cached_json(MANIFEST_PATH)["required_gate_ids"]
...
def test_a10_receipt_validates_independently_against_the_real_public_artifacts() -> None:
    assert a10.validate_receipt_independently(_cached_json(RECEIPT)) is None
...
def test_a10_receipt_binds_v4_sha_and_control_surfaces() -> None:
    ...
    assert _cached_json(RECEIPT)["bindings"]["a9_evaluation_package"]["sha256"] == a10.sha256_file(A9_RECEIPT_PATH)
    assert _cached_json(RECEIPT)["bindings"]["pilot_slot_manifest"]["sha256"] == a10.sha256_file(MANIFEST_PATH)
...
def test_a10_receipt_carries_forward_every_a2_a4_a5_a6_a7_a8_a9_residual_unresolved() -> None:
    ...
    for key in (
        ...
        assert all(e["status"] == "unresolved_carried_to_a10" for e in _cached_json(RECEIPT)[key])


def test_a10_receipt_does_not_claim_pilot_review_passed_while_the_gate_is_closed() -> None:
    assert _cached_json(RECEIPT)["review_gate"]["pilot_review_slice_ready"] is False
    assert _cached_json(RECEIPT)["status"] != "PILOT_REVIEW_PASSED"
    assert _cached_json(RECEIPT)["execution_counters"]["slots_review_ready"] == 0
...
def test_a10_receipt_never_claims_a_review_was_executed_or_a_self_review_occurred() -> None:
    ...
    assert all(entry["review_executed"] is False for entry in _cached_json(RECEIPT)["review_readiness_view"])
...
def test_a10_receipt_never_names_source_text_a_held_out_family_or_a_plaintext_source_id() -> None:
    ...
    assert _cached_json(RECEIPT)["a10_residuals"][0]["subject_id"].startswith("v4p-")
