...
def test_commitment_pool_never_invents_a_commitment_not_published_by_a4() -> None:
    forged_a4 = copy.deepcopy(_cached_json(A4_RECEIPT_PATH))
    ...
    with pytest.raises(assignment.SlotAssignmentError):
        assignment.validate_commitment_pool_matches_a4({**_cached_json(RECEIPT), "commitment_pool": pool}, ROOT)
...
def test_assignment_records_are_one_typed_entry_per_frozen_slot_never_a_silent_drop() -> None:
    gate = assignment.check_assignment_gate()
    records = assignment.derive_assignment_records(_cached_json(MANIFEST_PATH), _cached_json(A2_RECEIPT_PATH), gate)
    assert len(records) == 100
    assert len({r["slot_id"] for r in records}) == 100
    assert {r["slot_id"] for r in records} == set(assignment.a7.a6.all_frozen_slot_ids(_cached_json(MANIFEST_PATH)))
...
def test_receipt_validates_independently_against_the_real_public_artifacts() -> None:
    assert assignment.validate_receipt_independently(_cached_json(RECEIPT)) is None
...
def test_receipt_never_claims_dataset_rows_or_stronger_release_state() -> None:
    ...
    assert _cached_json(RECEIPT)["safety_assertions"]["epic_done_claimed"] is False
...
def test_receipt_never_names_source_text_a_held_out_family_or_a_plaintext_source_id() -> None:
    ...
    assert _cached_json(RECEIPT)["assignment_records"][0]["slot_id"].startswith("v4p-")


def test_receipt_never_binds_a_commitment_to_a_slot_no_stratum_commitment_link() -> None:
    assert all(record["commitment_sha256"] is None for record in _cached_json(RECEIPT)["assignment_records"])
    assert _cached_json(RECEIPT)["safety_assertions"]["stratum_to_commitment_binding_published"] is False
    assert _cached_json(RECEIPT)["safety_assertions"]["commitment_bound_to_a_slot"] is False
    assert _cached_json(RECEIPT)["commitment_binding_policy"]["stratum_commitment_binding_ever_public"] is False
