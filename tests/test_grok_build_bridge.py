...
def test_process_grok_build_invokes_native_registry_key(monkeypatch):
    ...
    def fake_invoke(*args, **kwargs):
        ...
        return SimpleNamespace(ok=True, response="native reply", session_id="sid-1", model="grok-4.7")
...
def test_grok_build_branch_review_uses_provisioned_checkout(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        or SimpleNamespace(ok=True, response="reply", session_id=None, model="grok-4.7"),
    )
...
def test_grok_build_dispatch_start_telemetry_has_defaults():
    with patch(
        ...
        for agent_name in ("grok", "grok-build"):
            ...
            assert telemetry.model == "grok-4.7"
