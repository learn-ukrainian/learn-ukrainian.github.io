...
only when realpath returns that same spelling. A symlink, a relative path,
and anything under ``/proc`` (cwd, directory fds) is resolved again on every
call, so re-pointing a symlink or changing cwd cannot be served a stale path.
...
# Absolute spellings realpath did not rewrite. Symlinks and /proc are absent.
...
def _realpath(text: str) -> str | None:
    ...
    try:
        ...
        return None
    if resolved is not None and _rememberable(text) and resolved == os.path.normpath(text):
