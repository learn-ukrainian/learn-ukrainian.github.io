...
| istorio | 3 | 136 | 100% Ukrainian | 3 modules |
...
### HIST (seminar, 100% Ukrainian, history track)

```bash
for n in 1 5 10 15 20; do
  .venv/bin/python scripts/proofread.py hist $n --dry-run --evaluate 2>&1 | tee tests/proofread-results/eval-b2hist-$(printf '%02d' $n).txt
done
```
...
### ISTORIO (seminar, 3 modules)

```bash
for n in 1 2 3; do
  .venv/bin/python scripts/proofread.py istorio $n --dry-run --evaluate 2>&1 | tee tests/proofread-results/eval-c1hist-$(printf '%02d' $n).txt
done
```
...
.venv/bin/python scripts/proofread.py istorio --all --dry-run 2>&1 | tee tests/proofread-results/full-c1hist.txt
...
.venv/bin/python scripts/proofread.py istorio --all --dry-run --evaluate 2>&1 | tee tests/proofread-results/eval-c1hist-batch.txt
...
| C1/ISTORIO/BIO | 2-5 | WORD_SALAD, SOURCE_VERIFICATION, HISTORICAL_INTEGRITY |
