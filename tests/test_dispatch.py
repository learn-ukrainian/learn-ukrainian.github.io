...
class TestDispatchAgent:
    ...
    def test_gemini_dispatch_rate_limit_falls_through_via_agy_to_flash(
        ...
        mock_invoke.side_effect = [
            ...
            RateLimitedError("agy", "gemini-3.8-flash-high", "agy 429"),
            Result(
                ok=True,
                agent="gemini",
        ...
        assert second_call["model"] == "gemini-3.8-flash-high"
...
@patch.dict("os.environ", {}, clear=True)
@patch("agent_runtime.runner.invoke")
def test_codex_dispatch_omitted_model_uses_sol(mock_invoke, tmp_path):
    from agent_runtime.result import Result
    mock_invoke.return_value = Result(
        ok=True, agent="codex", model="gpt-6-sol", mode="workspace-write",
    ...
    assert ok
    assert mock_invoke.call_args.kwargs["model"] == "gpt-6-sol"
