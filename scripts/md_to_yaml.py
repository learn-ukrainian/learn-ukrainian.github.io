...
def parse_cloze(content: str) -> str:
    ...
    while i < len(lines):
        ...
        if num_match:
            ...
            options = [o.strip() for o in options_str.split('|')] if '|' in options_str else [options_str]
...
def extract_activities_section(md_content: str) -> str | None:
...
def convert_md_to_yaml(md_path: Path, output_dir: Path | None = None,
                       dry_run: bool = False, force: bool = False,
                       strip: bool = False) -> Path | None:
    """
    ...
    with open(yaml_path, 'w', encoding='utf-8') as f:
        ...
        f.write("# Generated by md_to_yaml.py\n\n")
...
def convert_directory(dir_path: Path, output_dir: Path | None = None,
