#!/usr/bin/env python3
"""Frontend changed-path scope for CI cheap-exit (#6917 / #6930).

The ``frontend`` job uses this helper after checkout to decide whether the
diff touches the single-source denominator. Out of scope → exit 0 with a
loud auditable decision (job stays ``success``). In scope → continue the
remaining job steps.
