#!/usr/bin/env python3
"""Extract module plans from c1-hist meta.yaml files."""
...
def extract_sequence_from_id(meta_id: str) -> int:
    """Extract sequence number from module ID like 'c1-hist-45'."""
    match = re.search(r'c1-hist-(\d+)', meta_id)
    if match:
...
def convert_meta_to_plan(meta: dict, sequence: int) -> dict:
    ...
    plan = {
        ...
        'focus': meta.get('focus', 'historiography'),
...
def main():
    meta_dir = Path('curriculum/l2-uk-en/c1-hist/meta')
    output_dir = Path('curriculum/l2-uk-en/plans/c1-hist')
