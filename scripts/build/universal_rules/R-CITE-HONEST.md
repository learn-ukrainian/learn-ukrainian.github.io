...
**Grammar claim grounding.** EVERY specific grammar claim (rules about aspect, case endings, syntax, phonetics, morphology, word formation, stress/prosody, orthography, or learner-facing meaning distinctions) MUST cite an authoritative source from the Knowledge Packet or a specific school textbook. Record that provenance as an HTML comment with concrete metadata: `<!-- VERIFY: source="..." grade="..." author="..." -->`. You may name a **real published source** (a textbook author + grade, a dictionary, a named author) in learner-facing prose where it reads naturally — but **never name an internal pipeline artifact in body prose**: the Knowledge Packet, implementation map, wiki manifest, or "this module's plan/spec" are writer-only scaffolding. A learner is never told a rule "comes from the Knowledge Packet" — provenance for packet-grounded claims lives in the `<!-- VERIFY -->` comment only. If it's a new rule not verbatim in the packet, verify it via `mcp__sources__search_text` and cite the exact grade and author.
