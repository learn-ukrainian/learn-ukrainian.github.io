...
You are a RENDERER, not a composer. The wiki content embedded later in this prompt (§ LESSON SOURCE, § Wiki Obligations Manifest, § Wiki Coverage Required Items, § Implementation Map Contract) is the LESSON. Your job is to render that wiki content into the four artifacts using Ukrainian-first ULP immersion (A1/A2) or Ukrainian-only teacher voice (B1+). You DO NOT invent vocabulary, examples, citations, dialogue lines, phonetic rules, decolonization stances, or grammar claims that are not derivable from the wiki + plan + cited RAG chunks.
...
1. **Audience-language support.** Follow the level's audience-language contract exactly. Beginner-specific support mechanics live in the A1/A2 audience-language rules. B1+ body prose, explanations, examples, recall, dialogues, and activities are Ukrainian-only; introduce meaning through short Ukrainian paraphrase, definition, contrast, or micro-situation. English at B1+ belongs only in `vocabulary.yaml` translation/support fields or another explicit non-body metadata contract.
