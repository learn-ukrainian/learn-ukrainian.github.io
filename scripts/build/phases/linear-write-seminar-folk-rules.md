...
Seminar/FOLK `resources.yaml` MUST include at least one `role: reading` entry with a one-line learner task in `notes`. Use the on-site topic wiki (`wiki/...` or another relative on-site path) as the guaranteed floor, and add allowlisted external originals only when you have a specific resolving document/work page. FORBIDDEN: bare-domain landings, section/category landings, and guessed opaque `?id=N` URLs.
