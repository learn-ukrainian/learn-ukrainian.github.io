...
<!-- rule_id: #R-FOLK-PUBLIC-REGISTER -->
FOLK learner pages are Ukrainian-first seminar pages: no English UI labels such as `Texts you'll read`, no rendered `title_en` glosses in module reading lists, and no English CEFR/register labels such as `C1+ learner`. Reading lists show Ukrainian title, Ukrainian genre, and public reading link only. Every written FOLK module must pass a student-read fluency gate: read the rendered page top-to-bottom and fix broken syntax, sentence splicing, word salad, repetitive proof/source boilerplate, or unnatural Ukrainian before review. Source discipline must appear as coherent literary analysis grounded in public texts, not as meta-explanations of the validation workflow.

**FOLK public learner register hard stop.** Source verification is build/PR discipline, not student content. In `module.md`, activities, vocabulary, resources, generated FOLK MDX, and routable reading pages, do not expose build/process labels or validation-tool terms: `source hammer`, `source-first`, `learner-facing`, `chunk_id`, `source_chunk`, `corpus ID`, `service ID`, `verify_quote`, `hosted reading`, `public reading`, `lesson body`, or Ukrainian equivalents such as `службова позначка`, `внутрішній корпусний ідентифікатор`, `технічна інфраструктура джерел`. Public pages may show public source names and URLs. Describe source work in Ukrainian student terms: `публічний текст`, `першоджерело`, `уривок`, `джерельна сторінка`, `уважне читання`, `зіставлення джерел`. Never teach the validation workflow.
...
**FOLK FRAMING STANDARD (hard, religious-sensitivity + decolonization rule — the full standard is `docs/folk-epic/FOLK-FRAMING-STANDARD.md`).** Ukrainian folk culture is the heritage of a **Christian nation (1000+ years)**, taught as the **oral foundation of WRITTEN Ukrainian literature** (a pre-literature course) — myths, legends, songs, verbal art as **cultural/artistic heritage**. Four pillars bind every folk module:

1. **Pre-literature** — frame folklore as the oral root of the written canon (дума → Shevchenko's epic voice; казка/легенда → the literary fairy tale; пісня → the lyric), not as a worldview manual or standalone ethnography.
2. **Christian-heritage-first** — where a tradition is Christian, the Christian meaning gets **parity/primacy**. колядки/щедрівки = **Christmas/Nativity carols**, present them as **величально-побажальні** (praise + well-wishing); щедрування is an Eastern-rite folk-Christian tradition. **God created the world** — a pagan cosmogony is NEVER presented as *the* creation account.
3. **Written-text base + school-canonical sourcing** — build on verifiable text from the school canon (gr 5–8 ukrlit textbooks in `data/sources.db` = primary; ukrlib/wikisource = secondary). Host the **standard school form**, not archaic-dialect variants where a codified form exists.
4. **No occult / no pagan-as-held-belief** — never present demonology/occultism/witchcraft/spell-craft as real practice (the Russian "Ukrainians-as-witches/характерники" smear); pre-Christian motifs only as minor poetic/folk-art imagery or an explicitly-scholarly older layer, never doctrine, never foregrounded.

**NEVER** describe колядки/щедрівки (or any Christian-folk tradition — Великдень, Різдво, обжинки with a Christian layer) as «прикладна магія», a "magic" act, or with a «дохристиянське ядро / християнська оболонка (нашарування, шар)» pagan-core-Christian-veneer structure. That framing offends religious learners and reproduces the Soviet-atheist "pagan-survival" distortion (a known writer-bias trap, not a neutral scholarly default). If you mention the "magic/pagan-survival" reading at all, attribute it explicitly as a contested or Soviet-era frame and balance it — do not assert it as fact. Where older agrarian strata are genuinely relevant (spring веснянки/гаївки, купальські), treat them as **older poetic motifs/formulae in cited scholarly notes — never as the learner-facing thesis, never as "magic" the rite performs.** The scholarly term `продукувальна магія` may appear ONLY inside such a cited note, NEVER as framing and NEVER for колядки/щедрівки (= Christmas/Nativity and Щедрий-вечір/New-Year well-wishing songs). (Origin: teacher feedback / user directive 2026-06-25; supersedes the partial fix from user flag 2026-06-23 + PR #3745.)
...
5. **Keep the verbatim-quote convention for non-expository dialectal fragments.** Song, duma, or ritual fragments may contain authentic archaic/dialectal forms inside cited blockquotes or the module's verbatim-quote convention; do not turn unattested fragments into bare teacher prose. Hosted Хрестоматія MDX under `site/src/content/readings/` is different: it must pass full text via ``<PrimaryReading text={`...`} />`` so `ReadingText` renders the lines, never blockquote every line, and must include a public `source_url` when one exists plus explicit `published:` and `canonical:` frontmatter.
