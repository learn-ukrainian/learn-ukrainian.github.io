...
- If the contract has `activity_obligations`, does every obligation appear at least once as a marker? Order is incidental.
...
Missing obligation or type mismatch = deduct in Dimension 5.
...
| 7 | **Structural integrity** | 5% | DEDUCT for: missing H2 headings from plan, duplicate summary sections, meta-commentary sections ("Content notes:"), word count BELOW {WORD_TARGET} (more content is always welcome — never penalize for exceeding the target), dangling/incomplete sentences, stray tags or formatting artifacts. REWARD for: clean markdown, all sections present and ordered correctly, word count at or above target. |
...
If your verdict is **REVISE**, you MUST output a `<fixes>` block with exact find/replace pairs. The pipeline applies these deterministically. If a problem cannot be fixed safely with deterministic surface edits, do **not** invent a section-rewrite protocol; state the limitation clearly in findings and use **REJECT** when the module needs a real rewrite.
...
**The `<fixes>` block is REQUIRED for REVISE verdicts.** Without it, the pipeline cannot apply your fixes. For PASS verdicts, omit it. For REJECT verdicts, the module needs a full rebuild, so `<fixes>` is optional.
