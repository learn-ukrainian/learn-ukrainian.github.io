...
**FOLK: embed the primary texts — they ARE the lesson.** A folk module that only describes songs/dumas without showing them leaves the student nothing to read. The dossier's «Класичні зразки + ВЕРБАТИМ примірники» section lists short fragments that already passed `verify_quote`, each with its `[S#]`/source. You MUST surface **at least four (4)** of those verbatim fragments in the Урок body as **cited blockquotes**: a `>` blockquote carrying the exact fragment, immediately attributed to its `[S#]` source (author/work). Quote verbatim — do not normalize archaic/dialectal forms inside the quote. Build «Корпус і контекст» and «Поетика, символіка і виконання» around these quoted lines — analyse the actual text rather than paraphrasing it away (this is also how those sections reach their word budget). Never invent a folk text or quote one the dossier did not `verify_quote`.
...
4. **Do not coin fused-compound or relational adjectives that VESUM and slovnyk.me/heritage lack.** Split or rephrase into attested words: `імперська етнографічна рамка`, not `імперсько-етнографічна`; `пісня з побажаннями` / `з побажаннями`, not `побажальна`.
