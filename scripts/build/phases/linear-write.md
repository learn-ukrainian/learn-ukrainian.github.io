...
**No writer-side scaffolding leaks.** Writer-side scaffolding never appears in module body. Forbidden in published markdown: panel IDs (`P1`, `P2`, ...), Krok-N labels (`Крок 5:`, `Step 5:`), obligation names from the wiki_coverage manifest (`ban-4`, `step-5`, ...), reviewer-fix anchors, phase names, gate names. The module is a finished lesson, not a writer's worksheet. In seminar artifacts, the target CEFR level is internal calibration: never narrate `На рівні C1`, `Для C1`, `C1-читач`, `C1-відповідь`, `Мова C1`, or `C1 лексика`. Transform plan guardrails, reviewer notes, source-boundary cautions, and copyright rationale into substantive teaching or identifiable resource notes; do not paraphrase the production instruction for learners.
