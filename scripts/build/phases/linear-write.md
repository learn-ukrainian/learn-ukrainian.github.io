...
**1. Verify every Cyrillic word form in VESUM (exhaustive, not sampled).** Before finalizing, batch-verify every distinct Cyrillic word form in `module.md`, `activities.yaml`, `vocabulary.yaml`, and `resources.yaml` via `mcp__sources__verify_words`, except intentional bad-form marker spans and parser-excluded error fields (`error:` / `errorWord:`). This must include every Cyrillic content word, especially high-risk derived adjectives (`-ний` / `-овий` / `-альний` / `-увальний`), abstract `-ість` nouns, agent nouns, and every hyphenated compound; checking a sample is insufficient. For ANY miss that is not verified as heritage-attested via `mcp__sources__search_slovnyk_me` / `mcp__sources__search_heritage` with `is_russianism == false`, replace it with an attested synonym or rephrase before emitting; never ship an unverified content word. An unverified content word is a build failure. Before emitting any personal reflexive `-ся` form, verify it; traps such as `пити → *п'юся`, `читати → *читаюся`, and `писати → *пишуся` fail unless VESUM says otherwise.

<!-- rule_id: #R-VESUM-FIRST-PASS-CRITICAL -->
**VESUM vocabulary attestation is first-pass-critical.** The `python_qg` correction loop performs LITERAL find/replace only: it cannot rephrase a coined word or jargon borrowing into a standard synonym. A single unattested word fails `vesum_verified`, and a failed literal correction can delete content and then fail `word_count` too. Do not rely on correction to fix coinages, jargon, or unattested compounds; verify and replace/rephrase them before the first emitted artifacts.
