    ...
    for activity in draft.get("activities") or []:
        ...
        for idx, item in enumerate(activity.get("items") or []):
            ...
            if typ == "error-correction":
                ...
                corr = item.get("correction", item.get("answer"))
            ...
            if typ in {"quiz", "multiple-choice"}:
                ...
                has_dict = any(isinstance(opt, dict) and "correct" in opt for opt in options)

                if has_dict and not has_int and not has_str:
                    if len(dict_correct) == 0:
                        return failure(4, "answer_key_missing", "writer", activity=aid, token=str(idx)), {}
                    if len(dict_correct) > 1:
                        return failure(4, "answer_key_ambiguous", "writer", activity=aid, token=str(idx)), {}
                elif not has_int and not has_str and len(dict_correct) == 0:
                    return failure(4, "answer_key_missing", "writer", activity=aid, token=str(idx)), {}
...
def run_lesson(level: str, slug: str, n: int, *, draft: dict[str, Any], plan: dict[str, Any],
    ...
    if state_dir is not None and form_options:
        prov_doc = assembled.artifacts.get("provenance")
        if prov_doc:
            prov_doc, _ = rewrite_printable_provenance(state_dir, n, prov_doc, form_options)
            assembled.artifacts["provenance"] = prov_doc
    try:
