...
async function main(): Promise<void> {
  ...
  await connect(
    async (client) => {
      ...
      // 4. Test Gate
      const test = base.withDirectory('.', source).withExec(['npm', 'test']).sync(); // Execute

      console.log('✅ Tests passed');

      // 5. Build/Generate Gate (Verify it builds)
      const build = base.withDirectory('.', source).withExec(['npm', 'run', 'generate:all']).sync();
      ...
      const audit = base
        ...
        .withExec([
          'sh',
          '-c',
          'find curriculum/l2-uk-en -name "module-*.md" -print0 | xargs -0 -n 1 python3 scripts/audit_module.py',
        ])
        .sync();
