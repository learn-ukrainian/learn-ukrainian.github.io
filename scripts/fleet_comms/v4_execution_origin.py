...
def load_a3_packet_commitment() -> str:
    ...
    _require(isinstance(digest, str) and bool(HEX64_RE.match(digest)), "A3 packet receipt carries no packet_commitment_sha256 -- refusing")
    return digest
...
def derive_harness(resolved_recipient: str) -> str:
    ...
    _require(candidate in KNOWN_HARNESS_EXECUTABLES, f"recipient {resolved_recipient!r} is not a canonical V4 harness -- refusing")
    return candidate
...
def executable_name(review_cmd: list[str]) -> str:
    _require(bool(review_cmd) and isinstance(review_cmd[0], str) and review_cmd[0], "post-isolation argv has no executable -- refusing")
...
def blindness_from_prompt_profile(*, prompt_profile: str, transported_digest: str, authorized_digest: str) -> tuple[bool, bool, bool]:
    ...
    """
    _require(transported_digest == authorized_digest, "transported prompt digest does not match the authorized source-blind prompt -- refusing")
    _require(
