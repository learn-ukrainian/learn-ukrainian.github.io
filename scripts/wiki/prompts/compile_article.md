...
{dossier_section}## Матеріал джерел
...
**ПОВНОТА ІНЛАЙНОВИХ ПОСИЛАНЬ (критично — найчастіша причина провалу source_grounding).** Кожне ЗМІСТОВНЕ речення мусить нести інлайнове `[S#]` — НЕ лише прямі факти, а й:
- **синтез, узагальнення, місточки** («билини й думи стоять поруч зі світовими епосами…», «київський цикл окреслює…») — постав `[S#]` того джерела, що це підтверджує;
- **інтерпретаційні твердження** («княжий бенкет — це топос, а не побутова замальовка», «антагоністи є моделями історичного пограниччя») — або дай `[S#]`, або сформулюй як обережну атрибуцію («за такою-то рамкою…»), або постав `<!-- VERIFY -->`. НЕ лишай інтерпретацію без жодної опори;
- **перше речення кожного абзацу й кожен пункт таблиці** — це найчастіше «провисають» без посилання.
Перед здачею перечитай статтю абзац за абзацем: якщо змістовне речення не має ні `[S#]`, ні хеджа, ні `<!-- VERIFY -->` — додай посилання на конкретний доступний `[S#]` (усі потрібні наукові фрагменти вже є в наборі джерел) або переформулюй. Атрибуй точно: не приписуй джерелу формулювання, якого в його фрагменті немає (це MISATTRIBUTION), і не розширюй твердження понад те, що фрагмент стверджує (це OVERCLAIM).
...
- політично чи деколонізаційно спірне, **де українські джерела самі між собою не збігаються** (обставини репресій, колаборація, оспорюване авторство, дискусійні дати страти / смерті);
...
### Регістрова гігієна
- Не вживай англійський імпорт `вербатимний`. Пиши `дослівний`, `дослівна цитата`, `дослівний фрагмент`, `дослівне передання`.
- Не калькуй `приближення`: у звичайній прозі пиши `наближення`, `підхід`, `приблизний опис` залежно від змісту. `Приближення` доречне лише як спеціальний математичний термін.
- Не вживай `виступати` як копулу. Замість «X виступає ознакою / джерелом / доказом» пиши «X є ознакою», «X постає як джерело», «X слугує доказом».
- Уникай русизмів і кальок у власній прозі: `являти собою`, `в якості`, `на протязі`, `приймати участь`, `слідуючий`, `діючий`, `відноситися до`, `було встановлено`.
- Історичні цитати передавай дослівно, але не наслідуй їхні архаїчні чи русифіковані форми у власній авторській прозі.
