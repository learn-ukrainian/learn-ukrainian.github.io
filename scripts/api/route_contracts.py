...
ROUTE_CONTRACTS: tuple[RouteContract, ...] = (
    ...
    RouteContract(
        ...
        "exact",
        "http",
        "Opaque host occupancy: load fields, sanitized occupants {kind, agent, task_id, epic, optional instance_id}, and burn_state in {active, idle, unknown}; burn_sources has exactly atlas_job, driver, and foundry, each with state in {active, clear, unknown} and non-negative observation_age_s. Observer heartbeats partition by opaque host, with dynamic observer-only mac-operator and legacy cloud-observer behavior.",
        "In-process atlas-jobs load cache plus local atlas-jobs registry, local session-stream driver leases, optional occupancy markers, and the in-process observer presence store keyed by (host, agent, instance).",
    ...
    RouteContract(
        ...
        "exact",
        "http",
        "Loopback-only ingest for sanitized per-host project-state documents from remote reporters; schema v2 adds required workers block semantics (missing workers => unreported, [] => verified zero).",
