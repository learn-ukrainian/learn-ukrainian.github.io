...
ROUTE_CONTRACTS: tuple[RouteContract, ...] = (
    ...
    RouteContract(
        ...
        "prefix",
        "http",
        "Unified read-only observer for durable fleet-comms requests, authority queue jobs, messages, discussions, ACP rounds, formal reviews, endpoints, migrations, dead letters, and provenance.",
        "Fleet-comms v1 SQLite read models, endpoint registry metadata, and sanitized runtime/ACP projections.",
...
PAGE_CONTRACTS: tuple[PageContract, ...] = (
    ...
    PageContract(
        "fleet.html",
        "/fleet.html",
        "Consolidated read-only fleet-comms observer.",
        "/api/fleet/* over the existing durable-plane SQLite read models, endpoint registry, and sanitized runtime/ACP evidence.",
    ...
    PageContract(
        "channels.html",
        "/channels.html",
        "Read-only canonical channel bridge observer.",
        "/api/comms/channels*.",
        "Live broker reads; client preserves deeplink state.",
        ("humans", "agents"),
        "Replaces legacy comms chat for browser observation; sending remains in TUIs and project CLIs.",
        "low",
        "keep canonical",
    ),
    PageContract(
        "comms.html",
        "/comms.html",
        "Read-only legacy broker diagnostics, zombie, and batch progress UI.",
        "Deprecated legacy message reads plus broker/process health; build events use their dedicated page.",
        "Live broker/process reads.",
        ("humans",),
        "Keeps operational visibility without browser message composition; legacy APIs remain for CLI callers.",
        "medium",
        "retain as read-only legacy operations page until old users/CLI migrate",
    ),
    PageContract(
        "delegate.html",
        "/delegate.html",
