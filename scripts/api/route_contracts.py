...
ROUTE_CONTRACTS: tuple[RouteContract, ...] = (
    ...
    RouteContract(
        ...
        "Agent runtime inventory, usage, default-model headroom, Headroom proxy stats, recent calls, and auth mode.",
        "Runtime config, usage JSONL, local Headroom proxy stats/health, environment presence booleans, and OAuth credential presence.",
        "No route cache; live file/env reads plus bounded localhost Headroom probes. Auth reports booleans/source names only, never secret values.",
        ("runtime.html", "headroom.html", "routing.html", "index.html", "agents"),
...
PAGE_CONTRACTS: tuple[PageContract, ...] = (
    ...
    PageContract(
        "headroom.html", "/headroom.html",
        "Headroom proxy savings and health dashboard.",
        "/api/runtime/headroom/stats.",
        "Live localhost proxy probe with 5s client timeout and bounded server-side timeout.",
        ("humans",),
        "Complements runtime and cost dashboards.",
        "low; unavailable proxy returns a degraded JSON payload instead of a 5xx",
        "keep",
    ),
    PageContract(
        "cost.html", "/cost.html",
