...
ROUTE_CONTRACTS: tuple[RouteContract, ...] = (
    ...
    RouteContract(
        ...
        "exact",
        "http",
        "Opaque host occupancy: load fields, sanitized occupants {kind, agent, task_id, epic, optional instance_id}, burn_state in {active, idle, unknown}, top-level attention items empty_host_underused:<host_id> and empty_host_unknown_capacity:<host_id>; burn_sources has exactly atlas_job, driver, foundry, service, and observer, each with state in {active, clear, unknown} and non-negative observation_age_s. Default glance always includes host-teacher and observer-only mac-operator (host-job only when mapped); empty-map Linux API fills host-teacher in-process; live observer heartbeats merge into that Mac row, and legacy cloud-observer stays presence-gated with honest presence age.",
