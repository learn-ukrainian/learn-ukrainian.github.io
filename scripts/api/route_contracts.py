...
ROUTE_CONTRACTS: tuple[RouteContract, ...] = (
    ...
    RouteContract(
        ...
        "exact",
        "http",
        "Loopback-only ingest for sanitized per-host project-state documents from remote reporters.",
        "In-process TTL store keyed by opaque host_id; reporter must be allowlisted via MONITOR_OCCUPANCY_HOST_IDS or mac-operator.",
        "Upsert immediately; row expires after 15 minutes without a report; forbidden paths/hostnames/ports rejected with 400.",
        ("operators", "dispatch", "tests"),
        "Push complement to GET /api/fleet/projects/v1; API host self-reports in-process without POST.",
        "low with loopback-only mutation and a short TTL",
        "keep",
        mutates=True,
    ),
...
PAGE_CONTRACTS: tuple[PageContract, ...] = (
    ...
    PageContract(
        ...
        "/api/fleet/* over the existing durable-plane SQLite read models, endpoint registry, sanitized runtime/ACP evidence, bounded legacy operations read models, GET /api/occupancy for cloud-observer presence, and GET /api/fleet/projects/v1 for per-host checkout and serving-SHA drift.",
        "Client fetches bounded read-only projections with a 5s timeout, including occupancy and project-state panels; no client mutation controls or artifact-body retrieval.",
        ("humans", "operators"),
