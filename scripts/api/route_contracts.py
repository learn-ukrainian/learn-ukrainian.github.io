...
ROUTE_CONTRACTS: tuple[RouteContract, ...] = (
    ...
    RouteContract(
        ...
        "prefix",
        "http",
        "Tool timing, module-build token, and body-free legacy-route telemetry ingest/readback.",
        "Telemetry SQLite state used by Monitor instrumentation and module-build PR reporting.",
        "No route cache; POST mutates telemetry stores, GET reads current telemetry state. Legacy-route usage is retained only as hourly allowlisted aggregates for 90 days.",
        ("agents", "runtime tooling"),
        "Complements optional telemetry footer, runtime usage logs, analytics cost summaries, and legacy comms retirement evidence.",
        "low",
        "keep",
    ),
    RouteContract(
