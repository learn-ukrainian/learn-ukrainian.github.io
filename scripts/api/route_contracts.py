...
ROUTE_CONTRACTS: tuple[RouteContract, ...] = (
    ...
    RouteContract(
        ...
        "exact",
        "http",
        "Opaque host occupancy: load fields plus occupants {kind, agent, task_id, epic}.",
        "In-process atlas-jobs load cache plus local atlas-jobs registry; host keys from MONITOR_OCCUPANCY_HOST_IDS.",
        "Stale-while-revalidate via the existing load cache (fresh <=30s, stale <=300s); never probes SSH on the request path.",
        ("operators", "dispatch", "tests"),
        "Reuses GET /api/atlas-jobs/load cache; does not add a second probe board.",
        "low",
        "keep",
        mutates=False,
    ),
...
PAGE_CONTRACTS: tuple[PageContract, ...] = (
    ...
    PageContract(
        ...
        "/api/fleet/* over the existing durable-plane SQLite read models, endpoint registry, sanitized runtime/ACP evidence, and bounded legacy operations read models.",
        "Client fetches bounded read-only projections with a 5s timeout; no client mutation controls or artifact-body retrieval.",
        ("humans", "operators"),
