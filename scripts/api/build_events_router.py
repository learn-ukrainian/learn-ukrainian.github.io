...
def _scan_dispatch_meta_files(
    ...
    *,
    ...
    paths = sorted(
        ctx.roots.curriculum_root.glob(pattern),
...
def active_build_events(*, level: str | None = None, ctx: MonitorContext) -> dict[str, Any]:
    ...
    for (track, slug), (_path, meta) in _latest_dispatch_meta_by_module(level=level, ctx=ctx).items():
        ...
        if ts is None or ts < cutoff:
            continue
        state_path = safe_join(ctx.roots.curriculum_root / track / "orchestration" / slug, "state.json")
