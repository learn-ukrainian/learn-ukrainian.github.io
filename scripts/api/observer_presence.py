...
Grok Bot / QA Engineer heartbeats land here so occupancy can show the current
task without ``POST /api/agent-monitor/register``. In-process TTL store; a
Monitor restart drops rows (callers heartbeat). Never writes fleet-comms.
"""
...
ALLOWED_AGENTS = frozenset({"grok-bot", "qa-engineer"})
