...
_SUMMARY_OK = re.compile(r"^[A-Za-z0-9][A-Za-z0-9 .,'!?:;()/_-]{0,159}$")
...
def safe_summary(value: Any) -> str | None:
    """Short current-work text: no secrets, addresses, aliases, or filesystem paths."""
    if value is None:
        return None
...
def occupant(
    *,
    ...
    if kind == "observer":
        ...
        row["summary"] = safe_summary(summary)
    return row
