...
task id is the one derived for this unit and attempt, its ``prompt_sha256`` is that of the task file rendered here,
...
def check_dispatch_binding(
    ...
    with either would run under instructions this module never rendered: refused. The worktree block is generated by
    delegate from the worktree path alone, so it is the one block allowed. A record without the effective hash or
    the block list predates the binding and cannot prove what ran.
    """
