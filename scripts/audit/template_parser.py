...
def resolve_template(module_id: str, meta: dict) -> str:
    ...
    # Extract level from module_id - handle track levels like hist, bio
    ...
    if len(parts) >= 2 and parts[0] in ['a1', 'a2', 'b1', 'b2', 'c1', 'c2']:
        ...
        module_level = f"{parts[0]}-{parts[1]}" if parts[1] in ['hist', 'bio', 'pro'] else parts[0]
    elif parts[0] == 'lit':
        module_level = 'lit'
    else:
...
def parse_template(template_path: str) -> TemplateStructure | None:
    """
