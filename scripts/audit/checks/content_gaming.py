...
def check_section_balance(content: str, file_path: str = '') -> list[dict]:
    ...
    # Exclude enrichment content (Словник, Ресурси tabs) from balance check.
    # IMPORTANT: find("<!-- TAB:") matches TAB:Урок at pos 0 → empty content.
    # Find Словник or Ресурси specifically.
    slovnyk_idx = content.find("<!-- TAB:Словник -->")
    resursy_idx = content.find("<!-- TAB:Ресурси -->")
    if slovnyk_idx > 0:
        prose_content = content[:slovnyk_idx]
    elif resursy_idx > 0:
        prose_content = content[:resursy_idx]
    else:
        prose_content = content

    # Get narrative zones only
    narrative_zones = _split_narrative_zones(prose_content)
    narrative_text = '\n'.join(narrative_zones)
...
_UKRAINIAN_STOP_WORDS = frozenset({
    ...
    'і', 'й', 'а', 'але', 'або', 'чи', 'що', 'як', 'бо', 'не', 'ні', 'так', 'ще',
