...
def _resolve_vital_status_names(
    alt_names: list[str],
    vital_status: str,
    template: TemplateStructure,
) -> tuple[str | None, str | None]:
    """Determine preferred/forbidden section names based on vital status for biographies.

    Returns (preferred_name, forbidden_name).
    """
    if len(alt_names) <= 1 or template.level != 'c1' or 'biography' not in template.template_name:
        return None, None
    ...
    if 'living' in vital_status:
        if 'Сучасний етап' in alt_names:
            preferred_name = 'Сучасний етап'
        if 'Останні роки' in alt_names:
            forbidden_name = 'Останні роки'
        if 'Вплив' in alt_names:
            preferred_name = 'Вплив'
        if 'Спадщина' in alt_names:
            forbidden_name = 'Спадщина'
    else:
        if 'Останні роки' in alt_names:
            preferred_name = 'Останні роки'
        if 'Сучасний етап' in alt_names:
            forbidden_name = 'Сучасний етап'
        if 'Спадщина' in alt_names:
            preferred_name = 'Спадщина'
        if 'Вплив' in alt_names:
            forbidden_name = 'Вплив'

    return preferred_name, forbidden_name
...
def _find_matching_sections(
    ...
    """Find sections matching any of the alternative names for a required section."""
    found_alts = []
...
def _check_forbidden_tone(
    forbidden_name: str | None,
    preferred_name: str | None,
    vital_status: str,
    sections_data: list[dict],
) -> list[dict]:
    """Check if a forbidden header name (based on vital status) is used."""
    violations = []
...
def _is_sidecar_exempt_section(alt_names: list[str]) -> bool:
    """Check if a missing section is satisfied by sidecar files (vocab, activities, resources)."""
    normalized_name = alt_names[0].lower()
    is_vocab = any(v in normalized_name for v in ['словник', 'vocabulary', 'vocab'])
    is_activities = any(a in normalized_name for a in ['вправи', 'activities', 'practice'])
    is_resources = any(r in normalized_name for r in ['ресурси', 'resources', 'практики?'])
    return is_vocab or is_activities or is_resources


def _check_duplicate_sections(
    found_alts: list[dict],
    required: str,
    template: TemplateStructure,
) -> list[dict]:
    """Check for duplicate synonymous headers matching the same required section."""
    violations = []
    is_metalanguage = 'metalanguage' in template.template_name.lower()
    if len(found_alts) <= 1 or is_metalanguage:
        return violations
...
def _check_empty_sections(
    ...
    """Check for empty required sections (allowing parent containers with sub-sections)."""
    violations = []
    for section in found_alts:
...
def _check_required_sections(content: str, meta: dict, template: TemplateStructure) -> list[dict]:
    ...
    for required in template.required_sections:
        ...
        preferred_name, forbidden_name = _resolve_vital_status_names(
            alt_names, vital_status, template,
        )

        found_alts = _find_matching_sections(alt_names, required, sections_data, template)

        violations.extend(_check_forbidden_tone(
            forbidden_name, preferred_name, vital_status, sections_data,
        ))

        if not found_alts:
            if _is_sidecar_exempt_section(alt_names):
                continue
            ...
            violations.extend(_check_duplicate_sections(found_alts, required, template))
            violations.extend(_check_empty_sections(found_alts, sections_data))
