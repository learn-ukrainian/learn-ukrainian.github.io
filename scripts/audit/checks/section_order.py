...
OPTIONAL_SECTIONS = {'self_assessment', 'external', 'introduction', 'cultural', 'reading',
                     'warm_up', 'presentation', 'practice', 'review', 'checklist'}
...
def get_section_type(header: str) -> Optional[str]:
    ...
    for section_type, aliases in SECTION_ALIASES.items():
        for alias in aliases:
            if header_clean.startswith(alias):
                return section_type
    return None
...
def get_section_order_summary(content: str) -> dict:
    ...
    sections = parse_sections(content)
    return {
        'sections': [{'type': s.section_type, 'header': s.header.strip(), 'line': s.line_num}
                     for s in sections],
