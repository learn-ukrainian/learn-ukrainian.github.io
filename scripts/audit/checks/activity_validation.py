...
def _validate_morpheme_match(match, text: str, title: str) -> list:
    ...
    plain_text = re.sub(
        r"[а-яіїєґА-ЯІЇЄҐ]*\*([а-яіїєґА-ЯІЇЄҐ]+)\*[а-яіїєґА-ЯІЇЄҐ]*", lambda m: m.group(0).replace("*", ""), text
    )
    ...
    for idx, activity in enumerate(yaml_activities):
        if _get_activity_attr(activity, "type", "") == "reading":
            ...
            if act_id:
                ...
                missing_id_titles.append(_get_activity_attr(activity, "title", f"Activity {idx + 1}"))
                ...
                violations.append(
                    {
                        ...
                        "suggestion": (
                            f"Set min_correct: {actual_correct} OR mark exactly {min_correct} options as correct: true."
                        ),
                    }
                )
                ...
                violations.append(
                    {
                        ...
                        "severity": "critical",
                        "activity": title,
                        "message": (f"Question {idx} has {correct_count} correct options (must be exactly 1)."),
                ...
                violations.append(
                    {
                        ...
                        "severity": "critical",
                        "activity": title,
                        "message": (f"Item {idx} has {correct_count} correct options (must be exactly 1)."),
...
_POSSESSIVE_DATIVE_FORMS = {
    ...
    "нашій",
    "вашій",
    "своїй",  # singular feminine dative
    "моєму",
    "твоєму",
    "нашому",
    "вашому",
    "своєму",  # singular masc/neut dative
    "моїм",
    "твоїм",
    "нашим",
    "вашим",
...
def check_duplicate_options(yaml_activities: list) -> list:
    ...
    for activity in yaml_activities:
        ...
        for idx, item in enumerate(items, 1):
            ...
            str_options = [
                str(o).strip().lower() if not isinstance(o, dict) else str(o.get("text", "")).strip().lower()
                for o in options
            ]
...
NON_CASE_POS = {"prep", "conj", "part", "adv", "advp", "intj", "noninfl"}
...
CASE_DRILL_PROMPT_PATTERNS = [
    ...
    re.compile(r"\bяко?м(у|ий|ого)\s+відмінк(у|ок|а)\b", re.IGNORECASE),
]


def _is_case_drill_prompt(text: str) -> bool:
    """Check if prompt text matches case-drill patterns."""
    if not text or not isinstance(text, str):
        return False
    return any(p.search(text) for p in CASE_DRILL_PROMPT_PATTERNS)


def _check_word_indeclinable(
    word: str,
    ...
    Returns (is_indeclinable, pos_or_reason).
    """
...
def check_indeclinable_case_drills(
    ...
    """Check that case-drill prompts are not applied to indeclinables or non-case POS in VESUM.

    Issue #5917 audit gate.
    Hard-fails when:
    1. Prompt matches case-drill patterns ("correct case", "відмінок", etc.).
    2. Target word/lemma is an indeclinable or non-casing POS (prep, conj, part, adv, etc.,
       or has only one distinct form in VESUM).
    3. Numerals (numr) are NOT blanket-excluded because they decline.
    """
    violations = []
