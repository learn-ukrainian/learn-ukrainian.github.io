...
def convert_checkpoint(md_path: Path) -> bool:
    ...
    if not activities_section:
        print(f"⚠️  No activities found in {md_path.name}")
        return False
    
    # Parse activities
    activities = parse_activities_section(activities_section)
    if not activities:
        print(f"⚠️  Could not parse activities in {md_path.name}")
        return False
    ...
    print(f"✅ Created {yaml_path.name} with {len(activities)} activities")
...
def main():
    ...
    print(f"📊 Summary: {success}/{len(files)} modules converted")
