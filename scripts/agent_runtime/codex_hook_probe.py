...
def _run_case(repo: Path, case: ProbeCase, model: str | None, timeout: int) -> dict[str, Any]:
    if model not in {None, "gpt-6-sol"}:
        raise ValueError(f"Codex model {model!r} rejected; only gpt-6-sol is approved")
    ...
    command.extend(["--model", model or "gpt-6-sol", "-c", 'model_reasoning_effort="low"'])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", default=os.environ.get("CODEX_HOOK_PROBE_MODEL", "gpt-6-sol"))
    ...
    if args.model != "gpt-6-sol":
        parser.error("only gpt-6-sol is approved for Codex hook probes")
    return args
