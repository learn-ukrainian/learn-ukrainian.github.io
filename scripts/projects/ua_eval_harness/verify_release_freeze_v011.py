#!/usr/bin/env python3
"""Verify the immutable 0.1.1 release against its historical source bytes.

Three logical source artifacts resolve to exact archived evidence: the inherited
runner and the two verifiers. This does not certify their active replacements.
"""
...
V011_ARTIFACTS: tuple[tuple[Path, str], ...] = (
    ...
    (
        Path("tests/test_ua_eval_model_runner.py"),
        "v0.1.1_model_runner_regression_test",
    ),
)
...
# v0.1.1 records both compatibility and current verifier source. Their original
# bytes stay immutable even while active verification resolves archived sources.
HISTORICAL_VERIFIER_SOURCES = {
    Path("scripts/projects/ua_eval_harness/verify_release_freeze.py"):
        Path("archive/evidence/ua-eval-v0.1.1/verify_release_freeze.py.txt"),
    Path("scripts/projects/ua_eval_harness/verify_release_freeze_v011.py"):
        Path("archive/evidence/ua-eval-v0.1.1/verify_release_freeze_v011.py.txt"),
}
...
def _artifact(path: Path, role: str) -> dict[str, str]:
    return {
        "path": path.as_posix(),
        "role": role,
        "sha256": _sha256(_frozen_artifact_path(path)),
    }
...
def _validate_aggregate_report_is_public(
    heldout: Mapping[str, Any],
    report: Mapping[str, Any],
    *,
    report_path: Path,
) -> None:
    """Reject item-shaped fields or held-out content in an aggregate report."""
    forbidden = v010._forbidden_report_keys(report)
    if forbidden:
        raise FreezeError(
            f"aggregate report exposes item fields: "
            f"{report_path}: {forbidden[0]}"
        )
    item_ids = {str(row[0]) for row in heldout["items"]}
    protected_texts = {str(row[8]) for row in heldout["items"]}
    protected_texts.update(
        str(reference[1])
        for row in heldout["items"]
        for reference in row[12]
    )
    protected_hashes = {str(row[9]) for row in heldout["items"]}
    protected_hashes.update(
        str(row[3]) for row in heldout["exclusions"]
    )
    protected_hashes.update(
        str(reference[2])
        for row in heldout["items"]
    ...
    if any(item_id in serialized for item_id in item_ids):
        raise FreezeError(
            f"aggregate report exposes held-out item IDs: {report_path}"
        )
    if any(text in serialized for text in protected_texts):
        raise FreezeError(
            f"aggregate report exposes held-out source or target text: "
            f"{report_path}"
        )
    if any(receipt in serialized for receipt in protected_hashes):
        raise FreezeError(
            f"aggregate report exposes protected content hashes: {report_path}"
        )
...
def validate_freeze(freeze: Mapping[str, Any]) -> None:
    ...
    for artifact in artifacts:
        ...
        if artifact.get("sha256") != _sha256(artifact_source):
            source_note = f" (historical source: {artifact_source})" if artifact_source != ROOT / path else ""
