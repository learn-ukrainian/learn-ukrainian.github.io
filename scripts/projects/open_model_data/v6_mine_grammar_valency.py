...
INVALID_DOCUMENT_AKTU_RE = re.compile(
    ...
    r")(?!\s+(?:[а-яіїєґ\'’\-]+\s+){0,3}(?:агресії|вандалізму|тероризму|насильства|непокори|капітуляції|відчаю|милосердя|доброї\s+волі|героїзму|самопожертви|протесту|саботажу|диверсії|помсти|зради|каяття))",
    re.IGNORECASE,
)
PREDICATE_WORDS = {
