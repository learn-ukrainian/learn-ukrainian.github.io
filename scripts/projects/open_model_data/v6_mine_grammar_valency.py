...
VALENCY_FRAMES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "explanation": (
            ...
            "«знущатися з когось» (Є. Чак «Складні випадки українського слововживання»). "
            "Варіант керування з прийменником «над» («знущатися над ким») трапляється в класичній художній літературі "
    ...
    },
    {
        ...
        "head_word": "глузувати",
        "category": "F/Style",
        "correct_pattern": "глузувати (з кого/чого? з + родовий відмінок)",
        "incorrect_pattern": "глузувати (над ким/чим? над + орудний відмінок)",
    ...
    },
    {
        ...
        "head_word": "кепкувати",
        "category": "F/Style",
        "correct_pattern": "кепкувати (з кого/чого? з + родовий відмінок)",
        "incorrect_pattern": "кепкувати (над ким/чим? над + орудний відмінок)",
    ...
    },
    {
        ...
        "head_word": "сміятися",
        "category": "F/Style",
        "correct_pattern": "сміятися (з кого/чого? з + родовий відмінок)",
        "incorrect_pattern": "сміятися (над ким/чим? над + орудний відмінок)",
    ...
    },
    {
        ...
        "explanation": "Прийменник «по» в українській мові має обмежену сферу вживання (рух поверхнею або мета руху: піти по хліб). Його не слід калькувати у сфері діловодства, часу та регламенту (О. Пономарів, Б. Антоненко-Давидович).",
        "critique": "Прийменник «по» в українській мові має вузьку семантику; у діловому мовленні слід використовувати питомі прийменники «у», «за», «з», «щодо».",
        "examples": [
...
PARTICIPLE_FRAMES: list[dict[str, Any]] = [
    {
        "err_word": "бажаючий",
        "corr_word": "охочий",
        "lemma": "охочий",
    ...
    {
        "err_word": "відпочиваючий",
        "corr_word": "відпочивальник",
        "lemma": "відпочивальник",
    ...
    {
        "err_word": "подорожуючий",
        "corr_word": "подорожній",
        "lemma": "подорожній",
    ...
    {
        "err_word": "початкуючий",
        "corr_word": "початківець",
        "lemma": "початківець",
...
def load_brown_uk_sentences(
    ...
    if standard_genres & set(by_genre.keys()):
        ...
        n_hold = min(len(good_files), max(eval_count, 50))
        held_out_docs = good_files[:n_hold]
        train_docs = good_files[n_hold:]

    held_out_docs.sort(key=lambda p: p.name)
    train_docs.sort(key=lambda p: p.name)
...
def build_valency_trajectories(cur_ves: sqlite3.Cursor | None = None) -> list[dict[str, Any]]:
    ...
    for frame in VALENCY_FRAMES:
        verb = frame["verb"]
        pos_type = frame.get("pos_type", "verb")
        head_word = frame.get("head_word", verb)

        lemma, forms_cnt, attested = query_vesum_lemma_and_count(cur_ves, head_word)
...
def build_participle_trajectories(cur_ves: sqlite3.Cursor | None = None) -> list[dict[str, Any]]:
    """Build normative trajectories replacing active participles on -чий (G/Participle)."""
    trajectories = []
    for frame in PARTICIPLE_FRAMES:
        err_word = frame["err_word"]
        corr_word = frame["corr_word"]
        lemma_word = frame["lemma"]
...
def build_sft_dataset(
    ...
    # 1. Valency trajectories (single pass over unique valency examples - NO reps loop)
    for v in valency_items:
        if len(all_trajectories) >= target_count:
            break
        add_trajectory(dict(v))

    # 2. G/Participle trajectories (single pass over unique participle examples - NO reps loop)
    for p in participle_items:
        if len(all_trajectories) >= target_count:
            break
    ...
    for item in ua_gec_items:
        ...
        if len(all_errs) > 1:
            ...
            rules_bullets = " ".join(
                f"[{e['tag']}]: {CATEGORY_EXPLANATIONS.get(e['tag'], {}).get('rule', '').rstrip('.')}."
                for e in all_errs
            )
    ...
    # 4. Brown-UK training trajectories (PRESERVE controls + CONTRAST style)
    # Balanced between good and so-so without synthetic duplication
    good_items = [b for b in brown_uk_train if not b.get("is_error", False)]
    soso_items = [b for b in brown_uk_train if b.get("is_error", False)]

    remaining = target_count - len(all_trajectories)
    if remaining > 0:
        ...
        # Add so-so (CONTRAST) items: category Control/Usus, usus diversity
        soso_count = 0
        for b in soso_items:
            if len(all_trajectories) >= target_count:
                break
        ...
        # If so-so items alone didn't reach target_count, pull from remaining good_items
        if len(all_trajectories) < target_count:
            for b in good_items[good_count:]:
                if len(all_trajectories) >= target_count:
                    break
                s = b["text"]
                doc_id = b.get("doc_id", "")
    ...
    if len(all_trajectories) < target_count:
        ...
        while len(all_trajectories) < target_count and pool:
            ...
            idx += 1
            t = dict(src)
            t["trajectory_id"] = f"{src['trajectory_id']}.synth_{idx}"
            all_trajectories.append(t)
...
def shard_dataset(
    ...
    """Shard SFT dataset into numbered files and build manifest with deterministic shuffling."""
    sft_dir = output_dir / "sft"
    sft_dir.mkdir(parents=True, exist_ok=True)

    # Deterministic shuffle across sources so all shards have an even mix (resolves F10)
    shuffled = list(trajectories)
    random.Random(seed).shuffle(shuffled)

    trajectories_per_shard = len(shuffled) // num_shards
    remainder = len(shuffled) % num_shards
    ...
    for shard_num in range(1, num_shards + 1):
        ...
        shard_trajectories = shuffled[start_idx : start_idx + count]
        start_idx += count
    ...
    manifest = {
        ...
        "total_trajectories": len(shuffled),
...
def main() -> int:
    ...
    try:
        ...
        # 3. Ingest UA-GEC 20-category taxonomy (train split only)
        print("\n[3/5] Ingesting full 20-category UA-GEC taxonomy annotations (train partition only)...")
        ...
        # 4. Build VESUM case valency and participle frames
        ...
        trajectories = build_sft_dataset(
            ...
            participle_items=participle_items,
            cur_ves=cur_ves,
        ...
        vesum_valency_verified = all(
            ...
            if t.get("subtype") in ("valency_government", "participle_norm") and t.get("vesum_attestation")
        )

        tone_calibration_gate6_verified = all(
            verify_respectful_tone(t["final_response"], pejorative_words)
            and all(verify_respectful_tone(step, pejorative_words) for step in t["reasoning_steps"])
            for t in trajectories
        )

        precommit_file_ceiling_satisfied = all(f.stat().st_size / 1024.0 <= 2000.0 for f in shard_files)

        invariants_verified = {
            "zero_train_eval_leakage": zero_train_eval_leakage,
            "document_partitioning_enforced": document_partitioning_enforced,
            "full_20_category_taxonomy_ingested": full_20_category_taxonomy_ingested,
            "vesum_valency_verified": vesum_valency_verified,
            "tone_calibration_gate6_verified": tone_calibration_gate6_verified,
            "precommit_file_ceiling_satisfied": precommit_file_ceiling_satisfied,
        }
