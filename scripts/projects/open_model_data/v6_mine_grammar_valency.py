...
INVALID_DOCUMENT_AKTU_RE = re.compile(
    r"\b(?:"
    r"(?:прийняття|ухвалення|підписання|оприлюднення|скасування|затвердження|проекту|проєкту|положення|положень|норм|вимог|статті|тексту|розділу)\s+(?:[а-яіїєґ\'’\-]+\s+){0,3}акту\b"
    r"|(?:законодавчого|нормативного|підзаконного|ненормативного|установчого|нормативно-правового)\s+(?:[а-яіїєґ\'’\-]+\s+)?акту\b"
    r"|акту\s+(?:Верховн(?:ої|ою)\s+Рад(?:и|ою)|Президент(?:а|ом)|Кабінет(?:у|ом)\s+Міністрів|уряд(?:у|ом)|парламент(?:у|ом)|суд(?:у|ом))\b"
    r")(?!\s+(?:[а-яіїєґ\'’\-]+\s+)?(?:агресії|вандалізму|тероризму|насильства|непокори|капітуляції|відчаю|милосердя|доброї\s+волі|героїзму|самопожертви|протесту|саботажу|диверсії|помсти|зради|каяття))",
    re.IGNORECASE,
)
PREDICATE_WORDS = {
