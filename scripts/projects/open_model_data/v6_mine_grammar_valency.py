...
INVALID_DOCUMENT_AKTU_RE = re.compile(
    r"\b(?:"
    r"(?:прийняття|ухвалення|підписання|оприлюднення|скасування|затвердження|проекту|проєкту|положення|положень|норм|вимог|статті|дію|дії|тексту|розділу)\s+(?:[а-яіїєґ\'’\-]+\s+){0,3}акту\b"
