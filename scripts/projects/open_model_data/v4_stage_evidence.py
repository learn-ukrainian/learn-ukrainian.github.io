...
RESIDUAL_REASON_CODES = frozenset({REASON_RIGHTS_UNKNOWN, REASON_SOURCE_INCOMPLETE, REASON_INDEPENDENCE_UNAVAILABLE, REASON_STAGE_COMPLETION_NOT_YET_AVAILABLE})
...
def stratum_eligibility(manifest: dict[str, Any], a2_receipt: dict[str, Any], error_cls: type[Exception] = ValueError) -> list[dict[str, Any]]:
    ...
    * a coverage entry with an empty ``residual_ids`` list whose
    ...
    require(len({c["stratum"] for c in coverage_map}) == len(coverage_map), "A2 stratum_coverage_map carries a duplicate stratum entry -- refusing", error_cls)
    require({c["stratum"] for c in coverage_map} == set(manifest_by_stratum), "A2 stratum_coverage_map does not cover exactly the frozen manifest's strata -- refusing", error_cls)

    records = []
    for coverage in coverage_map:
        stratum = coverage["stratum"]
        for rid in coverage["residual_ids"]:
            require(rid in residual_ids, f"A2 stratum_coverage_map entry {stratum!r} references residual id {rid!r} absent from A2's own residuals -- refusing", error_cls)
...
def public_eligibility(eligibility: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [{key: record[key] for key in ("stratum", "rights_resolved", "assigned", "prerequisite_eligible")} for record in eligibility]
...
def stratum_a2_reason_codes(a2_receipt: dict[str, Any], error_cls: type[Exception] = ValueError) -> dict[str, str]:
    ...
    for coverage in a2_receipt["stratum_coverage_map"]:
        ...
        if not coverage["residual_ids"]:
            continue
        for rid in coverage["residual_ids"]:
            require(rid in reason_by_residual_id, f"A2 stratum_coverage_map entry {stratum!r} references residual id {rid!r} absent from A2's own residuals -- refusing", error_cls)
        reasons = {reason_by_residual_id[rid] for rid in coverage["residual_ids"]}
        require(len(reasons) == 1, f"A2 stratum_coverage_map entry {stratum!r} does not resolve to exactly one reason code -- refusing", error_cls)
        (reason,) = reasons
        require(reason in A2_REASON_TO_RESIDUAL_REASON, f"A2 stratum_coverage_map entry {stratum!r} carries an unmapped reason code {reason!r} -- refusing", error_cls)
        resolved[stratum] = A2_REASON_TO_RESIDUAL_REASON[reason]
    return resolved


def slot_residual_reason_code(stratum: str, eligibility_by_stratum: dict[str, dict[str, Any]], a2_reasons: dict[str, str]) -> str:
...
def completion_slot_ids(completions: list[dict[str, Any]], *, stage: str, total_slot_ids: set[str], error_cls: type[Exception] = ValueError) -> set[str]:
    ...
    for record in completions:
        require(record.get("stage") == stage, f"{stage} completion record does not carry stage={stage!r} -- refusing", error_cls)
        slot_id = record.get("slot_id")
        require(isinstance(slot_id, str) and slot_id in total_slot_ids, f"{stage} completion record references a slot id outside the frozen 100-slot denominator -- refusing", error_cls)
        ids.append(slot_id)
...
def validate_partition(total_slot_ids: set[str], completion_ids: set[str], residual_ids: set[str], *, label: str, error_cls: type[Exception] = ValueError) -> None:
    require(completion_ids.isdisjoint(residual_ids), f"{label}: completion and residual slot sets overlap -- refusing", error_cls)
    require(completion_ids | residual_ids == total_slot_ids, f"{label}: completion and residual slot sets do not exactly partition the frozen 100-slot denominator -- refusing", error_cls)


def validate_subset(smaller: set[str], bigger: set[str], *, label: str, error_cls: type[Exception] = ValueError) -> None:
