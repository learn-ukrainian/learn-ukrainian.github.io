...
PART_OF_SPEECH_WORDS = (
    r"слов[аоеіу]|словом|слові|слів|"
    r"форм[аиіуе]|формою|формі|"
...
NAMED_ENTITY_PATTERNS = [
    re.compile(
        rf"\b(?:{PART_OF_SPEECH_WORDS})\s+[«\"“']?([а-яіїєґa-z0-9'’ʼ\-]+)[»\"”']?",
        re.IGNORECASE,
    ),
    re.compile(
        rf"\b(?:{RELATION_WORDS})\s+(?:(?:у|в|для|до|від|щодо|з|із)\s+)?(?:(?:{RELATION_WORDS}|{PART_OF_SPEECH_WORDS})\s+)*[«\"“']?([а-яіїєґa-z0-9'’ʼ\-]+)[»\"”']?",
        re.IGNORECASE,
    ),
]
        ...
        for pat in NAMED_ENTITY_PATTERNS:
            for m in pat.finditer(analysis_text):
                ...
                if not tok or len(tok) <= 1:
                    continue
                if tok.startswith("-") or tok.endswith("-") or tok in EXEMPT_AFFIXES:
                    continue
                words = tok.split()
                if (
                    tok in all_target_tokens
                    or tok in EXEMPT_LINGUISTIC_ENTITIES
                    or tok in GRAMMATICAL_STOPWORDS
                    or tok in GRAMMATICAL_DESCRIPTORS
                ):
                    continue
                if any(
                    w in all_target_tokens
                    or w in EXEMPT_LINGUISTIC_ENTITIES
                    or w in GRAMMATICAL_STOPWORDS
                    or w in GRAMMATICAL_DESCRIPTORS
                    for w in words
                ):
                    continue
