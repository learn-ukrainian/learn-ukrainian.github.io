...
FORBIDDEN_COMPLETION_CLAIMS = tuple(sorted({*a11.FORBIDDEN_COMPLETION_CLAIMS, "TRAINING_READY_SILVER"} - {"GOLD_UPGRADE_READY"}))
...
def check_gold_overlay_gate(root: Path = ROOT) -> dict[str, Any]:
    ...
    *and* A11 itself still independently validates *and* A11's own silver
    ...
    a11_path = (root / "data/projects/open_model_data/admission/dataset_v4_a11_silver_release_gate_receipt_v1.json").resolve()
...
def derive_a12_slot_residuals(manifest: dict[str, Any], a2_receipt: dict[str, Any], gate: dict[str, Any]) -> list[dict[str, Any]]:
...
def build_gold_overlay_view(manifest: dict[str, Any], a11_receipt: dict[str, Any], a12_residuals: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    require(seen_slot_ids == expected_slot_ids, "A11 silver_release_view does not cover exactly the frozen slot manifest -- refusing gold overlay view")
...
def validate_gate_matches_receipt(receipt: dict[str, Any], root: Path) -> None:
    ...
    require(
        ...
        and declared["source_qualified_human_adjudication_recorded"] == gate["source_qualified_human_adjudication_recorded"]
    ...
    require(receipt["status"] != "TRAINING_READY_GOLD_SUBSET", "receipt claims TRAINING_READY_GOLD_SUBSET -- refusing (never a valid A12 claim)")
    require(declared["source_qualified_human_adjudication_recorded"] is False, "receipt overlay_gate claims a human adjudication was recorded, but no execution mechanism exists -- refusing")
    require(declared["gold_overlay_executed"] is False, "receipt overlay_gate claims a gold overlay was executed, but no execution mechanism exists -- refusing")
...
def validate_overlay_packet(receipt: dict[str, Any]) -> None:
    ...
    require(receipt["overlay_packet"] == OVERLAY_PACKET_REQUIREMENTS, "overlay_packet does not equal the frozen overlay-packet contract -- refusing")
...
def validate_residuals_and_gold_overlay_view(receipt: dict[str, Any], root: Path) -> None:
    ...
    require(receipt["a12_residuals"] == expected_a12_residuals, "a12_residuals does not reproduce from the live slot manifest, A2 receipt, and gate -- refusing")

    expected_view = build_gold_overlay_view(manifest, a11_receipt, expected_a12_residuals)
    require(receipt["gold_overlay_view"] == expected_view, "gold_overlay_view does not reproduce from the live A11 receipt and a12_residuals -- refusing")
    require(
        all(
            entry["gold_overlay_applied"] is False and entry["gold_label_tier"] is None and entry["adjudicator_source_qualification"] is None and entry["silver_row_released"] is False
...
def validate_eligibility_and_safety_all_false(receipt: dict[str, Any]) -> None:
    require(receipt["eligibility"] == GOLD_OVERLAY_ELIGIBILITY, "receipt eligibility does not equal the frozen all-false gold-overlay eligibility -- refusing")
    ...
    require(receipt["execution_counters"]["rows_overlaid_with_gold"] == 0, "receipt execution_counters.rows_overlaid_with_gold is not 0 -- refusing (no overlay-execution mechanism exists yet)")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--receipt", type=Path, default=A12_RECEIPT_PATH, help="A12 receipt JSON to verify (default: the tracked V4 A12 gold overlay gate receipt).")
