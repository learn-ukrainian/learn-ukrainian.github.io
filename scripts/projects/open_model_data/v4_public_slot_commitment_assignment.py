...
def check_assignment_gate(root: Path = ROOT) -> dict[str, Any]:
    ...
    a4_path = (root / "data/projects/open_model_data/admission/dataset_v4_a4_deterministic_extraction_receipt_v1.json").resolve()
    ...
    commitments_available = len(a4_receipt.get("builder_packet_consumption", {}).get("unit_commitments", [])) if a4_valid else 0
...
def build_commitment_pool(a4_receipt: dict[str, Any]) -> dict[str, Any]:
    ...
    require(algorithm["content_blind"] is True, "A4's unit_commitment_algorithm is not content_blind -- refusing to republish it as a safe pool")
...
def derive_assignment_records(manifest: dict[str, Any], a2_receipt: dict[str, Any], gate: dict[str, Any]) -> list[dict[str, Any]]:
...
def validate_residual_status_uniform_across_strata(records: list[dict[str, Any]], strata: list[dict[str, Any]]) -> None:
    ...
    require(set(by_stratum) == {s["stratum"] for s in strata}, "assignment records do not cover exactly the manifest's own strata -- refusing")
    for stratum, stratum_records in by_stratum.items():
        assigned = sum(1 for r in stratum_records if r["assignment_status"] == "assigned")
        require(assigned == 0, f"stratum {stratum!r} has a nonzero assigned count -- refusing (no stratum may ever show a different assignment shape today)")
        require(
...
def build_receipt(root: Path = ROOT) -> dict[str, Any]:
    ...
    a2_residuals_carried = [
        {"residual_id": entry["residual_id"], "origin_stage": "A2", "status": "unresolved_carried_to_public_slot_commitment_assignment"}
        for entry in a2_receipt["residuals"]
    ]
    a4_residuals_carried = [
        {"residual_id": entry["residual_id"], "origin_stage": "A4", "status": "unresolved_carried_to_public_slot_commitment_assignment"}
...
def validate_gate_matches_receipt(receipt: dict[str, Any], root: Path) -> None:
    ...
    require(declared == gate, "receipt assignment_gate does not match the state independently re-derived from the live public artifacts -- refusing (re-verify/regenerate required)")
    require(declared["assignment_ready"] is False, "receipt assignment_gate claims assignment_ready -- refusing (never a valid claim today)")
    require(declared["stratum_commitment_binding_available"] is False, "receipt assignment_gate claims stratum_commitment_binding_available -- refusing (structural, never true)")
...
def validate_commitment_binding_policy(receipt: dict[str, Any]) -> None:
    require(receipt["commitment_binding_policy"] == COMMITMENT_BINDING_POLICY, "commitment_binding_policy does not equal the frozen policy contract -- refusing")


def validate_commitment_pool_matches_a4(receipt: dict[str, Any], root: Path) -> None:
    a4_receipt = _load(A4_RECEIPT_PATH)
    expected = build_commitment_pool(a4_receipt)
    require(receipt["commitment_pool"] == expected, "commitment_pool does not reproduce from the live A4 receipt's own published commitments -- refusing")
    published = set(a4_receipt["builder_packet_consumption"]["unit_commitments"])
    require(set(receipt["commitment_pool"]["commitments"]) <= published, "commitment_pool republishes a commitment A4 never published -- refusing")
...
def validate_assignment_records(receipt: dict[str, Any], root: Path) -> None:
    ...
    require(receipt["assignment_records"] == expected, "assignment_records does not reproduce from the live slot manifest, A2 receipt, and gate -- refusing")
    slot_ids = {record["slot_id"] for record in receipt["assignment_records"]}
    require(len(receipt["assignment_records"]) == 100 and len(slot_ids) == 100, "assignment_records does not cover exactly the 100 frozen slots -- refusing")
    require(slot_ids == set(a7.a6.all_frozen_slot_ids(manifest)), "assignment_records slot IDs do not match the frozen manifest's own slot IDs -- refusing")
...
def validate_residuals_carried_forward(receipt: dict[str, Any]) -> None:
    ...
    for stage, source_ids, carried in (
        ...
        for entry in carried:
            require(
                entry["origin_stage"] == stage and entry["status"] == "unresolved_carried_to_public_slot_commitment_assignment",
...
def validate_eligibility_and_safety(receipt: dict[str, Any]) -> None:
    require(receipt["eligibility"] == ASSIGNMENT_ELIGIBILITY, "receipt eligibility does not equal the frozen all-false assignment eligibility -- refusing")
    ...
    require(receipt["execution_counters"]["assigned_slot_count"] == 0, "receipt execution_counters.assigned_slot_count is not 0 -- refusing")
    require(receipt["execution_counters"]["residual_slot_count"] == 100, "receipt execution_counters.residual_slot_count is not 100 -- refusing")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--receipt", type=Path, default=ASSIGNMENT_RECEIPT_PATH, help="Assignment receipt JSON to verify (default: the tracked receipt).")
