...
DEFAULT_LEDGER = REGISTRY_ROOT / "lexicon/practice-creation-review.json"
...
class CreationReview:
    ...
    def allows(self, kind: str, prompt: str, answer: str, contrast: str, source: dict) -> bool:
        ...
        if isinstance(receipt, dict):
            ...
            try:
                timestamp = (
                    datetime.fromisoformat(reviewed_at.replace("Z", "+00:00")) if isinstance(reviewed_at, str) else None
                )
            except ValueError:
                timestamp = None
            model = receipt.get("resolved_model")
            if (
                receipt.get("agent") == "agy"
                and isinstance(model, str)
                and re.fullmatch(r"gemini-[A-Za-z0-9][A-Za-z0-9._-]*", model)
