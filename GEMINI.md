...
- **Transliteration Ban (C1+)**: Latin transliteration STRICTLY PROHIBITED in C1-HIST/C1-BIO
...
```bash
.venv/bin/python -c "import json; print(json.dumps(json.load(open('schemas/activities-c1-hist.schema.json')), indent=2))" | jq '.definitions."true-false-c1-hist"'
```
