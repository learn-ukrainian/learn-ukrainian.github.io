#!/bin/bash
# Hook: Check for unread Claude messages on every prompt (BeforeAgent event)
...
# Count unclaimed, unacknowledged messages for Gemini
COUNT=$(sqlite3 "$DB" "SELECT COUNT(*) FROM messages WHERE to_llm='gemini' AND acknowledged=0 AND claimed_by IS NULL" 2>/dev/null)
...
PREVIEWS=$(sqlite3 -separator '|' "$DB" "
  SELECT
    ...
    timestamp
  FROM messages
  WHERE to_llm='gemini' AND acknowledged=0 AND claimed_by IS NULL
  ORDER BY id ASC
  LIMIT 5
" 2>/dev/null)

CONTEXT="CLAUDE INBOX: ${COUNT} unread message(s) from Claude waiting.
---"
...
CONTEXT="${CONTEXT}
---
Use mcp__message-broker__receive_messages(for_llm='gemini') to read full messages. Acknowledge after processing."
