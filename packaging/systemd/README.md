...
- **Secrets**: both services read `EnvironmentFile=%h/.secrets/learn-ukrainian-backup.env` (repository + password-file path). Never print or log its values — validation errors name the variable and the condition, not the configured value. The unit fails visibly when the file is missing.
