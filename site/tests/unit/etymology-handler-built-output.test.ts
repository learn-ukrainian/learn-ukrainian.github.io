...
describe("etymology inline handler on prerendered lexicon pages", () => {
  ...
  test.skipIf(!hasDist)("built dist lexicon HTML keeps the etymology handler when dist/ is present", () => {
    ...
    for (const name of articleDirs) {
      ...
      // Skip static redirect stubs generated by Astro redirects (e.g. /lexicon/practice, /lexicon/проєк)
      if (html.includes("<title>Redirecting to:") || html.includes('<meta http-equiv="refresh"')) continue;
      for (const marker of ETY_HANDLER_MARKERS) {
    ...
    if (process.env.ATLAS_STATIC_ROUTES === "1") {
