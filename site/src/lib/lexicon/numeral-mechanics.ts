 ...
 *
 * Implements Ukrainian Pravopys 2019 (§§ 105–111) and Academic Grammar rules for:
 *   1. Decades 50–80 Inflection (§ 107):
 ...
 *   2. Hundreds 200–900 Declension (§ 108):
 *      - Both roots decline:
...
export type NumeralMechanicsCategoryKey =
  (typeof NUMERAL_MECHANICS_CATEGORY_KEYS)[number];
...
export type NumeralMechanicsInterferenceKey =
  (typeof NUMERAL_MECHANICS_INTERFERENCE_KEYS)[number];
...
export function numeralMechanicsFeedbackFor(
  ...
  const matchedDistractor = card.distractors.find(
    (d) => d.text.trim() === normalizedSelection,
  );
...
export function resolveCardinal5080Rule(): NumeralRuleResolution {
  return {
    ...
    ruleUa: "У числівниках на позначення десятків 50–80 (п'ятдесят — вісімдесят) відмінюється лише друга частина (-десят); перша частина ніколи не змінюється: п'ятдесяти (не *п'ятидесяти*), шістдесятьма (не *шестидесятьма*).",
    ruleEn: "In numerals 50–80 (п'ятдесят — вісімдесят), only the second root declines (-десят); the first root remains invariant: п'ятдесяти (not *п'ятидесяти*), шістдесятьма (not *шестидесятьма*).",
  };
}
...
export function resolveCardinal200900GenitiveRule(): NumeralRuleResolution {
  return {
    ...
    ruleUa: "У числівниках на позначення сотень 200–900 у родовому відмінку відмінюються обидві частини, і друга частина має закінчення -сот: двохсот, трьохсот, чотирьохсот, п'ятисот, шестисот (не *п'ятиста*).",
    ruleEn: "In numerals 200–900 in the Genitive case, both roots decline and the second root ends in -сот: двохсот, трьохсот, п'ятисот, шестисот (not *п'ятиста*).",
  };
}
...
export function resolveCardinal200900DativeLocativeRule(): NumeralRuleResolution {
  return {
    citation: 'Правопис 2019 § 108',
    ruleUa: "У давальному відмінку числівники 200–900 мають закінчення -стам (двомстам, трьомстам, п'ятистам), а в місцевому — -стах (на двохстах, на трьохстах, на п'ятистах).",
    ruleEn: "In the Dative case, numerals 200–900 end in -стам (двомстам, п'ятистам); in the Locative case, they end in -стах (на двохстах, на п'ятистах).",
  };
}
...
export function resolveCardinal200900InstrumentalRule(): NumeralRuleResolution {
  return {
    citation: 'Правопис 2019 § 108',
    ruleUa: "В орудному відмінку числівники 200–900 мають закінчення -стами: двомастами, трьомастами, чотирмастами, п'ятьмастами (не *п'ятистами*).",
    ruleEn: "In the Instrumental case, numerals 200–900 end in -стами: двомастами, трьомастами, чотирмастами, п'ятьмастами (not *п'ятистами*).",
  };
}
...
export function resolveCardinal4090100Rule(): NumeralRuleResolution {
  return {
    ...
    ruleUa: "Числівники сорок, дев'яносто, сто мають лише дві форми: закінчення -о в називному й знахідному відмінках та закінчення -а в усіх непрямих відмінках (сорока, дев'яноста, ста).",
    ruleEn: "Numerals сорок, дев'яносто, сто have only two forms: ending -о in Nominative/Accusative and ending -а in all oblique cases (сорока, дев'яноста, ста).",
  };
}
...
export function resolveGovernment234Rule(): NumeralRuleResolution {
  return {
    ...
    ruleUa: "Числівники два, три, чотири керують іменниками у формі називного відмінка множини: два брати, три олівці, чотири студенти (калька родового відмінка *два брата* неприпустима).",
    ruleEn: "Numerals два, три, чотири govern nouns in the Nominative plural: два брати, три олівці, чотири студенти (calque *два брата* is incorrect).",
  };
}
...
export function resolveGovernment5PlusRule(): NumeralRuleResolution {
  return {
    ...
    ruleUa: "Числівники від п'яти й більше (п'ять, шість, десять, двадцять тощо) у називному відмінку керують іменниками у формі родового відмінка множини: п'ять братів, десять рулонів, сім книжок.",
    ruleEn: "Numerals from five onwards (п'ять, шість, десять...) in the Nominative govern nouns in the Genitive plural: п'ять братів, десять рулонів, сім книжок.",
  };
}
...
export function resolveGovernmentCompoundLastDigitRule(): NumeralRuleResolution {
  return {
    citation: 'Синтаксичні норми української мови',
    ruleUa: "У складених числівниках форма іменника визначається винятково останнім словом: на один — називний однини (двадцять один день), на два, три, чотири — називний множини (тридцять два дні), на п'ять і більше — родовий множини (сорок п'ять днів).",
    ruleEn: "In compound numerals, noun agreement is determined strictly by the last numeral: ending in один -> Nom sing, ending in два/три/чотири -> Nom plur, ending in 5+ -> Gen plur.",
  };
}
...
export function resolveCollectiveMasculineRule(): NumeralRuleResolution {
  return {
    ...
    ruleUa: "Збірні числівники (двоє, троє, четверо, п'ятеро тощо) природно вживаються з іменниками чоловічого роду — назвами осіб: двоє братів, троє друзів, четверо хлопців.",
    ruleEn: "Collective numerals (двоє, троє, четверо...) naturally combine with masculine animate nouns denoting persons: двоє братів, троє друзів, четверо хлопців.",
  };
}
...
export function resolveCollectiveFeminineRestrictionRule(): NumeralRuleResolution {
  return {
    citation: 'Правопис 2019 § 111',
    ruleUa: "Збірні числівники НЕ вживаються з іменниками жіночого роду на позначення дорослих осіб: вживаються лише власне кількісні числівники — дві жінки (не *двоє жінок*), три сестри (не *троє сестер*).",
    ruleEn: "Collective numerals are NOT used with feminine nouns denoting adult persons: only cardinal numerals are admissible — дві жінки (not *двоє жінок*), три сестри (not *троє сестер*).",
  };
}
...
export function resolveCollectivePluraliaNeuterRule(): NumeralRuleResolution {
  return {
    citation: 'Правопис 2019 § 111',
    ruleUa: "Збірні числівники обов'язково вживаються з іменниками, що мають лише форму множини (pluralia tantum: двоє дверей, троє ножиць, двоє саней), а також із назвами малят (четверо каченят).",
    ruleEn: "Collective numerals are mandatory with pluralia tantum nouns (двоє дверей, троє ножиць, двоє саней) and neuter nouns denoting young animals/beings (четверо каченят).",
  };
}
...
export function resolveFractionalPivtoraRule(): NumeralRuleResolution {
  return {
    ...
    ruleUa: "Дробові числівники півтора (для чоловічого та середнього роду) і півтори (для жіночого роду) завжди керують іменниками у формі родового відмінка ОДНИНИ: півтора року, півтори доби, півтора місяця.",
    ruleEn: "Fractional numerals півтора (masc/neut) and півтори (fem) always govern nouns in the Genitive SINGULAR: півтора року, півтори доби, півтора місяця.",
  };
}
...
export function resolveOrdinalCompoundDeclensionRule(): NumeralRuleResolution {
  return {
    ...
    ruleUa: "У складених порядкових числівниках відмінюється ЛИШЕ ОСТАННЄ слово; усі попередні слова зберігають початкову форму називного відмінка: у дві тисячі двадцять четвертому році (не *у двох тисячах*).",
    ruleEn: "In compound ordinal numerals, ONLY the last word inflects; all preceding words remain in the Nominative: у дві тисячі двадцять четвертому році (not *у двох тисячах*).",
  };
}
...
export function resolveTimeExpressionsRule(): NumeralRuleResolution {
  return {
    ...
    ruleUa: "Для позначення точного часу в українській мові вживають прийменник 'о' / 'об' із порядковим числівником (о десятій годині), форми 'чверть на одинадцяту', 'пів на дванадцяту', 'за двадцять третя' (кальки *в десять годин*, *без двадцяти* неприпустимі).",
    ruleEn: "Authentic Ukrainian time expressions use 'о' + ordinal numeral (о десятій годині), 'чверть на одинадцяту', 'пів на дванадцяту', 'за двадцять третя' (calques *в десять годин*, *без двадцяти* are errors).",
  };
}
...
export function resolveApproximateConstructionsRule(): NumeralRuleResolution {
  return {
    ...
    ruleUa: "Приблизну кількість в українській мові позначають інверсією іменника й числівника (років п'ять, хвилин двадцять) або прийменниками близько, понад, з (близько ста; прийменник 'біля' позначає просторову близькість, а не кількість).",
    ruleEn: "Approximate quantities in Ukrainian are expressed via noun-numeral inversion (років п'ять, хвилин двадцять) or prepositions близько, понад, з (близько ста; 'біля' denotes physical proximity, not quantity).",
  };
}
