...
<GroupSort client:only='react' groups={JSON.parse(`{"Hard pattern -ий/-а/-е": ["червоний олівець", "жовта стрічка", "зелене яблуко", "білий светр", "чорна сукня", "сіре пальто"], "Soft pattern -ій/-я/-є": ["синій светр", "синя ручка", "синє море", "сині речі"]}`)} instruction={"Sort the color chunks by adjective pattern."} isUkrainian={false} />
