...
<GroupSort client:only='react' groups={JSON.parse(`{"він / мій": ["стіл", "стілець", "телефон", "комп'ютер"], "вона / моя": ["книга", "кімната", "лампа", "ручка"], "воно / моє": ["вікно", "ліжко", "крісло", "дзеркало"]}`)} instruction={"Sort each noun by its safest A1 gender chunk."} isUkrainian={false} />
