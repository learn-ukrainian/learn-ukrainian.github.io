...
<GroupSort client:only='react' groups={JSON.parse(`{"Де?": ["у центрі", "на площі", "в Одесі", "на роботі"], "Куди?": ["у музей", "на вокзал", "в Україну", "на пляж"], "Звідки?": ["з України", "зі школи", "з роботи", "з Києва"], "Route or transport": ["на метро", "автобусом", "прямо", "направо"]}`)} instruction={"Sort each chunk by its place function."} isUkrainian={false} />
