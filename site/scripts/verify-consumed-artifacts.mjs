...
function expectedBrowseArtifacts(meta) {
  ...
  return Object.entries(shards).map(([letter, shard]) => {
    ...
    if (typeof path !== "string" || !path.startsWith("/lexicon/browse/") || !path.endsWith(".json")) {
      throw new Error(`site/src/data/lexicon-browse-meta.json has an invalid browse shard for ${letter}.`);
    }
...
function main() {
  ...
  try {
    ...
    for (const relativePath of expectedBrowseArtifacts(browseMeta)) requireArtifact(relativePath, missing);
  } catch (error) {
    console.error(`Invalid committed Atlas artifact: ${error instanceof Error ? error.message : String(error)}`);
