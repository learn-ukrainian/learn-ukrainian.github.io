...
`codexbar usage --json --provider <codex|claude|gemini|cursor|grok|kimi>` — which returns
`usage.secondary.usedPercent` and `pace.<window>.{stage,summary,deltaPercent,willLastToReset}`
with `dataConfidence: "exact"` for codex/claude. Read the SIGNAL (pace stage + reserve against
the window's expected usage), not just the transport. `/api/state/routing-budget` +
...
- **Upper bound — DISK, and disk wins every conflict:** every dispatch costs a git worktree
  ...
  quota remains; quota headroom never authorises exceeding disk headroom. Before adding width,
  check `df -h /` and `du -sh .worktrees`; reaping finished worktrees is what buys room for more
