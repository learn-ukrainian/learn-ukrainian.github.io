...
(**Sonnet 5 → Terra → Gemini 3.7 Flash → GLM → DeepSeek V4 Flash High**). Escalatory authority reviews select from the advisor quality prior (**Sol → Opus
...
* **Everyday Routine Formal Reviews** (practical seats @ `high` effort):
  ...
  * **Gemini seat**: `gemini-3.7-flash-high`
...
* **Escalatory Advisor / Critical Authority Reviews** (reserved for architecture, security, or design escalation):
  ...
  * **Other advisors**: `gemini-3.1-pro-high` · `gemini-3.7-flash-high` ·
    `kimi-k3-max` · `glm-5.3` @ `max` (advisory) · `grok-4.6` @ `high`.
  * `gpt-5.6-terra` is **not an advisor**; it remains the practical/routine
    Codex review seat.
  * `gemini-3.7-flash-high` is currently the strongest lane for Ukrainian.
    Flash 3.7 is the live AGY default (operator GO 2026-08-13); until the next 3.1 Pro ships,
...
* **ox-alpha / OpenRouter stealth** (operator 2026-08-23, #6943): cataloged as
  ...
  `opencode run --model openrouter/stealth/ox-alpha`. No automatic dispatch, no
  `--agent ox-alpha`, no `ask-ox-alpha`, no review ladder, no CF. Released 2026-08-20;
  OpenRouter lists $0 / 1M context; OpenCode ~one-week free preview (~through 2026-08-27).
  Provider retains prompts and completions (OpenRouter stealth terms). Family `zhipu` is
  unconfirmed (tokenizer fingerprint matches GLM-5.3 — not proof of identity).
...
| Q&A or single-shot review without commit | `ab ask-codex` / `ab ask-agy --to-model gemini-3.7-flash-high` for routine/agentic (top AGY default), `--to-model gemini-3.1-pro-high` only for deep single-shot (gemini-cli retired → agy) |
...
(default Flash pin `gemini-3.7-flash-high`, or Pro for deep single-shot), it receives a
...
| **glm / Z.AI** (`glm-5.3`) | deep security + large-context coherence audits | **LOCAL-ONLY** China egress; never CI/sensitive; prefer 5h when weekly hot |
...
| --- | --- | --- | --- | --- |
| **Coding / impl / fixtures** | **Ultra month (2026-08-13→~2026-09-13):** **cursor Auto** first when fit allows for mechanical + ordinary infra/code implement. Else **Luna @ `max`** for bounded work with exact owned paths + an objective scope ceiling; use a complete Sol envelope when consequential boundaries need definition. **Terra @ `high`** for broader autonomous integration or unresolved ambiguity | **agy** `gemini-3.7-flash-high` · **kimi** `k3-256k` · Luna/Terra when Cursor unfit or concurrency full | **deepseek-v4-flash** (tool-heavy / CF volume) · grok | LANGUAGE-LANES / advisor / authority never on Cursor; `cursor:auto` never CF identity; when Codex near_cap, **do not** park ruff/fingerprint/CI-fix on Terra; claude seat = only ≤5-LOC CI-fix-I-caused; Luna never sole authority; **Pro @ high = hard implement only** (complex multi-file, hard lookup — 2026-08-13) |
| **Code review** (cross-family = outside author's family) | **critical only:** Opus/Fable ↔ Sol (authority) | **high/medium/low formal CF defaults:** `gpt-5.6-terra` · `claude-sonnet-5` · `gemini-3.7-flash-high` · native `grok-4.6` (Cursor **`grok-4.6` explicit** if native dark) · Kimi K3 · GLM-5.3 · **DeepSeek V4 Flash @ OpenCode high** · pool **`laguna-s-2.1`** | **second dissent / volume:** Pool S 2.1 · Gemini 3.5 Flash | DeepSeek review = Flash only (Pro stays off the routine review ladder — Pro @ high = hard implement only, 2026-08-13); never critical authority; first-party `deepseek-direct` + native Entire capture |
| **UK content authoring** (author immersion-first, never translate) | **agy** (A1–A2 voice) ≈ **codex** | **claude** (B1–C2, sparingly — save the window) | **grok-4.6** | **LANGUAGE-LANES RULE below binds**: only these four; cursor/deepseek/kimi/pool/glm/gemma/stealth/ox-alpha excluded |
...
Operator directive: **Gemini 3.7 Flash is the AGY top default** (agentic workhorse).
Gemini 3.6 Flash remains an active previous pin.

| Model | Catalog slug | Tier | Use |
| --- | --- | --- | --- |
| Gemini 3.7 Flash (High) | `gemini-3.7-flash-high` | `frontier_practical` | Default AGY dispatch/Q&A; subagents; fast tool loops; day-to-day coding |
| Gemini 3.6 Flash (High) | `gemini-3.6-flash-high` | `frontier_practical` | Active previous Flash pin; explicit pin / back-compat |
...
**Tier calibration (operator claim checked):** 3.7 Flash is roughly **Terra / Sonnet 5 class** for agentic coding throughput — **not** Sol/Fable/Opus authority, and **not** a full replacement for 3.1 Pro on deep one-shot architecture. Prefer 3.7 Flash first; escalate to 3.1 Pro (or Sol/Fable) when depth matters more than loop efficiency.
