...
| Ukrainian CONTENT (authoring · russicism/quality review) — **we AUTHOR UK content, we do NOT translate EN→UK** | Content is written in Ukrainian, immersion-first, grounded in VESUM/`sources` MCP (not translated). **Authoring:** claude-tools (V7 writer, per `docs/decisions/2026-05-06-writer-selection…`). **Review (russicism/surzhyk/CEFR/VESUM):** deepseek-v4-pro + `sources` MCP (`verify_words`, `check_russian_shadow`, `query_cefr_level`); agy also strong (§7 cleared). ⚠️ **cursor injects russicisms on longer UK text** — verified «перекатні» (shadow 1.0, not in VESUM); do NOT route cursor as a primary UK-content lane without a VESUM/russicism gate. **Never pool/glm for UK content** (code models — glm anglicizes, pool worse). Which model authors/reviews UK content *best* is a content-lane bakeoff vs VESUM/corpus/CEFR — not asserted here from a spot-test. |
