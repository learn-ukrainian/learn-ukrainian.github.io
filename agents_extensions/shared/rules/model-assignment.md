...
`quality_tiers` — not a parallel ranking and not a `reviewed_on` refresh.
...
| AGY (`gemini-3.8-flash-high`) | fast scoped CF | `frontier_practical` | #7137 pass, wall ~47s, low noise, concrete finding |
...
* **Everyday Routine Formal Reviews** (practical roles; Codex @ `high`, other seats @ `high` effort):
...
* **Escalatory Advisor / Critical Authority Reviews** (reserved for architecture, security, or design escalation):
  * **Top advisors**: `claude-fable-5-1` (Fable 5.1) · `gpt-6-sol` @ `high` — the ONLY top-tier
    advisor seats. Sol is pinned at `high` for advisory and escalation. Fable's cost is accepted for advisor turns:
  ...
  * **Other advisors**: `gemini-3.1-pro-high` · `gemini-3.8-flash-high` ·
    `kimi-k3-max` · `glm-5.3` @ `max` (advisory) · `grok-4.7` @ `high`.
  * Legacy Terra qualification does not authorize a current Codex fallback; use Sol.
...
| Content Review with VESUM verification (load-bearing) | **LANGUAGE-LANES RULE binds (user 2026-07-17): agy / codex / claude / grok-4.7 only** — dispatch the reviewer on one of the four with the `sources` MCP (`verify_words`, `query_cefr_level`, `check_russian_shadow`). ~~deepseek-v4-pro default (#4358)~~ RETIRED for language seats by the same order; the #2112/# 4358 validation history stands as evidence only |
...
| Q&A or single-shot review without commit | `ab ask-codex` / `ab ask-agy --to-model gemini-3.8-flash-high` for routine/agentic (top AGY default), `--to-model gemini-3.1-pro-high` only for deep single-shot (gemini-cli retired → agy) |
...
(default Flash pin `gemini-3.8-flash-high`, or Pro for deep single-shot), it receives a
...
- **Orchestrator seats (fleet-comms stream #5512 / #4707)** — any of these may own a cold-start /
  ...
  | **codex** | `gpt-6-sol` @ high | **`gpt-6-astra` @ high** | Sol orchestrates and does advanced work. Luna @ high scouts. Sol @ high advises. Never co-owns a live lease |
...
* **Advisor = `gpt-6-sol` @ `high` (on-demand, NOT a standing worker).** The
  ...
  and escalation triggers. The advisory pin is `high`; Astra serves routine work at `low`.
...
* **Direct bounded execution:** use Astra @ `low` without an advisory envelope only
  ...
  ambiguity routes to Astra @ `low`, with Sol @ `high` advisory judgment when needed.
* **Review boundary:** Codex advisor and worker seats are both OpenAI-family. Sol's advisory
...
3. **Timed pause, not permanent neglect:** a lane at near_cap or operator-paused carries an
   explicit **return-at** timestamp → auto-return to rotation when the timed pause ends, not “Codex is dead forever.”
...
| **agy** (Gemini 3.8 Flash default) | agentic scripts, content (language lanes); **worker with complete briefs** (#5737) — not self-decomposing micro-PR spray | metered — cost-aware, not absent; complete brief required |
...
| **codex** Astra (1–4 drivers) | novel/hard impl; Astra bounded with scope ceiling | **throttle mechanical when near_cap / paused**; return-at resumes rotation; shed to Cursor `grok-4.7-high` / Flash / AGY / k3-256k / GLM |
...
| **Code review** (cross-family = outside author's family) | **critical only:** Opus/Fable ↔ Sol (authority) | **high/medium/low formal CF defaults:** `gpt-6-sol` @ `high` · `claude-sonnet-5` · `gemini-3.8-flash-high` · native `grok-4.7` (Cursor **`grok-4.7` explicit** if native dark) · Kimi K3 · GLM-5.3 · **DeepSeek V4 Flash @ OpenCode high** · pool **`laguna-s-2.1`** | **second dissent / volume:** Pool S 2.1 · Gemini 3.5 Flash | DeepSeek review = Flash only (Pro stays off the routine review ladder — Pro @ high = hard implement only, 2026-08-13); never critical authority; first-party `deepseek-direct` + native Entire capture |
| **UK content authoring** (author immersion-first, never translate) | **agy** (A1–A2 voice) ≈ **codex** | **claude** (B1–C2, sparingly — save the window) | **grok-4.7** | **LANGUAGE-LANES RULE below binds**: only these four; cursor/deepseek/kimi/pool/glm/gemma excluded |
| **Content / factual / CEFR review** (VESUM-gated) | **agy** (pedagogy/CEFR, + `sources` MCP) | **codex** · **grok-4.7** | **claude** (judgment tier) | **LANGUAGE-LANES RULE below binds**; NO grok as a QG judge seat (separate standing ban); FOLK stays cross-family GPT↔Claude per the folk rubric |
| **Research / recon / triage** | **Astra @ `low`** with exact owned paths + an objective scope ceiling; add a Sol envelope when the boundaries themselves need judgment | Astra @ `low` for broader work; Sol @ `high` for advisory judgment | agy | Workers never sole authority on consequential calls |
...
**Advisor (on-demand, HARD calls only): `gpt-6-sol @ high` by default** — architecture, high-stakes design/ADR review, difficult debugging, final synthesis, or a bounded advisory envelope. Workhorse (Astra) effort is `low`; reviewer (Sol) effort is `high`. **Excluded:** qwen (cost). **LOCAL-ONLY:** glm (China-egress, never CI). **Cross-family review gate holds:** the reviewer must be outside the author's model family.
...
| Code & infrastructure review | Kimi K3 · GLM-5.3 · Sol | Historical Terra review arcs on #5896/#5925/#5926/#5931 (defect-finding record) |
| Security-sensitive changes | Sol/GPT family · Kimi | ambient-Anthropic-token→z.ai leak caught pre-merge (#5931 arc) |
...
The current Codex panel participant is Sol; legacy results do not establish its qualification.
...
- **Default bounded work:** authority brief (**Fable** or **Sol**) → heap/practical implement
  (Astra @ low / Flash / …). A complete advisor packet makes weaker models viable.
...
`deepseek-direct/deepseek-v4-flash` at high effort, deny-all tools (the `ask-hermes` alias
...
`deepseek-direct/deepseek-v4-flash` `--variant high` by default (operator 2026-08-13).
...
| --- | --- | --- | --- |
| Gemini 3.8 Flash (High) | `gemini-3.8-flash-high` | `frontier_practical` | Default AGY dispatch/Q&A; subagents; fast tool loops; day-to-day coding |
...
**Tier calibration (operator claim checked):** 3.8 Flash is roughly **Terra / Sonnet 5 class** for agentic coding throughput — **not** Astra/Fable/Opus authority, and **not** a full replacement for 3.1 Pro on deep one-shot architecture. Prefer 3.8 Flash first; escalate to 3.1 Pro (or Astra/Fable) when depth matters more than loop efficiency.
...
| Standard reviewer | `gpt-6-sol` | `high` |
| Advisor / driver judgment / escalation | `gpt-6-sol` | `high` |
| Bounded scout / recon | `gpt-6-luna` | `high` |

Codex runtime routes accept `gpt-6-sol`, `gpt-6-luna`, and `gpt-6-astra`. Sol is the
orchestrator, standard reviewer, and advisor; Luna is the bounded scout; Astra remains
the implementation workhorse at low effort.
...
The route `execution_routing.sol_advised_bounded` designates **gpt-6-sol** as the live
advisor seat. Current designated advisors are **Fable** and **Sol**; **Kimi** consults non-Ukrainian
design/coding only and does not alone satisfy architecture GO. Model selections
for this route come from the catalog. Historical Terra and Luna evidence remains
historical and does not authorize a fallback. All are OpenAI-family models and cannot
satisfy independent CF for an OpenAI-authored PR.
...
the worktree (`scripts/orchestration/reap_worktrees.py --apply` / `drive-epic` §7a).
