...
**Kimi K3 onboarding (native `kimi`, added 2026-07-16):** dispatch with `.venv/bin/python scripts/delegate.py dispatch --agent kimi --model kimi-code/k3`. K3 is a separate Moonshot-family subscription lane with a 262K context window, tool/image/video input, and max-only effort. Until role-specific bakeoffs land, use it for bounded code implementation, recon, triage, and second-opinion canaries; it is not a sole architecture/release authority and is not routed to Ukrainian curriculum prose or factual review.
