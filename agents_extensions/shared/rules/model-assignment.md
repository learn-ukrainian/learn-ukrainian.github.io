...
**CodexBar / routing-budget (live 2026-07-07):** `/api/state/routing-budget` + `delegate --check-budget` = the never-trip window check for SUBSCRIPTION lanes (claude/codex/gemini) — consult before big fanouts. API-billed lanes (deepseek/grok/openrouter) are absent BY DESIGN (the user tracks dollar spend himself): **absence from the snapshot ≠ unavailable** — route them freely by quality fit. Any ranked-headroom view must treat absent lanes as unmetered (constraint pinned on #4640).
