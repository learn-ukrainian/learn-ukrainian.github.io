...
| **Claude Fable 5.1 (when in the driver seat)** | Apply the Fable 5.1 section of the `claude-api` skill's migration guide (`shared/model-migration.md`), not a remembered summary. Essentials: thinking is always on (never send `thinking: disabled`); start at `high` and step down to `medium`/`low` where results hold — level names do not mean the same depth across models; keep test-before-report and progress-grounding instructions (the Opus 5 advice to delete verification scaffolding does not apply); delegate independent subtasks asynchronously and keep working while they run; do not surface context-budget countdowns; final summaries re-ground the reader (outcome first, plain identifiers, no working shorthand). Corrections: state them plainly and briefly, then continue. |
| **Claude Opus 5 (when in the driver seat)** | Apply the Opus 5 section of the same migration guide: it MAY send `thinking: disabled` at effort ≤ `high` (never with `xhigh`/`max`). |
