...
description: >
  ...
  with calibrated probabilities for routing, ranking, verification, and cheap
  pre-LLM triage. Use when an agent or pipeline needs a structured decision
  instead of a full generative call; when brainstorming TypeSafe integrations;
  or when open-model-data / harness work needs confidence-gated classification.
when-to-use: >
  ...
  classification; replace prompt-and-parse with structured judgment
effort: medium
...
# TypeSafe (System One / Jev) — fleet skill
...
that path fights Layout A / agents_extensions deploy.
...
The following is the upstream TypeSafe skill body (MIT), retained so agents can
follow live-docs pointers and primitives without a separate install. Prefer live
docs over this snapshot when they disagree.
