# Inactive historical V4 dataset contract

This reference preserves the superseded skill for historical comparison only.
Its launch prompt, stage dispatches, original-row factory, and release instructions
are **inactive**. Do not execute them or treat them as authorization to resume
production. The active [skill entrypoint](../SKILL.md) records the operator's
human-authored Ukrainian source boundary and stopped generation state.

The frozen outcome hash inside the archive retains its original meaning. A7 is
still named **Independent original-row factory** in that historical contract;
it has not been renamed, reapproved, or converted into human-source ingestion.

Read only the historical section needed for a requested comparison. Search terms:
`Frozen V4 scope`, `Slot, row, and case identity`, `Functional ownership`,
`A0–A13`, `Operation-specific rights`, `VPS-first custody`, `Split readiness`,
`Completion and residual behavior`, `Launch prompt`.

Original SKILL.md bytes SHA-256: `47140c235af6cd45a3c790d76687c6a7103d00108de2c40e6e785973c58a9ac8`.
The archived text below is preserved verbatim; its imperative wording is historical.
